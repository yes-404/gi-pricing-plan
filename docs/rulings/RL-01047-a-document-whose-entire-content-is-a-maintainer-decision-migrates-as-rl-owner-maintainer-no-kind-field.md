...
id: RL-1047
family: ruling
title: a document whose entire content is a maintainer decision migrates as `RL-`, `owner: maintainer`, no `kind:` field
...
was: docs/plans/2026-09-03-w37-6-ruling-98-prose-migration.md
---

# W37-6 — RL-1047: how the maintainer's prose decisions under `docs/plans/` migrate under RFC-937 (2026-09-03)
...
`docs/plans/` migrate under RFC-937?"* — with a named starting set the delegating instruction
...
`docs/rulings/RL-01049-w37-6-the-maintainer-s-time-boxed-delegation-2026-09-03.md`, merged `e56d038` (PR #664, "renew the
W37-6 delegation, and rule condition 2 and the tagged evidence ref"). **Self-verified**: `git
show e56d038:docs/rulings/RL-01049-w37-6-the-maintainer-s-time-boxed-delegation-2026-09-03.md` read directly from the
...
**Filed under** delegation §1, which delegates *"RFC-937 §1/§4 amendments needed to reach a
...
## RL-1047 — a document whose entire content is a maintainer decision migrates as `RL-`, `owner: maintainer`, no `kind:` field
...
with the heading present returns one `RL-` draft, `owner: decision-maker` (RL-984's default
...
(`_PLAN_SUFFIX_KIND`, the four suffix-to-kind pairs from RFC-937 §5.2), `:1186-1189`
...
| `2026-08-30-nt-0017-maintainer-decisions.md` | yes | yes | "The maintainer's answers to RFC-898 §5's three open questions, quoted and dated...not the decision-maker's to rule" | **RL-, owner: maintainer** |
...
reader who checks only one will not know the other agrees**: RFC-937's own prose (§1.13,
...
*"Multi-ruling files → one per `## Ruling N`..."* (`docs/rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md:277`).
...
(`docs/rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md:319`), implemented verbatim at
...
(`docs/rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md:238`). This sentence's `handover` and
...
(`docs/rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md:149`, Owner column). All seven are decisions on scope
...
(`docs/rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md:38`, `RL`'s `kind:` column `—`, against `PL`'s
...
**(f) Nothing in RFC-937 supports "disclosed rather than migrated."** §4 step 1:
*"Assign one sequence to every row and document in `created`-date order"*
(`docs/rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md:276`) — unconditional, no carve-out for
...
`docs/rulings/INDEX.md#2026-09-03-w37-6-d1-d2-rulingsmd` — a **separate** document, per the frozen-file
...
`docs/rulings/RL-01049-w37-6-the-maintainer-s-time-boxed-delegation-2026-09-03.md` was appended, after this ruling was
...
(`docs/rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md:277`, and the parser it is coded against,
...
`corrected_by:` + a correcting `RL-`/`RFC-`"* (`docs/rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md:238`,
...
- This ruling amends nothing in `CLAUDE.md`, RFC-937 §1, or any template — it applies an
...
|---|---|---|---|
| **A — the shipped default, `PL- kind: leaf, owner: planner`** | Wrong family and wrong owner for all seven: `owner: planner` on documents none of which attribute their decision to the planner. | `docs/rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md:319` + `scripts/doc-id.py:1179-1206` (a real cell, not a straw option — it is what runs today absent this ruling) | Misattributes the maintainer's own "not yet," second withholding, delegation (twice more via §6.3/§6.4), RFC-898 answers, and WK-671 reopen direction to the planner in the stamped header — the exact false-authorship defect RL-995 already struck for `RL`'s old hardcoded-owner heuristic in the opposite direction (`scripts/doc-id.py:1195-1201`'s own comment). |
