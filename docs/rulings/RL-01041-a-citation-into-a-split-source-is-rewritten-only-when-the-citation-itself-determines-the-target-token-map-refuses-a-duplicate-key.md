...
id: RL-1041
family: ruling
title: a citation into a split source is rewritten only when the citation itself determines the target; `token_map` refuses a duplicate key
...
# W37-6 — RL-1041: a citation into a split source is rewritten only when the citation determines its target (2026-09-03)
...
**This ruling extends RL-980 from the line-offset case to the path-only case.** RL-980
already forbids the shape: *"A rewrite that changes only the path is forbidden"*
(`docs/rulings/RL-00980-a-line-number-citation-into-a-split-file-is-re-derived-or-replaced-a-path-only-rewrite-is-forbidden.md:62-65`, under the
heading at `:153`). RL-980 reached citations *carrying a line offset*; #672 created the same
failure for citations carrying **no** offset, which RL-980's text does not reach. Nothing
about the defect is new — only the class of citation it now applies to.

**Filed under** delegation §1, *"RFC-937 §1/§4 amendments needed to reach a completing, green
run"* (`docs/rulings/RL-01049-w37-6-the-maintainer-s-time-boxed-delegation-2026-09-03.md:19-21`). This is a **scope marker
...
## RL-1041 — a citation into a split source is rewritten only when the citation itself determines the target; `token_map` refuses a duplicate key
...
-oE '[0-9]+' | sort -n | uniq | tail -1` → `99` at `6195ca0`; `git grep -n "RL-1041"` over
...
snapshot"*, `docs/rulings/RL-01049-w37-6-the-maintainer-s-time-boxed-delegation-2026-09-03.md:249-264`, Amendment 1). After
...
`docs/rulings/RL-00910-q2-rl-906-s-mechanism-does-not-transfer-its-principle-does-and-the-answer-here-is-to-conform-the-corpus-and-red-gate-from-day-one.md` and names **RL-910** in the same sentence.
...
**the id resolves to RL-910's record and the path beside it resolves to RL-913's.** A
...
The id is RL-910's document; the path is **RL-913's** — the last of the five drafts.
...
- **The padding difference is not the defect and is by design.** `scripts/_docid.py:104-108`
  (`canonical`, *"The citation form — RFC-937 §1.1 rule 2: unpadded, always"*) against `:111-116`
  (`padded`, *"The filename form — RFC-937 §1.1 rule 3"*). An unpadded id beside a zero-padded
  filename is correct. **The number gap is the defect** — 990 against 993 here, 196 against 199
  as relayed.
- **The absolute numbers were wrong at this tree.** The observed pair is `RL-990` / `RL-993`.
...
`docs/rulings/INDEX.md#2026-09-02-w37-container-family-and-line-citations-rulingsmd` is itself a split
source: two `## Ruling N` headings, **88 at `:61` and 89 at `:153`**. RL-980's heading is the
...
|---|---|---|---|
| `docs/findings/FD-01017-migrate-cannot-complete-on-the-real-corpus-today-the-pending-proposals-container-has-no-discovery-code-and-the-unconditional-plan-reviews-census-is-what-catches-it.md:9` | RL-979 | full `docs/plans/…` | **yes → RL-980's record** |
| `scripts/doc-id.py:1679` | RL-979 | full `docs/plans/…` | **yes → RL-980's record** |
| `docs/research/RS-01003-ruling-acceptance-item-sweep-audit-record.md:374` | RL-979 | bare filename | no |
| `docs/research/RS-01003-ruling-acceptance-item-sweep-audit-record.md:375` | RL-980 | bare filename | no |
| `docs/plans/PL-00959-w37-6-the-migration-run-leaf-plan-superseding.md:122` | RL-979 | bare filename | no |
| `docs/rulings/RL-00984-owner-is-who-may-amend-the-a-series-takes-decision-maker.md:20` | RL-979 | bare filename | no |
| `docs/rulings/RL-00994-the-fixture-is-rebuilt-on-the-property-not-the-level-and-the-container-is-identified-positively.md:6` | RL-979 | bare filename | no |

**The relayed phrasing — "the citation of RL-980's own file is repointed to a wrongly-named
target" — is correct in outcome and inverted in direction, and the corrected form is worse.**
The two rewritten citations both say **RL-979** and both land **on RL-980's record**: the
...
`RL-1058` (RL-979) and `RL-1059` (RL-980); `RL-1059` is the last draft and wins the key.
...
`docs/closures/INDEX.md#closure-recordsmd` (21) and `docs/closures/INDEX.md#plan-reviewsmd` (12); the other 25 are
...
`.claude/skills/close-workstream/SKILL.md:646` (`docs/closures/CR-00830-plan-review-8-at-wk-670-s-close.md:233-251` →
`docs/closures/CR-00908-…`) and
`docs/research/w11-task-1-4-model-call-concurrency.md:7`
(`docs/rulings/RL-00868-score-one-s-real-time-path-async-evaluate-not-evaluate-executor-offload-and-whether-5-2-s-sync-convention-is-itself-the-defect.md:75-77` → an `RL-` record). The first is
**one of RL-980's own seventeen** — the overlap between the two rulings, made concrete.
...
**For a split source it names an arbitrary sibling's new path** — all five of RL-909–53's
...
- **(i) A document id adjacent to the path names the target.** The citing text carries the
  target's own identifier beside the path — `RL-910`, an `RL-`/`FD-`/`F<n>` id — and the
...
- **(iii) A line number maps to exactly one target through the split boundaries at the source
  tree.** This is **RL-980's re-derivation** (`:199-202`), performed **by the migration**
  rather than by hand — RL-980 permits (a) re-derivation or (b) replacement and leaves the
...
dispositioned by name before the run** — re-derived or replaced, in RL-980's own shape
...
**Why a deliberate dangle is the right residue.** RL-980 already settled the comparison, in
...
### 2.5 Superseded by RL-1042 — the undetermined case resolves to the split's index entry

**§2.4 is superseded, dated 2026-09-03, by RL-1042, stated at its own heading below.**
...
**What changed, in one line:** §2.4 made bucket (iv)'s **size** decide the window; RL-1042
makes bucket (iv) **empty by construction**, so there is no size left to decide anything.
**Read RL-1042 below for the rule, its two obligations and its new check** — it is not
...
|---|---|---|---|
| **A — the shipped behaviour, last draft wins** | Every path-only citation into all 27 split sources repointed to whichever draft iterated last. | `scripts/doc-id.py:5150`, `:5197` — a real, currently-shipped cell, not a straw option | The 171. Six citations naming RL-979 land on RL-980's record (§1.2); `register-lint.py:3` names RL-910 and points at RL-913 (§1.1). Every one resolves, so **no gate in the seven sees any of it** — the failure walks through condition 7 by construction, because condition 7 tests resolvability. |
| **B — a canonical target per split source** (pick one destination, rewrite every path-only citation to it) | Every citation resolves; a defensible-looking default; zero dangles; the gate stays green. | **No cell.** RL-980 (`:199-202`) forbids a rewrite that changes only the path, and names re-derivation and replacement as the two permitted repairs — a canonical target is neither. | **The maintainer's own grounds: *"it invents what RL-980 refused."*** It is strictly worse than A in one respect that matters: it makes the wrong answer look *chosen*. A reader who finds a citation pointing at a canonical target has no way to tell a determined target from a defaulted one, so the corpus loses the ability to distinguish a correct rewrite from a guess — permanently, since the original path is gone after the run. It also produces exactly the state RL-980 calls worse than failing loudly (`:208-209`). |
| **C — pure dangle: rewrite nothing into any split source** | All 171 dangle; condition 7 lists all 171; nothing is silently wrong. | RL-980 (`:199-202`) supports the *residue*, but no cell supports discarding a **determined** target: (b)'s *"the destination record's id"* is precisely a determinant the corpus already holds. | **The maintainer's own grounds: *"deriving nothing when the citation names its target is discarding evidence the corpus already holds."*** C is safe and lossy in the same act. It converts every (i)/(ii)/(iii) citation — where the target is *provable*, not guessed — into manual work, and puts them on the same list as the genuinely ambiguous ones, which is how the ambiguous ones get skimmed. Its cost is measured by bucket (i)+(ii)+(iii): every one of those is a rewrite C refuses to make on evidence it has. |
...
**Neither rejected reading reaches the halt condition.** Both have a cell (B: RL-980
