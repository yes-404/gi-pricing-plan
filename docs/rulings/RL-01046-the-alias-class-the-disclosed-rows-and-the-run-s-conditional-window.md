...
id: RL-1046
family: ruling
title: the alias class, the disclosed rows, and the run's conditional window
...
was: docs/plans/2026-09-03-w37-6-ruling-105-alias-class-h-breakdown-and-run-window.md
---

# W37-6 — RL-1046: the `F-W` alias class, the (h1) breakdown, three more instrument shapes, OQ-555, a §7(f) append, and the migration run's go-ahead (2026-09-03)
...
itself, exercising the maintainer's own delegation of WK-697/W37-6 decisions, confirmed to the lead
directly in this session on 2026-09-03: *"your goal is to instruct the team lead to achieve the
target of landing WK-697, where the first step is land W37-6; make decision on behalf of me as the
...
- **An amendment to what a maintainer's own prior ruling required is the maintainer's alone**
  (`CLAUDE.md` §12). Decisions A and B amend RL-1043 §2/§3's row scope; D1–D3 amend nothing
  written before them (they fill instrument rows RL-1043 §1 required but left unread); D4 is a
  dated append to RL-1043 §5; D5 is a dated append to RFC-937 §7(f); D6 closes OQ-555; D7
  opens what RL-1043's own text held shut, conditionally, and is itself a maintainer decision
  under the same delegation, not a re-reading of one already made.
- **The frozen records are not edited.** RL-1043, the delegation record, and RFC-937 stand as
  filed; this record supersedes or appends to the sentences it names, in the pattern RL-1043
  §3 and RL-1045 used.
...
## RL-1046 — the alias class, the disclosed rows, and the run's conditional window
...
git grep -ohE '^## Ruling [0-9]+' origin/main -- docs/ | grep -oE '[0-9]+' | sort -n | tail -1
  → 104
git grep -c 'RL-1046' $(git branch -r | grep -v HEAD) -- docs/ .claude/ scripts/
  → no match
```
...
**Sentence amended.** Delegation §8.5, `docs/rulings/RL-01049-w37-6-the-maintainer-s-time-boxed-delegation-2026-09-03.md:437`:
...
**Sentence read.** RFC-937 §7(h), `docs/rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md:426`: *"`audit-docs.py`
… green"*; RL-1043 §3's carve-out — *"Any H row without which `audit-docs.py` finds zero
...
`docs/plans/PL-01036-w37-6-row-h-the-named-h-rows-with-the-parser-failure-each-one-is-named-by-2026-09-03.md:143-151` (relayed) — check 35 (79) and
check 30 (77) are RFC-937 §5.1/§5.3/§5.4 **content** rows (stamping charters, skills, root
...
(d)/(g)/the script — W37-6's. `RFC-756`: an owner named twice goes stale; the row prints the
owner once.
...
RL-1043 §3 governs §1 (*"eight rows, not nine"*). `--verify` computes `(i)`, prints `owner:
...
### D3. `(h2)`'s OVER-EXEMPT is RL-1039's ruled outcome and is disclosed, not failed

`(h2)` now reports every denominator non-zero (533 / 111 / 31 / 111 / 437) and fails only on
*OVER-EXEMPT: check 37 exempts 366 of 437 on the `was:` field*. That is RL-1039's consequence,
accepted with its disclosure at delegation §6.3 — and since #693 the field it keys on carries true
provenance. **The over-exempt probe becomes `DISCLOSE`, printing RL-1040 §4's four figures
...
### D5. Dated append to RFC-937 §7(f)

RL-1044 §2.2's reading becomes the sentence's own text: *"unchanged across the migration on
...
### D6. OQ-555 closed: the standard's specimens become placeholders

RL-1044 §1.7 option 1. `docs/process/document-ids.md` and RFC-937 §1.1 rule 3 write their
specimens as `PL-<n>`, `PL-0<n>`, `PL-00<n>` (the shape the ADR examples already use), so no
specimen can ever resolve through `docs/INDEX.md`. OQ-555 → `closed` citing this ruling.
...
- **Task 1's executor (`executor-verify-2`) lands `EXPECTED_VERDICTS` first**, unchanged by this
  ...
  OVER-EXEMPT → `DISCLOSE` with RL-1040 §4's four figures together.
