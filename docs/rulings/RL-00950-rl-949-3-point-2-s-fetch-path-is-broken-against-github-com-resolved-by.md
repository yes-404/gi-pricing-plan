...
id: RL-950
family: ruling
title: RL-949 §3 point 2's fetch path is broken against github.com; resolved by
...
was: docs/plans/2026-09-01-ruling-60-census-provenance-checkout-depth.md
---

# RL-950 — RL-949 §3 point 2's fetch path is broken against github.com; resolved by
full-history checkout, not by repairing the fetch (2026-09-01)

**What this is.** RL-949 (`2026-09-01-nt-0016-slice2-fr-data-32-ruling.md`, merged as
...
AssertionError: docs/research/file-census-5ef559d.csv: names commit '5ef559d' as the tree it
...
not exempt a file it could not verify. RL-949 §4's third case (added in that record's own
...
1. `git grep -c "^## RL-950" docs/rulings/RL-00950-rl-949-3-point-2-s-fetch-path-is-broken-against-github-com-resolved-by.md`
   returns `1`, and `git grep -n "^## Ruling " docs/plans/` shows 60 filling the gap
   immediately after RL-949 with no duplicate and no skip.
...
3. §3 states precisely what changes and where (one workflow file, one line), leaving
   ...
   reason stated.
4. §4 confirms all three of RL-949 §4's broken-input cases still hold under the amended
...
6. `git grep -nE '\bFR-[A-Z]+-[0-9]|\bNFR-[A-Z]+-[0-9]|\bOQ-[A-Z]+-[0-9]|\bADR-[0-9]'
   docs/rulings/RL-00950-rl-949-3-point-2-s-fetch-path-is-broken-against-github-com-resolved-by.md` returns matches only
   to `FR-72` — no new requirement id, no spec amendment.
...
**Is RL-949 §3 point 2's mechanism sound? No** — not in its logic, which behaved exactly
...
the census's own design principle (`docs/research/RS-00952-file-census-rfc-897-stage-0.md`: "a concrete,
...
generated at a specific commit (`docs/plans/PL-00929-rfc-897-file-taxonomy-reference-coding-and-custody-research-and-the-slice-cut.md` §7's
Interfaces section: `file-census-<tree>.csv`, and `<tree>` has been treated as the short form
throughout RFC-897's own text, including this filename in RL-949 itself) — churn (2)
...
choice is not mistaken for an oversight: it is what makes RL-949 §4's third case
...
(`docs/findings/register.md`) records that concurrent gate runs sharing this repository's test
...
## 4. All three of RL-949 §4's broken-input cases, re-confirmed under the amended mechanism

RL-949 §4 specifies these against a **synthetic fixture git repository**, never the live
...
- **Unresolvable SHA** — unaffected; a synthetic fixture naming a commit absent from its own
  ...
  `fetch-depth: 0`, the real `docs/research/file-census-5ef559d.csv` never exercises the
...
Same finding as RL-949 §5, unchanged by this amendment: this is a CI/test-infrastructure
correction to an enforcement mechanism, not a reading of FR-72's text, which this record
does not revisit.
