...
id: RL-924
family: ruling
title: option (b): one computation, taught to answer the question it already claims to answer
...
was: docs/plans/2026-08-30-w11-service-account-permissions-ruling.md
---

# WK-671 Task 2B — a Service Account's permissions reach no enforcement path (2026-08-30)
...
**Mints no `FR-`/`NFR-`/`OQ-` id and makes no edit.** As with RL-915 this is `CLAUDE.md` §0's
first row — FR-347 already specifies that a Service Account holds these permissions, and
...
## RL-924 — option (b): one computation, taught to answer the question it already claims to answer
...
>
> Used by the API to tell the frontend what to show (FR-343's second sentence). … **it is the
