...
id: RL-984
family: ruling
title: `owner:` is who may amend; the A-series takes `decision-maker`
...
was: docs/plans/2026-09-02-w37-gap-1-ruling-86-owner-ruling.md
---

# Gap 1 — RL-995 §3 item 2 does not survive; the A-series takes `owner: decision-maker` (2026-09-02)

**What this is.** Gap 1 of `docs/plans/PL-00963-what-owner-means-the-family-table-the-historical-author-or-neither-a-derivation-for-ruling-2026-09-02.md` §227: *"does
RL-995 item 2 survive its premise failing?"* — framed there and deliberately not answered,
...
too.** The table is not silent on author-versus-owner; it settled the question and RL-995
read past it.

**And this is a self-correction rather than a new position.** [RL-979](RL-00980-a-line-number-citation-into-a-split-file-is-re-derived-or-replaced-a-path-only-rewrite-is-forbidden.md)
...
different drafter."* **RL-995 item 2 and RL-979 §2 are in direct contradiction and I did
not notice when I wrote the second.** Gap 1 is that contradiction surfacing, and RL-979 is
...
- **RL-995 is not edited.** An amendment to a filed ruling is a new dated artifact — the rule
  RL-995's own siblings state, applied to RL-995.
...
3. §3 says exactly which clause of RL-995 item 2 is struck and which survives — the failure
...
**(b) The premise RL-995 stood on does fail, as the derivation measured.** RL-995 §1(b)
quoted the 2026-08-30 delegation correctly — *"authorise you to approve RFC-842 RFC-843 and
RFC-895 landing on behalf of me"*, read narrowly as *"the landing … and nothing else"* — and
...
**(c) The contradiction with RL-979, in both records' own words.**

| | |
|---|---|
| **RL-995 §3 item 2** (`:126`) | *"`owner:` is not `decision-maker` for these three"* |
| **RL-979 §2** (`:95`) | *"§1.6's column is 'Owner — creates & amends', **not 'author'** … The planner's authorship is not erased by this; it is in the body and in `was:`."* |

RL-979 reached that against a planner derivation recommending the author reading, for the
`RFC` family — whose §1.6 row contemplates a different drafter in exactly the way the `RL` row
does. **Applying RL-979's own reasoning to `RL` yields `decision-maker`.**
...
files matching the delegation heading: 1
  lead           docs/plans/INDEX.md#2026-08-30-nt-0012-0013-0014-adoptionmd
```
...
**RL-995 §3 item 2 does not survive as to Rulings A1–A3. They take `owner: decision-maker`,
the value §1.6's `RL` row gives.**

`owner:` is **who may amend**, not who wrote. The attribution concern that motivated RL-995
item 2 is real — *a frozen record must not attribute a decision to a role that did not make
it* — and it is **not what this field expresses**. Authorship is preserved where RL-979
...
1. **RL-995 §3 item 2's first clause is struck as to the A-series.** They take
   `decision-maker`.
2. **Item 2's second clause survives untouched**: `_discover_plain_plans`'s
   ...
   **RL-996 §3 item 2 — which ruled `decision-maker` for those three — is unaffected and was
...
4. **The executor's design was right for the world RL-995 created**, and PR #603 implemented
...
5. **The lead's earlier objection to #603 on §1.6 grounds was wrong when made and is right
   now.** It was wrong because RL-995 governed; it is right because RL-984 does. Recorded
...
- **RL-996's `decision-maker` for the three standalone ruling files still holds after this
...
|---|---|---|
| **Whether a `written_by:` or equivalent authorship field should exist** | The derivation names the missing field as the proper home for attribution. Adding a field to §1.5's closed set is a §1 amendment, and RL-981 held that adding `decision:` *"is an edit to §1 … and it would have gone back to the maintainer"* | **The maintainer**, via the `RFC-` route, if anyone wants it. **Nothing depends on it**: RL-984 does not create the need, it declines to solve attribution with the wrong field |
...
the routing brief, and neither is the contradiction with RL-979; both were found by reading
