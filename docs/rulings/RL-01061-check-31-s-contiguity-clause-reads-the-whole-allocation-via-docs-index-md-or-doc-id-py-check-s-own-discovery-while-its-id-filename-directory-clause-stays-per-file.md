...
id: RL-1061
family: ruling
title: check 31's contiguity clause reads the whole allocation via `docs/INDEX.md` (or `doc-id.py check`'s own discovery), while its id/filename/directory clause stays per-file
...
was: docs/plans/2026-09-04-w37-6-ruling-108-check-31-contiguity.md
---

# W37-6 — RL-1061: check 31's contiguity clause reads the whole allocation, not a per-family scanner (2026-09-04)

## RL-1061 — check 31's contiguity clause reads the whole allocation via `docs/INDEX.md` (or `doc-id.py check`'s own discovery), while its id/filename/directory clause stays per-file
...
<!-- Structural note: this heading exists so `_discover_multi_ruling_files`
     ...
     `PL- kind: leaf, owner: planner` catch-all — the same defect RL-1059's and Ruling
     107's structural notes name (F96, `docs/findings/FD-01055-a-filed-ruling-that-omits-the-ruling-n-heading-migrates-as-pl-owner-planner-and-nothing-catches-it.md`). -->
...
**106 is highest, so 107 and 108 are the next two free numbers.** RL-1060 (filed
separately, PR #750, `docs/rulings/RL-01060-check-36-is-one-rule-at-two-times-with-d-and-must-carry-d-s-disclosed-classes-check-32-s-padding-resolution-clause-adopts-e-s-conjuncts-from-docid-not-a-private-re-typing.md`)
takes 107; this record takes **108**. `git grep -c "RL-1061"` returns nothing on
`origin/main` or on any open W37-6 remote branch (`origin/w37-6-h1-checks-31-32-36` / PR
#747, `origin/worktree-h1-checks` / PR #749, and every other branch checked for RL-1060's
...
`origin/w37-6-h1-checks-31-32-36`) cite "RL-1060" for check 31's contiguity clause — the
content filed here, under 108, not 107. RL-1060's own filing (this record's sibling)
...
> **1. Check 31 — exec-h1's revision is right, by the standard's own first rule.** RFC-937
...
> RL-1060's or already-discharged ground, not repeated here.]
...
> open-question rows (RFC-937 §1.2: they *"live in `docs/specs/<module>.md`"*, not in
...
- **`scripts/audit-docs.py` check 31** — its contiguity clause reads `docs/INDEX.md` (or the
  ...
  PR to write — PR #747 carries the code, and repoints its "RL-1060" comments at this
...
Entry 1's other three items — which are RL-1060's or already-discharged ground and are not
re-filed here.
...
from the bare string "RL-1060" to this record's path (108), under `lead.md`. This ruling
