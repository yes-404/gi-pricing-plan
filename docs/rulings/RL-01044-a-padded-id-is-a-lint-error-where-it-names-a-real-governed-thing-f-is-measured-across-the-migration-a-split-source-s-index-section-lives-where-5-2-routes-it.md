...
id: RL-1044
family: ruling
title: a padded id is a lint error where it names a real governed thing; (f) is measured across the migration; a split source's index section lives where §5.2 routes it
...
was: docs/plans/2026-09-03-w37-6-ruling-103-ef-readings-and-index-placement.md
---

# W37-6 — RL-1044: one reading each for §7(e) and §7(f), and RL-1043 §6's routing rule made computable (2026-09-03)

**Filed** 2026-09-03 by the decision-maker. **What this is.** RL-1043 §2 assigns the
decision-maker two of the six work-list rows — *"(e)/(f) — each gets one reading, ruled by the
decision-maker citing §7's sentence — not two"* — and RL-1043 §6 rules the cross-family
...
of which RL-1043 §1 and §2 already assign to `executor-verify`. **Until that PR is merged and
named, this ruling has changed no behaviour.**

## Authority

- **RL-1043 §2 assigns the (e) and (f) readings to this role by name**, and its acceptance
  ...
  `docs/rulings/RL-01043-no-further-delegated-window-until-7-a-i-is-an-instrument.md` on `main`, merged as **#684**
...
- **RL-1043 §6 is the maintainer's and is not reopened here.** Its rule is quoted verbatim in
...
- **The constraint RL-1043 §2 imposes on the method** — *"citing §7's own sentence … and not
...
## RL-1044 — a padded id is a lint error where it names a real governed thing; (f) is measured across the migration; a split source's index section lives where §5.2 routes it
...
<!-- Structural note: this heading exists so `_discover_multi_ruling_files`
     ...
     catch-all — the defect F96 (`docs/findings/FD-01055-a-filed-ruling-that-omits-the-ruling-n-heading-migrates-as-pl-owner-planner-and-nothing-catches-it.md`) was filed for. -->
...
git grep -n 'RL-1044' $(git branch -r | grep -v HEAD) -- docs/ .claude/ scripts/ → exit 1
```
...
`docs/rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md` §1.1 (line 426 carries §7; §1.1 is at lines 20–23) and
...
|---|---|---|
| **check 19** (`ADR-(\d{4})`, `scripts/audit-docs.py:418`) | RFC-937's width-5 ADR filename examples | **the document was distorted** — three examples rendered as `ADR-<nnnnn>` placeholders, and the audit record describing that had to split its own digit run as `ADR-000`+`01` so as not to retrigger the check it describes (`docs/research/RS-01002-rfc-937-verification-and-impact-sweep-audit-record.md:16-22`) |
| **check 28** (plan acceptance standard) | a ruling record filed in `docs/plans/` | **the document was changed** — *"The lead did not patch the check … and instead added a genuine `## Acceptance Standard` section to the record"* (`docs/findings/FD-01006-audit-docs-py-check-28-has-no-plan-kind-test-for-a-ruling-record.md:14-21`); the classification defect was filed as **F68** and carried forward with the S2 migration as its trigger |
| **`tests/test_notes_move_citations.py`** | RFC-937 and three documents about the migration | **the check's scope was narrowed** — `_SPECIFICATIONS_OF_THE_OLD_PATH`, a four-entry, individually-reviewed, per-file list, with an explicit prohibition on widening it: *"each needs its own reviewed line added here, not a widened pattern"* (`:111-114`) |
...
**Filed as [`F100`](../findings/FD-01051-discover-findings-docstring-describes-a-citation-rewrite-the-same-file-s-code-refuses-to-make-and-writes-a-padded-id-to-do-it.md)**, with the register row carrying the same
...
thing RL-1043 §2 turns these rows into. So the argument for one ruled reading is not
...
(e) because a legacy `ADR-703` does not resolve to a post-migration governed thing — it is a
...
where `NFR-502/501 → NFR-775/14` is already the named broken-input proof, and (e) has nothing
...
green before (e)'s number means anything**, which is also the order RL-1043 §2 puts them in —
...
its lift of RFC-937 §1.1 rule 3 … ) uses ids, some deliberately padded, to teach the grammar
...
**Filed as `OQ-555`** in `docs/open-questions.md`, mirrored into `docs/specs/00-overview.md`
...
ground was false.** `OQ-554` (`docs/open-questions.md:46`, mirrored at
`docs/specs/00-overview.md:554`) is a question purely about `audit-docs.py` checks 14 and 21,
filed under `OVR`; `OQ-548` was the same shape and was closed into an `audit-docs.py` check
plus `FR-22`. **The file already carries audit-tooling questions, and the premise was
...
it.** Filing it in `docs/findings/register.md` was proposed and is refused on two grounds, both read
...
**One thing left unplaced, deliberately.** `OQ-554` sits on **no** `docs/roadmap.md` §10 gate
...
**Third, RL-1043 §1 makes the instrument run *"the migration on a disposable snapshot"*.**
...
migration and the count after must be equal.** `8f5d57d` is read as the tree at which RFC-937
...
unchanged. **A row that cannot fail on the thing it names is precisely what RL-1043 §1 exists
...
one of them is a document about RFC-937 itself**: `docs/rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md`,
...
number and the rejected one produces the failing number, which is exactly the shape RL-1043 §2
warns against. **The grounds are independent of that**: §7's own "At the migration PR's merge
tree" preamble, §1.1 rule 4's "this standard never touches", and RL-1043 §1's disposable
...
## 3. RL-1043 §6 — the routing rule, made computable
...
`_discover_lettered_rulings` pattern. That is wrong, and the correction matters** — RL-1043
...
|---|---|---|---|---|---|
| 1 | `docs/closures/INDEX.md#closure-recordsmd` | `CR`×9, `LG`×10, `RS`×2 | `_discover_closure_records` | *"split into `CR-` files (`work`, `review`); preambles → `closures/README.md`"* | `docs/closures/INDEX.md` |
| 2 | `docs/closures/INDEX.md#plan-reviewsmd` | `CR`×12, `RFC`×2 | `_discover_plan_reviews`, which returns `_discover_proposal_containers`' drafts alongside its own | *"split into `CR-` files (`work`, `review`)"* — same §5.2 row | `docs/closures/INDEX.md` |
| 3 | `docs/plans/INDEX.md#2026-08-30-nt-0012-0013-0014-adoptionmd` | `PL`×1, `RL`×3 | `_discover_plain_plans` **and** `_discover_lettered_rulings` | the `plans/2026-*.md` row: *"the rest → `plans/`"* | `docs/plans/INDEX.md` |
...
are untouched by §6 and keep resolving to `docs/rulings/INDEX.md` under RL-1042 clause 1
unamended.
...
declared once from `docs/rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md` §5.2, and is never computed from the
...
3. **The section's contents are unchanged** — every target, from every family, with its path.
   ...
   `_write_split_source_indexes` already does, and RL-1042's grounds (*"not choosing a target
...
the routed rule alike, and proves nothing.** This is why RL-1043 §6 says the executor's
...
RL-1043 names it: *"an index section placed by sort order rather than by §5.2's route, once
...
Under RL-1043 §7's rule, applied to this role's own figures:
...
1. **The three cross-family sources are not all the plain-plan/lettered-ruling pattern** — §3.2.
   Two of the three are `closure-records.md` and `plan-reviews.md`, and the first is RL-1043
...
3. **`_MIGRATION_DIFF_FAMILY_INDEXES` resolves, but not where a reader would look.** It is
   **absent from `main` at `e97b97a`** — the only occurrence of the string in the working tree is
   the handover's own prose at `docs/plans/PL-01037-w37-6-the-extended-window-s-second-fail-and-the-handover-2026-09-03.md:325`. It is
...
predicates proves uncomputable inside `doc-id.py migrate --verify` as RL-1043 §1 shapes it.
A predicate that cannot be computed is a reading that failed, and RL-1043 §1 says so:
...
> (`docs/rulings/INDEX.md#2026-09-03-w37-6-ruling-100-split-source-citationsmd`), only `:91` is a path;
> `:94` and `:101` are both **prose**. **This role inferred "two paths" because two of the three
> hits carried the token `RL-993`, and never read the two lines separately** — a grep hit taken
...
*Fenced 2026-09-04 under RL-1044 §5.1; value unchanged.* — a padded id quoted **as the
...
prints it rather than this record merely recording it. It is `RFC-789`'s rule exactly: **a
...
**The control file is `docs/closures/INDEX.md#closure-recordsmd` — §3.2's split source 1.** It held all 8;
...
post-migration path.** That is RL-1043 §5's `was:` defect appearing independently in this
...
what RL-1043 §2 row 5 asked for.
...
into an executor's worktree during WK-670, the third write discarding uncommitted tracked files.
...
**RL-1044's four (e) conjuncts and two (f) conjuncts are implemented** by `executor-verify` in
PR **#695** (branch `w37-6-ruling-103-rows`, head `405adb6`, on `main` at `ac82256`). Both rows
are scored rather than `UNDETERMINED`, which is what RL-1043 §2 row 5 asked for.
