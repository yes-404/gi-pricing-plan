...
id: RL-994
family: ruling
title: the fixture is rebuilt on the property, not the level; and the container is identified positively
...
was: docs/plans/2026-09-02-w37-ruling-88-acceptance-amendment.md
---

# RL-979's second acceptance item, amended after the heading demotion (2026-09-02)

**What this is.** PR #609 (merged as `2fbce0c`) demoted `docs/closures/INDEX.md#plan-reviewsmd`'s
...
[RL-979](RL-00980-a-line-number-citation-into-a-split-file-is-re-derived-or-replaced-a-path-only-rewrite-is-forbidden.md)'s acceptance items —
it names a fixture that can no longer be built from the document it describes. **Ruled below as
RL-994**, which amends that item and adds one the demotion makes necessary.
...
[RL-985](RL-00986-the-ten-wk-661-slice-records-become-lg-carrying-work-and-no-slice-no-template-edit-is-needed.md) §2 rules toward. §1(c)
...
- **It lands as a new dated artifact, never an edit to the merged record** — the rule RL-979's
  own Authority section states, applied to RL-979.
- **Every figure is measured at `614c92c`**, `origin/main`'s tip when this record was written and
  this branch's base.
- **Nothing else in RL-979 is reopened.** The family, the `kind:`, the `status:`, the
  `owner:` disagreement, the P1-over-P2 finding and RL-980 all stand.
...
## RL-994 — the fixture is rebuilt on the property, not the level; and the container is identified positively

### 1. Verified first, at `614c92c`

**(a) The new shape.** `docs/closures/INDEX.md#plan-reviewsmd`:
...
**(b) The stale clause is real and it is an acceptance item.** RL-979 `:144` reads *"A
fixture whose only level-2 heading is followed by three record headings"*. **The file now has
zero level-2 headings**, so the fixture cannot be built from it. RL-979 `:67` (*"1155 is `##
...
**(c) #609 was right, and for a reason worth stating.** RL-979 ruled the container **is** a
record — an `RFC-`. Records in this file live at `###`. Before #609, a census would have needed
a **bucket 3** entry (RL-985 §2): *a `##` heading that is nevertheless a record*, declared in
...
RL-985 §2 holds that a bucket-3 entry *"that could have been derived is a defect in the fix
...
is a fourth instance of the class RL-985 exists for, this time in a *margin* rather than a
count.
...
RL-985's census counts *"every heading at any level — `^#{1,6}\s`"*, which is already the
...
**RL-979 §4's second acceptance item is replaced. Its violation is unchanged; its fixture is
rebuilt on the property rather than on a heading level.**

**Struck** — RL-979 `:144-146`:
...
> fixture is constructible from `docs/closures/INDEX.md#plan-reviewsmd` itself, which is the property the
> struck item lost.
...
> reclassify the container into a `CR- kind: review` if any of them were ever made. RL-979 §3
...
1. **RL-979 §4's second item is read as substituted from this record's date.** RL-979 is
   not edited.
2. **RL-979 `:67` and `:123` stand as written, stale in their heading-level detail**, with
...
- **`grep -c '^##[^#]' docs/closures/INDEX.md#plan-reviewsmd` returns 0, and the census still closes** with
...
| **Whether `#609`'s demotion should be mirrored in the roadmap's own mis-nestings** | RL-993 §3 routed the `#### Phase 1b status` case to the lead as a finding; the same treatment may apply | **The lead**, unchanged |
