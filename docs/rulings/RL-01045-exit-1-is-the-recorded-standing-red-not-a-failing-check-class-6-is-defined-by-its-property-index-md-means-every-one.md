...
id: RL-1045
family: ruling
title: exit 1 is the recorded standing red, not a failing check; class 6 is defined by its property; `INDEX.md` means every one
...
was: docs/plans/2026-09-03-w37-6-ruling-104-ci-signal-and-class-6.md
---

# W37-6 — RL-1045: the CI step reads the verdict set, and RL-989's class 6 is a property with the generated READMEs and every `INDEX.md` inside it (2026-09-03)
...
- **An amendment to what a maintainer's own prior ruling required is the maintainer's alone**
  (`CLAUDE.md` §12). Decision 1 amends RL-1043 §1's CI sentence; decisions 2 and 3 amend
  RL-989 §2's class 6. Both prior rulings are the maintainer's.
- **The frozen records are not edited.** RL-1043 and the preconditions rulings stand as
  filed; this record supersedes the sentences it names, in the pattern RL-1043 §3 used for
...
## RL-1045 — exit 1 is the recorded standing red, not a failing check; class 6 is defined by its property; `INDEX.md` means every one
...
git grep -hE '^#{1,6}[ \t]+Ruling[ \t]+[0-9]+' $(git branch -r | grep -v HEAD) -- docs/ \
  | grep -oE 'Ruling[ \t]+[0-9]+' | grep -oE '[0-9]+' | sort -n | uniq | tail -1     → 103
git grep -c 'RL-1045' $(git branch -r | grep -v HEAD) -- docs/ .claude/ scripts/  → no match
```
...
**The sentence amended.** RL-1043 §1
(`docs/rulings/RL-01043-no-further-delegated-window-until-7-a-i-is-an-instrument.md:48-49`): *"It runs in CI on
...
the change under review — F102 (`docs/findings/FD-01052-a-document-that-is-well-formed-today-and-unclassifiable-after-the-migration-with-nothing-before-the-migration-able-to-say-so.md`) measured exactly that: an
...
untouched, and RL-1043 §1's other clauses — snapshot only, nine rows as one table with
...
### 2. RL-989's class 6 is defined by its property; the four names are examples; the §5.2 generated READMEs are members

**The sentence amended.** RL-989 §2
(`docs/rulings/RL-00989-dp-3-g-is-a-property-of-the-script-its-filter-is-a-closed-enumeration-and-it-shares-check-34-s-predicate.md:56-57`): *"6. a generated
artifact regenerated in full — `INDEX.md`, `REDIRECTS.csv`, `docs/contracts/`, the core-JSON
digest (§4 step 7)."*

**The gap it left.** RFC-937 §5.2 makes several `README.md` files **generated** — the
...
produces"* and a *"further extension of RL-989's six-class enumeration"*, flagged for
ratification rather than assumed. The ratification never happened; the second-fail handover's
§11 recorded the phrase *"RL-989 class-6 ratification"* as resolving to nothing in `docs/`,
and the decision-maker's reading of §7(g) (PR #699, `docs/plans/PL-01035-w37-6-a-reading-of-7-g-s-ruled-predicate-the-six-classes-exist-they-are-a-widening-and-row-g-implements-one-of-them-2026-09-03.md`
§3.1) found the same gap independently. Under RL-989's own rule at `:268` — *"a hunk the
...
**Grounds.** A seventh class named for a file would be a class defined by path, which RL-989
...
**The word amended.** The same sentence's bare `INDEX.md`. RFC-937 §1.4 puts one at
`docs/INDEX.md`; RL-1042 and RL-1043 §6 create one **per family** — `docs/closures/INDEX.md`
...
tell which was meant — `RFC-777`'s shape, a reference that resolved only for its writer.
...
- **Task 21 (executor-verify, the six classes in `row_g`) proceeds under RL-989 as ruled,
...
- **Nothing here opens a window**, and the go-ahead remains the instrument's exit 0 at a quiet
  tree (RL-1043, *"What happens next"*). The Work close remains the maintainer's alone
  (`CLAUDE.md` §12).
...
*Violation: RL-989 §2's or RL-1043 §1's text edited in place rather than superseded by this
record.*
