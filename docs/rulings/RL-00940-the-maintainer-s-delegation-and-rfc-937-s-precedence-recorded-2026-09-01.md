...
id: RL-940
family: ruling
title: The maintainer's delegation and RFC-937's precedence, recorded (2026-09-01)
...
was: docs/plans/2026-09-01-maintainer-delegation-and-nt-0019-precedence.md
---

# The maintainer's delegation and RFC-937's precedence, recorded (2026-09-01)
...
[`RL-00914-rfc-898-the-maintainer-s-three-policy-decisions-recorded-2026-08-30.md`](RL-00914-rfc-898-the-maintainer-s-three-policy-decisions-recorded-2026-08-30.md) and
[`RL-00918-wk-671-reopen-the-maintainer-s-direction-recorded-2026-08-30.md`](RL-00918-wk-671-reopen-the-maintainer-s-direction-recorded-2026-08-30.md), these are the
...
RFC-937 at all, and **declined to treat it as verified** — flagging that a live instruction
...
reached a dispatch and no file; the first is the RFC-898 record above, and the mechanism was
identical.
...
| Not covered | Why |
|---|---|
| **A fact only the maintainer holds** | RFC-898's impact row 6 — whether the two repository settings are enabled — is *evidence*, not a decision. No authority substitutes for looking. It stays owed by the maintainer and this record does not discharge it |
| **Acceptance of a Work, Phase or Project close** | `CLAUDE.md` §12 reserves it to the maintainer. Past closes were delegated **specifically, conditionally and in writing** each time (the WK-671 second close, `2026-08-30-w11-reopen-direction.md` §4). This record does not read a general delegation as standing authority to close, and the lead will ask again when a close is next in front of it |
| **An amendment to `CLAUDE.md`'s own requirements** | §12 makes it the maintainer's. RFC-937 will amend `CLAUDE.md` extensively (§3 below) — that is authorised by §3, by name, not by this delegation |
| **Anything the maintainer rules on directly after this date** | A standing delegation is a default, not a bar |

## 3. RFC-937 outranks current practice
...
[`RFC-937`](../rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md) is therefore **not** a proposal to be
...
- **`CLAUDE.md` §5 — "Requirement IDs and section numbers are permanent: never renumber."**
  RFC-937 D2 renumbers `FR-154` to `FR-1187`. §5 yields. The note's §4 migration and §5
...
- **The per-family prefixes and their padding** (`FR-`/`NFR-`, `OQ-`, `ADR-`, `NT-`, `F-`), and
  the citation grammar RL-944 fixed for RFC-897 — which RFC-937 §9 records as lapsing by
  its own override clause, together with RL-942.
...
and RFC-937's own status obliges a roadmap row first.

## 4. The roadmap row rests on the note's own acceptance

Asked of the maintainer 2026-09-01 and answered *land the row with the plan*: **RFC-937's own
`accepted` status is the acceptance line its roadmap row cites.** WK-693 through WK-696 each cite
"the reconciliation's dated acceptance line" because each was reconciled from an `open` note;
RFC-937 was accepted outright at filing and has no reconciliation, so there is no second line
...
[Register](../findings/register.md) row F58 (artifact B has no live writer; the watcher charter's
...
Register row F61 (C2's `PreToolUse` layer is bypassable and, unlike the C3 that RL-920
dissolved, has no CI-equivalent backstop) offered the lead two dispositions and required one to
be named, with a date or a named event: **(a)** build RL-907(b)'s re-derivation cycle, or
...
1. **§1 and §2 (the delegation).** Every decision the lead makes on the maintainer's behalf
   ...
   provenance line.
2. **§3 (RFC-937's precedence).** The RFC-937 slice plan disposes of the three named collisions
   explicitly — `CLAUDE.md` §5, the per-family prefixes with Rulings 63/65, and check 19 —
   rather than designing around them. Violation, stated so it is detectable: a filed RFC-937
   plan whose text is silent on `CLAUDE.md` §5.
3. **§4 (the roadmap row).** `docs/roadmap.md` carries an RFC-937 row whose authority cell
   cites the note's own `accepted` status, and `git grep -n "RFC-937" -- docs/roadmap.md`
   returns it. Until it does, no RFC-937 work may start.
