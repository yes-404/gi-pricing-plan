...
id: RL-863
family: ruling
title: sampling cannot remedy either requirement, and the reasons differ
...
was: docs/plans/2026-08-29-w11-nfr-rate-2-sampling-structural-ruling.md
---

# WK-671 — sampling is not a remedy for NFR-489 or NFR-490, and FR-259's declines limb is why (2026-08-29)

**What this is.** The **structural half** of the NFR-490 remedy question, ruled at the lead's
...
## RL-863 — sampling cannot remedy either requirement, and the reasons differ

**Ruled: "lower the sampling rate" is eliminated as a remedy for NFR-490 and for
NFR-489.** The two eliminations are independent, and the first is stronger than the analysis
note claimed.

### 1. For NFR-490, the sampling rate does not appear in the requirement at all

NFR-490 (`03`:798), verbatim:
...
So for NFR-490, sampling is not a weak remedy or a partial one. **It is definitionally
...
### 2. For NFR-489, the arithmetic leaves a window of exactly 1 %, and it is already spent

NFR-489 (`03`:797), verbatim:

> **NFR-489** | Real-time scoring p99 < 50 ms server-side at 200 rps per replica for a
> ~200-step motor structure with one `exact` GBM call (NFR-454). Without a GBM call,
> p99 < 15 ms.
...
### 3. The answer to the question asked: yes, FR-259's design is implicated — and it is the declines limb

FR-259 (`03`:175) states the policy as *"sampled (default 1 %, configurable, plus 100 % of
declines and errors)"*.
...
all**, only on its being nonzero, which FR-259's existence presupposes: a tracing policy
written to capture declines is answering a case that occurs.

Two supporting readings, both verified rather than assumed:

- **Declines are inside NFR-489's population by construction.** FR-256 (`03`:167) makes a
...
- **The knob is configurable upward, never downward past the limb.** FR-259 makes the rate
...
**Eliminated:** any remedy of the form *"tune the sampling rate"*. For NFR-490 it is inert
(§1). For NFR-489 the only setting that satisfies the condition is *p* ≤ 1 %, which is
unreachable while FR-259's declines limb stands (§3) — and which, even if reached, satisfies
...
[`RFC-789`](../rfcs/RFC-00789-zero-calls-above-200k-tokens-measures-the-compaction-cap-not-discipline.md)'s boundary
...
- Amending a budget. If NFR-489's population is ever narrowed to exclude traced requests,
...
- Amending FR-259's declines limb. Recorded as available and flagged as the option to weigh
...
**The ruling is overridden** if the audit inverts that ordering, if NFR-489 is amended to
state a population that excludes traced requests, or if FR-259's declines limb is withdrawn
...
Slice 4's Task 4D projects NFR-500 from measured serialised size. **It must measure at the
~200-step scale NFR-489 names, or it projects from the flat part of a curve.** RL-871 §3
...
- **NFR-489, NFR-490 and FR-259 were read verbatim at their own lines** in
  `docs/specs/03-rating-engine.md` (`:797`, `:798`, `:175`), whole rows including dated
  amendments. **NFR-489 was read specifically for a scope clause that would have dissolved
  the question — it has none**, which is the check that made the rest necessary.
- **FR-256 was read** (`03`:167) to confirm a decline is a successful response rather than an
