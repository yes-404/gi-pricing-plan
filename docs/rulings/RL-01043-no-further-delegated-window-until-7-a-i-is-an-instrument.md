...
id: RL-1043
family: ruling
title: no further delegated window until §7 (a)–(i) is an instrument
...
was: docs/plans/2026-09-03-w37-6-ruling-102-verify-instrument.md
---

# W37-6 — RL-1043: the acceptance standard becomes an instrument, and the failing rows become the work list (2026-09-03)
...
## RL-1043 — no further delegated window until §7 (a)–(i) is an instrument
...
git grep -n 'RL-1043' origin/main -- docs/ .claude/ scripts/                    → no match
```
...
| # | row | the work |
|---|---|---|
| 1 | **(g)** | **The token-boundary bug.** *"A rewrite may not match inside a longer identifier."* **`NFR-502/501` is the broken-input proof.** |
...
rewrite that matches inside a longer identifier turns `NFR-502/501` into `NFR-775/14` —
...
> token under RL-1041 (i). Deferral is for the ambiguous; this isn't."**

**So the 74 are work, not a disclosure.** They are exactly RL-1041 clause (i) — a document
...
**condition 2's evidence**, accepted once by the maintainer with a disclosure under RL-1039,
...
### 6. RL-1042's cross-family placement — the route decides, not the sort order

**Ruled**, resolving the premise RL-1042 clause 1 assumed and which does not hold — three
...
**This is RL-1042's own principle applied one level up.** The section still chooses no
...
three numbers did not sum, and two figures (`137`, and `OQ-539`'s claimants) that an
...
*Violation: the (g) fix accepted without `NFR-502/501` exercised as a broken-input proof,
red before green.*
