...
id: RL-856
family: ruling
title: the resolver reports no maturity for a `rate_table`, and the exemption is declared and self-invalidating
...
was: docs/plans/2026-08-29-w11-1-2-rate-table-maturity-ruling.md
---

# WK-671 Task 1.2 — what a `rate_table` pin reports as its maturity (2026-08-29)
...
[`../plans/PL-00846-wk-671-slice-1-evaluator-core-its-prerequisites-and-the-latency-harness.md`](../plans/PL-00846-wk-671-slice-1-evaluator-core-its-prerequisites-and-the-latency-harness.md)`:833-835` tells the
...
14–15 [`RL-00881-dp2-fr-257-splits-into-four-limbs-wk-671-delivers-one-defers-two-with-owners-and-does-not-wire-a-gate-that-could-only-refuse-everything.md`](RL-00881-dp2-fr-257-splits-into-four-limbs-wk-671-delivers-one-defers-two-with-owners-and-does-not-wire-a-gate-that-could-only-refuse-everything.md)'s, 16–21
[`RL-00887-finding-3-the-framing-was-wrong-and-the-class-is-bigger-and-mostly-benign.md`](RL-00887-finding-3-the-framing-was-wrong-and-the-class-is-bigger-and-mostly-benign.md)'s.

**Mints one `OQ-` id — `OQ-620` — and no `FR-`/`NFR-` id and no error code.** Read against
`origin/main` at `06bdac5`, with `HEAD` identical.

---

## RL-856 — the resolver reports no maturity for a `rate_table`, and the exemption is declared and self-invalidating
...
- **`06` §3.3** (`:119`) gives it required evidence — *"Change note; diff vs previous; diff vs
  technical seed where seeded (`03` FR-230/231)"* — and FR-363 (`:109`) makes required
...
other. Resolving it is a spec change of real size and is raised as `OQ-620` below rather than
decided here.
...
- **Declared.** `pricing_core` carries a named constant beside `_APPROVED_OR_BETTER` — the name
  ...
  FR-20, `06` §2's Governed Artifact row and `OQ-620`, and states that membership is
...
- **Spec change in this commit**: `OQ-620` raised in `03` §10, mirrored into
...
- **(b) `06` is the wrong side.** A Rate Table Version is governed *through* the Rating Version
  ...
  edited artifact in the platform, where FR-233's bulk operations and FR-235's imports
...
- **(c) Split by phase.** Ungoverned individually in Phase 2; the lifecycle arrives in Phase 3
  with the rest of `06`'s surface (WK-677, which owns FR-351, FR-352, FR-353, FR-354, FR-355, FR-356, FR-357, FR-358, FR-359, FR-361, FR-363). Defers rather than answers, and
...
(b) is right and (a) is approving the same change twice. FR-229's *"The previous version stays
...
1. **`compile_bundle`'s gate is absolute where FR-20 is relative.** FR-20
   ...
   Rating Version's own status, and cites FR-20 in the message. A `draft` Rating Version
   ...
   F-W9-3's clause (2) territory ([`../findings/register.md`](../findings/register.md)`:25`).
2. **`rate_table` is a fourth instance of RL-886's class.** `06` §3.3 gives Rate Table Version
   ...
   (RL-886). Latent for the same reason: nothing submits one. It becomes live with
   `OQ-620`'s answer, and only under option (a).
3. **A stale next-free marker, which will mislead the next ruling.**
   [`../plans/PL-00855-wk-671-slices-2-4-planning-readiness-the-signals-that-release-each-and-what-a-leaf-plan-can-already-take-from-here-2026-08-29.md`](../plans/PL-00855-wk-671-slices-2-4-planning-readiness-the-signals-that-release-each-and-what-a-leaf-plan-can-already-take-from-here-2026-08-29.md)'s
   D5 row says a question there *"Needs an `OQ-RATE` (next free `OQ-620`)"*. This record takes
   `OQ-620`; **D5's is `OQ-RATE-8`.** A frozen document's next-free marker ages the moment
   anyone else allocates, and D5 is still unruled.
4. **Charter boundary, named rather than routed around — a second instance of RL-878's
   ...
   gate table **in the same commit**, and records four questions (OQ-562, OQ-543, OQ-644,
   OQ-582) that were raised, mirrored correctly, and invisible to the plan until 2026-08-15
   ...
   row and route it — nothing else in this PR depends on it, and RL-878's finding then has its
...
5. **Nine open questions are on no decision-gate row, found while discharging finding 4's
   obligation.** Running the `docs-audit` skill's coverage check over the tree with `OQ-620`
   added reports `OQ-620` correctly placed and **nine others missing**: OQ-602, -33, -34,
   OQ-549, -11, -12, -17, OQ-652 and OQ-656. `extra` and `duplicated` are both `none`,
   ...
   of the failure the skill records for OQ-562, OQ-543, OQ-644 and OQ-582: raised,
...
- `docs/specs/03-rating-engine.md` §3.3 `:130-147`, FR-240 `:136`, §10 `:800-813`.
- `docs/specs/06-governance.md` §2 `:64`, FR-363 `:109`, §3.3 `:119`, §4.2 `:251-348`.
- `docs/specs/00-overview.md` FR-20 `:223`.
- `docs/findings/register.md` F-W9-3 `:25`.
- `docs/plans/PL-00846-wk-671-slice-1-evaluator-core-its-prerequisites-and-the-latency-harness.md:820-845`.
