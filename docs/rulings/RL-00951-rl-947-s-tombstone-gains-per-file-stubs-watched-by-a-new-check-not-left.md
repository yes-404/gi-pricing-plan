...
id: RL-951
family: ruling
title: RL-947's tombstone gains per-file stubs, watched by a new check, not left
...
was: docs/plans/2026-09-01-ruling-61-notes-tombstone-stubs-watched.md
---

# RL-951 — RL-947's tombstone gains per-file stubs, watched by a new check, not left
unwatched (2026-09-01)

**What this is.** RL-947 (`2026-09-01-nt-0016-q4-q5-q6-q7-notes-rulings.md`, merged,
...
tombstone RL-947 specifies made `python3 scripts/audit-docs.py` fail with 9 broken-link
errors: 13 frozen plans under `docs/plans/` cite individual old-path notes (e.g.
`../../.claude/rfcs/RFC-00789-zero-calls-above-200k-tokens-measures-the-compaction-cap-not-discipline.md`), C4 forbids editing
...
1. `git grep -c "^# RL-951" docs/rulings/RL-00951-rl-947-s-tombstone-gains-per-file-stubs-watched-by-a-new-check-not-left.md`
   returns `1`, and `git grep -n "^# Ruling \|^## Ruling "  docs/plans/` shows 61 filling the
   gap immediately after RL-950 with no duplicate and no skip.
...
6. `git grep -nE '\bFR-[A-Z]+-[0-9]|\bNFR-[A-Z]+-[0-9]|\bOQ-[A-Z]+-[0-9]|\bADR-[0-9]'
   docs/rulings/RL-00951-rl-947-s-tombstone-gains-per-file-stubs-watched-by-a-new-check-not-left.md` returns no matches — no
   requirement id, no spec amendment.
7. `git diff --stat <merge-base>..<branch> -- docs/` names exactly this one new file — no
   frozen plan is edited, and `2026-09-01-nt-0016-q4-q5-q6-q7-notes-rulings.md` (RL-947's
...
| Without the stubs (README only), `audit-docs.py` fails with exactly 9 broken-link errors, all in `docs/plans/`, all citing `.claude/rfcs/000N-...md` | **Confirmed, reproduced directly** — moved all 18 stub files out, re-ran `python3 scripts/audit-docs.py`: exit `1`, exactly 9 `broken link in plans/...` lines, each naming an old-path note citation (0007, 0004, 0005, 0014, 0015, 0017, 0003, 0016×2). Restored the 18 files afterward; the check reproduces the dispatch's claim exactly, not merely plausibly |
...
| `git grep -l "\.claude/notes" -- .` returns exactly `.claude/rfcs/README.md` plus 13 `docs/plans/` files, nothing else — Slice 4's own acceptance item 1 | **Confirmed** — ran it directly: 14 lines, `.claude/rfcs/README.md` plus 13 named frozen plans. The 18 stubs pass this only because none of their bodies contains the literal string `.claude/notes` — confirmed by inspection of the template above, which names the target as `docs/notes/{name}` and never repeats the source path in words |
...
| `git log --follow docs/rfcs/RFC-00711-plan-review-at-each-phase-boundary.md` reaches the note's original commit | **Confirmed** — reaches `cccd809` ("a numbered working-notes directory, and the audit that keeps it honest (#54)"), through the intermediate rename commit `4ebc85d` and the move commit `e2eff6f` |
...
| RL-947's own rejection of a symlink turned on "both paths equally real, nothing forces migration, a future citer is equally correct either way" | **Confirmed, re-read at source** — `2026-09-01-nt-0016-q4-q5-q6-q7-notes-rulings.md:182-193`. The 18 stubs do not have this property: every stub is legible, explicit prose stating the file moved and where to, not a second live copy indistinguishable from the first — the risk this ruling addresses (unwatched content drift) is real but is not the identical two-homes defect RL-947 disqualified a symlink for, which is why this is a new evaluation rather than a reopening of RL-947's own verdict |
...
**Rejected: (1) — the stubs as built, with RL-947 amended to permit them and the gap
accepted.** This is an accepted cost with no detector — precisely the shape `docs/findin
gs/register.md`'s F58 names (a stated mechanism nothing runs, discovered by inspection rather
...
`docs/findings/FD-00954-audit-docs-py-check-1-s-link-checker-has-no-code-span-awareness.md` (a syntactic proxy carrying an exclusion list that does not
...
# each stub byte-identical to its rendered template — RL-951.
check_notes_tombstone()
```

**A frozen, closed registry, not a re-parse of the README's own table** — the same design
choice RL-949 made for the census carve-out, for the same reason: deriving the expected
...
3. **Exact content, per stub.** For each name in `OLD_NOTES_STUB_NAMES`, render:
   ...
   def rendered_stub(name: str) -> str:
       return (
           ...
           f"{OLD_NOTES_TOMBSTONE_DATE} (RFC-897 Slice 4). See [this directory's "
           "README] (README.md) for the full mapping and why this stub exists rather "
           "than a symlink (RL-947).\n"
       )
   ```
   ...
   in — fails (§4 case 2). This is deliberately not a regex or a prefix check: RL-949's
...
4. **`README.md` itself is not re-validated by this check.** RL-947 already specifies its
...
- **Stray file.** Add `.claude/rfcs/0099-not-a-real-stub.md` with arbitrary content
...
this ruling — the same standard `CLAUDE.md` §13 already states and RL-949/950 already
...
volunteered because §1's re-reading of RL-947 surfaced it: this ruling is not a
reopening of RL-947's own verdict (symlink vs. README), because per-file stubs were never
one of the two options RL-947 compared — RL-947's disqualification of a symlink turned
on both paths being *indistinguishably* real, which 18 explicit, legible redirect files are
not. Worth stating so a future reader does not take this record as evidence RL-947's own
