...
family: proposal
kind: process
title: One id per governed thing: one sequence, integer identity, a self-describing layout, and roles per family
status: active                  # draft → active → closed | retired | superseded (§1.2a)
created: 2026-09-01
owner: maintainer
supersedes: []
superseded_by: ~
...
# One id per governed thing: one sequence, integer identity, a self-describing layout, and roles per family
...
**Amended 2026-09-19 (W37-7, on `RL-1078`): `INDEX.md` carries reserved allocations as well as materialised ones.** A number reserved in `docs/REDIRECTS.csv` — a legacy register row assigned an id that never becomes a file — **is an allocation for the purposes of §1.1 rule 1**, and `doc-index.py` emits it as an `INDEX.md` row. **The four sources above are unchanged and are correct as written**: the allocator was never missing a source, the index was incomplete. That is why the remedy is an emission in the generator rather than a fifth scanner in `next` — a fifth scanner would make `next` step past the reserved block while the index still stopped short, opening a gap in the allocation that §1.11's check 31 fails on. A reservation whose number a materialised record already holds emits nothing; the document is the row (§1.4, *"one row per id"*). The emitted row's **number** and **family** are load-bearing — the allocator, contiguity and citation resolution all read them — while its title cell is a pointer to `docs/REDIRECTS.csv` rather than the reservation's own title, which would carry a pre-migration form into a file §1.11's check 36 scans.
...
| D2 | Requirement ids join the sequence: `FR-154` → `FR-1187` | The module segment encoded the file the clause lives in, which the file already tells you — the argument that retired `F-W9-3`, applied consistently |
...
| D13 | Owners: research → executor, except `RS- kind: audit` → auditor; RFC → maintainer; workflow → decision-maker (executor delivers); charters → maintainer | §1.6; closes every gap RFC-897 found, including the bespoke audit that `2026-08-29-w11-process-conformance-audit.md` had to file as a plan |
...
| Today | After |
|---|---|
| `docs/adrs/ADR-00703-pricing-core-is-dependency-free-and-owns-all-actuarial-maths.md` (2026-08-14) | `docs/adrs/ADR-<nnnnn>-pricing-core-is-dependency-free.md`, cited `ADR-1` |
| `FR-154` in `02-modelling.md` | `**FR-nnnn**` in place, cited `FR-nnnn`; `@pytest.mark.req("FR-nnnn")` |
| `W11-3` roadmap row | `SL-nnnn` row under `WK-nnnn`, `phase: P1b` |
| `## RL-941` inside a four-ruling file | `docs/rulings/RL-0nnnn-q1-category-set-amended.md`, cited `RL-nnnn`, `was: RL-941` |
| `docs/closures/CR-00832-work-item-record-w10-2-seeding-diffs-validation.md` | `docs/closures/CR-0nnnn-wk-nnnn.md`, `kind: work` |
| `F27` + `docs/findings/FD-00934-03-rating-shapes-vs-hand-authored-contracts.md` | `docs/findings/FD-0nnnn-rating-shapes.md`, register row `was: F27` |
...
| `.importlinter` | contract names `ADR-703`/`ADR-704`/`DEP-3` → `ADR-1`/`ADR-2`/`DEP-n` | M |
...
| `closures/CR-00709-phase-0-specification-status.md` | becomes the `P0` milestone section's exit-criteria record → `closures/CR-0nnnn-p0.md`, `kind: phase` | M |
| `skills-map.md` | citations rewrite | M |
| `workflows/wf-0n-*.md` (5) + README | `WF-0nnnn-*.md`, stamped; README table generated | M + H |
| `adrs/000n-*.md` (6) + README | `adrs/ADR-<nnnnn>-*.md`; bullet header → front matter (`accepted` → `active`, `deprecated` → `retired`); README generated | M + H |
| `rfcs/00nn-*.md` (18) + README | `rfcs/RFC-0nnnn-*.md`; prose header → front matter, statuses mapped (`open` → `draft`, `accepted` → `active`, `landed` → `closed`, `dropped` → `retired`, `superseded` unchanged) with `kind:` (`process` 0001, 0003–0010, 0013–0016, 0018; `enhancement` 0002, 0017; `incident` 0012); README rewritten, index table dropped for `INDEX.md` | M + H |
...
| `plans/PL-00853-wk-671-process-conformance-audit-is-the-team-following-delivery-process-md.md` | `research/RS-0nnnn-w11-process-conformance-audit.md`, `kind: audit`, owner auditor; its two unowned follow-ups become `FD-` rows | M + H (the two rows) |
| `findings/register.md`, `findings/register.md` | one `findings/register.md`; each row gains `status:` (`active`, or `closed` where a **Resolved** annotation exists) and `decision:` (the existing Decision cell); the phase register's rows merge in with `phase: P1b` (RFC-756: no second copy); per-phase views come from `doc-index.py --phase`; Finding-id cells → `FD-n` with `was:`; header prose rewritten | M + H |
| `audit/findings/F*.md` (5) + README | `findings/FD-0nnnn-*.md`; README rewritten | M + H |
| `audit/work/*/README.md` (15), `closures/CR-00822-phase-record-1b-modelling-workbench.md`, `closures/CR-00821-phase-1b-exit-demo-uat-acceptance-record.md` | `closures/CR-0nnnn-*.md`, `kind: work` / `phase` | M |
| `closures/INDEX.md#closure-recordsmd`, `closures/INDEX.md#plan-reviewsmd` | split into `CR-` files (`work`, `review`); preambles → `closures/README.md` | M + H |
...
| `process/retrofit-impossible.md`, `process/security-posture.md` | → `process/` | M |
| `audit/file-census*.{md,csv}`, `research/RS-00953-file-taxonomy-draft-rfc-897-stage-1.md` | → `research/RS-…` | M |
| `findings/README.md` | deleted; content to `findings/` and `closures/` READMEs | H |
...
| `frontend/src/` | 144 | requirement ids and `WF-698` in comments, test titles and one component doc-comment | M |
...
**Amended 2026-09-03 — RL-1046 D5, a dated append; the paragraph above stands as filed and is not edited** (`docs/rulings/RL-01046-the-alias-class-the-disclosed-rows-and-the-run-s-conditional-window.md`, D5). Row **(f)**'s *"unchanged from `8f5d57d`"* is read as, and from this date reads as, *"unchanged across the migration on the snapshot, per file through the routing table; `8f5d57d` is the tree the baseline was taken at, not the comparand."* This makes RL-1044 §2.2's reading the sentence's own text rather than a reading held beside it. §7's preamble already scopes every other row to the migration PR's merge tree, and this note's own header records `8f5d57d` as the tree at which its counts were measured — so a literal comparison against that commit would hold the migration answerable for every commit made in between, which is not a property of the migration. The routing table is RL-1044 §3.3's, not `REDIRECTS.csv`, which is one-to-one as generated and cannot express a split source (RL-1044 §5.2). A row **(f)** computed against a hard-coded `8f5d57d` is the violation this append exists to make detectable.
...
## 9. Relation to RFC-897 and Rulings 55–65

| Item | Disposition |
|---|---|
| RFC-897 C1, C2 | lifted — the one migration C1 guarded against, done once while cheap |
| C3, C4, C5 | kept; `status:` (forward only) and `superseded_by:` are C4's only post-freeze edits |
| Stage 0 census; RL-941 | kept as inputs; §1.2 is those categories with one home each, plus the row families |
| RL-942 | superseded via its own override clause |
| RL-943 | kept — check 38 is it as code |
| RL-944 | superseded via its own override clause — every family has an id |
| Rulings 55–58 | absorbed — matrix generated; notes move into `rfcs/`; tombstone → redirects; id citation universal |
| RFC-897 Stages 2–5 | replaced by §1, §4, §5; Stages 3–4 are the downstream Works |
...
git grep -ohE 'ADR-0[0-9]{3}' -- backend packages frontend | sort | uniq -c   # 91 × ADR-703 in code
