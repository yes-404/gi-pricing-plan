...
family: proposal
kind: process
title: A machine-readable core for the delivery process, so the rules a script can check stop being prose
...
relates: []                     # ids only
was: docs/notes/0014-machine-readable-process-core.md
---

# A machine-readable core for the delivery process, so the rules a script can check stop being prose

## 0. What this note is, and what it is not

**A note decides nothing** (`docs/rfcs/README.md`). This one records a proposal, the
...
| # | File | Change | Nature |
|---|---|---|---|
| 1 | `CLAUDE.md` §15 (Team Process, line 292) | Add one sentence: "Its mechanical extract is `docs/process/delivery-process.core.json`; the markdown is authoritative, the extract is derived and drift-checked (RFC-895 §3)." | Amend |
...
**Deliberately unchanged:** `docs/findings/register.md` (spec §9 mechanism untouched),
`docs/workflows/WF-698…05` (domain, not process — spec §4 / CLAUDE.md §15's own distinction),
...
1. **Freeze** — file this note as RFC-895. *(this artifact)*
...
`CLAUDE.md`, `docs/notes/`, `.claude/roles/`, `.claude/skills/` (directory listing),
`docs/process/`, `docs/plans/README.md`, `docs/roadmap.md`, `docs/open-questions.md`,
`docs/findings/register.md`. Recorded here because a proposal whose citations were never
...
|---|---|---|
| 1 | Note number `RFC-842` | `0012` and `0013` are taken; next free is **`0014`**. Applied throughout |
...
- "`CLAUDE.md` §4" for `WF-698…05` → §4 Documentation Suite names them (`:95`). ✓
...
- "`CLAUDE.md` §13's reference rule" → §13's bullet *"A reference carries its scope and its
  measurement"* (`:260-264`, citing `RFC-777`). ✓
...
- All cited artifacts exist: `docs/process/agent-settings.md`, `docs/plans/README.md`,
  `docs/open-questions.md`, `docs/findings/register.md`, `docs/roadmap.md`. ✓
- The rulings record `docs/plans/PL-00845-rfc-840-rfc-841-adoption-reconciliation-and-rulings-2026-08-29.md` exists;
  the WK-671 plan family confirms the Work/Slice naming the appendix's runtime-state example
  uses. ✓
...
```json
{
  "meta": {
    ...
    "authoritative": false,
    "note": "The markdown spec is authoritative. This file is the mechanical extract: state machine, guards, vocabularies, and runtime-state schema for hooks/watcher scripts. Regenerate or diff-check against the md on every spec change (RFC-895 script C4).",
  ...
  "artifacts": {
    "findings_register": "docs/findings/register.md",
  ...
  "runtime_state_schema": {
    ...
    "example": {
      ...
      "position": { "project": "gi-pricing-plan", "phase": "2", "work": "WK-671", "slice": "WK-671-S3", "flow_step": "implement_green" },
      "retry_counters": {
        "work:WK-671:replan": 0,
        "slice:WK-671-S3:fix_loop": 1
      },
