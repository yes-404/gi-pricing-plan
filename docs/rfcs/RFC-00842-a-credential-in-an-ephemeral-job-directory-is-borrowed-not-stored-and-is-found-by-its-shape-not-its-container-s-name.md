...
family: proposal
kind: process
title: A credential in an ephemeral job directory is borrowed, not stored, and is found by its shape, not its container's name
...
relates: []                     # ids only
was: docs/notes/0012-a-credential-is-borrowed-not-stored.md
---

# A credential in an ephemeral job directory is borrowed, not stored, and is found by its shape, not its container's name
...
excerpt containing one appears anywhere in this note** — per `docs/rfcs/README.md`'s own
...
**The instance.** A Slack posting token lived only inside a WK-670 job directory. That
...
There is none in the sense `docs/rfcs/README.md` means — no maintainer request produced
