...
family: proposal
kind: process
title: Slim the roadmap: split the plan from the archive
...
relates: []                     # ids only
was: docs/notes/0009-slim-the-roadmap.md
---

# Slim the roadmap: split the plan from the archive
...
4. **"Every phase has a stated outcome and exit criteria" is already CLAUDE.md §9 + §14.**
   Phase 1b's exit criterion is `WF-698` end to end; the §14 plan review is the exit-criteria
...
[[RFC-812]]). The roadmap keeps only the forward-looking plan: the header, the workstream
...
Schedule the restructure as its own workstream after WK-665: move the archive to `docs/audit/`,
slim the roadmap, and re-run the closure tripwire and `audit-docs.py` to prove no requirement
or status is silently dropped. The notes RFC-812 and RFC-813 land together.
