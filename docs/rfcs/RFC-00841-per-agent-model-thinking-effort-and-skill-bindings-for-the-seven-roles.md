...
relates: []                     # ids only
was: docs/notes/0011-per-agent-model-and-skill-settings.md
---

# Per-agent model, thinking effort and skill bindings for the seven roles
...
- **The decision-maker's hard boundary is correctly derived.** *No write access to any code
  worktree* is not a theoretical precaution: during WK-670 the decision-maker wrote into the
  ...
  fact — see `docs/plans/PL-00845-rfc-840-rfc-841-adoption-reconciliation-and-rulings-2026-08-29.md` Part B10. The
...
Practice resolves it in one direction and the document should say so: in WK-670 the auditor
authored `docs/closures/CR-00834-work-item-record-wk-670-rate-tables.md`, the register rows, and the spec-change PRs #308
...
records with no owner. This is the same distinction [`RFC-840`](RFC-00840-a-layered-slice-based-workflow-project-phase-work-slice-gated-at-every-layer.md)
...
point in WK-670 needed, or think as hard as possible on every ruling, which is cheap because
...
That is a real argument, but it has a cost the document does not price. In WK-670 the auditor
...
Nothing. Per `docs/rfcs/README.md`, a note is raw material. The tool-scope amendment in
...
with [`RFC-840`](RFC-00840-a-layered-slice-based-workflow-project-phase-work-slice-gated-at-every-layer.md) item 2.
...
Hold until [`RFC-840`](RFC-00840-a-layered-slice-based-workflow-project-phase-work-slice-gated-at-every-layer.md)'s items are ruled — this
...
[`RFC-840`](RFC-00840-a-layered-slice-based-workflow-project-phase-work-slice-gated-at-every-layer.md): the source reached this project as a loose
file outside the repository, and a note citing a path that will not survive is the custody
failure [`RFC-778`](RFC-00778-seven-deferred-items-with-no-durable-custody.md) records.
...
> team structure (`TEAM-STRUCTURE.md`, WK-670-era rules and incident lessons).
>
...
>
> **Cost rationale:** balance was the binding constraint in WK-670 (begin-close threshold
...
>   incidents in WK-670: cross-worktree checkouts discarded uncommitted work). Rule text to
...
>   (hard boundary after the WK-670 stop-order incident): ruling records only.
...
>   — the WK-671 lesson), gap lists, closure records, register deferral rows with named
...
> 5. **Per-slice audit axes** (WK-671 lesson) written into the auditor's agent body.
