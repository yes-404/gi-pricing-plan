...
family: proposal
kind: process
title: Two rules for reading an artifact: the one you name, and the one you verify
...
relates: []                     # ids only
was: docs/notes/0006-two-rules-for-reading-an-artifact.md
---

# Two rules for reading an artifact: the one you name, and the one you verify

## Why this is a separate note from RFC-778

[`RFC-778`](RFC-00778-seven-deferred-items-with-no-durable-custody.md) is a backlog: six items, each of
...
[`RFC-756`](RFC-00756-duplicated-status-in-claude-md-goes-stale.md) records, reproduced one level up: a single
status over two things whose lifecycles differ.

Both rules are the same defect as [`RFC-777`](RFC-00777-a-reference-that-resolves-only-in-the-writer-s-context.md)
seen from the reader's side. `RFC-777` is about writing a reference that resolves only for
...
**The instance.** Reviewing PR #167 (`OQ-549` and the double-lift strike), Claude named
...
[`RFC-777`](RFC-00777-a-reference-that-resolves-only-in-the-writer-s-context.md)'s missing-qualifier defect
...
**The instance.** `docs/workflows/WF-00698-dataset-to-approved-model.md` step D5 listed double lift among
the outputs of `compute_diagnostics()`, and cited `02` FR-171/172/183. The citation is
correct: `FR-171` is the universal-diagnostics requirement and is exactly the right
...
`FR-171` coverage under check 14 and passed check 21, independently of whether
`FR-171` says what D5 says it says. That gap is filed as
[`RFC-778`](RFC-00778-seven-deferred-items-with-no-durable-custody.md) item (f).
...
[`RFC-777`](RFC-00777-a-reference-that-resolves-only-in-the-writer-s-context.md)'s proposed fourth bullet
...
from failures in its own work during WK-664, one of which was caught by a peer session and one
