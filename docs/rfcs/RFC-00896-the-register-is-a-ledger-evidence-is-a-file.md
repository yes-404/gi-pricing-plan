...
relates: []                     # ids only
was: docs/notes/0015-the-register-is-a-ledger-evidence-is-a-file.md
---

# The register is a ledger; evidence is a file

One-line thesis: `docs/findings/register.md` has evolved a real discipline — ownership
...
2. **Unowned rows have no decay path.** F45(ii)+F46, F47, and F28's carried P5 are
   *correctly* unowned — each says why — but nothing structurally forces them onto an
   agenda. This is RFC-778's "deferred items with no durable custody" shape, sitting inside
...
5. **The owed list is compiled by hand at the moment of highest load.** F41, verbatim: the
   WK-671 close's owed list "already runs to thirteen items — a list that lost NFR-502/501
...
- **P3. `scripts/register-lint.py`, wired into the gate.** Deterministic, no LLM —
  RFC-895's C-class, pointed at this file. Checks: every Decision parses to a P1 shape;
...
- **P4. Split ledger from evidence.** Evidence essays move to
  ...
  when *status* changes — RFC-756's duplicated-status lesson applied to the register
...
- The register (and, after P4, the findings files) remain the record; scripts are views
  and guards, never a second source — same rule as RFC-895 §3.
...
- **Not** per-clause requirement-marker granularity (F35/F36/F43/F44/F48's shared shape).
  Real, already decided elsewhere (OQ-654 option f), owned by `req-coverage.py`'s
...
- **Not** a schema/JSON register. The table stays markdown and human-first; P3 parses it
  as-is. (If RFC-895's core-extract pattern later earns extension here, that is its own
  note.)
...
| # | File | Change | Nature |
|---|---|---|---|
| 1 | `docs/findings/register.md` header | P1 grammar paragraph + P2 decay sentence | Amend |
| 2 | `docs/findings/register.md` unowned rows (F45/F46, F47, F28-P5 at `7db62ca`; re-enumerate on filing) | Name each row's decay event per P2 | Amend, one line each |
...
| 7 | `docs/process/checklists/work-item-close.md` | Owed list is `register-owed.py` output, cited with tree; carried findings written per P1 grammar | Amend |
| 8 | `docs/process/checklists/phase-close.md` | Same at phase level | Amend |
...
Deliberately unchanged: `docs/closures/INDEX.md#plan-reviewsmd`, `closure-records.md`, existing
...
- **Q2 — Lint severity on legacy rows:** red-gate immediately, or warn on rows predating
  ...
  flag-day, same posture as RFC-895's Q3 on legacy plans.
...
- **Q5 — Findings file naming** where ids carry both forms (`FR-240 (F-W9-3)`):
