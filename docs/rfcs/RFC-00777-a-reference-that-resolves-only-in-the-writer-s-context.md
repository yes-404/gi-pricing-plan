...
family: proposal
kind: process
title: A reference that resolves only in the writer's context
...
relates: []                     # ids only
was: docs/notes/0004-a-reference-that-resolves-only-for-the-writer.md
---

# A reference that resolves only in the writer's context
...
|---|---|---|---|
| 1 | `generate-contracts.py` rewrote `custom-objective.schema.json` | **directory** — generated or authored? | Under the authored reading the sentence describes an **ADR-704 violation**: the contract flow is one-way and nothing generates the authored side |
...
| 6 | "nothing between `5f915d5` and `946725f` moved a contract **shape**" | **shape = the file, or its contents?** | Contents moved (4 files); membership did not. The loose reading reaches a phase reviewer as evidence that contracts do not drift — the exact opposite of what `OQ-649` is filed to ask |
...
| 10 | "`OQ-653` is now unconditional — **take it**" | **the verb's object** — an id's *availability* was confirmed and phrased as an instruction to *consume* one | Neither a count nor an enumeration: a verb applied to the wrong noun. A permanent requirement id, allocated once, **cannot be un-allocated** (`CLAUDE.md` §5). Cost one message because it was asked about rather than assumed |
...
That is the same reasoning `RFC-756` used for duplicated status: the copy nobody re-derives
...
skill is the `RFC-756` duplication pattern, and dating the copy mitigates staleness without
