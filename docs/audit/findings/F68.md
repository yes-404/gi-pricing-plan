...
Evidence essay for the register row self-named `(F68)` in `docs/audit/register.md`. The
...
The lead filed `docs/plans/2026-09-01-maintainer-delegation-and-nt-0019-precedence.md` — a
...
had no reason to carry. The lead did not patch the check (a shared gate, and NT-0019's own
...
`docs/plans/2026-08-30-nt-0017-maintainer-decisions.md` exists at `89dd2b1`, is a ruling
record in every respect that matters here (three maintainer policy decisions, recorded;
title "# NT-0017 — the maintainer's three policy decisions, recorded (2026-08-30)"), carries
...
it has one (it does not). NT-0019 §5.2 independently corroborates that this is not an
...
Some existing multi-ruling records in `docs/plans/` (per NT-0019 D9, "legacy multi-ruling
...
kind, which is a larger change than patching this one check and is what NT-0019 already does
...
- **Not fix-before-close.** Nothing live is failing today; the one real instance
  ...
  gate.
- **Superseded, not merely fixed, by NT-0019's own migration.** Per NT-0019 §8 Sequencing,
