...
**Register row:** `docs/audit/register.md`, the row self-naming `(F27)`. This file is the
...
Migrated 2026-08-31 as the NT-0015 P4 exemplar, chosen because it is the longest live row
and because Ruling 53 (`docs/plans/2026-08-30-nt-0015-q1-q5-rulings.md`) names this row's
...
all three as `"later-phase — 03 rating"`, a reason that expired when W9 and W10 built them.
...
`cancellation`, added 2026-08-18 with FR-RATE-63.

**`rate-table.schema.json` vs `model-schema`'s `RateTableVersion`**: missing `storage`
(added 2026-08-18 with FR-RATE-62 per `03:310`), `cells`, `created_by_operation`,
...
what FR-RATE-62 defines.** Its `required` list carries `rows`, typed `array` with
`minItems: 1`; `_cells_match_storage_mode` raises *"a parquet version addresses its cells by
a BlobRef, never inline rows (FR-RATE-62)"* whenever `storage != ROWS`. So a
parquet-stored `RateTableVersion` — the whole point of FR-RATE-62's spill-above-threshold
...
with a `_cells_match_storage_mode` validator, so FR-RATE-62 is implemented and only the
Phase-0 hand-authored contract was left behind.

## Decision — reasoning

**Carry forward with an owner, and W9/W10 are NOT reopened.** Reopening a Work close is the
...
Ruling 12's fourth obligation on Task 1.4, which keeps `scoring` from becoming the fourth
uncompared shape, the only part that is in W11's own scope.
...
**Owner decided (Ruling 29): the §14 review at W11's close**, bundled with F29 and the
