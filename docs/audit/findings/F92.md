...
declares them. That template change is W37-6's own §7.1 Task 1, under Ruling 70's licensing
instrument.
...
leaf plan [`../../plans/2026-09-02-w37-6-migration-run-leaf-plan.md`](../../plans/2026-09-02-w37-6-migration-run-leaf-plan.md)
...
[`F87.md`](F87.md) was filed for this exact shape, in its own words: the fact *"lived only in a
...
**`NT-0005`, quoted by F83's own register row, is the rule**: *"a deferred item with no owner is
...
canonical home is elsewhere — `NT-0003`'s failure, already live in the same W37 row this close
