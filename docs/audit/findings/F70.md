...
Evidence essay for the register row self-named `(F70)` in `docs/audit/register.md`. The
...
tonight and you have caught several"). Verified independently below; one detail of the
relay does not hold up and is corrected in its own section rather than silently dropped.
...
docs/plans/2026-08-24-w32-11-certificate-floors-and-two-generated-sides-ledger.md
...
process adoption (NT-0010/0011) and all of W33 through W36.
...
```
$ git grep -oi "ledger" origin/main -- docs/plans/2026-09-01-nt-0019-id-standard-map-plan.md | wc -l
10
```

The load-bearing instance, `docs/plans/2026-09-01-nt-0019-id-standard-map-plan.md`, Slice
W37-1:
...
## What the W37-1 execution actually did — corrected from the relay

**Confirmed:** PR #562 (`docs(process): lift NT-0019's document-id standard and its 13
family templates (W37-1)`, still open, not merged) creates no `docs/plans/*-ledger.md` file
(`gh pr view 562 --json files` lists 14 files, all under `docs/_templates/`,
`docs/process/document-ids.md` and `docs/README.md` — none a ledger), and its Summary and
Verification sections do carry the substance the acceptance line asks for: "checked by
reversing the shift and diffing against `docs/notes/0019-one-id-per-document.md` at
`1b3bc21` — zero differences," repeated as a checked test-plan item. **The diff verification
the acceptance line requires is genuinely present, in the PR body, not in a ledger.**

**Not confirmed, and searched for directly rather than assumed:** the relay's claim that the
executor "declined to revive a dead convention unilaterally... and flagged it." Searched
PR #562's body (`gh api repos/.../pulls/562 --jq '.body'`, grepped case-insensitively for
"ledger", "dead convention", "lapsed", "declined" — zero matches), its one commit message
(same search, zero matches), and its comments (`gh pr view 562 --json comments` — empty).
Also checked every open or merged PR whose title mentions "W37" or "ledger" — none besides
#562 touches this slice, and none discusses a ledger. **If this reasoning happened, it left
no artifact this review can find.** That is not a smaller version of the finding — it is the
same finding one layer down: even an executor's own judgment call about a lapsed convention,
if real, is not itself durable here. The corrected shape is not "the gap was flagged and
handled well" but "the gap was worked around, silently, and this review is the first place
it is written down."

## The lead's disposition (recorded here so this row cites it rather than reopening it)

The lead ruled *proceed without a ledger* for W37's slices, under the replan-vs-proceed call
...
directory, its own lint, an owner in §1.6) would create files in a form NT-0019 is about to
...
NT-0019 lands it as a family.** This is a disposition on *this slice's* instance, not a fix
...
F58 (`docs/audit/register.md`): `.claude/roles/watcher.md` states the watcher writes runtime
...
- **Not fix-before-close.** The lead's disposition already resolves W37's own exposure; this
  row is the underlying gap the disposition explicitly does not fix.
- **Not a criticism of the W37-1 executor's engineering** — the diff verification performed
  is sound and is exactly what the acceptance line needed substantively. The gap is that the
  form (ledger vs. PR body) diverged from the plan's own text with nothing recording that it
  had to.
- **Proposed disposition** (a proposal, not a verdict — CLAUDE.md §13's four verdicts and the
  ...
  NT-0019 lands `LG-` as a first-class family with its own directory, lint and §1.6 owner —
  ...
  admitted to be mid-supersession. Which of the two (wait for NT-0019, or a cheap interim
