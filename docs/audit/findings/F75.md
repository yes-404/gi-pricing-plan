...
Evidence essay for the register row self-named `(F75)` in `docs/audit/register.md`. The
...
> **backgrounded the poller**, and ended its turn anyway. Three stalls on 2026-08-30 (W11
...
## The keyword sweep, reproduced independently — numbers differ from the dispatch, conclusion does not

Case-insensitive `grep -c` for `background|wait|notif|turn|sleep|async|long-running|Monitor`
against each of the seven charters, at `main` = `39ee30c`:

| File | This review's count | Dispatch's stated count |
|---|---|---|
| `executor.md` | 9 | 7 |
| `lead.md` | 4 | 1 |
| `reporter.md` | 3 | 2 |
...
The raw counts disagree in magnitude and, for `watcher.md` vs `reporter.md`, in *rank* — a
discrepancy most likely explained by a different exact command (matching lines vs total
occurrences), not investigated further because it is not load-bearing: **what matters is
which hits are the rule, not how many keywords a file contains**, and every non-executor hit
was read directly rather than counted. `reporter.md`'s three hits are `Monitor` used as the
...
two hits are about a different finding (F31's stale-timestamp lie) and about proving a
`Monitor` task is alive, not about ending a turn while one runs. **None of the six
non-executor charters contains the rule** — confirmed by reading every matched line, not by
the count. The three-way zero (`auditor`, `decision-maker`, `planner`) is the one number
this essay and the dispatch agree on exactly.
...
several findings has precedent here (F28, the NT-0010/0011 pilot's fourteen sub-findings).
...
## NT-0019 §1.6 — charter ownership, read directly

Same standard as F74, not re-derived: `docs/notes/0019-one-id-per-document.md` §1.6,
