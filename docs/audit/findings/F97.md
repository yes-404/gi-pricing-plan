...
'\bF[0-9]{2,3}\b' -- docs/audit/register.md docs/audit/findings/` at `e97b97a` returns a
...
`docs/notes/0012-a-credential-is-borrowed-not-stored.md:16-19` states its incident's shape:
...
**And the same asymmetry `NT-0007` and F84 record applies**: a guard that aborts announces
itself; a residue no check covers does not. The predecessor's halt was *correct* — the
handover (`docs/plans/2026-09-03-w37-6-second-fail-handover.md` §10.1) discharges the halt
