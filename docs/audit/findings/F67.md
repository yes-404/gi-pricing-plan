...
Evidence essay for the register row self-named `(F67)` in `docs/audit/register.md`. The
...
- **The defect is repaired in the local script**, and this row plus `watcher-state.md` are
  ...
  finds only this finding's sources and a W11-era plan reference to a predecessor path) —
