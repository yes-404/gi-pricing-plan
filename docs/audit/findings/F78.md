...
Evidence essay for the register row self-named `(F78)` in `docs/audit/register.md`. Filed
...
roadmap rows into one draft (Ruling 91) and refuses on two conditions rather than choosing
silently. Its own docstring states both:

> The one thing this still refuses on: a work whose several rows disagree on status
> (Ruling 91 obligation 1), or a work whose occurrences span more than one phase section
...
`_discover_roadmap`'s status-disagreement refusal (Ruling 91 obligation 1) **is** tested
