# F86 — Ruling 49's decay rule has no faithful check, the wrong population, and a blind backstop
...
makes them live**, because Ruling 49's own text names the review as the fallback when the check
does not fire — and the tool that builds every review's agenda cannot see the shape either.

## The rule

Ruling 49 Text B (`docs/plans/2026-08-30-nt-0015-q1-q5-rulings.md:387-389`), copied into
`docs/audit/register.md`'s own header:
...
Run on a clean tree, because the tool refuses a dirty one (Ruling 52). **12 of 83 rows surface.**
...
**Ruling 50 is where this became invisible.** Its §3 acceptance item names three deliberately
...
three exist. Ruling 50's own text is sound — it states an observable violation.** What the
...
**The violation, in Ruling 29's form:** an unowned row whose Decision cell names neither a
