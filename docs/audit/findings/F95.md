# F95 — two of NT-0019's nine declared Document families, `WF` and `FD`, are absent from `doc-id.py`'s migration maps
...
**NT-0019 §1.2 declares nine Document-kind rows** (`docs/notes/0019-one-id-per-document.md`,
...
**110** of those classify `"none"`. **45 of the 110 are under `docs/audit/`**, which `NT-0019
§4` step 4 (`:279`) requires dissolved — *"Move into the family directories; dissolve
`docs/audit/`; delete the `.claude/notes/` stubs"* — and is not, because among other gaps
...
a gap that does not," `NT-0007`'s asymmetry) **in the one family whose gap the visible `none`
...
not). Checked first, per `NT-0003`/`NT-0006`: no existing register row or finding names
...
`docs/audit/findings/F*.md` with `owner: auditor` (§1.6) and `was:` per NT-0019's own worked
