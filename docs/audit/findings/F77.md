...
## Update, 2026-09-02 — built, and proven on the broken input Ruling 94 named

Landed in W37-5c (scope item 4 of
[`../../plans/2026-09-02-w37-5c-slice-decision.md`](../../plans/2026-09-02-w37-5c-slice-decision.md)),
...
emit. A draft-level check could not see Ruling 94's mutation at all, which is the concrete
...
`_stamp_header`'s skip tuple — writes the ten real `W5 ... (in progress, not closed)`
records of `docs/audit/closure-records.md` through it, and measures **10 `slice:` values and
...
signature Ruling 94 §4 names as the violation is a check that reads the same before and
after.
...
**Ruling 84 §4's third item is built in the same pass**, as Ruling 94 §4 obliges — *"an
...
its broken input is again real: withhold the roadmap's `W5` draft and `_write_document_drafts`
