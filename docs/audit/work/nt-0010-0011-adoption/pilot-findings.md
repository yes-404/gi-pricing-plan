...
| **P4** | **fixed** — `delivery-process.md` §4 now distinguishes the process-slice from a plan's `Slice N` heading, and says to disambiguate at each use. Ruled for W11 (five sequential process-slices, frozen map untouched). **The definitional defect remains**: `delivery-process.md` must distinguish the process-slice from a plan's slice heading, or the plan template must stop using the word for the group | §15 step 7 |
