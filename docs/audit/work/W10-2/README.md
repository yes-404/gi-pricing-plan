...
Derived from `docs/plans/2026-08-28-w10-rate-tables.md` slice W10-2 (T1–T4) first, then
evidenced. W10-2 delivers the pricing actions behind FR-RATE-16 (seed a table from an
approved model's relativities), FR-RATE-17 (cell-level diffs with exposure weighting) and
FR-RATE-19 (named validation on save, checked before the version persists):

- T1 seeding: `seed_from_model()` in `pricing_core.rate_tables.operations`, approval gate
  (FR-OVR-14 → `PIN_NOT_APPROVED`), `seeded_from` `{model_ref, seeded_at}` lineage
- T2 diffs: `diff_vs_previous()` / `diff_vs_seed()` with caller-supplied exposure weights
  (DP1), exposure-weighted mean change; parquet deferral (FR-RATE-62)
...
The parquet half of FR-RATE-62 was out of W10-2's reach (nothing can yet write parquet
...
**Requirement coverage:** FR-RATE-15/16/17/19 each carry `@pytest.mark.req` evidence in
test files (req-coverage green at head `4b33b28`: FR-RATE-15 ×2, FR-RATE-16 ×9,
FR-RATE-17 ×8, FR-RATE-19 ×2, FR-RATE-62 ×2 files; no nonexistent-ID FAIL). Endpoint
evidence is marker-visible: the API suite's 13 tests carry markers (F1 gap from the first
audit pass, fixed and re-audited).

| Requirement | Verdict |
|---|---|
| FR-RATE-15 | ✓ delivered and tested — immutable version, change_note required (bad-body test asserts its absence → 422) |
| FR-RATE-16 | ✓ delivered and tested — seeded_from lineage, approval gate, extraction (non-log links skipped), decimal strings (R2) |
| FR-RATE-17 | ✓ delivered and tested at the core — diff_vs_previous/seed, changed cells, max-abs and exposure-weighted mean (hand-computed weighting test); **carry-forward**: the endpoint passes no weights (DP1 substrate absent — the portfolio-dataset join is built nowhere) — finding F-W10-2 |
| FR-RATE-19 | ✓ delivered and tested — four named failure modes, checked before the version persists; boundary codes asserted (RATE_TABLE_KEY_DUPLICATE) |
| FR-RATE-62 | (partial) parquet half = W10-3 scope — W10-2 records the 501 refusal via the dated §5.2 amendment and proves it on deliberately broken input (hand-inserted parquet version → 501 RATE_TABLE_PARQUET_UNBUILT) |
...
|---|---|---|---|
| F-W10-2 | FR-RATE-17 exposure-weight wiring: pricing-core accepts caller weights (DP1) but the diff endpoint passes none; the portfolio-dataset join is scheduled in no slice | carry forward with an owner — portfolio-dataset integration | closed-with-findings |
