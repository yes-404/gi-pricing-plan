...
W37 row still reads *"W37-6 has not run and its go-ahead has not been asked for"* at its most
...
1. **"NT-0019 §7(b)–(k) hold at W37-11's close, not at W37-6's" is not what the filed record
   says.** `docs/notes/0019-one-id-per-document.md:426` states acceptance items (a)–(k) as one
   list with no per-item owner; the owner split lives in
   `docs/plans/2026-09-01-nt-0019-id-standard-map-plan.md:828`, and it reads **"Scope:
   ...
   (`docs/plans/2026-09-03-w37-6-time-boxed-delegation.md` §2, §7.3), and the W37-11 slice text
...
2. **"Ruling 68's class-6 extension ratification" and "the lettered-ruling nested-span floor"
   do not resolve against `docs/` at `198ea5d`.** `git grep` for "class-6", "class 6",
   "ratif", "nested-span", "nested span" and "lettered" across `docs/` finds Ruling 68's own
   six-class permitted-diff predicate (`docs/plans/2026-09-02-w37-6-migration-run-leaf-plan.md`
   and siblings) and `_discover_lettered_rulings` (the Ruling-A1–A3 discovery function,
   `docs/audit/findings/F90.md`), but no ratification of a class-6 *extension*, and no finding
   ...
   (#26 "File Ruling 101 as §2.5 of the Ruling 100 record", #27 "Append §8 to the delegation
...
rows**, one further excluded as opening with a resolution marker (F28, unrelated to W37 and not
re-verified here — out of this review's scope). Split by whether the row predates W37 or was
filed inside this window, because the two need different treatment.

**Eleven rows predate W37 and are not re-derived here** (`FR-RATE-25`/`F-W9-3`, F27, F29, F31,
...
detail and none has moved since (`git diff --stat 567eea2..198ea5d -- docs/audit/register.md`
...
**Ten rows are W37's own** (F86, F87, F88 limb 2, F89, F90, F91, F92, F93, F94, F96). Verified
directly against `docs/audit/register.md` at `198ea5d`, not recalled from the brief:

| Row | Register text (verified) | This review's disposition |
|---|---|---|
| F86 | Ruling 49's decay rule — carry forward, explicit decay, no owner, written 2026-09-02 at W37-5c's close | **No change.** The disposition was already made at the right event; this review does not re-open it. |
...
| F88 (limb 2) | `docs/audit/phases/1b/register.md` reached by no discovery function — limb 1 discharged, limb 2 open, owner W37-6, decays to the §14 review **at that close** if undischarged | **Not yet decayed here** — its named event is W37-6's close, which has not happened. Listed for completeness; no disposition owed from this review. |
...
| F90 | Check 37 / `###`-nesting — dispositioned by Ruling 96/97, re-measured at `15ed00d` | **No change.** Already ruled at the right authority level (maintainer, via D1/D2), already re-measured; this review's job is not to re-litigate a ruling. |
| F91 | NT-0014 runtime-state writer stale since 02:02Z on 2026-09-02 — carry forward unowned | **No change.** Confirmed independently: `write_runtime_state.py show` at this tree still reports `written_at: 2026-09-02T02:02:13Z`, position pinned at slice `W37-4` — nineteen hours and seven slices stale. Decay event is the next watcher re-arm; not this review's to own. |
...
| F93 | NT-0019 §1.5 vendored-manifest header requirement contradicts the maintainer's own exemption ruling — unowned, decay event named as "the next amendment to NT-0019, failing which the next §14 review" | **Decayed here. Disposition: carry forward, unowned, into W37-11's closure record** — this is exactly the shape W37-11's charter already names ("a verdict for every requirement of the note that has no evidence"), and NT-0019 is not otherwise due an amendment before then. Not a W37-6 gate item: it does not stop or blind the run. |
...
and nothing in this window's evidence bears on that axis — W37 is a documentation-corpus
migration, not a requirement delivery, so the closest completion measure is NT-0019 §7's own
acceptance items, and W37-6 has not closed. **Answer: not applicable to derive a completion
number now; the honest statement is that `docs/roadmap.md`'s W37 row is itself out of date**
...
task list's live item #25 names `OQ-OVR-11` claimed by two generated ids — silently overwrite
...
`assert classified == seen` pattern `docs/audit/register.md`'s own header already names as the
...
or `docs/notes/0019-one-id-per-document.md` gains one line — every `dict` built inside
...
**`docs/roadmap.md`'s W37 row is stale, confirmed by direct read (§ preamble above)** — it
predates the delegation, both renewals, and both halted windows. This is the same defect class
NT-0003 exists to name (a duplicated/carried status going stale) and F95 exists to exemplify
one level down.

**F95's register row is stale, verified directly rather than taken from the handover.**
`docs/audit/register.md:136` reads: *"not started, `WF` half; `FD` half in progress."*
...
register rows at all** — verified by their absence from `docs/audit/register.md` at this tree:
...
structural drift in `docs/audit/phases/1b/register.md`. The shared task list's own items #17
...
shape `docs/audit/register.md`'s own header warns against ("a deleted row leaves every citation
...
The map plan (`docs/plans/2026-09-01-nt-0019-id-standard-map-plan.md:828`) gives W37-11 a
...
2. **Amendment 3's backfill clause ("the backfill is W37-11's") — the mechanism is right, the
   ...
   window) and is already working as designed — Ruling 100's own record carries
   `implementation: owed` exactly as specified. But "the backfill is W37-11's" was written as a
   blanket routing rule before any concrete backfill existed to route. Now one does: Ruling 98
...
migrated tree itself non-compliant with NT-0019's own ruled terms (Ruling 98's implementation,
...
structure and its dependency chain (`docs/plans/2026-09-01-nt-0019-id-standard-map-plan.md:364-
...
| 4 | Drift | Roadmap W37 row stale; F95's register cell stale (verified against `2f0467e`); three found-not-filed findings (F12 range, `was:` corruption, F23/24/25) | Update the register cell; file the three findings before the next go-ahead ask |
...
rule rather than silently acted on: the NT-0019 §7 owner split is (a)-(i) W37-6 / (j)-(k)
W37-11, not (a) vs. (b)-(k); and "Ruling 68's class-6 extension ratification" / "the
...
- `docs/notes/0019-one-id-per-document.md:424-430` (§7 acceptance, §8 sequencing) — read
  directly.
- `docs/plans/2026-09-01-nt-0019-id-standard-map-plan.md:364-368,824-844,892-894` (dependency
  table, Slice W37-11, spec-coverage map) — read directly.
- `docs/plans/2026-09-03-w37-6-time-boxed-delegation.md` (all seven sections) — read directly
  in full.
- `docs/plans/2026-09-03-w37-6-window-handover.md`,
  `docs/plans/2026-09-03-w37-6-renewed-window-handover.md` — read directly in full.
- `docs/audit/register.md` rows F86–F96, F92, F95 — read directly at `198ea5d`.
- `docs/roadmap.md:382` (W37 row) — read directly.
