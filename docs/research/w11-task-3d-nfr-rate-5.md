# NFR-493 — batch scoring throughput per worker (WK-671 Slice 3, Task 3D)

`docs/specs/03-rating-engine.md` §9 (`:886`): *"Batch scoring ≥ 1 M risks/hour per worker
(NFR-455), linear in workers."* **The requirement has two clauses, and this note measures
...
`docs/plans/PL-00849-wk-671-slice-3-batch-scoring-the-pure-transform-the-checkpointing-handler-and-the-route.md` Task 3D's acceptance standard, which
...
**Not a CI gate.** RL-872's governance carried over unchanged
(`docs/rulings/INDEX.md#2026-08-29-w11-slice1-rulingsmd`): a timing assertion on a shared runner
...
- **Fixture — no `model_call` step, and this is a disclosed simplification, not an
  ...
  booster. Row width: **1 input column.** This is far simpler than NFR-489/490's own
  ~200-step motor structure (`bench-rating.py`), and **this note's numbers are not
  comparable to NFR-489/490's** — the same disclaimer `w11-3b-compiled-for-content-hash-
  delta.md` makes for its own narrower fixture. NFR-493's budget is about the batch
  **pipeline** (chunking, the manifest, scratch I/O, the final parquet write) at scale;
  per-row algorithm cost is NFR-489/490's budget, already measured separately. This note
...
- **Tree.** `3dc8d6b1f8920aa135736328677be39a9d0df043` — `origin/main`'s tip after WK-671
...
| budget (NFR-493, throughput clause) | ≥ 1,000,000 risks/hour/worker |
...
the final content-addressed parquet write — sustains throughput well above NFR-493's
...
- **Throughput against a realistic motor structure.** No `model_call`/GBM step is in this
  fixture. A production batch run against NFR-489's own ~200-step structure would spend
...
- **Linearity across workers (NFR-493's second clause).** Only one worker ran. "Linear
...
- **Linearity clause — not started.** No second worker was run in this slice; nothing
  under WK-671 attempts a multi-worker throughput measurement. This is not a deferral of a
  failing result — it is an unattempted measurement, and it is named here rather than
  silently folded into a single "NFR-493 met" line, which is precisely the shape that
  produced the NFR-499 clause-collapse this note's own dispatch cites as the standing
  cautionary instance.
