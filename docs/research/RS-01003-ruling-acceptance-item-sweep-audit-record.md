...
|---|---|
| **Date** | 2026-09-02 |
| **Auditor** | this session, dispatched by the lead to sweep every filed ruling's acceptance item after RL-1000 routed it |
...
| **Corrects** | [RL-994](../rulings/RL-00994-the-fixture-is-rebuilt-on-the-property-not-the-level-and-the-container-is-identified-positively.md) ("twelve ruling records now carry acceptance items") and [RL-1000](../rulings/RL-01000-the-property-stands-the-instrument-is-amended-because-a-broken-input-need-not-be-a-document.md) ("Fourteen ruling records now carry acceptance items") — both figures were asserted in the paragraph that routed this sweep, not independently derived. §6 states the growth-versus-undercount split per figure. Neither ruling is edited; this record is filed alongside them, findable from a `corrects:`-shaped cross-reference in the same spirit as the `614c92c` correction (`CLAUDE.md` §15). |
...
59-61) and RL-916's own phrasing ("Two tests, each stated as a violation that must become
...
RL-907, self-titled "The acceptance test, stated as the violation that must become
...
Implements the enumeration half of this sweep as a re-runnable script, carrying RL-985's
property (`docs/rulings/INDEX.md#2026-09-02-w37-guard-arithmetic-and-ledger-family-rulingsmd`): **every
...
| `exception` | 1 | RL-916, hand-verified, named because a regex fitted to one sentence would not generalise |
...
RL-916" framing this sweep was asked to commit understates the corpus by ten rulings. Each
...
hortatory," and (RL-950, its own sub-case) a re-confirmation of a **different, named**
...
the WK-697-series form (`### N. Acceptance — the violation that must become detectable`) going
...
**RL-986 §4 item 2 (VACUOUS AT BIRTH) — REPLACE shape.** Original text: *"A check that no
...
record, so the check can never see the fixture it names. Register **F77**. Ruled by RL-1000
...
**RL-979 §4 item 2 (INVALIDATED).** Original text named "a fixture whose only level-2
heading is followed by three record headings" in `docs/closures/INDEX.md#plan-reviewsmd`. Was constructible
when written — RL-979 measured "exactly one level-2 heading" at its own pin. PR #609
(`2fbce0c`, confirmed an ancestor of this record's pin) demoted that heading to level-3;
`grep -c '^## ' docs/closures/INDEX.md#plan-reviewsmd` returns **0** at this pin. The fixture the item names
cannot be built the way it is stated. Rebuilt by RL-994 on a property (shared heading level
with no higher level anywhere in the file) rather than a specific level number.

**RL-951 (INVALIDATED) — documented, deliberate trade, not a silent accident.** §4
...
(`scripts/audit-docs.py:55-64`) names the resolution: RFC-937 §5.5 gave slot 30 to its own
...
`docs/REDIRECTS.csv`, not the old notes root under `.claude`. So: today, neither of RL-951's two named
...
**RL-995 §4 item 3 (INVALIDATED) — confirmed by a later commit, not merely by a later
ruling.** Original text: *"Each of the three new `RL-` records carries an `owner:` that is not
`decision-maker`."* RL-984 (`7082c38`/#618, landed during this sweep) reversed the decision
...
— the departure regexes are deleted, not left inert. RL-995 item 3's assertion (`owner ≠
...
unrelated reason (implementing RL-984's own obligation, not this record's finding).** A
...
**RL-984 §3 item 3 / §4 item 3 (VACUOUS AT BIRTH) — RE-INSTRUMENT shape, distinct from
RL-986's.** §4 item 3: *"RL-996's `decision-maker` for the three standalone ruling files
...
**non-interference claim** — did RL-984's own diff disturb territory it deliberately left
...
from RL-986's shape: RL-986 checked a positive obligation the code never attempted
...
- **RL-987 acceptance item 2** (original, reversion-based), withdrawn by RL-970: measured
...
- **RL-882 acceptance item 1**, withdrawn via a same-day cross-file correction filed under a
  non-canonical heading in a *different* file — `docs/rulings/RL-00859-the-remainder-spli
  ts-and-the-split-is-the-answer.md:139`, "## Correction to RL-882, discharging register row F32" — exactly the
  addendum trap `docs/plans/README.md` §"enumerate the rulings by diffing the record" warns a
  heading-only sweep misses. Register **F32**, confirmed verbatim: *"Only RL-882's
...
builds or runs it. Two confirmed: RL-887's "Acceptance test" is a standard for judging a
...
that threshold itself rather than finding it stated. RL-911's "acceptance evidence is the
...
**CANNOT_DETERMINE** (2): RL-997 item 3 has no `Violation:` clause among two genuine
siblings in the same §4 — reads as disclosure prose for the maintainer's weighing, and the
question "is this vacuous" may be the wrong one to ask of it. RL-860's own override clause
("unowned"/"owner TBD" in the cell) is textually triggered by register row F72 today; whether
RL-909's later register-conformance sentence retroactively legitimises that phrasing, or
F72 is the live regression RL-860 exists to catch, is not resolved here.

## 6. The growth-versus-undercount split

RL-994's own commit: `22d8d64`, `2026-09-02T12:53:04+01:00`. First-commit timestamp of every
one of the 30 files carrying a genuine item (`git log --format=%aI --follow -- <path> | tail
-1`):

**28 of 30 existed at or before RL-994's own commit; 2 arrived after** — RL-1000's own
file (`13:27:48`) and RL-984's own file (`14:28:36`). Of the gap between RL-994's
"twelve" and this record's 30: **2 is growth, 16 is undercount** (28 files already carried a
genuine item when RL-994 said twelve). RL-1000's "fourteen," filed at `13:27`, could in
principle have seen up to 29 of the 30 (everything but RL-984's own file, filed after it) —
so that revision undercounted by at least 15.

## 7. Behavioural evidence, where it exists

RL-887 is the only item in this sweep with an *observed* failure rather than a mood
...
(`backend/tests/test_lineage.py`, FR-72) flagged the CSV as bundled reference data.
RL-949's carve-out
...
| 16 | 2026-08-29-w11-slices-2-4-rulings.md | CONSTRUCTIBLE | item1 WITHDRAWN (originality not substance) + item2 degraded-read built | register F32; docs/rulings/RL-00859-the-remainder-splits-and-the-split-is-the-answer.md:139 Correction to RL-882 |
...
| 19 | 2026-08-29-w11-slices-2-4-rulings.md | CONSTRUCTIBLE | EVIDENCE_FLOOR vs FR-364 named list, one read of each | docs/specs/06-governance.md:145 carries the amendment citing RL-885 |
...
| 23 | 2026-08-29-w11-slices-3-4-rulings.md | CONSTRUCTIBLE | trace-delete refusal under NFR-459 floor | backend/tests/test_traces.py:258,284 both pass |
...
| 27 | 2026-08-29-w11-ruling-vs-plan-scope.md | CONSTRUCTIBLE | no resolver hardcodes status; was failing at filing now fixed | historically red at 39cb58e; fixed by RL-859 |
...
| 29 | 2026-08-29-w11-algorithm-pin-maturity.md | CANNOT_DETERMINE | unowned register row with no decay event | F72's Decision cell literally contains unowned; whether RL-909's later conformance sentence legitimises that is open |
| 30 | 2026-08-29-w11-fr-rate-65-attribution.md | CONSTRUCTIBLE | FR-243 absent from roadmap; fix outstanding | grep -c FR-243 docs/roadmap.md = 0; known tracked gap |
...
| 40 | 2026-08-30-w11-reopen-hooks-and-bundle-resolution-rulings.md | CONSTRUCTIBLE | retry-cap hook: synthetic runtime state at cap+1 and cap-1 | tests/test_retry_cap_hook.py cites RL-920 §5 verbatim; 11 tests pass |
...
| 42 | 2026-08-30-w11-reopen-scope-and-batch-frame-contract-rulings.md | CONSTRUCTIBLE | three concrete prohibitions - stated by the ruling as testable not hortatory | docs/closures/CR-00927-work-item-record-wk-671-scoring.md §10.5 confirms all three citing RL-922 |
...
| 44 | 2026-08-30-w11-4b-trace-environment-ruling.md | CONSTRUCTIBLE | two tests: multi-env key resolves to issued env; no-env caller writes no row | backend/tests/test_score.py:796,843 cite RL-916 part3; auth/service.py:230-234 implements it |
...
| 49 | 2026-08-30-nt-0015-q1-q5-rulings.md | NONE_FOUND |  | own text is a one-time PR with no stated check; its Text A/B/C become testable via RL-910 |
...
| 59 | 2026-09-01-nt-0016-slice2-fr-data-32-ruling.md | CONSTRUCTIBLE | three named broken-input census cases (Broken-input proof convention) | backend/tests/test_lineage_census_carveout.py cites RL-949 x2; all 3 cases present |
| 60 | 2026-09-01-ruling-60-census-provenance-checkout-depth.md | CONSTRUCTIBLE | checkout-depth config confirms RL-949's cases still hold | .github/workflows/python.yml:124-126 fetch-depth:0 matches §3 |
| 61 | 2026-09-01-ruling-61-notes-tombstone-stubs-watched.md | INVALIDATED | check_notes_tombstone() must red on a stray file and on an edited stub body | function absent (0 matches); scripts/audit-docs.py:55-64 module docstring: slot 30 given to RFC-937's header check protective job ends with this commit by that same resolution until W37-6 deletes the stubs entirely; name repurposed to check_redirects/slot 36 which watches REDIRECTS.csv not the old notes root under .claude |
| 62 | 2026-09-01-nt-0016-q1-q2-q3-q7-general-rulings.md | NONE_FOUND |  | only an Overridden-if clause; lapsed by its own clause per RFC-937 §9 |
| 63 | 2026-09-01-nt-0016-q1-q2-q3-q7-general-rulings.md | NONE_FOUND |  | only an Overridden-if clause; lapsed by its own clause per RFC-937 §9 |
...
| 65 | 2026-09-01-nt-0016-q1-q2-q3-q7-general-rulings.md | NONE_FOUND |  | only an Overridden-if clause; lapsed by its own clause per RFC-937 §9 |
| 66 | 2026-09-02-w37-migration-preconditions-rulings.md | CONSTRUCTIBLE | item1 checks 30/31/33/36 built; item2 (reversion-based) WITHDRAWN by RL-970 | check 34 DP-7 shared predicate confirmed shared between doc-id.py and audit-docs.py |
...
| 70 | 2026-09-02-w37-field-set-and-rollup-rulings.md | CONSTRUCTIBLE | four items: wrong-carrier check, hardcoded-policy check, silent-coverage check, ledger extra | check_header_fields/derive_field_policies read from templates; FD.md already excludes decision: citing RL-981 |
...
| 72 | 2026-09-02-w37-field-set-and-rollup-rulings.md | CONSTRUCTIBLE | four fixtures: invisible slice, mid-flight, replanned, no-catch-all | _rollup_map_plan docstring cites RL-983; precedence table present, no trailing catch-all |
| 73 | 2026-09-02-w37-6-leaf-plan-findings-rulings.md | CONSTRUCTIBLE | withdrawal of RL-987 item2 + 2a/2b/2c substitute; own §5 two falsifiability items | sweep_legacy_forms measured wrong in both directions on the real corpus |
...
| 79 | 2026-09-02-w37-template-parser-conflicts-rulings.md | CONSTRUCTIBLE | three items: row-template parses; RL-981 mutation; kind: on WK- rejected | tests/test_template_headers.py:378,407,426 all pass |
...
| 83 | 2026-09-02-w37-guard-arithmetic-and-ledger-family-rulings.md | CONSTRUCTIBLE | five items: census fails today naming real headings; two mutations; legacy-guard nuance; roadmap arithmetic not yet built | _check_multi_ruling_files_not_silently_unrecognised live; _reconcile_census raises citing RL-985 §3 item 3 |
...
| 86 | 2026-09-02-w37-ruling-a-series-and-standalone-ruling-files.md | INVALIDATED | item3: each new RL- record carries an owner: that is not decision-maker | RL-984 (a later ruling) reverses the decision this item encoded; post-sweep confirmed by commit 2e48960/#620: _ruling_file_owner now unconditionally returns decision-maker for every input |
...
| 88 | 2026-09-02-w37-container-family-and-line-citations-rulings.md | INVALIDATED | item2 (original text): a fixture whose only level-2 heading is followed by three record headings | PR #609/2fbce0c demoted the heading; grep -c '^## ' docs/closures/INDEX.md#plan-reviewsmd = 0; rebuilt by RL-994 on a property not a level |
...
| 92 | 2026-09-02-w37-roadmap-transform-rulings.md | CONSTRUCTIBLE | every Depends-on id resolves to a WK- row (WK-662 control); WK-662 retired with successors named | WK-662 verified by execution: status=retired, body names WK-663/WK-664 |
| 93 | 2026-09-02-w37-ruling-88-acceptance-amendment.md | CONSTRUCTIBLE | rebuilt property-based fixture + positively-identified container (replaces RL-979 item2) | grep -c '^## ' docs/closures/INDEX.md#plan-reviewsmd = 0 confirmed at current tree |
| 94 | 2026-09-02-w37-vacuous-acceptance-item-ruling.md | CONSTRUCTIBLE | substituted count-and-print check + _stamp_header mutation test (replaces RL-986 item2); design constructible | register F77: not yet built as of this pin - no commit had touched scripts/doc-id.py or tests/test_doc_id_migrate.py for this purpose since 09b7e9b |
| 95 | 2026-09-02-w37-gap-1-ruling-86-owner-ruling.md | VACUOUS | item3 (originally worded): RL-996's decision-maker owner for the 3 standalone files still holds after this amendment | scripts/doc-id.py:1541 owner=_PLAN_KIND_OWNER[leaf]=planner; zero code paths emitted decision-maker for those 3 files as of this pin - non-interference property holds, nothing to observe; re-instrumented post-sweep by commit 2e48960/#620 (test_ruling_87_standalone_files_are_untouched_by_this_amendment) |
...
1. First pass counted `nt-0015-q1-q5-rulings.md` (Rulings 49-53) as carrying acceptance items
   ...
   enforces their substance (`register-owed.py`'s dirty-worktree refusal for RL-912; the
   findings-directory naming convention `docs/audit/findings/README.md` ties to RL-913).
2. Re-reading Rulings 52 and 53's *own text* against the boundary found neither states a
   ...
   making on RL-921 earlier in the same sweep — found a second time, in this sweep's own
...
naming which heading fell into which bucket, not by confirming a printed sum — RL-985's own
...
- **RL-987 acceptance item 2** (original, reversion-based test), withdrawn by RL-970.
  ...
  caught either way. §5's CONSTRUCTIBLE count for RL-987 reflects its live replacement
  (2a/2b/2c, demonstrated falsifiable against the real corpus in RL-970's own §5); the
  original item is what this correction names.
- **RL-882 acceptance item 1**, withdrawn the same day it was filed (`docs/rulings/RL-008
  59-the-remainder-splits-and-the-split-is-the-answer.md:139`, "Correction to RL-882," discharging register row
...
run. On the *blinds* limb: **RL-986 §4 item 2's substituted check (RL-1000) is to be
built**; **RL-995 §4 item 3 is to be rebuilt so it can pass on some input**. Neither stops
...
| INVALIDATED (3) | rebuilt or a bounded trade | R88 by RL-994; R86 by `e2296ec`; R61 a documented trade, horizon W37-6 |
...
### CANNOT_DETERMINE 1 of 2 — RL-860 versus register row F72: **no violation**

**Determinable after all, and the answer is that the override applies.** RL-860's clause
(`docs/rulings/RL-00860-owners-for-the-seven-unowned-register-findings-and-one-new-row.md:93-97`) ends *"This ruling is overridden
...
**What is open is a different, later rule.** RL-909 Text B — *"An unowned row must name the
...
### CANNOT_DETERMINE 2 of 2 — RL-997 item 3: **not an acceptance item**

**RL-997 §4 carries two acceptance items, not three.** Items 1 and 2 each carry a `Violation:`
...
**RL-887.** The ruling is *"no change"*, and its own text puts the unresolved question outside
...
finding**: register row `FR-240` (`F-W9-3`) clause (5) records *"no `control`-intent factor
in a rateable path (`02` FR-88) — no implementation anywhere"* with a disposition attached.
Verified at `7186dca`: the code is declared in `docs/specs/03-rating-engine.md:617` and
`docs/workflows/WF-00699-approved-models-to-approved-rating-version.md`, and is absent from
`backend/src/app/errors.py`.

**RL-911.** Its §2 says in its own words *"This ruling does not fix its value; it fixes its
...
RL-1000's *"a vacuously true acceptance item does not satisfy itself"* arriving by the opposite
