# NFR-RATE-4 measurement — W11 Task 1.3, `CompiledBundle`/`load_bundle`
...
`.venv` this worktree built with `uv sync --all-packages`. NFR-RATE-4
...
"real large" on both axes NFR-RATE-4 cares about independently: many rating steps (so
...
- A real, trained XGBoost booster: 12 features, 300 trees, max depth 6, fit on 5,000
  ...
  payload exactly as Task 1.2's real resolver does (Ruling 7) — `booster_content`, a
...
- Timed `compile_bundle(version, resolver)` directly (`time.perf_counter()`), and measured
  ...
  form, per Ruling 7's reading of the budget ("measured on the persisted form, not an
  in-memory estimate"), not this task's own `load_bundle`/`CompiledBundle`, which is never
  itself serialised (FR-RATE-65) and so has no size of its own to measure against this NFR.
...
Both halves of NFR-RATE-4 pass by a wide margin on this structure. `load_bundle()` itself
(not this NFR's own metric — Ruling 7 scopes NFR-RATE-4 to the persisted `Bundle`, and
...
because it is the number Ruling 8's "one deserialisation, not N" property is *about*: paid
...
ADR-0003) inflates a binary booster's size rather than shrinking it. Ruling 7 already named
...
rounds — would exhaust NFR-RATE-4's budget on the booster alone, before counting a single
rating step. **This is a finding against NFR-RATE-4's margin for a large real deployment,
not against this task or against Ruling 7's ruling**, which already flagged it; recorded
...
fitting or database I/O on this path (NFR-RATE-3). Step count is not the risk this NFR
should be re-measured against; booster size is.

## Scope

This measures `NFR-RATE-4` alone, Task 1.3's own assignment in the plan's requirement
coverage table. `NFR-RATE-1` (component), `2`, `3`, `7`, `8` and `14` are Task 1.4/1.5's to
