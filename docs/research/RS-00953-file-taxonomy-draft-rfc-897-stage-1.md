...
family: research
kind: measurement
title: File taxonomy — draft (RFC-897 Stage 1)
status: active                  # draft → active → closed | retired (§1.2a)
created: 2026-09-01
owner: executor
corrected_by: []
relates: []                     # ids only — the FR-/ADR-/RFC- target a spike's `closed` cites
was: docs/audit/file-taxonomy-draft.md
---

# File taxonomy — draft (RFC-897 Stage 1)
...
[`2026-08-31-nt-0016-investigation.md`](../plans/PL-00929-rfc-897-file-taxonomy-reference-coding-and-custody-research-and-the-slice-cut.md) §4 defines
(RFC-897 §10). Nothing here amends `docs/plans/README.md`'s four declared kinds, adds a
...
([`RS-00952-file-census-rfc-897-stage-0.md`](RS-00952-file-census-rfc-897-stage-0.md)'s own "circularity" note explains why a census cannot
...
by re-running `python3 scripts/file-census.py` per RFC-897 Slice 3 Step 1 — each figure says
which. The corpus for every count is `git ls-files` (`docs/research/RS-00952-file-census-rfc-897-stage-0.md`'s corpus
...
`5ef559d` census's own committed values are clean: `docs/research/file-census-5ef559d.csv`
...
`grep -c '^docs/audit/file-census' docs/research/file-census-5ef559d.csv` returns `0` —
...
per RFC-897's Global Constraints) counts *tracked files whose content contains the
...
`docs/research/file-census-5ef559d.csv` itself. Excluding matches that originate solely from
`docs/research/file-census-5ef559d.csv` and `docs/research/RS-00952-file-census-rfc-897-stage-0.md` (the census's own
...
(`2026-09-01-nt-0016-slice2-fr-data-32-ruling.md`, "RL-949," and
`2026-09-01-ruling-60-census-provenance-checkout-depth.md`, "RL-950"). RL-949's file
is genuinely referenced (RL-950's own text cites it by name) and is not part of either
population. RL-950's file is unreferenced, but for an entirely different, mundane reason
...
slice does not touch it (RFC-897 Global Constraints) — and it recurs every time a census is
...
`docs/closures/INDEX.md#closure-recordsmd` and `docs/roadmap.md` shows that a closed workstream's
closure record cites the **requirement id** (`FR-199`), the **slice id** (`W6b-10`,
`W32-4`) or the **PR number**, and not, in the cases checked, the plan's own filename. For
example, `docs/closures/INDEX.md#closure-recordsmd`'s WK-664 delivery table cites `W6b-10 browser auth`
...
RFC-897 §8's own text scopes code-file conventions under `backend/`, `packages/` and
...
— matching RFC-897 §0's own opening-evidence scope (`docs/` + `.claude/` populations, not the
whole tree).
...
Twelve categories carried forward from RFC-897 §3's working hypothesis, each re-measured at
this draft's tree rather than assumed. Home, mutability and id family are the census's own
column definitions (`docs/research/RS-00952-file-census-rfc-897-stage-0.md` "Column rules"); "rejected reading" is the
...
- **Census evidence, at `4f95fb3`, mutually-exclusive classification of all 124
  ...
  eras**: the four WK-664-era files use the token `slice-map` (one revised twice more, each a
  distinct filename rather than an edit — `docs/plans/README.md`'s frozen-file rule read
  literally, since a map plan is itself frozen once filed); the one WK-672-era file uses
...
- **Rejected reading.** Treating "map plan" as its own category, separate from "leaf plan."
  Rejected because RFC-897 §3's table names one row, "plan (map / leaf)," and the two are
...
- **Purpose.** The dated, sibling home for a decision-maker's ruling on a decision point —
  ...
  plan").
- **Mutability.** `write-once-amend` (RFC-897 §3's own term; the census's directory rule
...
- **Census evidence.** **24 files** at `4f95fb3` match a filename ending in `-ruling.md`,
  ...
  `b551060` re-run of RFC-897 §9's token sweep ("ruling 10... rulings 11," one family, 21
  ...
  [`2026-08-31-nt-0016-investigation.md`](../plans/PL-00929-rfc-897-file-taxonomy-reference-coding-and-custody-research-and-the-slice-cut.md)
...
- **Mutability.** `append-only` (RFC-897 §3's term; census reports `frozen` by directory,
  same caveat as §1.3).
...
- **Census evidence.** **16 files** at `4f95fb3` match a filename ending in `-ledger.md`
  (mutually-exclusive classification), matching RFC-897 §0's own `7db62ca` figure of 16
...
- **Mutability.** `write-once` (RFC-897 §3's term).
- **Today's home.** **Three** homes, not the two RFC-897 §3 names as a "known tension" —
  ...
  2. `docs/closures/INDEX.md#closure-recordsmd` — **one file**, a single append-only document holding
  ...
  3. `docs/plans/*-closure*.md`-shaped filenames — **3 files** at `4f95fb3`
     ...
     classification).
- **Id family.** Work ids (`WK-661`, `WK-664`, `WK-692`, `WK-670`, `WK-671`...) cited within; no id minted
...
- **Rejected reading.** Collapsing the three homes into one count. Rejected — they behave
  ...
  work id); `docs/closures/INDEX.md#closure-recordsmd` is a single running document that has never
  ...
  `docs/plans/README.md`'s frozen-file rule. RFC-897 §3's "known tension" is confirmed real
...
- **Purpose.** The append-only decision grammar (register) and the per-finding evidence
  essay (findings), split by RFC-896 (`.claude/roles/auditor.md`: "Evidence essays live at
...
- **Mutability.** `write-once-amend` (register rows amended in place per their decision
  ...
  correction.
- **Today's home.** `docs/findings/register.md` (one file, register rows) and
...
- **Census evidence.** `docs/audit/findings/` = **5 files** at `4f95fb3`, with a
  ...
  already covers as its own near-universal pattern, §1.11 below). `docs/findings/register.md`
...
- **Rejected reading.** RFC-897 §3's table names this one row, "register + findings." This
...
- **Purpose.** Working research findings, not yet (or never) promoted to a spec, plan or
  finding.
- **Mutability.** `frozen + correcting annotations` (RFC-897 §3's term); census reports
  `unknown` (`docs/research/` carries no directory-level mutability rule in the census
  script, per `docs/research/RS-00952-file-census-rfc-897-stage-0.md`'s column rules — everything outside the four
...
- **Rejected reading.** Treating the coarse `generated` mutability label as evidence the
  ...
  limitation (§ "Column rules," `docs/research/RS-00952-file-census-rfc-897-stage-0.md`) is that `mutability` is a
...
- **Purpose.** Three related but distinct things RFC-897 §3 groups as one row: the living
...
- **Rejected reading.** Treating "process / charter / skill" as genuinely one category
  because RFC-897 §3 gives it one table row. Rejected for the purposes of §3 of this draft
...
- **Purpose.** The project's design memory — an investigation, a finding, a proposal not
  yet a spec change.
- **Mutability.** `frozen proposal` (RFC-897 §3's term), amended only by dated correction —
  the same convention this very document (RFC-897) demonstrates in its own "Corrections
  after filing" section.
- **Today's home.** `.claude/notes/` at this draft's tree — **19 files** at `4f95fb3`.
  RFC-897 Slice 4 (blocked on Slice 1 having merged and on Q5/Q6/Q7's notes half being
...
- **Rejected reading.** None on the category itself — Q5, Q6 and the notes half of Q7 are
  ...
  precisely because it did not appear in RFC-897 §3's twelve-row table at all: **no role
...
- **Id family.** `wf-NN` (`WF-698` through `WF-702`).
...
## 2. Findings against RFC-897 §3's own twelve-row table
...
- **RFC-897 §3's "known tension" undercounts by one home.** The note names two homes for
  "closure / audit record" — `docs/audit/work/**` and `docs/plans/*-closure*.md`. §1.5
  finds a **third**: `docs/closures/INDEX.md#closure-recordsmd`, a single 4782-line running document
...
- **Two candidate categories each carry two live, mutually unintelligible filename
  ...
  (map / leaf)" category's map sub-kind is spelled `slice-map` in four WK-664-era files and
  `map-plan` in one WK-672-era file — the census's own `name_pattern` clustering does not
  ...
  alone**: it means the hypothesis in RFC-897 §3 names the right categories but the
...
- **Splitting register from findings by home**, not just by RFC-896's decision-grammar
...
**These are findings, not assignments** — RFC-897
...
`docs/research/file-census-5ef559d.csv` or `docs/research/RS-00952-file-census-rfc-897-stage-0.md` as the only "referrer,"
...
(terminal-by-design — acceptable, but declared) are RFC-897 §6's two verdicts that apply to
...
`docs/closures/INDEX.md#closure-recordsmd` and, where relevant, `docs/roadmap.md`, for a closure
...
|---|---|---|---|---|
| 1 | `2026-08-19-custom-metrics-final-review.md` | final-review | **4** | Declared final-review kind (`docs/plans/README.md`): "findings against a finished branch, and their verdicts." FR-154 is confirmed delivered in the WK-661 closure narrative by id and prose, not by this filename. |
| 2 | `2026-08-19-custom-metrics-ledger.md` | ledger | **4** | Declared ledger kind. Its own header cites an upstream plan under `.planning/` (git-ignored, never committed) — this ledger is the sole durable record of that work, not a downstream artifact of a committed plan. FR-154 confirmed delivered (WK-661 close). |
| 3 | `2026-08-19-glm-approximation-as-model-ledger.md` | ledger | **4** | Same shape as #2. FR-137 confirmed delivered — WK-661 closure narrative's "Transparency artifacts" row names it directly. |
| 4 | `2026-08-19-psi-comparison-selector-ledger.md` | ledger | **4** | Same shape as #2. FR-63 / VR-DST-1, upstream plan also `.planning/`-only. |
| 5 | `2026-08-19-paired-quantile-models.md` | leaf plan | **4** | FR-199, no sibling ledger exists for this plan. Explicitly narrated as delivered in `docs/closures/INDEX.md#closure-recordsmd` ("the slice... that makes FR-199 real") and cited in a `scope-audit.py MODEL --endpoints` line in the same record. |
| 6 | `2026-08-21-ebm.md` | leaf plan | **4** | FR-140, no sibling ledger. Confirmed in the WK-661 closure narrative ("EBM via interpret-core exports terms and bins directly rather than a serialised estimator, FR-140"). |
| 7 | `2026-08-21-offset-from-another-model.md` | leaf plan | **4** | FR-116, no sibling ledger. Confirmed in the WK-661 closure narrative ("offsets including offset-from-another-model, GLM-to-GLM, FR-116"). |
| 8 | `2026-08-21-tweedie-profile-likelihood.md` | leaf plan | **4** | FR-114, no sibling ledger. Confirmed in the WK-661 closure narrative ("Tweedie with the power estimated by profile likelihood and its 95% interval recorded, FR-114"). |
| 9 | `2026-08-22-gbm-weights-and-dropped-eval-metrics.md` | leaf plan | **4** | FR-159/160, no sibling ledger. Extensively narrated across `docs/closures/INDEX.md#closure-recordsmd` (multiple dated sub-sections discuss `eval_metrics` being honoured, FR-159). |
| 10 | `2026-08-22-w5-audit-remediation-ledger.md` | ledger | **4** | Declared ledger kind, execution record for a WK-661 remediation pass within WK-661's own closed window (WK-661 closed 2026-08-22, same date). |
| 11 | `2026-08-22-w5-closure-ledger.md` | ledger | **4** | Declared ledger kind; its content is literally the execution trail of WK-661's own close, dated the same day WK-661 closed. |
...
| 14 | `2026-08-23-w32-3-dataset-list-derived-fields-ledger.md` | ledger | **4** | Declared ledger kind, cites its own upstream plan (`2026-08-23-w32-3-dataset-list-derived-fields.md`, itself referenced and not in this population) by filename. WK-692 closed 2026-08-24 with a dedicated W32-3 heading in its closure record. |
| 15 | `2026-08-23-w32-4-ebm-predict-arm-ledger.md` | ledger | **4** | Same shape as #14; WK-692 closure record carries a dedicated W32-4 heading. |
| 16 | `2026-08-23-w32-5-partial-dependence-exposure-ledger.md` | ledger | **4** | Same shape as #14; WK-692 closure record carries a dedicated W32-5 heading. |
| 17 | `2026-08-23-w32-6-backtest-and-objective-endpoint-tests-ledger.md` | ledger | **4** | Same shape as #14; WK-692 closure record carries a dedicated W32-6 heading. |
| 18 | `2026-08-24-w32-11-certificate-floors-and-two-generated-sides.md` | leaf plan | **4** | Its own header states it was "allocated on 2026-08-24" from a decision inside `2026-08-23-w32-closure-proposal.md`'s own Part C — i.e., it is itself a product of WK-692's closure process. WK-692 closure record carries a dedicated W32-11 heading (one of "12 slice headings — W32-1 ... W32-11 plus W32-1b" the closure record's own acceptance table names). |
| 19 | `2026-08-24-w6b-1b-diagnostics-view.md` | leaf plan | **4** | WK-664 closed 2026-08-27. Closure record's own text: "Earlier WK-664 slices (W6b-1..9, W6b-13, W6b-14) merged in prior sessions; their delivery records are in the roadmap §6 slice records" — this slice (W6b-1b) is inside that named range. |
...
| 28 | `2026-08-25-w6b-10-browser-auth.md` | leaf plan | **4** | WK-664 closure record's own delivery table names this slice directly: "W6b-10 browser auth, PR #250, verified." |
...
| 35 | `2026-08-28-w10-rate-tables-rulings.md` | rulings record | **4** | Own text: "the plan itself is frozen... and is not amended; this record is the dated home of the rulings." WK-670 closed 2026-08-28 (`docs/roadmap.md`'s WK-670 row cites `docs/closures/CR-00834-work-item-record-wk-670-rate-tables.md`); a rulings record's declared purpose — the dated record of a decision at the time it was made — matches RFC-897 §6's own verdict-4 example ("frozen records cited only at creation") directly. |
...
Stated per RFC-897 §10 and the investigation plan §4a item 7. Each is a choice; this draft
recommends nothing and rules nothing.

### Q1 — Is the closed category set ruled as drafted, amended, or rebuilt from the census?

- **Option A — drafted.** Adopt RFC-897 §3's twelve-row table as written, unchanged by
...
- **Option C — rebuilt from the census.** Discard the twelve-row table as a starting
  ...
  the ground RFC-897 §10 itself already recommends ("rule on the census-clustered set, not
...
- **Option A — grammar-in-place.** All of `docs/plans/`'s undeclared sub-kinds (rulings,
  ...
  filename grammar Stage 2 defines. RFC-897 §10's own recommendation.
...
- **Option B — category attribute.** The declaration lives on the file itself (a header
  field, per RFC-897 §4's reference-coding stage).
- **Option C — derived, not declared, wherever a closure record already exists.** §4's
  ...
  stale independently of the closure record it duplicates (`CLAUDE.md` §0, RFC-756's own
...
- **Option A — id-only, universally.** Every category cites another by a minted id,
  extending the notes' NT-id precedent (RFC-897 §1a's own argument: "id citations survived
...
- **Option C — mixed, split by whether the category has an independent id.** Categories
  ...
  which RFC-897 C2 and this draft's own Global Constraints forbid. §4's own evidence is
