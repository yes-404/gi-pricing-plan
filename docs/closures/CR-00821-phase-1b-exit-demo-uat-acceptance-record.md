...
family: closure
kind: phase
title: Phase 1b exit-demo UAT — acceptance record
...
was: docs/audit/exit-demo-uat.md
---
...
of the core `WF-698` journey (`scripts/demo.py` with the full 678 013-row freMTPL2 seed),
...
| 5 | The postcondition check passes over HTTP (`_verify_journey_postconditions`) | PASS — `GET /api/v1/models?status=approved&limit=5` returned 200; the demo printed "WF-698 demo subset: 1 approved model(s)" |
...
| 7 | The total seed time stays within the NFR-529 budget (300 s) | PASS — 90 s |
...
- Item 6: the frontend dev server serves on :5173 and the router registers the four
  routes; the demo guide (FR-408) maps them to the seeded workspace.
