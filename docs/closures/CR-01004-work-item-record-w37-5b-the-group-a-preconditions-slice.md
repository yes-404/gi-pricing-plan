...
family: closure
kind: work
title: Work-item record — W37-5b (the group-A preconditions slice)
...
was: docs/audit/work/W37-5b/README.md
---
...
commits while this audit ran — `09b7e9b` (#614, RL-1000), `2eef65b` (#615, `git-hygiene`
...
[`2026-09-02-w37-5b-slice-decision.md`](../plans/PL-00956-w37-5b-the-lead-s-decision-on-the-obligations-proposal-2026-09-02.md)
§3 states group A as **eighteen rows: 1–15, 30, 31, 36**, against
[`2026-09-02-w37-6-outstanding-obligations.md`](../plans/PL-00961-w37-6-everything-it-owns-before-the-run-one-list-with-each-item-s-state-and-what-discharges-it.md)
...
| 3 | A new dated leaf plan for W37-6, superseding the frozen one by name and date, carrying RL-970's amendment inline | **Met** | [`2026-09-02-w37-6-migration-run-leaf-plan-v2.md`](../plans/PL-00959-w37-6-the-migration-run-leaf-plan-superseding.md), filed draft (`958cb7d`), lifted to `status: active` (`f9975f1`) once RL-979 resolved its one blocking row. Header states: *"Supersedes: [the frozen plan]... which is frozen at its date and is not edited."* Item 11 restated in full inline, not by reference. Frozen original confirmed untouched: `git diff --stat b648c22..3f41d60 -- docs/plans/PL-00960-w37-6-the-migration-run-leaf-plan.md` is empty. |
...
| 4 | Ten `WK-661 —` closure headings raise on the first, stopping the run | `614c92c` (#599) | **Delivered, tested — see §3 for a correction to this commit's own body** | Independently counted `docs/closures/INDEX.md#closure-recordsmd`: **21** real `###` headings, reconciling to 8 `CR-work` + 1 `CR-phase` + 2 `RS-audit` + 10 `LG-`, exact match to `_discover_closure_records(ROOT)`'s live output and to `test_closure_records_real_corpus_decomposes_into_ruling_84s_four_buckets` (ran narrowly, passes). |
...
| 6 | `RL-902`–`A3`: family undetermined, blocking the census | derivation `04f47b2` (#595), ruled `cc17404` (#598, RL-995) | **Ruled — three `RL-` records; discovery code not yet written — filed as F81** | The guard still, correctly, reds on the real file today: `_check_multi_ruling_files_not_silently_unrecognised(ROOT)` raises naming `docs/rulings/RL-00902-rfc-842-s-credential-lifetime-rule-lands-in-claude-skills-secret-hygiene.md`'s real `### RL-902` heading as unclassified — proving the guard works and the discovery gap is real, not that row 6 failed. See §6, F81. |
| 7 | "Pending proposals" container: family undetermined | derivation `44ec54e` (#597), ruled `f4cbbb7` (#601, RL-979), amended `22d8d64` (#612, RL-994) | **Ruled — `RFC-`, `kind: process`, `owner: maintainer`; discovery code not yet written — see F80** | `docs/closures/INDEX.md#plan-reviewsmd` re-counted: **0** level-2 headings, **12** level-3, **28** level-4, matching RL-994's replacement-fixture requirement. But the guard this slice itself built (row 1) now correctly reds on this same container's *undecided disposition*: `_check_plan_reviews_heading_census(ROOT)` raises naming line 1155's "Pending proposals" heading — filed as **F80** (§6). |
...
| 10 | `REFERENCE.md` still teaches the rejected criterion | `574d536` (#611) | **Delivered** | `docs/_templates/REFERENCE.md:39-49` now states "a hand-kept list, not a filesystem property... RL-990 rejected that detection rule" — read directly. |
...
| 15 | Does the multi-ruling splitter try to split the three h1 ruling files? | measured in `a31d509` (#603), `d7c9b08` (#608) | **Measured — no** | Ran `_discover_multi_ruling_files(ROOT)` directly: 35 source files touched, none of the three named h1 files among them. **One false lead corrected in drafting this record**: `docs/rulings/RL-00994-the-fixture-is-rebuilt-on-the-property-not-the-level-and-the-container-is-identified-positively.md`'s h1 title, "# RL-979's second acceptance item...", matches a loose `^# Ruling [0-9]` grep but is not a standalone ruling record in canonical form — it is RL-994's own file, titled as a description of the ruling it amends. Confirmed by reading the heading directly; the count stays **three**, per the obligations list. |
| 30 | `_discover_requirements`: no guard at all | `d7c9b08` (#608) | **Delivered, tested — guard's own live finding is new and unowned, filed as F82** | `_discover_requirements(ROOT)` → **660** (unchanged count, confirming nothing regressed). The new guard correctly reds today, naming `docs/specs/00-overview.md`'s real, unclassified `DEP-1`, `DEP-537`, `DEP-2`, `DEP-3` — a genuine disclosed gap the guard was built to find, not a false positive. See §6, F82. |
...
| 36 | §8-vs-Ruling-66 stage-boundary conflict (carve-out — routed) | ruled `e74a683` (#596, RL-997) | **Ruled — reading 1: §8 is sequencing, not part of the accepted standard; RL-987 stands unamended** | RL-997's verdict rests on `docs/process/document-ids.md` (the maintainer-owned standard) lifting only §1.1–§1.13, never §4/§5/§7/§8 — confirmed by direct read of the document's own title, "RFC-937 §1, lifted verbatim." |
...
**What was accurate in `614c92c`'s body**: its *second* disclosed gap — RL-986 §4's
...
slice, named `W37-6` as owner via RL-998, and was never revisited after row 12's fix
...
### 5a. RL-986 §4's second acceptance item (`slice:` vacuity) — ruled mid-audit

Filed as register row **F77** ([essay](../findings/FD-01015-rl-986-4-s-second-acceptance-item-is-vacuously-true-no-code-path-can-ever-write-the-slice-value-the-acceptance-item-asks-a-broken-fixture-to-test.md)). Verified independently:
...
`09b7e9b` (RL-1000, PR #614) mid-session. RL-1000 holds: *"an acceptance item that is
...
first time a ledger legitimately carries `slice:`, since RL-986 §3 item 5 keeps it
...
RL-1000 also corrects a stale-tree quotation error in an earlier version of the lead's own
...
the tuple correctly before RL-1000 was written.**

**What RL-1000 leaves open**, verified directly at `3f41d60`: neither the substituted
...
today's real corpus (`_discover_roadmap`'s one `WK-661` draft, phase `P1b`, matches all ten real
...
**Proposed verdict**: the interpretation question is **discharged by RL-1000**, cited
...
Filed as register row **F78** ([essay](../findings/FD-01016-discover-roadmap-s-phase-spanning-refusal-has-no-fixture-exercising-it-its-author-disclosed-the-gap-rather-than-calling-the-row-done.md)). `4cbfa62`'s own body:
...
1. `_check_multi_ruling_files_not_silently_unrecognised` — the real RL-902/A2/A3 file
   (`docs/plans/INDEX.md#2026-08-30-nt-0012-0013-0014-adoptionmd`), ruled `RL-` (Rulings 86/87) but
   with no discovery code yet — earliest in the pipeline (`doc-id.py:2984`), so a real run
   would halt here first. Filed as **F81** ([essay](../findings/FD-01018-migrate-also-halts-on-the-real-rl-902-a2-a3-file-ruled-rl-not-yet-discoverable.md)), unowned.
2. `_check_plan_reviews_heading_census` — the "Pending proposals" container, ruled `RFC-`
   (RL-979/994) but with no discovery code yet (`doc-id.py:3000`). Filed as **F80**
   ([essay](../findings/FD-01017-migrate-cannot-complete-on-the-real-corpus-today-the-pending-proposals-container-has-no-discovery-code-and-the-unconditional-plan-reviews-census-is-what-catches-it.md)), unowned. Not one of the obligations list's 39 rows:
   RL-979 postdates `b648c22` by several commits, so the list could not have named a
   defect in a ruling that did not yet exist.
3. `_check_requirements_not_silently_unrecognised` — `DEP-1`, `DEP-537`, `DEP-2`, `DEP-3` in
   ...
   ([essay](../findings/FD-01019-migrate-also-halts-on-four-real-dep-ids-in-docs-specs-00-overview-md-module-less-by-design-and-unrecognised-by-discover-requirements.md)), unowned.
...
- **F79** — `scripts/register-lint.py`'s `assert classified == seen` is exactly the
  count-comparison shape RL-985 rules against, not retrofit by this slice (disclosed in
  `d7c9b08`'s own body: *"deliberately not fixed... does not itself satisfy RL-985"*).
...
- **The `owner:` field has no home for any document family** — surfaced resolving rows 6/7
  ...
  ([`2026-09-02-w37-rfc-bucket-c-owner-values.md`](../plans/PL-00966-rfc-draft-the-owner-value-for-1-6-s-four-unsourced-rows.md),
...
- **PR count, corrected once already in drafting this record and re-checked here**: the
  dispatch said "seventeen" merged PRs. `git log --oneline b648c22..09b7e9b | wc -l` gives
  **22** — `#593` through `#614`, every number present (an earlier pass of this record
  stopped enumerating at `#613`, undercounting by exactly the PR carrying RL-1000 itself;
  caught re-deriving the figure rather than re-typing it, the same discipline this record
  applies to everyone else's counts). Of the 22: `01bd0bd` (#594) is the decision record that
  creates the slice rather than a row-fix; `d47a5f5` (#613) is the RFC draft explicitly filed
  as a disclosure outside the slice; `09b7e9b` (#614, RL-1000) is a pure ruling record with
  no code. The other 19 derive, rule, fix, or measure a group-A row or a slice-decision
  acceptance item. Not reconciled to seventeen under any grouping this audit could construct;
  flagged rather than guessed at.

## 8. Owed list — generated, not recalled

`register-owed.py <work-id>` takes a Work id, not a Slice id — `W37-5b` itself returns zero
rows (nothing in the register has ever named a slice in its Work item column; verified:
`python3 scripts/register-owed.py W37-5b` against `bbbe333` reports "0 owed row(s)"). What
this close actually needs is everything the register attributes to **W37-6**, since that is
where every row this slice touches or creates was filed before this slice existed to receive
it — run against the committed revision carrying this record, not a placeholder:

```
$ python3 scripts/register-owed.py W37-6
Generated by `python3 scripts/register-owed.py W37-6` against `bbbe333 (audit/w37-5b-closure)`.
Mode: work item 'W37-6'. 5 owed row(s), 1 matched but excluded as opening with a resolution
marker (listed below — verify none carries a residual item; the register's own header names
five rows where a status marker and further carried content share one cell).

- F77 [RL-986 §4's second acceptance item... is vacuously true and untestable as stated] —
  carry forward, unowned — the interpretation question is Ruled 2026-09-02 (RL-1000, PR
  #614)... [full text: register.md]
- F78 [`_discover_roadmap`'s phase-spanning refusal... has no fixture exercising it] —
  deferred with an owner — W37-6... [full text: register.md]
- F80 [`migrate()` cannot complete on the real corpus today: ... "Pending proposals"...] —
  not started — discovery code turning the heading into an `RFC-` draft... [full text: register.md]
- F81 [`migrate()` also halts on the real RL-902/A2/A3 file...] — not started — discovery
  code splitting the three letter-suffixed sub-headings... [full text: register.md]
- F82 [`migrate()` also halts on four real `DEP-` ids...] — not started — a named data-source
  decision for the four `DEP` ids... [full text: register.md]

Excluded as opening with a resolution marker — verify:
- F76 [An uncaught `HeaderError` in check 39 silently drops six later checks...]
```

(Trimmed to first line plus disposition per row for this record's own readability; the
script's real output carries each row's full Decision-cell text, reproduced in full in
`docs/findings/register.md` itself.) **Reconciliation, per §5b's own requirement**: every id in
the block above (F76, F77, F78, F80, F81, F82) appears in this record with a resolution — F76
in §4 (resolved), F77 in §5a (ruled, implementation not started), F78 in §5b (deferred, owner
W37-6), F80/F81/F82 in §6 (not started, unowned). This section adds nothing the block does
not already carry; the Findings table below adds only items with **no** register row at all
(the roadmap staleness) plus F79, which the block does not surface because no register row
names a Work item for it (its own Work item cell is `—`, by design — see F79's own row).
...
- **Does not rule** row 36 (already ruled, RL-997) or the `owner:` RFC (the maintainer's).
- **Does not edit** any frozen plan.
- **Does not close WK-697** or claim the retrofit-impossible mapping — that is a Work-level
...
discharged by RL-1000 mid-audit, one — F78 — still open for W37-6); one new, materially
