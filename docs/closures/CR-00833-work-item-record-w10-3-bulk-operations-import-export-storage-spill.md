...
family: closure
kind: work
title: Work-item record — W10-3 (Bulk Operations, Import/Export, Storage Spill)
...
was: docs/audit/work/W10-3/README.md
---
...
Derived from `docs/plans/PL-00829-wk-670-implementation-plan-rate-tables-seeding-diffs-bulk-operations-import-export.md` slice W10-3 (T1–T4) first, then
evidenced. W10-3 delivers FR-233 (bulk operations that record their parameters),
FR-235 (CSV/XLSX export/import with strict round-trip), and the storage half of
FR-232 (workspace-configurable cell-count threshold, parquet spill, 202-with-Job
diff), plus the backend endpoints behind FR-230/234/232's write paths:
...
- 3B (pricing-core ops, #307): `uplift_table` / `uplift_by_filter` / `floor_and_cap` /
  `rebase_to_level` (each validating before persisting, FR-234), `decide_storage_mode`
  (FR-232), `export_to_csv` / `export_to_xlsx` / `import_from_csv` /
  `import_from_xlsx` with the shared strict pipeline (FR-235)
...
| `storage` spill decision at write time | DP2: threshold changes apply to new versions only; old versions keep their storage (immutability, FR-232) |
...
evidenced (100 %). req-coverage marker counts: FR-233 ×24, FR-235 ×26,
FR-232 ×25 test files (W10-3's share; FR-230 ×11, FR-231 ×10, FR-234 ×5
carry the slice's guard and import tests too). No nonexistent-ID FAIL.

| Requirement | Verdict |
|---|---|
| FR-233 | ✓ delivered and tested — four bulk operations record their parameters (`uplift_table: percentage=…` auto-notes mirror 04 §4.4); each validates before persisting (FR-234); named refusals `FILTER_UNKNOWN_KEY` / `FLOOR_ABOVE_CAP` / `REBASE_NO_MATCH` / `REBASE_AMBIGUOUS` / `REBASE_ZERO_REFERENCE` |
| FR-235 | ✓ delivered and tested — export CSV/XLSX + import with strict round-trip (`_checked_import` shared by preview and confirm: header equality, ragged-rows check, type check, completeness); named refusals `IMPORT_KEY_MISMATCH` / `IMPORT_TYPE_MISMATCH` / `IMPORT_PARSE_ERROR`; import presents the diff, `confirm: true` creates the version (201, same wire shape as bulk-operation); cannot-override test proves 422 + no version created |
| FR-232 | ✓ delivered and tested — `rate_tables.cell_threshold` REGISTRY (INT 250 000, min 1), read at version-creation time only (DP2); `decide_storage_mode`; rows → PostgreSQL rows, parquet → content-addressed blob; 202-with-Job diff where either version is `storage: parquet` (baseline resolved before the storage check — same refusals as the 200 path); worker computes the same artifact and returns `JobResult(kind="blob", ref=sha256)` fetchable from `/blobs/{sha256}`; the 501 (`RATE_TABLE_PARQUET_UNBUILT`) retired and superseded in 03 §5.1 with a dated W10-3D amendment |
