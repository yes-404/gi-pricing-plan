...
family: closure
kind: work
title: Work-item record — WK-671 (Scoring)
...
was: docs/audit/work/W11/README.md
---

# Work-item record — WK-671 (Scoring)
...
**WK-671's named hard target, NFR-489, is measured and failing** — not unmeasured, not
marginal. **Three of its ten functional requirements were never started**, because the
maintainer stopped the workstream at the end of Slice 2. This record closes WK-671 on what it
...
authorised you to decide WK-671 close"* — recorded alongside the adoption's delegation in
`docs/plans/INDEX.md#2026-08-30-nt-0012-0013-0014-adoptionmd` §1.1. The delegation is read narrowly:
it covers this close, not WK-672 and not any later phase.
...
evidence** contain **thirteen** requirement ids: FR-250, FR-251, FR-253, FR-254, FR-255, FR-256, FR-257, FR-258, FR-259, FR-260, FR-261, FR-262 and FR-252.

The roadmap's WK-671 row names **ten**: `FR-250, FR-251, FR-253, FR-254, FR-255, FR-256, FR-257, FR-258, FR-259, FR-252`.

**The row is right and the section boundary is the wrong scope line.** `FR-260, FR-261, FR-262` —
Golden Quotes, the Regression Suite, the Quote Sandbox — are named explicitly in **WK-672's own
row** (`docs/roadmap.md:377`, "Testing: golden quotes, property assertions, regression runs |
FR-260, FR-261, FR-262"). §3.8 straddles two workstreams: FR-258 and 42 (trace) are WK-671's, and 43
to 45 (testing) are WK-672's. No correction to either row is needed; this paragraph exists
because deriving from the section heading alone would have over-scoped WK-671 by three
requirements, and the next reader will make the same derivation.

**WK-671's scope is therefore:**

- **Ten FRs** — FR-250 to 42, and FR-252.
- **Three NFRs named in the row** — NFR-489 as "the hard target", joined by NFR-502
  and 14, carried forward from WK-669 via register row F-W9-1.
...
|---|---|---|
| **1 — evaluator core** | #406, #415, #416, and Task 1.1 at `0d942b3` | `CompiledBundle`, `load_bundle`, JDM wire translation, `score_one` on `async_evaluate()`, the bare-metal latency harness, RL-859's maturity check |
| **2 — real-time scoring endpoint** | `378483d`, #443, #446, #452 | `NO_LIVE_RATING_VERSION` and the per-worker bundle slot; `POST /api/v1/score` with bundle resolution and Service Account enforcement; the blank-change-summary guard and its documented 422; **the full-path NFR-489 measurement** |
| **3 — batch scoring** | — | **nothing.** Plan filed (`docs/plans/PL-00849-wk-671-slice-3-batch-scoring-the-pure-transform-the-checkpointing-handler-and-the-route.md`), every blocking decision ruled (D6, Rulings 31/32), no code |
| **4 — trace sampling and persistence** | — | **nothing.** Plan filed (`docs/plans/PL-00850-wk-671-slice-4-trace-sampling-the-row-plus-blob-store-and-the-retention-floor.md`), Rulings 34/35 and the always-capture correction landed, no code |
...
|---|---|---|
| **FR-250** | **delivered and tested** | Real-time scoring of one quote. 5 markers, `backend/tests/test_score.py:123,146,162,…`. Three limbs (F43): L1 ref resolution and L2 explicit ref delivered; **L3, the p99 budget, is NFR-489 and it fails — see §3.2** |
| **FR-251** | **delivered in part; the remainder deferred with an owner (WK-674)** | Explicit `rating_version_ref` delivered and tested, 1 marker at `test_score.py:527`. The requirement's other content is **one** restriction, not two — approved-only *and* a rewritten `what_if` purpose both sit inside its own `prod` clause, and WK-671 has no environments. Deferred deliberately by **RL-880 clause 3**, documented three ways (the ruling, `score.py:226-229`, the test docstring). Not a bare gap |
| **FR-253** | **not started** | Batch scoring route `POST /api/v1/score/batch`. Zero markers. No route registered, no `SCORE_BATCH` job kind. Slice 3 never ran |
| **FR-254** | **not started** | Chunked, resumable, progress-reporting batch. Zero markers. **`def score_batch` exists nowhere in the tree**; `pricing-core/rating/score.py:8` says so in terms — *"not built here"*. **This is 0 of 3 limbs built, not "2 of 3 broken"** — an earlier reading in the backlog said the latter and was wrong: nothing exists to be broken |
| **FR-255** | **delivered and tested** | Error taxonomy on the scoring path. 8 markers, the best-evidenced requirement in the workstream |
| **FR-256** | **delivered and tested** | 4 markers across `backend/tests/test_score.py` and `pricing-core/tests/test_rating_score.py` |
| **FR-257** | **delivered in part; three limbs reassigned** | The approval-evidence gate, 1 marker at `test_rating_versions.py:270`. F44's four limbs: the gate is WK-671's and delivered; the **regression-suite** limb is **WK-672's**, the **dislocation-run** limb is **WK-673's**, and the **GIPP** limb is a permission with no failing case — untestable until something exercises it |
| **FR-258** | **delivered and tested** | Trace production. 1 marker at `pricing-core/tests/test_rating_score.py:494`, **stacked with NFR-490** — see §3.2 for why that stack matters |
| **FR-259** | **not started** | Production trace sampling. Zero markers. Slice 4 never ran. **Its own amendment already settles the batch question** (2026-08-29): batch contributes nothing to the sampled stream, so `score_batch` takes no sampling policy |
| **FR-252** | **delivered and tested** | 3 markers in `pricing-core/tests/test_rating_score.py` |
...
| Requirement | Verdict | Evidence |
|---|---|---|
| **NFR-489** | **measured and FAILING — the workstream's named hard target** | Task 2D (#452, `98eca40`), `docs/research/w11-task-2d-nfr-rate-1-full-path.md`. `_fetch_bundle` **alone** costs p99 **66.294 ms** against a **50 ms whole-request** budget on a 2,039,114 B bundle; without GBM, p99 **44.283 ms** against 15 ms. **Over budget at every rung from 10 rps** — 5 % of the required 200. At that rung p99 queue wait is 7.179 / 4.191 ms, so **the cause is fetch, not saturation and not the box**. Resolving *which* bundle to score costs about **three times** what scoring it costs |
| **NFR-502** | **owed, not delivered** | Bounded but **not isolated**. The ~12 ms residual after subtracting fetch and `score_one` from the handler mean contains framework, routing, auth, DI *and* serialisation. An upper bound containing four other things is not a measurement of the one, and booking it against WK-668's 0.070 ms would be a synthetic-for-real substitution. The executor declined to report it as delivered; that judgement is adopted |
| **NFR-501** | **delivered** | Measured in WK-668: p99 1.626 ms, 3.3 % of its budget. 2 markers |

### 3.3 Non-functional requirements the work touched, claimed by no roadmap row

| Requirement | Verdict | Evidence |
|---|---|---|
| **NFR-490** | **split: correctness delivered and tested; latency measured and failing** | 1 marker at `test_rating_score.py:494-495`, which correctly checks the R3 clause (traced-vs-untraced equality). The **≤ 20 % latency clause has zero assertions anywhere** and is measured at **+723 %**, over by ~36×. See F35's 2026-08-30 correction: `req-coverage.py` has no per-clause granularity, so one marker on a two-clause requirement reads as whole coverage |
| **NFR-497** | **degradation delivered and tested; availability limb deferred with an owner (WK-674)** | 5 markers including an end-to-end HTTP degraded read at `test_score.py:587`. The 99.95 % monthly target is unmeasurable pre-deployment — no measurement code for it exists anywhere. See F41's 2026-08-30 amendment |
| **NFR-498** | **delivered and tested, and unplanned** | Audit events on compile, `e16c459`, marker at `test_rating_version_compile.py:655`. **It appears in no plan and no roadmap row** — the scope derivation found it, not the plan. Recorded here rather than as a register row, because the register holds open findings and this is closed |
| **NFR-499** | **two clauses of four delivered and tested; one not started; one ruled** | 3 markers at `test_score.py:214,231,563`, all exercising **authentication and scoped credentials**. The **per-client rate limit is enforced by nothing** — `rate_limit_rps` is written, stored, returned and consulted by nothing; `RATE_LIMITED` is registered and raised nowhere in `src/`. Register row **F48**. The quote-input-logging clause is settled by **RL-917** |
| **NFR-500** | **not started** | Trace storage under 200 GB/year at 1 % sampling. Zero markers, and it cannot be evidenced before FR-259 exists — sampling is what it measures |

**A roadmap gap, not a delivery gap.** NFR-RATE-2, 9, 10, 11 and 12 were all worked on inside
WK-671 and are named by no workstream row. That is the same class as FR-243 (RL-861):
...
|---|---|---|---|
| NFR-489, fetch alone, with GBM | p99 **66.294 ms** | 50 ms *whole request* | **FAIL** |
| NFR-489, fetch alone, without GBM | p99 **44.283 ms** | 15 ms *whole request* | **FAIL** |
| NFR-490, traced vs untraced | **+723 %** | ≤ 20 % | **FAIL**, ~36× over |
| NFR-501 | p99 1.626 ms | 50 ms | pass, 3.3 % of budget |
...
## 5. Why NFR-489's failure does not block the close

It is the workstream's named hard target and it fails, so this needs stating rather than
assuming.

**The failure is not in what WK-671 built.** `score_one` on the real fixture costs mean 12.326 ms
...
to the NFR-497 degradation branch and **cannot move to the happy path as it stands** — a
re-pointed ref would then serve a stale bundle. Correctness traded for latency, silently.

**So the remedy is an architectural ruling, not a fix inside WK-671's scope**, and holding the
workstream open for a decision nobody has taken would not produce it. What WK-671 owes is the
...
| Finding | Resolution |
|---|---|
| **NFR-489 fails at the full path** | **carry forward, owner: an architectural ruling before WK-674 deployment.** The question is whether a `ref` may be served from a memo without a metadata read, and what staleness window that admits. Register row owed at the next register pass |
| **NFR-502 owed, not delivered** | carry forward with the same owner — isolating it needs the same instrumentation an NFR-489 remedy would add |
| **NFR-490 latency, +723 %** | carry forward. F35, corrected 2026-08-30. Remedy gated on RL-862's off-path capture |
...
| **NFR-500 not started** | reassigned with FR-259; it cannot be evidenced before sampling exists |
| **NFR-499's rate limit (F48)** | carry forward, provisionally WK-674 — a limit enforced in one replica's memory is not a limit |
...
| **F27(c), F29, F33 — one gate-coverage item** | **F33 materially advanced** by `c8d3c81`: mypy now covers the test trees and reports no issues over 163 files. **F27(c) and F29 remain open**, and the RFC-895 adoption record names them so a later reader cannot mistake silence for a decision |
...
WK-671/WK-672/WK-673/WK-674 cut held: FR-257's four limbs distributed cleanly across exactly those
workstreams, which is evidence the boundary was drawn on something real.

**Question 2 — do the workstream cuts?** **No, for WK-671 specifically.** Four slices in one
...
were worked on inside WK-671 while being claimed by no roadmap row** (§3.3). This is the third
instance of the class — FR-243 and FR-252 preceded it. **Proposal: the roadmap's
...
FR-241's citation of FR-247 where it means FR-270 — **the latter is still
...
`docs/plans/INDEX.md#2026-08-30-nt-0012-0013-0014-adoptionmd` §1.1.

## 8. Close

**WK-671 is closed 2026-08-30 as a reduced-scope close**, on seven of ten functional requirements
...
[`docs/rulings/RL-00918-wk-671-reopen-the-maintainer-s-direction-recorded-2026-08-30.md`](../rulings/RL-00918-wk-671-reopen-the-maintainer-s-direction-recorded-2026-08-30.md)
...
[RL-919](../rulings/RL-00921-a-ref-may-not-be-served-from-the-memo-without-a-metadata-read-and-it-does-not-need-to-be-the-content-hash-is-already-in-hand-after-the-first-read-and-is-discarded.md#ruling-39--w11-is-reopened-under-its-own-id-the-closure-record-is-appended-to-never-amended-and-the-roadmaps-status-marker-moves-while-its-close-note-stays-verbatim)
§1: **FR-253, FR-254, FR-259**, and — riding with FR-259 — **NFR-500**,
which §6 above tied to it. **Adoption slices E, F and G are not part of this reopen**; they
continue under `docs/plans/INDEX.md#2026-08-30-nt-0012-0013-0014-adoptionmd`, a separate Work with its
...
and NFR-500 — §6's *"reassigned — a future batch-scoring slice (36, 37) and a future
sampling slice (42)"* (and, for NFR-500, *"reassigned with FR-259; it cannot be
evidenced before sampling exists"*) is **superseded**: the work is back under WK-671 rather than
awaiting a future slice's workstream row. No other row of §6 is touched by this section.

**§6's NFR-489 carry-forward row is discharged, not resolved as passing.** §6 recorded:
*"carry forward, owner: an architectural ruling before WK-674 deployment. The question is whether
a `ref` may be served from a memo without a metadata read, and what staleness window that
admits."*
[RL-921](../rulings/RL-00921-a-ref-may-not-be-served-from-the-memo-without-a-metadata-read-and-it-does-not-need-to-be-the-content-hash-is-already-in-hand-after-the-first-read-and-is-discarded.md#ruling-41--a-ref-may-not-be-served-from-the-memo-without-a-metadata-read-and-it-does-not-need-to-be-the-content-hash-is-already-in-hand-after-the-first-read-and-is-discarded)
...
**Read honestly: this is not a fix and NFR-489 is not shown reachable.** RL-921 mints no
requirement id, amends nothing in `docs/specs/`, and is explicit that its own shape *"does not
establish that NFR-489 passes."* With the fetch's dominant cost already excluded, the
without-GBM limb's own component re-measure reads **p99 23.027 ms against a 15 ms budget** —
over. RL-921 §4 names this as "the requirement's own half" still in the dock, and states
that if a re-measurement with the blob read actually removed still fails that 15 ms limb, that
failure is the trigger for a further decision on NFR-489 itself — not answered here, not
assumed here. NFR-489's remediation (the code change RL-921 §2 describes) is, per
`docs/rulings/RL-00918-wk-671-reopen-the-maintainer-s-direction-recorded-2026-08-30.md` §3, explicitly **not** decided as part of this
...
[`docs/findings/register.md`](../findings/register.md) F50 (the `bundle_slot.py:28-31`
immutability argument RL-921 §3 found wrong as stated) and F51 (a false premise in
`docs/research/w11-task-2d-nfr-rate-1-full-path.md:74-75` that RL-921 §2 refutes).
...
NFR-500, and NFR-489's disposition), re-verdicting none of the seven requirements
closed on 2026-08-30, and accepted by the maintainer with a fresh dated line — the 2026-08-30
delegation does not reach it (RL-919 §5).

## 10. Second close

**Audited against `origin/main` `b749acb`** (RL-919/922's reopened scope: FR-253,
FR-254, FR-259, NFR-500, and NFR-489's disposition). §§1–9 above are not
edited by this section (RL-919 §2, RL-922 §6) — verified: the only two commits ever to
...
FR-253,FR-254,FR-259,NFR-500` confirms RL-919 §1's four-item list against
...
run reports (FR-260/261/262) are WK-672's, per §1 above — correctly outside this scope, not a
gap in it.
...
| Requirement | Verdict | Evidence |
|---|---|---|
| **FR-253** (batch scoring route) | **delivered and tested** | `POST /api/v1/score/batch` (`backend/src/app/api/score.py:394`, Task 3C, `3dc8d6b`/#475) submits a `score.batch` Job and nothing else. 1 marker, `backend/tests/test_score_batch_api.py:143`. `--endpoints` confirms the route is published in the generated contract (absent from the missing-endpoint list) |
| **FR-254** (chunked, resumable, progress-reporting batch) | **delivered and tested** | `score_batch` (`packages/pricing-core/src/pricing_core/rating/score.py:979`, Task 3A, `59407f2`/#465) is a pure chunked transform taking a `ProgressCallback`; the `score.batch` worker handler (Task 3B, `eda70d6`/#471) drives it. 4 markers: `packages/pricing-core/tests/test_rating_score_batch.py:156,203`, `backend/tests/test_scoring_handlers.py:193,397` |
| **FR-259** (production trace sampling) | **delivered and tested** | `decide_sampling` (100 % of declines/errors, configured rate otherwise) and off-path trace production per RL-862's correction (Task 4B, `003f9d4`/#485); trace rows and migration (Task 4A, `25c5688`/#480); `GET /api/v1/traces` and its access control (Task 4C, `87dd4b7`/#490). 20 markers across `backend/tests/test_traces.py`, `test_traces_api.py`, `test_score.py` |
...
### 10.3 NFR-500 — measured, not asserted

| | measured | budget | verdict |
|---|---|---|---|
| NFR-500, blob storage at the ~200-step reference structure | 516.07 GB/year | 200 GB/year | **PROJECTED OVER, ~2.58×** |
| NFR-500, row storage (upper-bounded) | 0.23 GB/year | — | negligible, does not change the verdict |
...
~200-step reference figure (1,032,137 B) by NFR-500's own stated volume (500,000 sampled
quotes/year). **The projection is conservative, not an upper bound**: it excludes
FR-259's 100 % decline/error sampling floor, so real persisted volume is higher than
...
schema/semantics change outside Task 4D's measurement scope. **Verdict: NFR-500 is
...
### 10.4 NFR-493 — its first verdict, split at both clauses (F52)
...
| | measured | budget | verdict |
|---|---|---|---|
| NFR-493, throughput/worker | 5,093,947 risks/hour/worker | ≥ 1,000,000 | **PASS, 5.09×** |
| NFR-493, linear in workers | — | — | **NOT MEASURED** — one worker ran |
...
multi-worker run. This section discharges the register row's own request that "the WK-671
second close... needs to add NFR-493 to its NFR table for the first time, split at its
two clauses" — done above; F52 itself stays open, unowned, per its filed disposition.

### 10.5 NFR-489 — discharged, not passing

RL-921 answers §6's architectural question (a `ref` may not be served from the memo
without a metadata read, and does not need to be — the content hash is already read and
discarded on `_fetch_bundle`'s first statement) and RL-922 rules the resulting code
change **into** the reopen while explicitly ruling NFR-489's verdict **out** of it.
...
- The component delta is measured with tree, host, pass count and ref cardinality all
  named, exactly as RL-922 §4/§6 requires: hit mean 3.657 ms vs. full-path mean
  11.637 ms, on `feat/w11-3b-batch-handler` off `59407f2`, one run, one ref, a shared 4-core
  box — explicitly **not** an NFR-489 re-measurement, on a minimal no-GBM fixture not
  comparable to NFR-489's own budget.
- **NFR-489's verdict does not move: measured and FAILING.** The without-GBM limb's own
  component re-measure (RL-921 §4, unchanged by Task 3B) reads p99 **23.027 ms against a
  15 ms budget**, with the fetch already excluded — over. No artifact produced this reopen
  describes NFR-489 as passing, improved to passing, or re-measured; `bundle_slot_capacity`
  is not raised. RL-921 §4's 15 ms trigger — the condition that would put NFR-489 itself
  in further question — is **not** treated as fired by this delta, per RL-922 §4's explicit
  prohibition.
- **Carry forward, owner WK-674**: the requirement re-measurement on a dedicated host, more than
  one pass, and any `bundle_slot_capacity`/TTL/refresh-channel change (RL-882 clause 4),
...
assigned to this same task by RL-922 §7 and both closed by `eda70d6`/#471 — verified
against the diff, not the commit message: the docstring now states the true safety argument
(safe because the memo is read only on the NFR-497 degradation branch, never the happy
...
|---|---|---|
| **NFR-502** owed | carry forward, same owner | **Unchanged.** No instrumentation for isolating it landed this reopen; still owed |
| **NFR-490 latency, +723 %** (F35) | carry forward, remedy gated on RL-862's off-path capture landing | **Its blocking precondition is now met** — RL-862's off-path capture landed with Task 4B (`003f9d4`), for the production/sampled stream. **The latency clause itself is unchanged and still failing**: the explicit `ctx.options.trace=True` synchronous path (FR-258's, which is what F35 measured) is untouched by Task 4B — the commit message states this directly ("FR-258's caller-requested `ctx.options.trace` is unchanged"). Flagged rather than re-verdicted: the remedy (trimming what a trace records per node) is now unblocked and shares its lever with F55 (10.3) — nobody has connected the two, and the lead should decide whether to note the now-open gate in F35's row |
| **NFR-499 rate limit** (F48) | carry forward, provisionally WK-674 | **Unchanged.** `rate_limit_rps` is still accepted, persisted and returned, and consulted by nothing; `RATE_LIMITED` is still raised nowhere under `src/` |
| **F27(c), F29** (gate-coverage bundle with F33) | open, owner: the §14 review at WK-671's close | **Unchanged.** F33's partial landing (`3edd75a`, mypy `files` widened) predates the reopen (2026-08-29, before `8fd48b7`); nothing in this reopen's work touches error-code cross-checking (F29) or the `03` shape-vs-contract comparison (F27(c)) |

### 10.7 New findings from this audit

| Finding | Concerns | Decision | Status |
|---|---|---|---|
| **`w11-task-4d-nfr-rate-12.md`'s conservative caveat is buried** | Readability of the NFR-500 research note | fix before close | closed — one sentence added to §0's headline verdict line, cross-referencing §5's existing caveat rather than duplicating it; §5's bullet is unchanged |
...
unowned**, attributed to `main` rather than to any WK-671 task.

### 10.8 Preconditions

`docs/roadmap.md`'s WK-671 row states two, neither waivable by the lead: every reopened slice
...
FR-253 delivered and tested; FR-254 delivered and tested; FR-259 delivered and
tested; NFR-500 measured and FAILING (~2.58× over, conservative); NFR-493 measured
and split — throughput PASS at 5.09×, linearity NOT MEASURED (F52, unowned); NFR-489
discharged architecturally, verdict unchanged — measured and FAILING, carried to WK-674.

**I am satisfied.** Every reopened requirement has a verdict backed by re-run tests or a
measurement note read to its own numbers, §§1–9 are unedited, RL-922's five prohibitions
all hold (no artifact in this reopen's work claims NFR-489 passing, the 15 ms trigger is
...
delegation (`docs/rulings/RL-00918-wk-671-reopen-the-maintainer-s-direction-recorded-2026-08-30.md` §4) is the lead's to record here
with a dated line, per RL-919 §5's supersession.

**Accepted by the lead, 2026-08-30.** The auditor's six proposed verdicts in 10.9 are
**adopted unamended**: FR-253, FR-254 and FR-259 delivered and tested;
NFR-500 measured and **FAILING** at ~2.58× over, conservative; NFR-493 split —
throughput PASS at 5.09×, linearity **NOT MEASURED**; NFR-489 discharged architecturally
with its verdict **unchanged and still failing**, carried to WK-674.
...
**What this close does not claim.** WK-671's own hard target is **not met**: NFR-489 remains
measured and failing, and RL-921 discharged the *architectural question* it was carried
for without making the requirement pass. NFR-500 is over budget on a **conservative**
basis, so the real figure is worse. NFR-502 stays owed, NFR-490's latency clause stays
...
**One disposition the audit left to the lead (10.6), decided here.** NFR-490's remedy
precondition — RL-862's off-path capture — is met by Task 4B, while the measured clause
...
**The §14 plan review and the RFC-895…RFC-898 reconciliation are separate artifacts on this
