...
family: closure
kind: work
title: Audit record — nt-0012-0013-0014-adoption (`docs/process/checklists/work-item-close.md`)
...
was: docs/audit/work/nt-0012-0013-0014-adoption/README.md
---

# Audit record — nt-0012-0013-0014-adoption (`docs/process/checklists/work-item-close.md`)

> Not a `CLAUDE.md` §13 numbered-workstream closure — RFC-842, RFC-843 and RFC-895 are notes
> under `docs/notes/`, not a roadmap `W`-id, and this Work closes under the maintainer's
> 2026-08-30 delegation (`docs/plans/INDEX.md#2026-08-30-nt-0012-0013-0014-adoptionmd` §1.1), not
...
> record, [`nt-0010-0011-adoption`](CR-00891-audit-record-nt-0010-0011-adoption-claude-md-15-step-5.md).

## Scope

**The plan**: `docs/plans/INDEX.md#2026-08-30-nt-0012-0013-0014-adoptionmd`, filed against
...
| C | RFC-842's two rules given durable homes (Rulings A1, A2) | `97965be` | #456 |
| D | RFC-843's "remove the relay" (RL-904) | `97965be` | #456 |
| E | Runtime state file — RFC-895 artifact B | `b551060` | #506 |
...
| G | C2, the retry-cap hook (C3 dissolved, not built — RL-920 §3) | `9e8783d` | #516 |
...
the plan's slice G, was **dissolved by RL-920** §3
(`docs/rulings/RL-00920-q2-is-answered-differently-for-c2-and-c3-c3-is-dissolved-c2-gets-claude-settings-json-and-slice-g-is-re-cut-and-still-blocked.md:38-56`) — verified
...
the plan quotes the maintainer verbatim — *"authorise you to approve RFC-842 RFC-843 and
RFC-895 landing on behalf of me"* — and scopes it explicitly to landing these three notes,
**not** WK-671's close, WK-672, or any later phase. That is what §1.1 says; the lead's relay matches
it.
...
instruction is to confirm with the planner whether one is due; RFC-842/843/895 is a Work
under a note-adoption process, not a numbered workstream, and plan review 10 already ran at
WK-671's second close the same week — flagging for the lead to confirm with the planner rather
...
- **RL-902** (credential-lifetime rule → `secret-hygiene`): confirmed at
  `.claude/skills/secret-hygiene/SKILL.md:35`, heading *"A credential in a job directory is
  borrowed, not stored (RFC-842)"*.
- **RL-903** (search-by-shape rule → `close-workstream`, beside "A false zero argues"):
  confirmed at `.claude/skills/close-workstream/SKILL.md:310`, heading *"Search for the
  thing's shape, not its container's name (RFC-842)"*.
- **RL-904** (RFC-843's "remove the relay" → `delivery-process.md` §15, after the existing
  ...
  heading *"Remove the relay, do not merely distrust it (RFC-843)"*.

**Verdict: delivered and tested** (each ruling's own text is the acceptance criterion; each
landed verbatim where ruled).

**Finding, not in the lead's brief, found independently**: both `docs/rfcs/0012...md` and
`docs/rfcs/0013...md` still carry `Status: open` at `9e8783d`, unchanged since they were
raised — even though the rules each note asked for are now built and landed. `docs/rfcs/R
EADME.md`'s own definition of `open` is *"Raised, assessed, not agreed... nothing is
built"*, which is no longer true of either. Same shape RFC-895's own note hit and was
...
RL-907(d)'s acceptance test (a no-op cycle produces a byte-identical file, verified failing
...
already-filed carried finding, **F58** (`docs/findings/register.md`): no live process invokes the
...
**The substantive gap** (the lead's framing, verified rather than assumed): RL-920 §3
...
### RL-908's citation — cosmetic, verified

`delivery-process.md:110-113`'s §6 step 4 parenthetical currently reads *"(rulings record:
`docs/rulings/RL-00908-impact-matrix-row-4-does-not-sit-forever-it-inverts-and-part-c-row-5-closes-here.md`, RL-908, closing Part C row 5)"*,
where RL-908's own ruled text read `(RL-920)`. **Reworded, not wrong**: RL-908 is the
ruling that actually wrote this replacement sentence (closing Part C row 5), so citing Ruling
48 rather than RL-920 — which the sentence's *content* still credits by name in-body ("CI
runs the full gate...") — is arguably more accurate, not less. The body text after the
parenthetical is **verbatim** against RL-908's ruled replacement, confirmed word-for-word.
No finding filed.
...
|---|---|---|---|
| F61 (new) | C2's `PreToolUse` registration is bypassable the same way C3's git hook was, and unlike C3 has no CI-equivalent backstop — RL-920 never reconciled its own §3 and §4 against each other | carry forward with an owner — build the re-derivation cycle (RL-907(b)) or have the lead/maintainer explicitly accept the residual gap; no slice names this today | closed-with-findings |
...
| RFC-842/843 `Status: open` (new, unfiled) | Both notes' rules landed (slices C, D) but their own `Status` fields still read `open`, contradicting `docs/rfcs/README.md`'s definition | not the auditor's write scope (each note's `Owner` field names the lead/maintainer); flagged in this record and in the report to the lead rather than filed as a register row, since it is a one-line note correction, not a workstream-scale gap | open — needs the lead to correct both notes' `Status` fields |
...
| RL-908 citation reword | Cosmetic | accept — body verbatim, citation arguably improved | closed |
...
- **C3** — not delivered, **deliberately**: dissolved by RL-920 §3, discharged rather than
  built. `delivery-process.md` §6 step 4 carries the ruled replacement text.
- **The re-derivation/CI-equivalent backstop for C2** (RL-907(b)) — not built, no owner
...
bypassable in exactly the way RL-920 disqualified C3's git hook for — session-disabled
...
disqualifying defect without C3's saving grace, and RL-920 never reconciled the two. What
...
**One correction this close prompted.** RFC-842's and RFC-843's own `Status` rows still read
`open` while their rules had been in force since `97965be`, and RFC-895's read `accepted` after
