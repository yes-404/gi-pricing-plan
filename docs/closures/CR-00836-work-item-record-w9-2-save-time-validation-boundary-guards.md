...
family: closure
kind: work
title: Work-item record — W9-2 (save-time validation + boundary guards)
...
was: docs/audit/work/W9-2/README.md
---
...
Derived from `docs/plans/PL-00818-wk-669-implementation-plan-rating-algorithm-contract-validation-bundle-compilation.md` slice W9-2 first, then
evidenced. W9-2 validates a `RatingAlgorithm` at save time and enforces the four WK-668-
confirmed boundary guards: T1 `validate_algorithm` (03 §5.2), T2 the guards
(FR-273/274/275/276), T3 the API. The slice gate: an invalid graph is refused at save
...
|---|---|---|
| T1 — `validate_algorithm` (03 §5.2) | `packages/pricing-core/src/pricing_core/rating/compile.py` — result-type compatibility (FR-227), deterministic evaluation (FR-216/246), the four guards. **Spec-reconciliation vs `03` §5.2: PASS** — the signature `validate_algorithm(algo) -> list[ValidationIssue]` matches exactly | delivered |
| T2 — the four guards, each proven to fail on broken input | FR-273 `assert_integer_minor_round_trip` (self-check); FR-274 unguarded division refused + guarded accepted; FR-275 scale-cap-28 overflow refused; FR-276 foreign function refused (compiled against the engine). Pricing-core focused run: 8/8 passed | delivered |
