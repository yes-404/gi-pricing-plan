...
family: closure
kind: work
title: Audit record — nt-0010-0011-adoption (`CLAUDE.md` §15 step 5)
...
was: docs/audit/work/nt-0010-0011-adoption/README.md
---

# Audit record — nt-0010-0011-adoption (`CLAUDE.md` §15 step 5)

> Not a `CLAUDE.md` §13 workstream closure record — the RFC-840/841 adoption is not a
...
> procedure: whether Task 5 of `docs/plans/PL-00844-rfc-840-rfc-841-adoption-implementation-plan.md` delivered
> what the ruled proposal (`docs/plans/PL-00845-rfc-840-rfc-841-adoption-reconciliation-and-rulings-2026-08-29.md`)
...
`.claude/roles/*.md`, and WK-671 slice 1 run as the pilot — all of it before the maintainer
...
- All seven `.claude/roles/*.md` files re-read in full. §14 split correctly carried by both
  ...
  `docs/plans/PL-00844-rfc-840-rfc-841-adoption-implementation-plan.md` by path (`watcher.md:27-28`,
...
- `audit-docs.py` clean; 533 requirements defined (up from 532, PR #340's FR-243).
...
→ an unattended team stand-down → a new team respawned from `.claude/roles/*.md` → WK-671
...
**The pilot ran. Its findings are [`../findings/FD-00894-rfc-840-841-adoption-pilot.md`](../findings/FD-00894-rfc-840-841-adoption-pilot.md)** — §15 step 6's
...
- **#18** — `.claude/roles/**` (and `.claude/skills/**`) outside CI; two-halved fix, not
  landed.
- **#25** — five WK-671 decision points reduced to bare names in every durable artifact.
...
- **#30** — three further promotion candidates surfaced by the same handover-slimming pass
  that produced this record (the verify-don't-accept evidence trail, WK-671's true
...
the pilot findings. On acceptance `RFC-840` and `RFC-841` received dated `superseded` status
...
1. **The pilot was Task 1.1, not all of Slice 1.** Ruled by the maintainer mid-pilot. The
   Slice 1 plan (`docs/plans/PL-00846-wk-671-slice-1-evaluator-core-its-prerequisites-and-the-latency-harness.md`) still says at line 57 that
...
4. **Acceptance discharges nothing carried.** Register row **F28** lists every pilot finding
   ...
   four to the planner's §14 review at WK-671's close. They are enumerated there rather than
