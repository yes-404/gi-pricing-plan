...
id: WF-699
family: workflow
title: Approved Models to approved Rating Version
...
was: docs/workflows/wf-02-model-to-rating-version.md
---

# WF-699 — Approved Models to approved Rating Version
...
**Outcome:** An `approved`, fully-pinned Rating Version ready for deployment (WF-701).
...
| A2 | Pricing Actuary | Repeats for every rateable factor across perils. The technical rate is now the baseline, explicitly. | `03` FR-230 |
| A3 | Pricing Actuary | Edits cells with commercial judgement — softens `17-20` from 1.92 to 1.84. The editor shows the change against **both** the previous version and the technical seed, with the exposure weight behind each cell. | `03` FR-231 |
| A4 | Pricing Actuary | Applies a bulk operation: +2 % across the expense table. It is recorded as a parameterised operation, not 40 unexplained cell edits. | `03` FR-233 |
| A5 | Backend | Validates on save: complete key coverage, no nulls, values within declared bounds, no duplicate keys. | `03` FR-234 |
| A6 | Pricing Actuary | Saves each as a new Rate Table Version with a mandatory change note. | `03` FR-229 |
...
| B2 | Pricing Actuary | Declares the input contract: types, ranges, enum domains, nullability, descriptions. | `03` FR-213 |
| B3 | Pricing Actuary | Adds a `lookup` step: outcode → rating area, as at `effective_date` — not "now". | `03` FR-221 |
...
| B5 | Pricing Actuary | Adds `table` steps for each rate table, `expression` steps for the loading chain, and `constraint` steps for minimum premium and the maximum year-on-year increase, each with a `reason_code`. | `03` FR-220/225 |
| B6 | Frontend | Flags an error **on the node**: `s_office` consumes `commission_factor`, which nothing produces. The graph cannot be saved. | `03` FR-212, §5.3 |
| B7 | Pricing Actuary | Mounts the reusable `sub_graph:ncd-ladder@4` rather than re-drawing it. | `03` FR-217 |
| B8 | Pricing Actuary | Adds `output` steps with explicit rounding (`half_even`, 0 dp on pence). Rounding happens exactly once. | `03` FR-226 |
| B9 | Backend | Type-checks on save. An `expression` step multiplying a `money_minor` by a float-typed value is refused: `MONETARY_FLOAT_REFUSED`. | `03` FR-227/245, R2 |

### Phase C — Compile and pin (Pricing Actuary)

| # | Actor | Action | Refs |
|---|---|---|---|
| C1 | Pricing Actuary | `POST /rating-versions` — declares the algorithm version and every pin: rate tables, peril structure, reference tables. | `03` FR-237 |
| C2 | Pricing Actuary | `POST /rating-versions/{id}/compile` → `202` + Job (`rating.compile`). | `07` FR-399 |
| C3 | Worker → pricing-core | `compile_bundle()` validates everything at once: DAG structure, reference resolvability, artifact maturity, type compatibility, no `control`-intent factor in a rateable path, no unapproved custom objective transitively reachable. | `03` FR-240 |
| C4 | Worker | First attempt fails: `PIN_NOT_APPROVED` — the windscreen burning-cost model is still `review`. | `03` FR-240, FR-20 |
| C5 | Pricing Actuary | Waits for that model's approval (WF-698 phase E), recompiles. | — |
| C6 | Worker | Produces the Bundle: content hash, 84 MB, self-contained. It will score with no database access at all. | `03` FR-239, NFR-491 |
...
| E6 | Approver #2 | Rating Versions require two approvers by default policy. Requests changes: the +10 % movers need a communication plan. | `06` §4.2, FR-355 |
| E7 | Pricing Actuary | Adds the plan as a Commentary Block in the dossier, resubmits. Both the change request and its resolution stay in the trail. | `06` FR-355/377 |
| E8 | Approver #2 | Approves. | `06` FR-353 |
| E9 | Backend | Transitions to `approved`, pins the decision to the exact version and evidence ids, emits Audit Events. | `06` FR-356, R2 |
...
|---|---|---|
| Graph has a cycle or an unresolved reference | Rejected at save time, shown on the node | `03` FR-212, `RATING_GRAPH_CYCLIC` |
| Monetary value would be float-typed | `MONETARY_FLOAT_REFUSED` at save | `03` R2, FR-227 |
| A pinned artifact is not yet approved | `PIN_NOT_APPROVED` at compile | `03` FR-240 |
| A `control`-intent factor reaches a rate table | `CONTROL_FACTOR_IN_RATEABLE_PATH` | `02` FR-88, `03` FR-240 |
| Rate table has a gap in key coverage | `RATE_TABLE_INCOMPLETE` at save | `03` FR-234 |
| Golden quote mismatch | Promotion blocked until each mismatch is deliberately accepted | `03` FR-260 |
| Evidence stale relative to the artifact | `EVIDENCE_INCOMPLETE` at submission | `06` FR-356 |
| Submitter tries to approve | `SUBMITTER_CANNOT_APPROVE` | `06` R1 |
| Referenced model later flagged `dataset_invalidated` | Flag propagates to the Rating Version and to the inbox; approval blocked | `01` FR-53, `06` FR-359 |
...
- A dossier assembled from all of the above (`06` FR-376).
...
|---|---|
| A — Rate tables | `03` FR-228, FR-229, FR-230, FR-231, FR-233, FR-234, FR-235, FR-236 |
| B — Algorithm | `03` FR-RATE-1..13, 28..33 |
| C — Compile | `03` FR-237, FR-238, FR-239, FR-240, FR-241, FR-242; FR-20 |
| D — Evidence | `03` FR-257, FR-258, FR-259, FR-260, FR-261, FR-262, FR-263, FR-264, FR-265, FR-266; `04` FR-294 |
