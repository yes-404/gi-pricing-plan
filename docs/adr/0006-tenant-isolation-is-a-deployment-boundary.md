# ADR-0006 — Tenant isolation is a deployment boundary

- **Status:** accepted
- **Date:** 2026-08-15
- **Deciders:** maintainer
- **Related:** OQ-OVR-1 (decided by this ADR), FR-OVR-13, FR-OVR-15, FR-OVR-16,
  `07-platform.md` §2, §3.5 and §3.6, `06-governance.md` FR-GOV-24, OQ-PLAT-4, OQ-GOV-1
...
Three options were on the table (OQ-OVR-1):
...
- **Nothing that carries tenant data is reachable from more than one tenant's deployment**
  ...
  audit chain are per tenant (FR-OVR-15).
- **What may be shared is what carries no tenant data**: container images, infrastructure
  code, CI, and public reference data (ONS/OGL, bank holidays — `01` FR-DATA-32).
...
what makes FR-OVR-16 necessary: a dossier regenerated in one deployment must name the build
...
the answer changes what W14 builds in Phase 2, and a deployment story written on the
