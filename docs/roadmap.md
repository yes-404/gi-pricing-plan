...
| **5. EBM** | FR-MODEL-37 | Verdict on file: not started, owner is "the slice that first fits an `ebm` model" — and `ebm` is one of the four Model types in `CLAUDE.md` §7's vocabulary, so W5 owns it unless reassigned. The stated cost is `interpret` as a third heavy dependency serving one requirement *(2026-08-21: delivered by the EBM slice — see the slice record.)* |
