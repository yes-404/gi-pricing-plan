...
> Transparency Artifact** (FR-132). Fitting a black box is allowed; pricing with an
...
| **Holdout** | The named split (from `01` FR-76) not used for fitting, on which reported performance is measured. |
...
> FR-115 would recognise as a surfaced failure. **The spec was right and the code was
...
| **FR-115** | Non-convergence, separation, rank deficiency, and aliased columns are surfaced as explicit, named fit errors with the offending factors identified — never as a silently returned degenerate fit. |
...
| **FR-169** | *(appended 2026-08-26, OQ-613 decided (a))* **A surrogate's spec carries a companion `slug@version` address, and the id stays the lookup key.** `GlmSpec.approximates_model` is a nested block — `model_slug` in the shared `Slug` pattern and `model_version` (`≥ 1`) — following `interval_for`'s id-and-version shape with the id left out: `approximates_model_id` remains the lookup key, and a block carrying the same id a second time is the fact stated twice `CLAUDE.md` §2 refuses. The pair is set **if and only if** `approximates_model_id` is set, refused at the type in both directions on FR-141's pattern. The slug half alone resolves to the family's latest version, which may legitimately differ from the pin, so the type refuses nothing beyond the iff — but the pair **joins the `spec_hash` payload** (FR-206, `v10` → `v11` in the same commit), and at reservation `MODEL_APPROXIMATION_INVALID`'s compared set grows from three to five: a companion naming a slug or version different from the pinned model is refused by name, the way `dataset_version_id`, `split_ref` and `factors` already are. What the bare id could not say to a human — which model, at which version — the companion says directly, and a reviewer no longer resolves one id to get the version. |


> **Amendment to FR-152, 2026-08-25 (W6b-7, the objective library and certificate slice).**
> "Not refused outright" above is a rule about the **verdict**, and `CertificateResult.outcome_of`
> is where it is computed: a `violated` check yields `certified_with_findings`, never `failed`.
> It has never been a rule about the **surface**, and nothing until now stopped a view from
> receiving that non-blocking verdict and rendering the `violated` row as a failure anyway —
> which reinstates, in the only place a human reads it, exactly the outright refusal this
> requirement exists to prevent. The rule is therefore extended to the surface rather than split
> across a second id: **a view rendering a certificate must not style, label, group or order a
> `violated` check as a failure.** A certificate reads as blocked only where its `overall` is
> `failed`; `certified_with_findings` reads as certified. **That is this requirement's rule, and
> citing FR-146 for it inverts it** — read literally, 42's "must pass … hessian
> non-negativity over the sampled domain" makes a `violated` check blocking, which is the
> tension this requirement was written to resolve. 42 gates submission on a certificate; which
> outcomes carry an objective past that gate is settled here, and in `platform/objectives.py`'s
> `certify`, where `overall is FAILED` is the whole of the test and anything short of it lands
> the objective in `certified`. Where a check is `violated`, the clipping strategy this
> requirement already requires be declared is shown beside it — a finding without the strategy is
> the half an Approver cannot act on. **The rule is on the status, not on the check that emits
> it.** `violated` is convexity-only by implementation today, emitted at a single site, but
> `CheckStatus` is a free enum on `CertificateCheck.status` and `outcome_of` branches on the
> status alone without asking which check carried it; a rule written as "the convexity check"
> would narrow silently the day a second check emits `violated`. **What discharges this is a
> rendering test, not this id's existence** — a view given `certified_with_findings` that still
> presents the `violated` row as a failure must fail. This discharges FR-24's fourth
> carve-out, which falls due at the plan of the slice that builds the view carrying it.
### 3.8 Diagnostics

| ID | Requirement |
|---|---|
| **FR-170** | Every fit produces a persisted **Diagnostics** artifact. Diagnostics are computed once at fit time and read thereafter; the UI never recomputes them. |
| **FR-171** | Universal diagnostics (all model types): actual-vs-expected by factor level and by banded continuous factor (exposure-weighted, with CIs); lift/gains curves by predicted decile; Gini / normalised Gini; calibration by predicted decile; residual summaries; overall A/E ratio on train and holdout. *(Amended 2026-08-17, WK-661. **Double lift is removed from this list** and lives on the comparison artifact, FR-186 and §4.11. It was listed here and `PartitionDiagnostics.double_lift` was populated by nothing — nothing could populate it: double lift is pairwise, the comparison model is unknown at fit time, and FR-170 makes diagnostics computed once at fit time and read thereafter, so it could not be filled later either. A field that is structurally always null is worse than an absent one, because a reader takes it for a measurement that came out empty.)* |
| **FR-172** | GLM-specific diagnostics: deviance, null deviance, AIC/BIC, dispersion estimate, degrees of freedom, per-factor type-III deviance test with p-value, relativity plots with confidence bands, standardised deviance and Pearson residual plots, leverage/Cook's distance on a sample, and a VIF/aliasing report. |
...
> than exact — the elastic-net grid FR-112 scans starts at zero and moves away from
...
> **FR-196 addendum, 2026-08-19 (WK-661, the paired-quantile slice) — the boundary
> holds and gains a second door.** FR-196 says the platform offers exactly one
> interval kind and names `prediction_interval` as the only future second. Building
> FR-199 found a third case it did not anticipate: a paired-quantile interval
...
> `prediction_interval`. **FR-198 already refuses a GBM approximation on exactly this
...
> *mean* was not decided by FR-198 and is decided by FR-200 — a requirement
...
**Amended 2026-08-18 (WK-661, the interaction slice).** `operand_factor_ids` is new, and it is
what makes FR-83's `interaction` type expressible: the field carries the Factors being
crossed, by id, for the reason `banding_id` carries a Banding by id — a slug changes meaning
the next time someone re-cuts a boundary. FR-92 has the design and its consequences.
...
- **`expression` is still printed here and still does not exist on the type.** That is
  FR-208's recorded verdict (OQ-582, 2026-08-17) and not an oversight this slice
...
> **Two corrections to this section's own contract, 2026-08-15 (WK-661), both found by
> implementing it.**
>
...
>   requirement (`01` FR-61). A band *is* a level, so the banding schema now points at
...
>   check. FR-100 says *configurable*, and configuration a reviewer cannot read is a
>   default with extra steps: two fits of the same banding could apply different floors and
>   the artifact would record neither.

> **Where the last band ends (added 2026-08-15, WK-661).** Under `closed: "left"` band *i* is
...
(FR-106). `credibility_pk` is the `(p, k)` pair the standard was derived from — 1 082
...
> **Amended 2026-08-22 (WK-661, the audit-remediation slice).** Bühlmann–Straub is now built —
> OQ-579 decided on 2026-08-15 that WK-661 builds *two* methods, and until this date one was
...
>   FR-106's "re-derive `Z` rather than take it" literally true, and a test does exactly
...
>   not run.
> * **A book that cannot support the estimate is refused, not silently shrunk** — FR-118.

> **Amended 2026-08-15 (WK-661).** Three corrections the implementation forced, and one it
> **got wrong**:
>
...
>   of the same numbers. A band is a level, `01` FR-61 already defines a level's
...
> (FR-116 as amended). **Live 2026-08-21** (the offset-from-another-model slice). A `GbmSpec`
...
applied to the **response at fit time** (FR-127). Two models differing only in their
...
> (FR-206; WK-661, the closure slice).** This section recorded `v4 → v5` and `v8 → v9`
...
> omitted a transition *both* of them held — `modelling.py:113` and FR-200's row in §3
...
> before `v3`.
>
> - **`v3 → v4`** (2026-08-19, FR-200) — `interval_for` joined the payload: a bound's
...
>   each other by FR-204's dedup.
> - **`v5 → v6`** (2026-08-21, FR-112/182) — `select_by` and `cv` joined the payload:
...
> - **`v6 → v7`** (2026-08-21, FR-114) — `tweedie` joined the payload: a power
>   estimated over `tweedie.p_grid` is a different fitted question than a fixed one.
> - **`v7 → v8`** (2026-08-21, FR-116) — `offset_model_ref` joined the payload: the
...
> - **`v9 → v10`** (2026-08-22, FR-111) — **the first bump for an interpretation
...
>   FR-204's dedup would otherwise answer the next caller with an unweighted fit for a
...
```json
{
  ...
  "approximates_model": null
}
```

> **Corrected 2026-08-17 (WK-661, OQ-582).** This block used to show a nested
...
> on the GLM arm are **absent entirely** and owned by Phase 1b (FR-207). *(Amended
...
> is the one the FR-112/FR-182 CV path built, mirroring `GbmSpec`'s nested
...
> note named — the shape the FR-112/FR-182 CV path built, mirroring `GbmSpec`'s
> nested `early_stopping`. **`method` is `01` FR-73's three fold-construction
...
> **Added 2026-08-21 (FR-114).** `tweedie` carries the profile-likelihood grid when
...
> **`interval_for` is live from 2026-08-19 (WK-661, the paired-quantile slice)** —
> FR-207's staging rule, and the last of the fields OQ-582 listed as
...
> model's interval, and FR-199 makes that an opt-in the actuary pays 2–3× a fit
> for. On the spec rather than on `Model` because it changes the model's identity —
> FR-200(i) has the argument and the FR-137 precedent it rests on.
```

`objective.kind = "custom"` replaces `name` with `ref: "custom_objective:<slug>@<version>"`.

> **Amended 2026-08-17 (WK-661, the GBM arm).** Three corrections, made by building the union.
>
...
>   declaration.** FR-121 says the platform *constructs* `base_margin` from the
...
>   **fit result** (`GbmFitResult.base_margin`), which is where FR-126's load-time
>   assertion needs it.
> * **`loss_treatment` sits on the common block, not on this arm.** FR-127 applies it
...
>
> * `categorical_handling` has **no default** — a default would be FR-131's silence.
> * `early_stopping.on` has **no `train` value**, and `on: "holdout"` requires `split_ref`.
>   FR-124 refuses training-set early stopping; reaching it by omitting the split is
>   the same defect arrived at more quietly.

> **Amended 2026-08-17 (WK-661, found by the `WF-698` journey test).** **A banded Factor is
...
>
> FR-131 is unaffected: it refuses the silent label-encoding of an **unordered**
> categorical, and a band is ordered with its map persisted. FR-125's dtype
...
> **Amended 2026-08-18 (WK-661, the custom-objectives slice).** **`response` is required
...
> family, so `response` becomes the only thing FR-153's applicability check and the
...
>
> **Amended 2026-08-26 (W6b-17, OQ-613 decided (a)).** The field is no longer bare:
> the companion `approximates_model` — `model_slug` and `model_version`, following
> `interval_for`'s shape with the id left out — sits beside it, FR-169. The sentence
> above, "a block under a different name would contradict the requirement", describes the
> state before this date: the block exists now, under the name the requirement's own
> amendment names, and what a reviewer once resolved one id to get, the pair says
> directly. `spec_hash` moves `v10` to `v11` with the pair (FR-206), for the reason
> the v5 move above was stated: the model a surrogate approximates, including how to
> address it, is part of what that surrogate *is*.
>
...
> **Added 2026-08-21 (WK-661, the EBM slice, FR-140).** The common block is inherited
...
> (triples) is **declared-and-unbuilt** (FR-207); `max_bins` defaults to 64 and must
...
>
> **Refused by name, 2026-08-21 (FR-207):** §7's families and binomial `log_loss` are
...
> **FR-206:** the EBM fields join the `spec_hash` payload and the tag moves `8 → 9`.

### 4.5 Custom objective — `template` catalogue

Shipped templates, each with analytic gradient/hessian in `pricing-core` (FR-143):
...
| `quantile` | `alpha` | Pinball loss | Paired quantile models — the only supported GBM prediction interval (FR-194, FR-199) |
...
Each template declares its `applicability` block (FR-153) and its own parameter
validity ranges (e.g. `tweedie.p ∈ (1, 2)` exclusive, `cap > 0`).

**This catalogue is the whole of Phase 1's custom-objective surface** (FR-150,
OQ-573 decided 2026-08-15): `expression` objectives ship in Phase 2, behind a flag that
is off until they do. A template still certifies (§4.7, FR-151) — the machinery is
...
> **Amended 2026-08-18 (WK-661), three cells corrected by building the catalogue.**
>
...
>   Model Spec (§4.4, FR-127/128), where it is part of `spec_hash` and demands its
...
Shared by `expression` custom objectives (FR-144), `expression` factors
(FR-95), preparation `derive_expression` (`01` FR-36), and `expression`
...
Rules enforced by the parser (FR-145):
...
> **§4.6 measured against the implementation 2026-08-22 (WK-661, the audit-remediation slice),
> and it diverges in three ways — none previously recorded, and the third is the one that
> matters.** The parser being described is `pricing_core.data.expressions`, built in **WK-660**
> for `01` FR-36; §4.6 was written for the Phase 2 `expression` objective grammar and
> has never been checked against it. All three are stated rather than resolved: this is a
> §0 case, and which side is wrong is **OQ-573's slice (WK-690)** to decide, since it owns
...
Produced by `POST /custom-objectives/{id}/certify`; required for submission (FR-146).
...
```json
{
  ...
  "checks": [
    ...
    {"name": "symbolic_vs_numeric_hessian", "status": "pass",
     "detail": "max relative error 3.8e-4, within the step-aware tolerance for h=1e-6 on a loss of this curvature (FR-149)"},
    ...
    {"name": "branch_discontinuity", "status": "warn",
     "detail": "gradient and hessian are discontinuous at the branch boundary exp(f) = y; 41 of 10,000 sampled points fell within h of it and were excluded from the derivative comparison (FR-147/148)"},
...
>   `library_versions`. The JSON above shows them flat. The split is ADR-703: `certify_objective`
...
>   step they actually used, which is the reason FR-149 asks for it.
...
> **`MetricCertificate` (FR-157), added 2026-08-19.** Same two-object split as
> `ObjectiveCertificate` and for the same ADR-703 reason: `certify_metric` computes a
...
>   the direction the metric declares (FR-156). This is the metric's analogue of
...
> **`tweedie` is live from 2026-08-21 (FR-114)** — the estimated Tweedie power,
...
> **`split_ref` and `diagnostics_id` are live from 2026-08-16 (WK-661, diagnostics).** Both
> were among the fields OQ-582 named as declared-and-dead, and `diagnostics_id` was its
...
> `expression` and `filter` stay unimplemented, and OQ-582 was **decided 2026-08-17**
> on exactly that staging rule — FR-207 carries it, and names them with their owners.
>
> **`split_ref` is required to reach `fitted`**, though the field itself is optional: a
> spec may be explored without one, but FR-183 makes a diagnostic without its holdout
> counterpart a defect and FR-202 makes diagnostics the condition of `fitted`, so a
...
> **`covariance_blob` is live from 2026-08-18 (WK-661, the prediction slice)** — FR-207's
...
> FR-195 exists for: the artifact holds `p` numbers where the matrix is `p x p`, so no
...
> **`flags` and `approval_request_id` are live from 2026-08-17 (WK-661, the lifecycle).** Both
> were on OQ-582's list of fields declared and dead; the slice that creates an approval
...
> detail.** `01` FR-53 makes validation re-runnable on an already-validated version, so
...
> FR-205 exists to stop. It is derived from the Dataset Version's *current* status on
...
> now a stated verdict with an owner rather than an open question: OQ-582 was decided
> 2026-08-17 and FR-207 lists them, while FR-211 restates R3 in the direction the
> link actually runs.
...
> **R3 restated 2026-08-17 (WK-661, OQ-582) — FR-211.** It used to read
...
> `diagnostics_id`** *(amended 2026-08-22, WK-661's audit-remediation slice).* `status ≥ fitted
...
>
> **The evidence is traced to `00` FR-4, not to a §4.8 requirement id, because §4.8 has
> none for immutability.** R2 is a §1.3 hard rule with no id of its own, and the ids §4.8
> cites are about presence, transparency and lineage. FR-4 — *"every Artifact is
> immutable once it leaves `draft`"* — is the requirement R2 instantiates for a Model, and
> is what the new tests carry alongside FR-203, which the existing raw-`UPDATE`
...
2026-08-22 (WK-661, the audit-remediation slice)**. §4.8 has carried a GLM example since Phase 0
and an EBM one since 2026-08-21, and never a GBM one, which is why FR-161's
...
wrongly. `booster_format` is ADR-705 (`pickle` is not a refused value, it is not a value);
`base_margin` is FR-126, and omitting the offset at scoring fails **silently** on both
...
> **The fit result IS the model (2026-08-21, WK-661, the EBM slice, FR-140).** An EBM is
...
> estimator**: the declarative artifact alone is enough to rescore the model (ADR-705).
>
...
> **`ebm_shape_functions` is the third kind, added 2026-08-21 (WK-661, the EBM slice).** An
...
> shape (FR-140's requirement text stays as the contract). The block is `terms_blob`
...
> (FR-168, OQ-601).** No build produces it yet: `build_shap_summary` publishes
...
> today — a fixture copied from it would be rejected. *(Noted 2026-08-24, WK-692 closure proposal
...
>
> *(Amended 2026-08-25, W6b-5a: **built.** `ShapInteraction` carries the field, so the example above **is** a valid instance and a fixture copied from it is accepted — pinned by a test that constructs it exactly as printed rather than by a reader comparing two blocks. The field is emitted carrying `null` where no ratio is defined, rather than omitted: omission would need `exclude_none` at every serialisation site, and a per-site setting holds at one and not another.)*
...
**Six corrections, 2026-08-18 (WK-661, the peril-structure slice).** §4.10 printed an example
...
- **The reconciliation carries a per-peril breakdown**, which is new. It is where
  FR-128's treatment is stated beside the number it produced, and the total is checked
...
- **Every large-loss treatment but `none` carries its `evidence_blob`**, which is
  FR-189's "recorded with its calibration evidence" made structural. A restoration
...
- **Money is integer minor units and `ratio`/`tolerance` are exact decimals rendered as
  strings** (FR-10), which the example's bare `0.9959` did not show.
- **A structure has a lifecycle**: `draft → reconciled → review → approved → superseded →
  archived`. `draft → review` is **not** an edge — FR-190 makes the reconciliation the
  evidence FR-191's approval reads, so a structure reaching an approver without one is
...
Submission additionally refuses a reconciliation whose status is `fail`. FR-190 makes
...
*Added 2026-08-17 (WK-661, the comparison slice).* §5.2 has named `ModelComparison` as a return
...
**Invariants** — `|model_refs| ≥ 2` (FR-186 compares *two or more*; one model measured
...
**The shared holdout is stored, not promised.** `01` FR-76 records the split on the
...
question a selection decision (`WF-698` E2) actually turns on.

**Immutable, and enforced at the privilege layer** (FR-43): `06` §3.3 makes a comparison
required evidence for a Model approval where a predecessor exists.

### 4.12 `Backtest`

*Added 2026-08-18 (WK-661, the backtest slice).* FR-187 has named the operation since Phase
...
declared from Phase 0 and typed `null`, and nothing could ever have populated it: FR-170
...
`PartitionDiagnostics.double_lift` was removed before it (FR-171, 2026-08-17): a field
that is structurally always null reads as a measurement that came out empty. `cross_validation`
stays: FR-182 computes it *at fit time*, so `Diagnostics` is where it will land.

**One `PartitionDiagnostics`, and it is not called a holdout.** FR-183's "train and
...
themselves Dataset Versions (`01` FR-76) with ids the contract has never been shown, so
...
**Immutable, at both layers** (FR-43) — privileges narrowed to `SELECT, INSERT` *and* the
...
privileges and not the row trigger until `e1f2a3b4c5d6` (2026-08-18); `01` FR-44 has
...
other kind (FR-150's rule, applied to metrics). `params` must be exactly the named
...
says capped. A status past `draft` requires a `certificate_id` (FR-157). `direction`
has no default (FR-156). `applicability` must be no wider than §4.5's own applicability
for `template` — an author may narrow it further, never extend it, because widening claims
the analytic loss is valid somewhere `pricing-core` never established for it (FR-155).
...
| Method | Path | Purpose |
|---|---|---|
| `POST` | `/api/v1/factors` | Create/version a Factor (FR-83) |
...
| `POST` | `/api/v1/bandings/propose` | Propose boundaries by method against a dataset version (FR-98) |
| `POST` | `/api/v1/bandings/evaluate` | Recompute band statistics for **edited** boundaries, persisting nothing (FR-102) |
...
| `POST` | `/api/v1/groupings/propose` | Propose a grouping by method (FR-105) |
| `POST` | `/api/v1/groupings/evaluate` | Recompute the deviance/df evidence for an **edited** mapping (FR-102) |
| `POST` | `/api/v1/groupings` | **201** Persist a Grouping |
| `GET` | `/api/v1/groupings?dataset_id=` | List groupings, latest version first |
| `POST` | `/api/v1/model-specs/validate` | **200** with `SpecValidation` — `ok` plus every problem, never only the first. A spec that merely cannot be fitted is not a bad *request*, so it is not a 4xx; a spec naming a version that does not exist is a 404 (FR-153, FR-185) |
| `POST` | `/api/v1/models` | **202** Fit → Job; returns existing model on `spec_hash` match (FR-204) |
| `GET` | `/api/v1/models` | List models, filtered by `family` and `status`, cursor-paginated newest first (`00` §5.2). Every listed model reports `flags: []` — FR-205's flag is a per-model read, so a page of 50 would be 51 round trips to decorate rows the caller is about to narrow; `GET /models/{slug}` answers it, and is the only place it gates anything |
...
| `POST` | `/api/v1/models/{id}/transparency` | **202** Build a transparency artifact (FR-132) |
| `GET` | `/api/v1/models/{id}/transparency` | The model's most recent artifact (FR-139) |
| `POST` | `/api/v1/models/{id}/backtest` | **202** Backtest against another dataset version (FR-187) |
| `GET` | `/api/v1/models/backtests/{id}` | The stored backtest artifact (§4.12, FR-94) |
| `POST` | `/api/v1/models/compare` | **202** Comparison artifact for 2+ models on a shared holdout (FR-186) |
| `GET` | `/api/v1/models/comparisons/{id}` | The stored comparison artifact (§4.11) |
| `POST` | `/api/v1/models/{id}/predict` | **200** Score rows with FR-194's uncertainty (dev/debug scale, row-capped; production scoring is `03`) |
| `POST` | `/api/v1/models/{id}/submit` | Submit for approval (`06`) — `fitted → review`, `If-Match` required |
| `POST` | `/api/v1/models/{id}/archive` | `draft \| fitted \| superseded → archived` (FR-202), `If-Match` required |
...
| `POST` | `/api/v1/custom-objectives/{id}/derive` | Symbolically derive gradient/hessian from `loss` (FR-144) — Phase 2, refused with `OBJECTIVE_KIND_NOT_ENABLED` (FR-150) |
...
| `POST` | `/api/v1/custom-metrics` | **201** Create → `draft` (FR-154, FR-155) |
| `GET` | `/api/v1/custom-metrics` | The workspace's metrics, cursor-paginated, filterable by `status` and `slug`, each row carrying `usage_count` (FR-167) |
| `GET` | `/api/v1/custom-metrics/{id}` | The metric, its status and its certificate outcome (FR-162) |
| `POST` | `/api/v1/custom-metrics/{id}/certify` | **202** Run §4.7's metric checks (FR-157) |
| `GET` | `/api/v1/custom-metrics/{id}/certificate` | The latest `MetricCertificate` for that version (FR-162) |
| `POST` | `/api/v1/custom-metrics/{id}/submit` | Submit for approval (FR-154's lifecycle) |
| `GET` | `/api/v1/custom-metrics/{id}/usage` | Blast radius: models using this metric version (FR-162) |
| `POST` | `/api/v1/peril-structures` | **201** Create/version a Peril Structure (FR-188) |
| `GET` | `/api/v1/peril-structures` | The workspace's peril structures, cursor-paginated, filterable by `status` and `slug` (FR-167) |
| `GET` | `/api/v1/peril-structures/{id}` | The structure and its reconciliation (FR-192) |
| `POST` | `/api/v1/peril-structures/{id}/reconcile` | **202** Recompute reconciliation (FR-190) |
| `POST` | `/api/v1/peril-structures/{id}/submit` | Submit for approval, `reconciled → review` (FR-192) |

> **Amended 2026-08-22 (WK-661, the audit-remediation slice): three interface rows, and §5.2
...
> `backtest_model`, leaving FR-116 documented at length in §3.4 and unreachable from
...
> banding's own stored `minimums` (FR-100). Measured on a banding declaring
...
> **Amended 2026-08-15 (WK-661, the GLM spine).** Two corrections, made by building it:
>
...
>   the specification has already been fitted (FR-204). The table said 202; the second
...
> **Amended 2026-08-15 (WK-661, bandings and groupings).** The four banding and grouping routes
...
>   (FR-101), which is a creation whatever the slug already was.
>
...
>   resolving them needs a database, which ADR-703 forbids `pricing-core`. The spec's
...
>   **Closed 2026-08-15 (WK-661):** `fit_glm` takes `progress` again and reports six stages;
...
> OQ-582 rather than settled here. *(Decided 2026-08-17 — the staged contract, as
> FR-207. All four fields named in this paragraph are now live.)*
>
> **Narrowed 2026-08-15 (WK-661):** §4.1's `banding_id` and `grouping_id` are now live fields
> on `Factor`, because their slice landed — which is what OQ-582's recommendation
> ("re-widen it as the slices land") describes. `expression` remains declared and unbuilt,
> and FR-208 states its verdict: selectable, unresolvable, and refused by name.
...
> **Amended 2026-08-17 (WK-661, the comparison slice).** `POST /models/compare` is built, and one
...
> one needs a database that ADR-703 forbids `pricing-core`. `predict_glm` and
...
> * **Candidates must agree on their weighting scheme** (FR-184), refused with
...
> **Amended 2026-08-17 (WK-661, the model lifecycle).** `POST /models/{id}/submit` is built, and
> one route is **added to the table above**:
>
> * `POST /api/v1/models/{id}/archive` — FR-202 names `archived` and no endpoint reached
...
> * **`If-Match` is required on both** (`00` §5.4, FR-450), and what it guards is stated
...
>   409 and only one of them is actionable. WK-660 deferred the header for being the second kind
...
> **Amended 2026-08-17 (WK-661, the GBM arm).** No route is added or removed. `POST /models`
...
>
> * **FR-153's *objective applicability* half is now built for the GBM arm.**
>   `SpecProblemKind` gains `objective_unsupported`, and a spec naming an objective outside
>   FR-120's set — or a Custom Objective while FR-142 is unbuilt — comes back as a
...
> * **`Diagnostics.gbm` is populated (FR-174), and the evaluation curve lives there.**
>   The code briefly put the curve on `GbmFitResult`; `diagnostics.schema.json` has had it
>   under `gbm` since Phase 0 and FR-174 calls it a diagnostic asking for **train and
>   holdout**, which is FR-183's shape. The **code was wrong and moved**. Only
>   `best_iteration` stays on the fit result, because scoring needs it and diagnostics are
>   not loaded to score.
>
> A GBM's `ComplexityDiagnostic.parameter_count` is its **leaf count** (FR-185). A
...
> **Amended 2026-08-18 (WK-661, the backtest slice).** `POST /models/{id}/backtest` is built, and
> one route is **added to the table above**:
>
> * `GET /api/v1/models/backtests/{id}` — FR-94. The table declared the `POST` and no
...
> Three decisions the building settled, recorded here rather than left in the code:
>
> * **`backtest_model` lives in `diagnostics.py` and reuses `_partition`.** FR-187's
...
> **The catalogue reconciled against `errors.py`, 2026-08-22 (WK-661, the audit-remediation
> slice), applying `00` FR-22's own six verdicts rather than inventing new ones.**
> OQ-548 decided this on 2026-08-21 and FR-22 lists exactly these six; what had not
...
>   query and re-raises `06`'s `EVIDENCE_INCOMPLETE` (FR-211), and pickle is refused by
...
>   Phase 2's `expression` objectives (OQ-573). They are not struck because the slice that
>   raises them is scheduled. `submit_for_review` stands in with `VALIDATION_FAILED` today.
>
> **The machine check that would keep this true is FR-22's and the maintainer's**, with
...
> **`GLM_FIT_FAILED` added 2026-08-22 (WK-661, FR-115's remainder).** `fit_glm` caught
...
> **Added 2026-08-22 (WK-661, the EBM slice).** `ebm_monotonicity_verified` refuses to
...
> **Added 2026-08-21 (WK-661, the EBM slice).** `EBM_MONOTONE_CONSTRAINT_INCOMPLETE` refuses an
...
> so the refusal arrives as a code, not a wrong model. (Amended 2026-08-21, the WK-661 EBM
> fit task: the original note claimed `interpret` raises a bare `ValueError`; the pinned
> library does not.)

> **`MODEL_INTERVAL_PAIR_INVALID` added 2026-08-19 (WK-661, the paired-quantile slice).**
...
> side that already has one (FR-199, FR-200). Its own code rather than
...
> only in one of those three is accepted today. *(Amended 2026-08-26, W6b-17, OQ-613
> decided (a): the compared set is **five**. `model_slug` and `model_version` — the
> companion `approximates_model`, FR-169 — join the three above, because a companion
> naming a slug or version different from the pinned model is a relation failure and the
> same refusal must name it; the deliberate non-comparison of `offset`, `weight` and
> `seed` is untouched.)*
...
> **Added 2026-08-17 (WK-661, the GBM and transparency slices).** The ten codes above are new;
...
> `SCORING_FEATURES_MISMATCH`: FR-126 exists because omitting the offset at scoring
...
> **Added 2026-08-18 (WK-661, custom objectives).** Six `OBJECTIVE_*` codes arrive with the
...
> `OBJECTIVE_REF_MISMATCH` are the caller's wiring (ADR-703 — `pricing-core` is handed the
...
> `OBJECTIVE_REQUIRES_OFFSET` are FR-153's applicability, refused before any boosting
> round; `OBJECTIVE_EARLY_STOPPING_UNSUPPORTED` is the honest edge of what is built, since a
> custom eval metric is FR-154 and deferred; `OBJECTIVE_HESSIAN_STRATEGY_UNSUPPORTED`
> is FR-152 meeting a template with no Gauss-Newton form to drop a term from.
>
...
>
> **Added 2026-08-21 (WK-661):** FR-116 also raises it when a `kind="model"` spec reaches
> fit or scoring without the resolved offset array.

An **invalid lifecycle transition** (FR-202) is `VALIDATION_FAILED` at `409`, not a code
...
module's submission and approval paths (FR-363 R4, FR-205).

> **Amended 2026-08-18 (WK-661, the custom-objectives slice).** Five of the six
...
> FR-166. The fifth artifact in this module to declare its writes and no read, and the
> sharpest case of it: certification is a **202**, so without the read there was no way to
> learn its verdict, and FR-163 puts an Approver in front of a certificate they could
> not fetch.
>
...
>   `OBJECTIVE_KIND_NOT_ENABLED`. It is the `expression` path, and FR-150 gates it
...
> * ~~**`POST /custom-metrics` (FR-154) is not built, and is deferred to Phase 1b with
...
>   stopping under a custom objective turned out to need one (FR-160). A custom
...
> **Amended 2026-08-19 (WK-661, the custom-metrics slice).** Three `METRIC_*` codes arrive
> with the Custom Metric endpoints (FR-162). `METRIC_REF_UNRESOLVED` is
...
> ahead of the code that raises them: they belong to FR-159's refusals from the
...
> the scanned path (FR-112, FR-182) — the `key_column`/`time_column` skew
...
> **`GLM_TWEEDIE_POWER_GRID_EDGE` added 2026-08-21 (FR-114).** Raised by the
...
> `GlmFit` (glm.py's dataclass, FR-112/FR-182) and the interface comment lagged
...
def check_banding(frame: pl.DataFrame, banding: Banding, *,
                  ...
                  # None ⇒ fall back to banding.minimums (FR-100)
...
def predict_gbm(result: GbmFitResult, booster: bytes | object, data: pl.DataFrame,
                ...
                # booster: bytes (loads fresh) or load_gbm_booster's return (RL-874)
                # nthread: added 2026-08-29 (WK-671 Task 1.4, F-W11-1-2) — NFR-501's
...
def load_gbm_booster(model_type: Literal["xgboost", "lightgbm"], booster: bytes,
            ...
            # added 2026-08-29 (WK-671 Task 1.3, RL-874) — the loaded-booster seam:
            # deserialise once, hand the result to predict_gbm on every subsequent call
            # nthread: added 2026-08-29 (WK-671 Task 1.4, F-W11-1-2) — set once here, at
...
def approximation_spec(spec: GbmSpec, *, source_model_id: UUID,
                       source_model_slug: str, source_model_version: int) -> GlmSpec
def build_glm_approximation(result: GbmFitResult, booster: bytes, spec: GbmSpec,
                            ...
                            source_model_slug: str, source_model_version: int,
                            bandings=None, groupings=None,
...
> **Amended 2026-08-27, at WK-664's close (plan review 5, proposal 4.2).** The two
> approximation functions were missing `source_model_slug` and `source_model_version`, which
> the code has required since `OQ-604`'s ruling (#246) made `reserve_model` derive the
> stored slug as `{source family slug}-approx`: the slug and version are the human-readable
> half of the surrogate's address (`FR-169`), and a caller copying these signatures
> could not name the source model the functions describe. The code was right and this block
> was behind it.
>
...
> (W32-5).** It is how FR-175's cap is spelled, it has existed since the cap did,
...
> (FR-168, OQ-601).** Unlike the two notes above, this block is **not** behind the
> code here — it is in front of it. The built signature takes no `holdout`; the kwarg was added
> with FR-168, whose owner is the slice that builds the factor workbench's suggestion
...
> easy to misread as built. *(Noted 2026-08-24, WK-692 closure proposal Part D item 1. The
...
>
> *(Amended 2026-08-25, W6b-5a: **built.** The signature above is now the built one — `holdout` is required, with no default, matching the sibling `build_glm_approximation`. It was declared required here before it existed, and that is what the code was made to match; an optional keyword would have let a forgotten argument produce the pre-ratio artifact silently, indistinguishable from the legitimate absence a `null` ratio records.)*
...
> **Two signatures corrected, 2026-08-16 (WK-661, diagnostics).** `predict_glm` and
...
> database, which ADR-703 forbids this package. This is the same correction already
...
> `with_interval`: FR-194's interval comes from the covariance matrix, which the fit
...
> > (ADR-703 keeps the blob store out of this package) and returns
...
> > `fit_gbm` already made for the booster, and for the same ADR-703 reason: `pricing-core`
...
> **Corrected again, 2026-08-17 (WK-661, the GBM arm).** `predict_gbm` was the third instance
> of the same defect, and it is now the third one fixed: it took a `Model`, whose
> references need a database ADR-703 forbids this package. It takes the `GbmFitResult` and
> the booster **bytes**.
>
...
>   to the booster (FR-125) and this package cannot store a blob (ADR-703). Content
...
> * **`holdout` is a parameter.** FR-124's early stopping needs the rows, and
...
>
> `apply_loss_treatment` is new and was declared nowhere — FR-127's cap is applied to
...
>
> > **A defect in this function, found and fixed 2026-08-18 (WK-661, custom objectives).**
...
> > objectives slice needed the link recorded anyway (FR-130), and the defect was
...
> **Corrected a fifth time, 2026-08-18 (WK-661, peril structures).** `assemble_risk_premium`
...
> (`model:motor-ad-frequency@7`), and resolving one needs the database ADR-703 forbids
...
> * **`observed`, `exposure` and `tolerance` are parameters.** FR-190 compares modelled
...
>   carries numbers and not their provenance, and FR-128 requires the treatment to be
...
def make_xgb_objective(fns: ObjectiveFns):
    def objective(preds: np.ndarray, dtrain: xgb.DMatrix):
        ...
        g, h = fns.grad(y, f, w), fns.stabilise(y, f, w)   # FR-152 strategy
        _finite_or_abort(fns, g, h, y, f, round_index)     # FR-165
        return g, h
    return objective
```

> **Four corrections to this section, 2026-08-18 (WK-661, custom objectives).** Each is the
...
>   backend adapter re-implementing FR-152, and two of the three strategies silently
>   becoming the third.
...
>   certificate carries an id, a job and a `certified_at` that ADR-703 forbids this package
...
> the whole of `ObjectiveKind.EXPRESSION` (FR-144/145). They are gated behind
> `expression_objectives_enabled`, off throughout Phase 1 (FR-150), and WK-661 shipped the
...
| View | Route | Contents |
|---|---|---|
| Factor workbench | `/factors/:datasetVersionId` | Column list with profile one-ways (`01` FR-61), banding editor with draggable boundaries and live band stats, grouping editor with relativity-ordered levels and merge tolerance slider, monotonic-direction and intent controls, interaction suggestions ranked by SHAP interaction strength which the actuary adds as explicit Factors or ignores (FR-135) *(the exposure-share and holdout-lift columns are withdrawn, 2026-08-23; what stands beside a candidate is its holdout strength ratio, FR-168)* |
| Model spec builder | `/models/new` | Dataset/split pickers, response & offset/weight, factor multi-select, model-type tabs, objective picker (builtin or approved custom), hyperparameters, live spec validation (FR-153) |
| Model list | `/models` | List of fitted models with slug, status and owner; link to Model detail. *(Added 2026-08-27, closure-audit standard: `00` §5.6 declares `/models` as the canonical list route, which this table lacked.)* |
| Model detail | `/models/:slug` | Spec summary, coefficient/relativity tables with CI bars, fit metadata, lineage strip, flags. `?version=` selects one; the latest by default *(Amended 2026-08-27, plan review 5, proposal 4.3: the **lineage strip is not rendered** — `ModelDetailView.vue` renders none, and no build note recorded the departure. Under FR-24 this is a declared-prose cell; the binding surface is the generated contract.)* |
| Diagnostics | `/models/:slug/diagnostics?version=` | Train/holdout side-by-side throughout; A/E by factor, lift, calibration, residuals, GBM eval curves and importances, CV fold dispersion. *(Amended 2026-08-24, WK-664. **Double lift is struck from this cell.** FR-171 removed it on 2026-08-17 — it is pairwise, the comparison model is unknown at fit time, and `PartitionDiagnostics.double_lift` was structurally always null — and it lives on the comparison artifact, FR-186, which the Model comparison row below already carries. The struck instrument survived here for a week and was then transcribed into a load-bearing chart count in a filed plan, which is how it was found. **This cell is prose, not an enumeration**, and it is non-exhaustive in the other direction too: it names none of `partial_dependence`, `permutation_importances`, `monotonicity` or `quantile_crossing` from `GbmDiagnostics`, nor `type_iii_tests`, `vif`, `aliasing` or `leverage_blob` from `GlmDiagnostics`, all of which the generated contract carries. **Derive no binding chart set from this cell** — under FR-24 this is a *declared-prose* cell, the affordance OQ-549's decision of 2026-08-24 named after finding it already invented here; the binding enumeration is the generated contract.)* |
| Model comparison | `/models/compare?ids=` | Aligned metric table, double-lift chart, factor-by-factor relativity diff |
| Custom objective library | `/objectives` | List with status, applicability, usage count; editor with live parse errors (expression authoring is gated by `expression_objectives_enabled` and off throughout Phase 1 — FR-150), derived gradient/hessian display, loss-curve preview at chosen parameter values *(Amended 2026-08-27, plan review 5, proposal 4.3: the derived **gradient/hessian display and the loss-curve preview are not rendered** — the view is list-only. The cell's Phase-gating note covers the editor only; these two departures had no note until this one.)* |
| Objective certificate | `/objectives/{id}/certificate` | Per-check `pass` / `warn` / `violated` / `failed` (§4.7) — `violated` presented as a **finding**, never as a failure, since it is the ordinary result for a legitimate non-convex pricing loss (FR-152) — convexity heatmap over the sampled `(y, f)` domain, smoke-fit result |
| Custom metric library | `/metrics` | *(registered 2026-08-24, WK-692 closure proposal Part D item 3)* List with status, applicability and usage count, the mirror of the objective library above; editor with live parse errors and a link to the metric certificate (FR-162). Backed by `GET /custom-metrics` (FR-167), which this table asked for before it named the screen that consumes it *(Amended 2026-08-27, plan review 5, proposal 4.3: the **editor with live parse errors is not rendered** — the view is list-only. The view's header comment records the certificate-gap departure but not the editor; this note records the editor.)* |
| Peril structure library | `/peril-structures` | *(registered 2026-08-24, WK-692 closure proposal Part D item 3)* List with status and slug, each row linking into the per-structure detail view below. Backed by `GET /peril-structures` (FR-167). **No usage count** — that requirement's count is of Model Specs referencing the artifact, and the reference runs the other way for a Peril Structure (§4.10), so the column is absent by specification rather than pending |
| Peril structure | `/peril-structures/{id}` | Per-peril model pins, large-loss treatment, reconciliation panel |
| Backtest | `/models/:slug/backtests/:backtestId` | *(registered 2026-08-23; **declared-prose**, FR-24)* Period-by-period A/E and lift ~~against the held-out window~~ **on the Dataset Version the backtest names**, the backtest's own definition (~~split,~~ periods, exposure basis) shown beside its results, and the fit it was run against. Addressed by backtest id, not by model — a model has many (FR-94). *(Amended 2026-08-25, WK-664. **The struck words describe a split this artifact does not have**, and they are how this was found: a builder scoping the view read them and hand-wrote a `"Holdout"` caption. §4.12 and FR-187 both settled on 2026-08-18 that a backtest holds one `PartitionDiagnostics`, and that naming it a holdout "would claim a split nobody made"; `BacktestSummary` carries no split field at all. This cell was registered five days after that amendment, to close a roadmap gap, and imported the pre-amendment vocabulary — which is how a cell that binds nothing still produces a defect. **The single partition's caption is "Backtest".** Not the period and not the `slug@version`: the four shared instruments interpolate the caption into their own column headings (`${label} A/E`, `${label} exposure years`), and the view is addressed by one backtest id, so there is nothing to disambiguate inside it. Dating the series is what `05-monitoring.md` reads §4.12's many-per-model backtests for, and captioning for that here would be building ahead of the phase. Not "out-of-time" either: FR-187 requires only a version *other than* the one fitted on, so a caption asserting *later* claims an invariant the type does not enforce — the same defect as "Holdout", at a smaller scale. **This cell binds none of that**, and correcting prose leaves it prose (FR-24). What binds is **FR-187, extended on the same date** to state the rule the caption is an instance of: a caption for that partition may not assert a relationship the artifact does not carry. It went there rather than into a new requirement because the rule governs any surface naming the partition — this view, an export, `05-monitoring.md` later — and because a new frontend-only id is what this table's 2026-08-23 note argues against: `req-coverage.py` scans backend tests only, so it would arrive permanently unbacked, while FR-187 is already evidenced over HTTP. OQ-587's contract-is-the-floor rule is not what is at work here either — it governs a view needing *data the contract does not carry*, its words being "if WK-664's diagnostic view needs those fields", and a caption needs no field, no computation and nothing persisted. `PartitionLabel` is the control: a hand-authored caption over two contract fields, built with the Diagnostics view, for which no requirement was ever raised or owed.)* |
| Prediction | `/models/:slug/predict` | *(registered 2026-08-23)* Ad-hoc scoring against a fitted Model: input row or uploaded batch, the prediction with its interval where the model type offers one, and the refusal by name where it does not (FR-180) |
...
the deviance/df trade-off (FR-107). An actuary should never have to fit a model to
find out whether a grouping was sensible.

> **Built 2026-08-15 (WK-661).** The factor workbench is routed at `/factors/:datasetVersionId`
...
>
> The interaction requirement is met through **FR-150's `/evaluate` routes**, which
...
> path was removed from §5.1 by the 2026-08-15 amendment, and FR-192 and FR-166 —
...
> (FR-94, FR-187, FR-180); what was missing was the registry entry.
>
...
> fifty-one registered views have no FR; the demo guide (`07` FR-409) parses these tables as
...
> **Not built, 2026-08-18 (WK-661, custom objectives).** Neither `/objectives` nor
> `/objectives/:slug@:version/certificate` has a view, and both stay owned by **WK-664** with
> the rest of `02` §5.3. The slice built the API and the certification behind them
> (FR-166 added the two reads a certificate screen needs), so what is missing is the
...
> and only the screen.
>
> Two Contents items are worth flagging before WK-664 starts, because both are harder than the
> column implies:
>
...
>   legitimate non-convex pricing loss (FR-152) — rendering it as a failure would tell
...
Full journey: [`WF-698-dataset-to-model.md`](../workflows/WF-00698-dataset-to-approved-model.md).
Custom objective path: [`WF-702-custom-objective-lifecycle.md`](../workflows/WF-00702-custom-objective-lifecycle.md).
...
|---|---|---|
| **glum** | All GLM fitting (FR-110, FR-111, FR-112, FR-113, FR-114, FR-115, FR-116) | `GeneralizedLinearRegressor`; `std_errors()` and `covariance_matrix()` (non-robust, robust HC-1, clustered) and the coefficient table with CIs and p-values satisfy FR-113 directly — verified to exist, not assumed; Tweedie/Poisson/Gamma, native offset handling, elastic-net CV paths |
| **statsmodels** | Fallback/cross-check diagnostics (FR-172) | Type-III deviance tests, residual diagnostics, coefficient cross-validation against glum |
| **XGBoost** | Primary GBM (FR-119, FR-120, FR-121, FR-122, FR-123, FR-124, FR-125, FR-131) | Custom objective `(grad, hess)` signature, `base_margin`, `monotone_constraints`, `interaction_constraints`, JSON model IO, `QuantileDMatrix` for memory |
...
| **NFR-482** | A stored Model round-trips: export → import into a clean instance → predictions identical to the last representable digit (FR-5). |
| **NFR-483** | Security: user-supplied expressions never reach `eval`/`exec`; the parser rejects out-of-grammar input with a position-accurate error; compiled objectives cannot allocate unbounded memory or exceed their per-round time budget (FR-165). |
| **NFR-484** | Audit: factor/banding/grouping creation and edit, fit start and completion, objective derivation/certification/approval, and every status transition emit Audit Events with before/after state. |
| **NFR-485** | Memory: fitting a 5 M × 60 dataset stays within 32 GB, using `QuantileDMatrix`/streaming construction rather than duplicating the design matrix. |
| **NFR-486** | Diagnostics artifacts stay under 50 MB per model; larger evidence (SHAP dependence, residual scatter) goes to content-addressed blobs referenced from the artifact. |
| **NFR-487** | **The GLM per-factor diagnostic block — FR-172's type-III tests — costs no more than one fit wall-clock per *tested* factor.** The unit is one element of `type_iii_tests`, **not** one element of `factors`: interaction operands carry no design column and are skipped (FR-92), a spec with fewer than two factors yields none, and a refit that will not converge is skipped rather than reported. The denominator is a **warm** fit of the same spec at the same scale, for the reason §9's measurement note records. The bound is the mechanism rather than a target: each iteration runs one full `fit_glm` over the remaining factors plus one full `predict_glm`, so a refit of a strictly smaller model that costs more than the fit it is a refit of names a defect rather than a scale. (Added 2026-08-22, OQ-572 decided.) |
...
> **NFR-477 measured 2026-08-15 (WK-661), and it is met for three of the four methods
...
> | `propose_grouping`, `credibility_weighted`, 10 000 levels | 4.24 s | **met**, and only after NFR-478 — it was 8.59 s while the source summary was computed twice |
...
> (`01` FR-61 holds the one-way summaries), or replace Ward with a contiguous 1-D
...
> **NFR-477 re-measured 2026-08-22 (WK-661, the audit-remediation slice), and the record
...
> Two limits travel with it: `01` FR-61 stores one-ways only for the columns the
...
> **NFR-479 measured 2026-08-22 (WK-661), and it is not met — because of another requirement
...
>
> Everything `compute_diagnostics` does *except* FR-172 fits the budget: 9.0 % and
> 9.5 % at the small scales, 32.1 % at 678 013 × 60 — 2.1 points over, which is tuning. The
> rest is FR-172's type-III tests, which **drop each factor and refit**. That is one
...
>
> **The budget and FR-172 cannot both hold as written.** A diagnostic containing *F*
> refits of the model cannot cost 30 % of one fit for any *F* above zero, and FR-170
...
> is raised as **OQ-572** with three options rather than settled here — §14 makes a
...
> so OQ-572 must also say which denominator the budget means.
> 
> **Amended 2026-08-22 (OQ-572 decided — option (a), denominator settled as fit wall-clock).** Every number above stands: it is what was measured, and its verdict column records what was believed on the day. What changed is the requirements the numbers are read against.
> 
> | Fit | Block | Read against | Measured | Verdict |
> |---|---|---|---|---|
> | GLM, 50 000 × 12 | all but type-III | NFR-479, ≤ 50 % | 8.9 % | **met** |
> | GLM, 50 000 × 24 | all but type-III | NFR-479, ≤ 50 % | 9.5 % | **met** |
> | GLM, 678 013 × 60 | all but type-III | NFR-479, ≤ 50 % | 32.1 % | **met**, 1.56× |
> | GLM, 5 M × 60 | all but type-III | NFR-479, ≤ 50 % | 34.1 %, *projected* | **met**, 1.47× — projected, never measured |
> | GLM, 50 000 × 12 | type-III | NFR-487, ≤ 1.0× per tested factor | 0.417× cold, 0.95× warm-corrected | **met** |
> | GLM, 50 000 × 24 | type-III | NFR-487, ≤ 1.0× per tested factor | 0.574× cold, 0.98× warm-corrected | **met** |
> | GLM, 678 013 × 60 | type-III | NFR-487, ≤ 1.0× per tested factor | **> 1.61×**, censored | **not met** |
> | GBM, 75 000 × 60 × 500 | permutation + partial dependence | NFR-488, ≤ 0.06 fits per pass | 0.0480 fits per pass | **met**, 1.25× |
> 
> **Three things the re-read turned up that the original note did not.**
> 
> **The "9.0–9.5 %" that justified keeping 30 % is the two small arms only.** The third GLM arm is 32.1 %, so the non-type-III ratio *triples* across the measured range rather than sitting flat, and it projects to 34.1 % at 5 M × 60 — NFR-475's own scale. It is not scale-free because diagnostics are roughly linear in rows while the fit is measured at n^0.837, so a percentage stated without a scale is not a budget. NFR-479 now names one.
> 
> **The per-factor multiple is an artefact of a cold denominator; corrected, it is flat at ≈ 1.0.** Each type-III iteration calls the full `fit_glm` plus a full `predict_glm`, so a refit over one fewer factor *ought* to cost about one fit — yet the raw multiples are 0.417× and 0.574×. That is only possible if the denominator carries fixed cost the refits do not: the bench's first fit runs cold, one row-and-factor pair per process. Solving the two refit costs for a fixed term gives 1.59 s and 1.26 s against denominators of 2.82 s and 3.05 s — two independent estimates agreeing that **41–56 % of each denominator is one-time cost**. Corrected, the multiples are 0.95× and 0.98×, which is exactly what the mechanism predicts. **Those two figures are derived, not measured**, and a warm-denominator run is owed before NFR-487 is called met at any scale. The original note's "growth is super-linear because each refit is over a wider design matrix" does not survive the third point: a two-point law fitted to F = 12 and F = 24 predicts 1 307 s at F = 60, and the run exceeded 2 400 s — under-predicting by at least 1.84×. Two points cannot evidence a growth exponent, and the only other observation falsifies the one they give.
> 
> **The GBM's 3 002 % is not "over 60 factors".** It is over roughly 625 full-population scoring passes, and 90 % of them are partial dependence: one pass per partition, 61 for permutation importance at one repeat, and 560 for partial dependence — 20 categorical factors × 8 levels plus 40 numeric × 10 quantiles. That is 0.694 s per pass, or 0.0480 of a fit, which is what NFR-488 is set against. Per *factor* it is 0.500 fits, a number that means nothing outside this exact factor mix. Permutation importance and partial dependence are not equal partners in the cost; they split roughly 1 to 9.
> 
> **What is still not met, and who owns it.** NFR-487 is breached at 678 013 × 60 — more than 1.61× per tested factor against a 1.0× bound — and the observation is **censored**, so the true figure is unknown rather than large. The excess is diagnosable rather than mysterious: every refit re-resolves the factors and rebuilds the design matrix from scratch, and each adds a full-frame `predict_glm` that the original fit never pays. **Owner: Phase 1b**, together with the warm-denominator run the corrected multiples depend on. NFR-479 and NFR-488 are met at every arm measured.

---

> **NFR-480 measured 2026-08-22 (WK-661), and it is met with fifty times the headroom.** All
...
> constraint. **Owner: none required.** If a Phase 2 `expression` objective (FR-144)
...
> **NFR-486 measured 2026-08-22 (WK-661), and it is met by nearly three orders of
...
> **NFR-475 and NFR-485 measured 2026-08-22 (WK-661) at four scales below the one they
...
> reaches **25.9 GB — 81 % of NFR-485's budget**, on a machine that cannot hold it to
...
> linearly; and none of this exercises the streaming construction NFR-485 names — the
> curve above is the dense GLM design matrix, precisely the path `QuantileDMatrix` does not
> apply to.
>
> **NFR-476 is measured once and its growth is not.** A GBM at 75 000 rows × 60 factors
...
>
> NFR-476's second clause — an `expression` objective adding no more than 25 % — is
...
> **NFR-481, -7, -8 and -9 given verdicts 2026-08-22 (WK-661), two of them correcting what
> the plan believed.**
>
> * **NFR-484 — evidenced** for every act that has a before: `backend/tests/test_model_nfrs.py`
...
> * **NFR-483 — half met, and the half that is met is now tested.** The `eval`/`exec`
...
>   interchangeably. **~~The position-accurate clause is not met~~ — met 2026-08-22 (WK-661, the
...
>
>   **The per-round objective time budget is not implemented anywhere**; FR-165's NaN/inf
>   abort is, with four markers. ~~**Owner: WK-661** for the error position~~ — **delivered**; the
>   per-round budget travels with FR-165 and is unchanged by this.
> * **NFR-482 — out of Phase 1 scope. Maintainer verdict 2026-08-22**, on plan review 3's
...
>   **FR-5 carries zero markers**, and its serialisability half is evidenced only
>   incidentally by `model-schema` round-trip tests naming other requirements.
>
>   **It is not a WK-661 defect and never was**: no workstream row has ever named a Model export
>   path, so there is nothing WK-661 failed to build. The requirement is **not superseded** — ids
...
>   written. Recording that is what lets WK-661 close honestly: an unevidenced requirement with no
>   owner would otherwise read as WK-661 owing a feature it was never asked for.
>
>   **Verdict reversed 2026-08-27 (WK-665, OD2).** The finding the verdict rested on — "no Model
...
>   **delivered**, and its parent FR-5 now carries NFR-482's marker.
> * **NFR-481 — ~~half evidenced, and the roadmap called it evidenced~~ both halves now
>   carry markers, 2026-08-22 (WK-661, the closure slice).** It asks for identical GLM coefficients
...
>
>   **A defect found while evidencing it, raised as OQ-599 and since resolved: `fit_glm`'s
...
>   seed argument, and `spec.seed` is the single seed for every fitter (FR-179). This
