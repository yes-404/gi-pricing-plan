...
> citing FR-MODEL-42 for it inverts it** — read literally, 42's "must pass … hessian
...
> presents the `violated` row as a failure must fail. This discharges FR-OVR-21's fourth
...
>
> **Amended 2026-08-26 (W6b-17, OQ-MODEL-43 decided (a)).** The field is no longer bare:
> the companion `approximates_model` — `model_slug` and `model_version`, following
> `interval_for`'s shape with the id left out — sits beside it, FR-MODEL-129. The sentence
...
> directly. `spec_hash` moves `v10` to `v11` with the pair (FR-MODEL-86), for the reason
...
> only in one of those three is accepted today. *(Amended 2026-08-26, W6b-17, OQ-MODEL-43
> decided (a): the compared set is **five**. `model_slug` and `model_version` — the
> companion `approximates_model`, FR-MODEL-129 — join the three above, because a companion
...
> **Amended 2026-08-27, at W6b's close (plan review 5, proposal 4.2).** The two
> approximation functions were missing `source_model_slug` and `source_model_version`, which
> the code has required since `OQ-MODEL-34`'s ruling (#246) made `reserve_model` derive the
> stored slug as `{source family slug}-approx`: the slug and version are the human-readable
> half of the surrogate's address (`FR-MODEL-129`), and a caller copying these signatures
...
| Backtest | `/models/:slug/backtests/:backtestId` | *(registered 2026-08-23; **declared-prose**, FR-OVR-21)* Period-by-period A/E and lift ~~against the held-out window~~ **on the Dataset Version the backtest names**, the backtest's own definition (~~split,~~ periods, exposure basis) shown beside its results, and the fit it was run against. Addressed by backtest id, not by model — a model has many (FR-MODEL-92). *(Amended 2026-08-25, W6b. **The struck words describe a split this artifact does not have**, and they are how this was found: a builder scoping the view read them and hand-wrote a `"Holdout"` caption. §4.12 and FR-MODEL-57 both settled on 2026-08-18 that a backtest holds one `PartitionDiagnostics`, and that naming it a holdout "would claim a split nobody made"; `BacktestSummary` carries no split field at all. This cell was registered five days after that amendment, to close a roadmap gap, and imported the pre-amendment vocabulary — which is how a cell that binds nothing still produces a defect. **The single partition's caption is "Backtest".** Not the period and not the `slug@version`: the four shared instruments interpolate the caption into their own column headings (`${label} A/E`, `${label} exposure years`), and the view is addressed by one backtest id, so there is nothing to disambiguate inside it. Dating the series is what `05-monitoring.md` reads §4.12's many-per-model backtests for, and captioning for that here would be building ahead of the phase. Not "out-of-time" either: FR-MODEL-57 requires only a version *other than* the one fitted on, so a caption asserting *later* claims an invariant the type does not enforce — the same defect as "Holdout", at a smaller scale. **This cell binds none of that**, and correcting prose leaves it prose (FR-OVR-21). What binds is **FR-MODEL-57, extended on the same date** to state the rule the caption is an instance of: a caption for that partition may not assert a relationship the artifact does not carry. It went there rather than into a new requirement because the rule governs any surface naming the partition — this view, an export, `05-monitoring.md` later — and because a new frontend-only id is what this table's 2026-08-23 note argues against: `req-coverage.py` scans backend tests only, so it would arrive permanently unbacked, while FR-MODEL-57 is already evidenced over HTTP. OQ-MODEL-15's contract-is-the-floor rule is not what is at work here either — it governs a view needing *data the contract does not carry*, its words being "if W6b's diagnostic view needs those fields", and a caption needs no field, no computation and nothing persisted. `PartitionLabel` is the control: a hand-authored caption over two contract fields, built with the Diagnostics view, for which no requirement was ever raised or owed.)* |
...
> the certification behind these views were built by W5, and what is missing is still the screen
> and only the screen.
>
...
| ~~**OQ-MODEL-36**~~ | ~~**Does `POST /api/v1/model-specs/validate` answer `422` for a spec that merely cannot be fitted?**~~ §5.1's row states the rule without qualification — *"A spec that merely cannot be fitted is not a bad **request**, so it is not a 4xx"* — and the handler's docstring gives the reason: a 422 *"would say the request was malformed, and §5.3's live validation would then be a form that errors on every keystroke."* But the request body is typed as the `ModelSpec` union, so FastAPI parses it before the handler runs and `GlmSpec`'s three `model_validator(mode="after")` guards raise at parse time — a Poisson spec with `offset.kind == "none"`, a Tweedie power outside 1 to 2 exclusive, and a surrogate whose response column disagrees with itself. The route declares 422 and returns it for all three, so a spec that merely cannot be fitted gets one. `docs/roadmap.md`:2487 claims the 200-not-4xx behaviour as W5-delivered, so the contradiction sits in a closure record too. Measured 2026-08-25: the actionable sentence, which names FR-MODEL-19, is carried **only** in `errors[].message` — `title` and `detail` are generic. Not a duplicate of `SpecProblemKind.OFFSET_MISSING`, which fires on a *declared* offset column absent from the version. Options are an unparsed body re-validated through the same union so errors become `SpecProblem`s, amending this §5.1 row so a contract-level invariant is a 422, or moving the family-keyed validators into `validate_spec` with a matching refusal on `POST /models` — the last being FR-MODEL-81's existing two-presentation shape rather than a loss of the storability guard, and costing instead the type-level unrepresentability that `CLAUDE.md` §7 argues an actuarial invariant deserves. Recommendation is the first, held with a narrow margin. **No owner:** all three are a §5.1 or backend change, `W6b` is a frontend workstream, and `W32` — which owned Phase 1b's backend half and built this route — is closed, so it needs a new workstream or a later phase. **Nothing is blocked meanwhile** — `W6b-4a` renders and tests the 200, 404 and 422 branches distinctly. Raised 2026-08-25 by `W6b-4a`. **Open**, owned by the maintainer; options and evidence are in `docs/open-questions.md`. **DECIDED 2026-08-26: (a) — unparsed body, re-validated through ModelSpec, 200 with the complete SpecProblem list** |
| ~~**OQ-MODEL-37**~~ | ~~**`GbmSpec.objective` takes a closed set of four and `GbmSpec.eval_metrics` takes an open one, through the same `GbmFunctionRef` — so nothing in the contract says which objectives the platform supports.**~~ FR-MODEL-26's set lives in `pricing-core` (`gbm.py`:107-115), whose comment states the hazard: *"Two hand-written lists would eventually disagree about which objectives the platform supports, and the disagreement would show up as a spec that validated and then failed."* `GbmFunctionRef.name` is open for a metric reason that does not apply to the objective arm, and `model-schema` depends on `pydantic` alone so it cannot import the set. The contract carries it nowhere machine-readable — zero occurrences in `generated.json`, and the hand-authored `model-spec.schema.json` lists the four names in a description string ending in an ellipsis, explicitly *"Not an enum"*. `W6b-4a` therefore authored `BUILTIN_GBM_OBJECTIVES` in the frontend, the second hand-written list that comment warns against. Options are a closed type for the objective position only, an endpoint publishing the set, or keeping the frontend list behind a divergence test; the first is recommended, since it removes the second list rather than policing it. **No owner** — the first two are `model-schema` or `02` §5.1 changes and `W32` is closed. **Nothing is blocked meanwhile**: `W6b-4b` ships the divergence test. **Widened 2026-08-25: a second surface.** `FITTABLE_OBJECTIVE_STATUSES` (`model_schema/objectives.py`:174) is the `{certified, review, approved}` set a fit accepts, distinct from R4's `approved`-alone rule for a model reaching approval. `ObjectiveStatus` is generated so the type reaches clients; the subset does not, so a picker offering fittable objectives hand-writes a fourth copy. **Option (a) does not transfer** — a subset of an enum's members is not a field type — so only the endpoint or the divergence test apply here. Raised 2026-08-25 by `W6b-4b`. *(Amended 2026-08-25, W6b-5b: **(c)'s CI prerequisite is shipped.** That option needed a frontend-side test reading `gbm.py` as text **and** a `paths` addition so it runs on a `pricing-core` edit. `frontend.yml` now carries `packages/pricing-core/src/pricing_core/modelling/**` on **both** triggers, added by `W6b-4b`, and `W6b-5b` relies on the same path for a second such guard (`factorIntents.test.ts`, `OQ-MODEL-41`). So (c) no longer costs a CI change for any future guard over that directory — which lowers its cost relative to (a) and (b) without changing the recommendation, since (c) still leaves the frontend authoring a modelling fact.)* **Open**, owned by the maintainer; options and evidence are in `docs/open-questions.md`. **DECIDED 2026-08-26: (a) — GbmSpec.objective gets a Literal-typed ref; eval_metrics stays open** |
| ~~**OQ-MODEL-38**~~ | ~~**FR-MODEL-128's holdout strength ratio does not have `1` as its null.**~~ The published five are the top five *by in-sample strength*, and the holdout is a lookup on those same five — so the denominator is a **selected maximum**, biased upward by the selection, while the numerator is an independent re-measurement and unbiased. The expected ratio therefore sits below `1` even where the structure is identical in both partitions, and **even at equal N**. A second, weaker mechanism compounds it: the row cap `_INTERACTION_ROW_CAP` is applied per call inside `_pair_strengths`, so a partition smaller than the cap contributes fewer rows and equal caps give unequal N — which widens the spread without shifting the centre, since `mean(\|·\|)` is unbiased at any N. The requirement's interpretive sentence — "a ratio near `1` says the structure survives out of sample" — is written as though `1` were the unbiased null, and it is not. Options are equalising N (partial, and not absorbable: lowering the training cap changes the published `strength` that does the selecting), debiasing the denominator, publishing a null band beside the ratio, or ranking on the holdout instead; the band is recommended, since the field supports a judgment and a judgment needs a baseline. Whichever is chosen, the interpretive sentence needs amending with it. **No owner in W6b** — every remedy is a spec change and `W32` is closed. Raised 2026-08-25 by `W6b-5a`, which built the requirement as written. **Open**, owned by the maintainer; options and evidence are in `docs/open-questions.md`. **DECIDED 2026-08-26: (c), with (b) if a band cannot be estimated cheaply** |
| ~~**OQ-MODEL-42**~~ | ~~**What does `deprecated` mean for a Custom Objective — may new specs select one, and do existing pinned uses continue?**~~ `02` FR-MODEL-46 names `deprecated` as the lifecycle's terminal state; FR-OVR-1 (nothing hard-deleted) backs the pinned-uses half — an existing pin keeps resolving. Nothing states whether a **new** spec — a new Model, Peril Structure or Rating Version — may select a `deprecated` version. `wf-05` D6 asserts *"existing pinned uses continue, new specs cannot select it"* with only FR-MODEL-46 cited, which states neither half's second clause. *Recommendation: (a) selection is forbidden — `deprecated` is read/execute-only, pins continue, new specs cannot select; costs a selection-time validation and an error code. (b) selection with a warning — advisory, no new enforcement. (c) no selection semantics — a label only. A label new work may ignore does no governance work, so (a).* Open; raised 2026-08-25 (W6b decision maker) by the wf-01…05 journey-prose sweep. **DECIDED 2026-08-26: (a) — deprecated is read/execute-only: existing pins continue, new specs cannot select** |
| ~~**OQ-MODEL-41**~~ | ~~**`FactorIntent` publishes four arms, the platform honours two, and `POST /factors` accepts all four.**~~ FR-MODEL-116 supersedes `offset` and FR-MODEL-120 `diagnostic`, both deliberately keeping the arm in the published contract, so the union will never narrow and no machine-readable permitted subset exists — only the complement, `REFUSED_FACTOR_INTENTS` in `pricing-core`. Harder: that constant is referenced **nowhere under `backend/`**, so the only refusal is `resolve_factors` on the fit path. A superseded intent is accepted, stored and audited, then detonates at fit — "let the backend refuse" is not a fallback that exists, and a UI control is currently the only guard. Two surfaces, one remedy space, so one question. Options are refusing at creation, publishing the live/superseded distinction, both, or neither; **both** is recommended, refusal first, since it closes a hole rather than a duplication. **No owner** — backend and `model-schema` surfaces, and `W32` is closed. `W6b-5b` ships the control and a divergence test. Raised 2026-08-25. **Open**, owned by the maintainer; options and evidence are in `docs/open-questions.md`. **DECIDED 2026-08-26: (c), (a) first if sequenced** |
| ~~**OQ-MODEL-40**~~ | ~~**`GET /api/v1/models` cannot filter by dataset version**~~, so a view scoped to one — the factor workbench — filters in the client over a paginated list. A filter applied to one page renders "no models" while matches sit on a later page, indistinguishable from a version that has none. `OQ-MODEL-35`'s shape one route over, and the only other recorded instance. A quieter consequence: `Model` carries **no timestamp**, so "most recent" exists only as the route's `ORDER BY id DESC` over UUIDv7 ids — a client that filters must also preserve order, and nothing in the type system says so. Options are a `dataset_version_id` filter on the route, a route on the version, a bounded and disclosed client filter, or publishing `created_at` on `Model`; the route filter is recommended, with `created_at` alongside. **No owner** — `02` §5.1 or `model-schema` changes, and `W32` is closed. `W6b-5b` ships the bounded client filter. Raised 2026-08-25. **Open**, owned by the maintainer; options and evidence are in `docs/open-questions.md`. **DECIDED 2026-08-26: (a), with (d) alongside** |
| ~~**OQ-MODEL-39**~~ | ~~**An all-`null` set of holdout strength ratios cannot be told from an artifact that predates them.**~~ `TransparencyArtifactRow.payload` stores the artifact whole as JSONB and is insert-only, so a pre-`W6b-5a` artifact carries no `holdout_strength_ratio` key while a later one carries `null` where no quotient exists; `to_artifact` validates the payload into `TransparencyArtifact` before it leaves the platform layer, so both reach a panel as `None`. The two warrant opposite readings — "rebuild to compute it" against "the booster found no interaction structure" — and rendering the first as the second discards a pair on evidence that does not exist. **Narrowed:** `_ratio` returns `None` iff in-sample `strength` is `0.0`, so any candidate carrying a float proves the artifact is post-`W6b-5a` and only an all-`null` one is undecidable; that rests on the call site rather than the type. Recommended: an explicit capability flag, the house answer `interactions_available` already gives this shape. **No owner** — a `model-schema` change, and `W32` is closed. `W6b-5b` ships wording true of both causes. Raised 2026-08-25. **Open**, owned by the maintainer; options and evidence are in `docs/open-questions.md`. **DECIDED 2026-08-26: (a) — holdout_ratio_available on ShapSummary, the interactions_available precedent** |
| ~~**OQ-MODEL-43**~~ | ~~**A surrogate's source is pinned by UUID while every address derived from its slug resolves to the source family's latest version — so a rendered or derived link can land on a version the surrogate does not approximate.**~~ Raised 2026-08-26 (W6b decision maker) out of OQ-MODEL-34's ruling. `GlmSpec.approximates_model_id` pins one model by UUID (model-schema `modelling.py`:1040) with no companion family-and-version address, the surrogate's slug is derived from the source *family* slug with `-approx` appended (FR-MODEL-102), and `/models/:slug` resolves to the family's latest version — so the fidelity numbers a reader follows from the derived address can be those of a different version's predictions, the misreading FR-MODEL-96(i) exists to prevent. `IntervalFor` already establishes the shape: the central model is "named by **id and version**" (FR-MODEL-78/100, model-schema `modelling.py`:1279-1280). Options are (a) a companion `slug@version` pair on `GlmSpec` following `IntervalFor` — joins `spec_hash` (FR-MODEL-86), costs a contract field and a bump; (b) resolve the pin in the rendering surfaces only — no schema change, and every slug-derived address still lands on latest; (c) document the slug-derived address as family-level — cheapest, leaves a link that can disagree with the pin. Recommendation: (a), because it makes the surrogate self-describing in the sense FR-MODEL-102 already claims, at the cost of a contract field and a `spec_hash` bump; the id stays the lookup key. **Open**, owned by the maintainer; options and evidence are in `docs/open-questions.md`. **DECIDED 2026-08-26: (a) — GlmSpec gains a companion slug@version address following IntervalFor, joining spec_hash; the id stays the lookup key** |
