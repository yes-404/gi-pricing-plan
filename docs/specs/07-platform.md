...
| **OQ-PLAT-6** | How does the browser authenticate to the API — authorization-code + PKCE in the SPA, or a BFF session cookie? §3.7 specifies the API side (discovery, JWKS, bearer verification) and nothing specifies the client side; the frontend consequently sends no credential at all, and reaches a running API only through the development proxy W6a added. |
