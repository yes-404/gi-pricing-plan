...
| **FR-PLAT-45** | Workspace settings include: currency, locale/timezone for display, default validation thresholds, trace sampling rate, approval policy reference, retention windows, model complexity thresholds (`modelling.max_factor_count`, `modelling.min_exposure_per_parameter` — both unset by default, `02` FR-MODEL-81), and feature flags (e.g. `sql_validation_check_enabled`, `expression_objectives_enabled`). **`rating.batch_abort_failure_rate` joins the list, unset by default** *(added 2026-08-29, W11 Slice 3. `03` FR-RATE-38's "declared threshold" had no home anywhere in the suite — swept, and FR-RATE-38 is its only mention. Unset by default for the reason the two model-complexity thresholds above are: the requirement reads "does not abort … unless the failure rate exceeds a declared threshold", so an undeclared one means no rate-based abort, with the counts-and-samples half unaffected. A batch run may pass a value of its own, but only one that **lowers** the effective threshold; that argument is a Job parameter recorded under FR-PLAT-14, never a fourth tier in FR-PLAT-43's precedence. Ruled in `docs/plans/2026-08-29-w11-slices-3-4-rulings.md` Ruling 24.)* |
...
| ~~**OQ-PLAT-16**~~ | ~~**Does the frontend need a request-shaped generated type, or does every write path cast?**~~ `generate:api` runs `openapi-typescript` `^7.13.0` with no flags, so `--default-non-nullable` takes its default of *true* and every property carrying a JSON-Schema `default` is emitted as a *required* TypeScript property — `GlmSpec` renders `alpha: number` with no `?` beside `approximates_model_id?: string \| null`. Right for reading a response, wrong for writing a request, and one generated type serves both directions. The generated OpenAPI is not at fault: `GlmSpec` lists 3 required and 13 defaulted properties with no property in both sets. The consequence today is three `as unknown as ModelSpec` casts in `ModelSpecBuilderView.vue` — the only non-test `as unknown as` in `frontend/src` — disabling compiler checking of every outgoing spec body. Options are the status quo cast, `--default-non-nullable false` at the cost of every response read, or a second generated type set aliased for request bodies only; the last is recommended, since both sets are generated and neither hand-writes a contract shape. **`W6b` can own it** — frontend tooling, not backend work or a `02` §5.1 change. **Nothing is blocked meanwhile**: `W6b-4b` reuses the documented cast. Raised 2026-08-25 by the `W6b-4a` arbitration. **Open**, owned by the maintainer; options and evidence are in `docs/open-questions.md`. **DECIDED 2026-08-26: (c) — two generated type sets: strict for responses, permissive for request bodies** |
| ~~**OQ-PLAT-17**~~ ✔ | ~~**How does the browser learn the OIDC issuer and its `client_id`?**~~ **DECIDED 2026-08-25 (W6b decision maker): (a) — `FR-PLAT-66` states the channel.** `FR-PLAT-55` specifies the flow but not its bootstrap: a discovery document resolves the provider's endpoints once the issuer is known and never carries a `client_id`, and **no `oidc_client_id` exists anywhere** — `config.py` has `oidc_issuer`, `oidc_audience`, `oidc_jwks_url`, `oidc_jwks_ttl_s`, `dev_auth_enabled` and no other, `oidc_audience` being the resource server's audience rather than a client registration. `FR-PLAT-58`'s realm already presupposes such a client without saying where it is configured. So the SPA carries **no authentication code at all, in any mode**: `local`/`dev` works only because the Vite dev proxy injects `x-dev-principal-id` on its behalf — the path `FR-PLAT-55` excludes — and that vanishes from a built artifact, leaving nothing to extend. A built SPA presents none of the three header credentials the API accepts, and a cookie is not a silent fallback because nothing in `backend/src/app` reads one. `/api/v1/settings` cannot carry it — authenticated, and scoped to a workspace where an issuer is a deployment value. Options are a new unauthenticated `GET /api/v1/auth/config` on the `/version` precedent, build-time `import.meta.env` injection, or an auth block on `/version`; the first is recommended, the second states the issuer twice — `FR-PLAT-59` forecloses it outright, since production runs on the deployer's own provider — and the third hand-writes a contract shape. Raised 2026-08-25 (W6b), allocating the id that `docs/plans/2026-08-25-w6b-14-local-oidc-provider.md` Finding 3 routed here and deliberately left unminted; it blocks `W6b-10`. **Decided 2026-08-25 (W6b decision maker): (a) — an unauthenticated `GET /api/v1/auth/config`, stated as `FR-PLAT-66`.** Options and evidence are in `docs/open-questions.md`. |
