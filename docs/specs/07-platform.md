...
| Term | Definition |
|---|---|
| **Workspace** | The top-level tenancy container (FR-OVR-13). All artifacts belong to exactly one. |
...
| **Celery + Redis** | Job execution, queue routing, cancellation | Queue routing by kind, revocation semantics, worker memory limits, result backends, avoiding the "lost task" failure modes |
...
| ID | Question |
|---|---|
| **OQ-PLAT-1** | Celery or a Postgres-backed queue (e.g. `procrastinate`, `pgqueuer`)? Celery is the mature default and the stack already runs Redis; a Postgres queue removes a component, gives transactional job enqueue alongside the domain write (which would make `06` R2 simpler), and is markedly easier to reason about — at the cost of throughput headroom and ecosystem. |
...
| **OQ-PLAT-3** | Should the scoring service be a separate deployable *application* (its own image, its own minimal dependency set) rather than the same image run with a different role? A separate image gives a much smaller attack surface and faster start on the latency path; one image is simpler to build and version. |
| **OQ-PLAT-4** | How are workspace-level resource quotas expressed (concurrent compute jobs, total storage), and are they needed at all in a single-tenant deployment (FR-OVR-13)? |
