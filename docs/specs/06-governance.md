...
| ID | Requirement |
|---|---|
| **FR-351** | The approval lifecycle is uniform across artifact types: `draft → review → (approved \| changes_requested \| rejected)`. Post-approval states (`live`, `superseded`, `retired`) belong to the owning module but are governed by the same audit rules. |
...
| **FR-353** | **Separation of duties**: the submitter cannot approve, and where two approvals are required they must be distinct Principals (R1). Enforced in the backend. |
...
| **Validation Rule** | Successful dry-run result against a real Dataset Version (`01` FR-50) |
...
| **Rate Table Version** | Change note; diff vs previous; diff vs technical seed where seeded (`03` FR-230/231) |
