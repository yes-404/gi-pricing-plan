...
> views and their routes (FR-OVR-22), so it governs. A module's §5.3 may carry detail
...
| **OQ-OVR-2** | ~~Project licence: Apache-2.0 (permissive, adoption) vs AGPL-3.0 (protects against closed SaaS forks).~~ **DECIDED 2026-08-14: Apache-2.0.** AGPL was rejected because it blocks the insurers embedding `pricing-core` that the project exists for, and NFR-OVR-11's dependency check now has a concrete target: every runtime dependency must be Apache-2.0 compatible. |
