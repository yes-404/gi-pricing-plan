...
| An **embedded** notebook server | A client library is provided instead (OQ-546, decided 2026-08-14). Embedding JupyterLab would put arbitrary code execution inside the platform's security and audit boundary — the same problem custom objectives pose, and not one to solve twice. Revisited in Phase 4 once that sandboxing exists |
| Customer PII storage as a system of record | Datasets are pseudonymised snapshots (see FR-12) |
...
orchestrated by the **scheduler tick** (`07` FR-413), with the pipeline code it
...
entity** — one tenant, one deployment, nothing shared (FR-16, FR-17, ADR-710). A
...
```json
{
  ...
  "currency": "ISO 4217, e.g. GBP — the workspace's single currency (OQ-542)",
...
- All `POST` endpoints that create jobs or artifacts accept an `Idempotency-Key` header;
  a repeat returns the original result. *Amended 2026-08-23 (`07` OQ-647): the
  **24-hour window this line used to state is withdrawn** — it was never implemented, and
  ...
  be attempted again, while success and cancellation do not. `07` FR-414 is the
  authority, and FR-450 binds the API to this section as amended.*

### 5.5 pricing-core function conventions

`pricing-core` is the only place actuarial maths lives (FR-8). Its public API obeys:
...
> *(Recorded 2026-08-27, closure-audit standard.)* The Route column is the canonical
> route. A module's §5.3 Route cell must agree with it: `00` §5.6 names the canonical
> views and their routes (FR-25), so it governs. A module's §5.3 may carry detail
> routes the inventory does not list, but a route §5.6 declares must appear in the owning
> module's §5.3. Two named views on one route are two §5.3 rows carrying the same route,
> not one missing row — a check must diff routes, never view names.
...
|---|---|---|
| `WF-698-dataset-to-model.md` | Raw file → validated Dataset Version → fitted, diagnosed, approved Model | 01, 02, 06 |
| `WF-699-model-to-rating-version.md` | Approved Models → peril structure → rating DAG + rate tables → approved Rating Version | 02, 03, 06 |
| `WF-700-rate-change-impact.md` | Proposed rate change → dislocation + optimisation + GIPP evidence → decision | 03, 04, 06 |
| `WF-701-deploy-and-monitor.md` | Approved Rating Version → deploy to environment → live scoring → drift/AE monitoring → alert | 03, 05, 07 |
| `WF-702-custom-objective-lifecycle.md` | Define custom objective → validate/sandbox → approve → use in model → audit | 02, 06, 07 |
...
| **PLAT** | — | — | — | — | — | audit sink, permission check (DEP-537) | — |
...
- **DEP-537** — **Two `GOV` interfaces are cross-cutting and sit outside that order: the
...
| **FastAPI + Pydantic v2** | The single API surface (FR-14); Pydantic models are the contract source of truth (FR-9) |
| **SQLAlchemy 2.x async + Alembic** | Metadata persistence, transactional audit writes (FR-7), migrations |
| **PostgreSQL 16** | Entities, JSONB artifact bodies, audit log, row-level constraints for state machines |
| **Celery + Redis** | Job execution and queueing (FR-13); Redis also caches live rating bundles |
...
| **openapi-typescript** | Generated API client types (FR-9) |
...
| ID | Requirement |
|---|---|
| **NFR-454** | Real-time scoring: p99 < 50 ms server-side for a single quote against a live Rating Version, excluding network, at 200 rps sustained per replica. |
| **NFR-455** | Batch scoring throughput: ≥ 1 M risks/hour per worker for a typical motor rating structure (≈ 200 DAG steps). |
| **NFR-456** | GLM fitting: 5 M rows × 60 factors converges in < 10 min on a 16-core worker. GBM fitting: same data, 500 trees, < 20 min. |
| **NFR-457** | Interactive UI: p95 < 300 ms for metadata reads; any operation exceeding 2 s becomes a Job with progress. |
| **NFR-458** | Audit completeness: 100 % of governed state transitions produce an Audit Event; the audit log is append-only at the database privilege level. |
| **NFR-459** | Retention: artifacts and audit events retained ≥ 7 years (UK regulatory expectation); scoring traces sampled and retained ≥ 13 months. |
| **NFR-460** | Recoverability: RPO ≤ 15 min, RTO ≤ 4 h for the pricing metadata database; the blob store is versioned and replicated. |
| **NFR-461** | Security: all traffic TLS 1.3; secrets from environment/secret manager only; no credentials in artifacts; dependency and container scanning in CI. |
| **NFR-462** | Portability: the full stack runs locally via `docker compose up` with no cloud dependency, using MinIO for object storage. |
| **NFR-463** | Accessibility: the SPA meets WCAG 2.2 AA; all charts have an accessible tabular equivalent. |
| **NFR-464** | Licensing: every runtime dependency must be OSI-approved and compatible with the project licence; copyleft-incompatible or source-available-only dependencies are rejected (see `open-questions.md` OQ-541 for the licence choice). |
