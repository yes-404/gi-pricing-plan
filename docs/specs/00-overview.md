...
| ~~**OQ-OVR-7**~~ ✔ | ~~Should `01` FR-DATA-26's one-way row rename `severity_minor` and `burning_cost_minor`? They are means, and therefore floats, while `_minor` is reserved for integer minor units (FR-OVR-7).~~ **DECIDED 2026-08-17: rename to `mean_severity` and `mean_burning_cost`, in the slice that next changes the profile contract — not as a change of its own, and not never.** Specified as `01` FR-DATA-46, which carries the trigger and the owner. Until then the two names are excluded by name from the money scan, and the exclusion may not grow: a money-discipline allow-list is exactly the kind of exception nobody revisits. Raised 2026-08-15 by generating the `banding` and `grouping` schemas, which embed the row. |
