...
| **OQ-OVR-8** | `DecimalStr` silently accepts a `float`: `OneWayRow(exposure_years=0.1+0.2)` yields `Decimal('0.30000000000000004')`, binary error preserved inside a field FR-OVR-7 calls exact. Should it refuse floats outright? 26 fields across 7 modules are affected; `LevelCount.exposure_years` is currently the only strict one. |
