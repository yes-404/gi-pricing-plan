...
| **OQ-OVR-10** | What is a Contents cell in each module spec's §5.3 Frontend views table — an enumeration of what the view must show, or a prose summary of its character — and how does a cell declare which kind it is? Raised 2026-08-24 (W6b): a filed slice map read `02` §5.3's Diagnostics cell as an enumeration and derived from it a chart count that then bound a slice. The cell was wrong in both directions — it still named double-lift a week after FR-MODEL-50 struck it, and it named none of the eight further fields `GbmDiagnostics` and `GlmDiagnostics` carry in the generated contract. Cells in the same table do both things: two rows above are category-noun summaries, the Objective certificate row enumerates all four `CheckStatus` values with a presentation rule, and the Peril structure library row enumerates its *negative* space. Nothing distinguishes them. Mirrored in `docs/open-questions.md`. |
