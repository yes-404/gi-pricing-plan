...
Adopted 2026-08-29 from RFC-841 (`docs/rfcs/RFC-00841-per-agent-model-thinking-effort-and-skill-bindings-for-the-seven-roles.md`),
companion to `docs/process/delivery-process.md`, reconciled and ruled in
`docs/plans/PL-00845-rfc-840-rfc-841-adoption-reconciliation-and-rulings-2026-08-29.md` ("the rulings record").
...
plugin installation or `superpowers:` namespace is involved (see `docs/plans/PL-00844-rf
c-840-rfc-841-adoption-implementation-plan.md` Task 2 for the two places that still said otherwise, now fixed).
...
- **The lead's tool row** (Part B9): **"full read; git merge authority (sole merge
  authority); write to handover/status files only."** This is RFC-841's version, not
  RFC-840's — RFC-840 §2's lead row disagreed (read-only + plan/map files, no merge
  ...
  `TEAM-STRUCTURE.md`'s table both match RFC-841's row, not RFC-840's.
...
1. **Verdict split:** auditor proposes → **lead** adopts/amends/rejects and merges.
   Decision-maker rules decision points and spec conflicts only. (Replaces RFC-840's
...
5. **Per-slice audit axes** (the WK-671 lesson) are written into the auditor's role file.
