# W10 Implementation Plan — Rate tables: seeding, diffs, bulk operations, import/export
...
**Architecture:** Three slices on the existing Phase 1b seam. Slice W10-1 defines the `RateTable` and `RateTableVersion` shapes in `model-schema` (keys, value column, storage mode, immutability invariants). Slice W10-2 builds seeding from an approved model's relativity table, implements cell-level diffs with exposure weighting, and enforces validation rules. Slice W10-3 implements bulk operations (uplift, floor/cap, rebase), CSV/XLSX import/export with strict round-trip verification, and the parquet-spill threshold logic (FR-RATE-62). The workspace-configurable cell-count threshold (default 250 000) determines storage mode at write time and is immutable with the version.
...
**Spec:** [`03-rating-engine.md`](../specs/03-rating-engine.md) FR-RATE-14..21, FR-RATE-62 · `04` FRs for bulk-operation metadata (lifted from optimisation) · [`04-optimisation.md`](../specs/04-optimisation.md) for the data model of bulk operations.

**Highest ids in use:** FR-RATE-64 (instalment loading, Phase 2). Next free: FR-RATE-65, NFR-RATE-15.
...
- A Rating Version pins an exact RateTableVersion per referenced table; all versions are immutable (FR-RATE-15).
- The cell-count threshold is stored on RateTableVersion and immutable with it (FR-RATE-62).
- Cells stored as PostgreSQL rows use SQL joins for diffs; cells above the threshold spill to parquet and degrade to Job-based diff (FR-RATE-17, FR-RATE-62).
...
| Requirement | Slice |
|---|---|
| FR-RATE-14 (Rate Table definition) | W10-1 |
| FR-RATE-15 (immutability) | W10-1, W10-2 |
| FR-RATE-16 (seeding from model) | W10-2 |
| FR-RATE-17 (cell-level diff with exposure weighting) | W10-2 |
| FR-RATE-18 (bulk operations: uplift, floor/cap, rebase) | W10-3 |
| FR-RATE-19 (validation on save) | W10-2 |
| FR-RATE-20 (CSV/XLSX import/export with round-trip) | W10-3 |
| FR-RATE-21 (rateable vs diagnostic) | W10-1 |
| FR-RATE-62 (storage threshold, rows vs parquet) | W10-1, W10-3 |

---

## Findings (verified 2026-08-28 against origin/main `eb9b6a1`)

**F1. Rate tables are greenfield beyond the Phase 1b seam.** `backend/src/app/platform/rating_versions.py` carries the Phase 1b minimal `RatingVersion` pinning a single model. No `RateTable` or `RateTableVersion` shape exists in `model-schema`, no `rate_tables/` module in `pricing-core`, no rate-table routes, and no FR-RATE-14..21 marker exists in the codebase.

**F2. The seeding, diff and validation scope is non-trivial.** FR-RATE-16 requires tracking the source model and version; FR-RATE-17 requires exposure-weighted diffs fetched from the portfolio dataset; FR-RATE-19 requires complete key-domain coverage validation. These are not CRUD operations; they are analytical operations over multiple versioned artifacts.

**F3. The storage threshold decision is specified but deferred implementation-wise.** FR-RATE-62 is decided (OQ-RATE-3, 2026-08-18) and has fixed implementation details: `storage` is a version property (`rows | parquet`), immutable at write time, and determines whether FR-RATE-17 diffs run as SQL joins or as Jobs returning the same artifact (202 instead of 200).
...
- [ ] Add `RateTable` to `packages/model-schema/src/model_schema/rating.py`: `slug`, `version`, `rateable` (bool), `storage` (`rows \| parquet`, immutable), `keys`, `value`, `default_row` (FR-RATE-14, FR-RATE-21, FR-RATE-62).
...
- [ ] Add the `storage` field to `RateTableVersion`: `rows` or `parquet`, decided at write time based on the workspace's cell-count threshold, immutable with the version (FR-RATE-62).
- [ ] Add `seeded_from` to track the source model reference and timestamp (FR-RATE-16).
- [ ] Add `change_note` as a required field on version creation (FR-RATE-15).
...
- [ ] Enforce that a Rate Table Version is immutable: editing produces a new version, never in-place modification (FR-RATE-15).
- [ ] Enforce the `storage` mode is immutable with the version (FR-RATE-62): once written, a version's `storage` field cannot change.
...
- [ ] Implement `seed_from_model()` in `packages/pricing-core/src/pricing_core/rate_tables/operations.py`: accept an approved Model or Peril Structure, extract its relativity table, create a `RateTableVersion` with `seeded_from` metadata (FR-RATE-16).
- [ ] Validate that the model reference is approved (maturity check per FR-OVR-14).
...
- [ ] Implement `diff_vs_previous()` and `diff_vs_seed()` in `pricing_core.rate_tables.operations`: compare cells by key, report absolute and relative change (FR-RATE-17).
- [ ] For tables stored as PostgreSQL rows: join against the portfolio dataset to fetch exposure weights per cell, compute exposure-weighted mean change (FR-RATE-17).
- [ ] For tables stored as parquet: defer to a Job that re-fetches and re-computes the diff, returning the same artifact structure; API answers 202 instead of 200 (FR-RATE-62).
...
- [ ] Implement `validate_rate_table()` per `03` §5.2 (FR-RATE-19): check complete coverage of the declared key domain (or an explicit `default_row`), no null values, values within declared bounds, no key duplication.
...
- [ ] Implement `uplift_table()`, `uplift_by_filter()`, `floor_and_cap()`, `rebase_to_level()` in `pricing_core.rate_tables.operations` (FR-RATE-18).
- [ ] Each operation records its parameters (percentage, filter expression, bounds, base level) as metadata on the new version, not just the resulting cells (FR-RATE-18).
- [ ] All operations produce a new immutable version with the updated cells.
- [ ] Each operation validates the result before persisting (FR-RATE-19).
...
- [ ] Implement `export_to_csv()` and `export_to_xlsx()` in `pricing_core.rate_tables.operations` (FR-RATE-20).
- [ ] Export includes keys, value column, and all declared metadata (types, bounds, banding references).
- [ ] Implement `import_from_csv()` and `import_from_xlsx()` with strict round-trip validation (FR-RATE-20): keys, types, and completeness must match the table schema.
...
- [ ] Implement `decide_storage_mode()` in `pricing_core.rate_tables.operations`: given a cell count and the workspace's configured threshold (default 250 000), return `rows` or `parquet` (FR-RATE-62).
...
- [ ] Both paths serve the same API; only latency and status differ (FR-RATE-62).
...
- `docs/specs/03-rating-engine.md`: FR-RATE-14..21, FR-RATE-62, §4.2, §5.1.
- `docs/roadmap.md` §7: the W10 row and the Phase 2 workstreams.
- `docs/specs/04-optimisation.md` §4: bulk-operation metadata structure (to be refined in spec if W10 requires it).
