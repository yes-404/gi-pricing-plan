...
family: plan
kind: leaf
title: The PSI Comparison Selector (`01` §5.3, FR-63) Implementation Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-19-psi-comparison-selector.md
---

# The PSI Comparison Selector (`01` §5.3, FR-63) Implementation Plan
...
on `/data/:slug/v/:version/profile` that calls FR-63's already-served `compare`
...
**zero callers** since WK-663; this slice gives it its caller. The view gains a `<select>`
...
- [`docs/specs/01-data-management.md`](../specs/01-data-management.md) §5.3 (the Profile
  row and its 2026-08-19 note), §5.1 (the `compare` endpoint row), FR-63, FR-60.
...
- **`exactOptionalPropertyTypes` is on.** Spread an optional rather than assigning
  `undefined`: `...(x ? { k: x } : {})`.
- **Exact decimals are strings and are rendered, never parsed** (FR-10). `psi`,
...
- Highest ids in use: **FR-67**, **OQ-566**. Next free: `FR-68`, `OQ-556`.
...
FR-63 nor §5.3 asks the reference to survive navigation, and no other requirement does.
...
**Whichever is chosen, Task 1 records it as `OQ-556` with the options and the outcome.**
...
| `docs/specs/01-data-management.md` | §5.3's note resolved (Task 6); OQ-556 mirrored into §10 (Task 1). |
| `docs/open-questions.md` | OQ-556 raised and decided. |
...
## Task 1: Raise `OQ-556`, and get it decided

**Files:**
- Modify: `docs/open-questions.md` (the DATA table, after the `OQ-566` row)
...
```bash
grep -rn "OQ-556" docs/ .claude/ || echo "FREE"
```
...
```markdown
- **OQ-556** — where the PSI comparison selector's chosen reference version lives: the
...
git commit -m "docs(data): OQ-556 — where the comparison selector's reference lives"
```
...
`~~**OQ-556**~~ ✔`, following the shape of the decided rows above it. Commit that as a
separate commit.
...
2. **A version with `profile_id === null` is listed but `disabled`, suffixed "(no profile)".**
   ...
   FR-62's one-way handling deliberately avoids; `DatasetVersion.profile_id` already
...
PATH="$HOME/.npm-global/bin:$PATH" pnpm --dir frontend type-check
git add frontend/src/views/ProfileView.vue frontend/src/views/__tests__/ProfileView.test.ts
git commit -m "feat(frontend): the profile view fetches FR-63's comparison"
```
...
- Modify: `docs/open-questions.md` (OQ-556's status, if Task 1 Step 6 left it open)
...
Leave the note's remaining paragraphs (the OQ-565 resolution) untouched.
...
- [ ] **Step 4: Close OQ-556 if it is still open**

If Task 1 Step 6's answer has not yet been written into `docs/open-questions.md`, do it now:
strike the question, prefix the id with `~~**OQ-556**~~ ✔`, set Status to `**decided**`,
...
FR-409's guide is derived rather than written, so there is nothing to update — but the
...
gh pr create --title "feat(frontend): the PSI comparison selector (01 §5.3, FR-63)"
```

The PR body states: what was built; that `compareProfiles()` went from zero callers to one;
OQ-556 and its answer; that `psiBand` was narrowed and why; the measured test counts
...
fourth — "PSI comparison selector" — is Tasks 3–5. FR-63's endpoint is consumed (Task 4)
...
name. `VR-DST-1`'s thresholds are the bands (Task 2). **No requirement is added**: FR-63
