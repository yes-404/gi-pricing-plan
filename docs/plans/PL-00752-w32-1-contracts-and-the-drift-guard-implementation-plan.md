...
family: plan
kind: leaf
title: W32-1 — Contracts and the drift guard Implementation Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-22-w6b-contracts-and-drift-guard.md
---
...
the committed contract; CI fails on drift (`FR-451`).

**Spec:**
- [`../specs/02-modelling.md`](../specs/02-modelling.md) — `FR-107` and its 2026-08-22
  amendment, which names this slice's owner.
- [`../specs/00-overview.md`](../specs/00-overview.md) §4.3 — the artifact envelope;
  `FR-9`.
- [`../specs/07-platform.md`](../specs/07-platform.md) — `FR-451`, the committed contract
  and its drift check.
...
- [`../adrs/ADR-704-model-schema-single-source-of-truth.md`](../adrs/) — the rule this guard
  enforces.
...
- **Money is integer minor units; exact decimals are strings** (`FR-10`). `OneWayRow`
  carries both, and its ratios (`frequency`, `mean_severity`, `mean_burning_cost`) are
  **float statistics, not amounts** — `FR-64` is explicit, and getting this backwards is
...
`FR-107` requires a Grouping to store "its method, parameters, **source Level
...
`FR-107` tests. `_book()`, `_proposal()` and `DATASET` are the module's existing
...
Expected: PASS, including the three existing `FR-107` tests.
...
git commit -m "feat(model): FR-107 — the grouping evidence carries its source levels"
```
...
```json
        "source_level_stats": {
          "$ref": "#/$defs/OneWayRows",
          "description": "The source levels this grouping collapsed (FR-107). Added to the model 2026-08-22 having been declared here since Phase 0 and absent from GroupingEvidence throughout - nothing caught it because the field is nested under evidence and the field-name comparison reads top-level names only."
        },
        "target_level_stats": {
          "$ref": "#/$defs/OneWayRows",
...
`FR-64` is explicit that a mean is a statistic, not an amount, and typing them as integers
...
least eight** checks on a certificate and the model requires one. `FR-151`'s certification
...
- [ ] **Write the ledger** — `docs/plans/PL-00751-w32-1-contracts-and-the-drift-guard-execution-ledger.md`,
...
**1. Spec coverage.** `FR-107`'s "source Level statistics" — Task 1, with the contract in
Task 2 and the control in Task 3. `FR-451`'s drift check — Tasks 4, 5, 6, controlled by
Task 7. `FR-9` — Task 4's marker. Plan review 3's question 2(b), the constraint-level
