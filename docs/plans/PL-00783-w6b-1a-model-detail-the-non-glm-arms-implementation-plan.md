...
family: plan
kind: leaf
title: W6b-1a — Model detail, the non-GLM arms: Implementation Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-24-w6b-1a-model-detail-non-glm-arms.md
---
...
[`PL-00786-wk-664-the-revised-slice-map.md`](PL-00786-wk-664-the-revised-slice-map.md) §5 proposes
...
**Sections 1–7 below were drafted by a general-purpose subagent** under the WK-664 work lead's
...
- **E6 and Task 8 came from the WK-664 work lead, not from this review or the draft.** The lead
...
- **Nobody hand-writes a shape that already exists in `model-schema`** — FR-9 defines every
...
- **A JavaScript number is a float64.** A field whose contract declares it an exact decimal
  string stays a `string` on the way through — never `parseFloat` one (FR-10).
...
and *"The GBM detail view is WK-664's (`02` §5.3), together with the transparency artifact that
...
| 2 | Quantile intervals | `GbmSpec.interval_for` (FR-199, FR-200) |
| 3 | The surrogate link | `GlmSpec.approximates_model_id` and `glm_approximation.approximating_model_id` (FR-137) |
| 4 | The EBM arm | `EbmSpec` (§4.4) and `EbmFitResult` (§4.8) |
| 5 | The transparency artifact | `TransparencyArtifact` (§4.9), read by `GET /models/{id}/transparency` (FR-139) |
...
- **Any chart at all.** NFR-463 attaches an accessible tabular equivalent to *each* chart,
...
- **Three Contents-cell items the common arm does not render** — confidence-interval *bars*
  ...
  the code, but whether three is the *complete* count depends on `OQ-549`, which is open on
...
| Early stopping | `spec.early_stopping` | FR-124 refuses training-set stopping; the metric and the rounds are what say the stop was honest |
...
| Feature dtypes, categorical maps | `fit.feature_dtypes`, `fit.categorical_maps` | FR-125 makes the dtype expectations and encoding maps travel with the booster, because the exported model cannot express them on its own |
| Base margin | `fit.base_margin` | FR-121: exposure enters as a `base_margin` the platform builds from the declared offset — never as a feature, never as a weight. Which construction was used is the difference between a frequency model and a nonsense one |
| Inverse link, and who applies it | `fit.inverse_link` | FR-130: the field names the transform the platform must apply, or null where the library already did. Both readings must be spelled out, because the value alone cannot distinguish them |
| Booster reference | `fit.booster_blob`, `fit.booster_format` | ADR-705: a declarative artifact, never a pickle. The digest is the audit trail |
| Dropped eval metrics | `fit.dropped_eval_metrics` | FR-161: a declared metric the backend could not evaluate. Silent absence is the defect the field exists to remove |
...
FR-199: each bound is a Model in its own right — same Model Family, dataset version, split
...
it("says a GLM surrogate is a surrogate, and what its diagnostics are against", async () => {
  // FR-137: a surrogate's diagnostics are against the source model's predictions, never
...
rendered only when present, because FR-132 allows any combination and an absent block is
...
**`holdout_strength_ratio` gets no column.** FR-168 declares it and the code that builds
...
the GBM example beside it carries *"added 2026-08-22 (WK-661, the audit-remediation slice)"*.
...
**Raised by the WK-664 work lead at review, not by the draft.** It is in this plan because Task 1
...
```
grep -c '^### Task ' docs/plans/PL-00783-w6b-1a-model-detail-the-non-glm-arms-implementation-plan.md
```
...
Expected: the second command returns nothing and exits 1. NFR-463's tabular-equivalent
obligation therefore gains no new surface from this slice.

---

## 6. Escalations — routed to the WK-664 work lead, decided by nobody here
...
FR-199 makes the bound share its central model's family slug and the platform enforces it,
...
2. **`model-schema` is the single source of truth and the example is not.** ADR-704 generates
   `docs/contracts/` from it and CI fails on drift (FR-451). A hand-typed JSON block in a
...
`ModelFlag` exists and a dataset-invalidated flag gates approval (FR-205). All three belong
...
list. That is exactly the question `OQ-549` is open on (PR #167), and this cell is one of
the two it names. **So: the three omissions are unconditional; the claim that three is the
complete count is conditional on `OQ-549` and vacated if it resolves toward "summary".**
...
only when the WK-664 work lead asked whether it had been. The answer came back benign — `Spec
...
WK-664 work lead at review. *"As a fraction of the estimate"* against a body returning `high - low`;
...
sixteen distinct IDs the plan cites — FR-9, FR-10, FR-MODEL-27, 30, 31, 33, 34, 50, 67,
78, 84, 94, 96, 100, 111, 128 and NFR-463 — **and found no fourth mis-attribution.** Two are
...
- **`MODEL_INTERVAL_PAIR_INVALID` survived the sharpest version of the test.** Task 1's
  ...
  family slug, dataset version, split ref and the factor set"*, while FR-200(iv) attaches
...
- **FR-161's amendment is overtaken, not wrong.** It states that §4.8 *"has never carried
