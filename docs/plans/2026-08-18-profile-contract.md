...
**Goal:** Close the `ColumnProfile` histogram divergence — the field `01` §4.7, the committed contract and `01` §5.3's view all promise and no code produces — and, because that makes this the slice that changes the profile contract, discharge FR-DATA-46's deferred rename and publish a generated counterpart so the profile contract stops being unchecked.
...
**Spec:** `docs/specs/01-data-management.md` — §4.7 (`Profile` contract), §5.3 (Profile view), FR-DATA-25/26/27, FR-DATA-46. Contract: `docs/contracts/schemas/profile.schema.json`. Divergences recorded in `docs/roadmap.md` (~line 768) and the §5.3 audit row (~line 701).
...
- **Requirement IDs are permanent** (`CLAUDE.md` §5). Never renumber. The **maximum** existing id — not the last one you read — is `FR-DATA-47`, so **the next free id is `FR-DATA-48`**. `01`'s requirement tables are not in numeric order; re-derive with `grep -o "FR-DATA-[0-9]*" docs/specs/01-data-management.md | sort -t- -k3 -n | tail -1` before appending. Max `OQ-DATA` is `8`, so **the next free question is `OQ-DATA-9`**.
- **Money is integer minor units or an exact decimal string, never float** (FR-OVR-7). Exposure is `DecimalStr`.
...
- **Do NOT rename `modelled_burning_cost_minor` or `observed_burning_cost_minor`** (`packages/model-schema/src/model_schema/perils.py`, `pricing_core/modelling/perils.py`). They are genuine integer `MoneyMinor` fields and FR-OVR-7 requires the `_minor` suffix on them. FR-DATA-46 targets **only** `OneWayRow`'s two float means. A blind `sed` over `burning_cost_minor` breaks money discipline.
...
|---|---|---|
| `docs/open-questions.md` | `OQ-DATA-9` raised with options and a recommendation | 1 |
| `docs/specs/01-data-management.md` | `OQ-DATA-9` mirrored into §10; `FR-DATA-48` appended; §4.7 dated note; FR-DATA-46 marked delivered | 1, 2, 5, 8 |
...
| `backend/tests/test_contracts.py` | Nested conformance for `profile`; delete the FR-DATA-46 exclusion | 5, 6 |
...
## Task 1: Raise OQ-DATA-9 — the Dataset display fields
...
**Files:**
- Modify: `docs/open-questions.md` (append `OQ-DATA-9`)
...
**Interfaces:**
- Consumes: nothing.
- Produces: the id `OQ-DATA-9`. No later task in this plan depends on it.
...
grep -o "OQ-DATA-[0-9]*" docs/open-questions.md docs/specs/01-data-management.md \
  | grep -o "OQ-DATA-[0-9]*" | sort -t- -k3 -n | tail -1     # expect OQ-DATA-8
sed -n '148,170p' packages/model-schema/src/model_schema/datasets.py
sed -n '799,800p' docs/specs/01-data-management.md
```

Expected: max is `OQ-DATA-8`; `Dataset` carries `created_at` and `archived_at` and none of the three; §5.3's Dataset list row reads "Datasets with latest version, status badge, last validated, owner".

- [ ] **Step 2: Append OQ-DATA-9 to `docs/open-questions.md`**
...
In `docs/roadmap.md`, the "Two unresolved model/contract divergences" row: the `ColumnProfile` half is now owned by this slice, the `Dataset` half by `OQ-DATA-9`. Rewrite so neither half is unowned. Do **not** delete the row — it records that both were built around in silence.
...
Expected: `All checks passed`. If it reports `OQ-DATA-9 listed in open-questions.md but raised in no spec`, Step 3's row is missing or its id is not backticked the way the audit reads it.

- [ ] **Step 6: Commit**

```bash
git add docs/open-questions.md docs/specs/01-data-management.md docs/roadmap.md
git commit -m "docs(data): OQ-DATA-9 — the dataset list asks for three fields Dataset lacks"
```

---

## Task 2: `Histogram` in model-schema, and FR-DATA-48

The shape first, with no producer. `01` §4.7 and `docs/contracts/schemas/profile.schema.json:35` both define a histogram; FR-DATA-25's enumerated statistics do not mention one. **The contract is right and the requirement is incomplete** — so the requirement gains the obligation (`CLAUDE.md` §14: resolve, never soften) rather than the contract losing the field.
...
- [ ] **Step 5: Append FR-DATA-48 to `01`**

In `docs/specs/01-data-management.md`, in the table FR-DATA-25 lives in, append:
...
> *(2026-08-18)* The example above has always shown a `histogram`, and so has the committed contract; `ColumnProfile` did not carry one until FR-DATA-48. **The contract was right and the requirement was incomplete** — FR-DATA-25 enumerates the statistics and never named this one. The example's uneven edges were illustrative, not a specification: FR-DATA-48 fixes equal-width bins, because two engines must agree and quantile-derived edges collapse to duplicates on a low-cardinality column, with each engine deduplicating differently.
...
git commit -m "feat(data): a Histogram shape, and FR-DATA-48 naming what produces it"
```

Expected: audit passes; `req-coverage.py` exits 0 with FR-DATA-48 evidenced. If it reports `FR-DATA-48 claimed but nonexistent`, Step 5's row did not land in a table the script reads.
...
git commit -m "feat(data): profile_frame computes FR-DATA-48's histogram
...
## Task 5: FR-DATA-46's rename

FR-DATA-46 (`docs/specs/01-data-management.md`, ~line 160) decided this rename and assigned it to "the slice that next changes the profile contract". **This is that slice.** The requirement also says the money-scan exclusion "may not grow" — deleting it is how this task proves the rename landed.
...
- Modify: `docs/specs/01-data-management.md` (FR-DATA-46 marked delivered)
- Regenerate: `docs/contracts/schemas/generated/*`, `docs/contracts/openapi/generated.json`, `frontend/src/api/generated/`

**Interfaces:**
- Produces: `OneWayRow.mean_severity: float | None` and `OneWayRow.mean_burning_cost: float | None`, replacing `severity_minor` and `burning_cost_minor`. Both stay floats and stay expressed in the workspace currency's minor unit — **only the names change** (FR-DATA-46).
...
Expect roughly a dozen sites in each of the first two groups and a handful in the third. **Do not run a bare `sed` over `burning_cost_minor`** — it would rename the peril structure's genuine `MoneyMinor` fields and break FR-OVR-7.
...
> **`backend/tests/test_demo_guide.py` is derived (FR-PLAT-54)**, and it reads the profile contract. If it fails, the derivation needs the new names — never hand-edit the guide.

- [ ] **Step 7: Mark FR-DATA-46 delivered**

In `docs/specs/01-data-management.md`, replace FR-DATA-46's "**Not yet delivered**: …" through "Owner: the next slice to change the profile contract." with:

> **Delivered 2026-08-18**, in the slice that added FR-DATA-48's histogram — the change to the profile contract OQ-OVR-7 was waiting for. The hand-written money-scan exclusion in `backend/tests/test_contracts.py` is deleted: `mean_severity` and `mean_burning_cost` do not match the scan's pattern, so nothing needs excluding.
...
git commit -m "refactor(data): FR-DATA-46's rename — mean_severity, mean_burning_cost

A mean is not a minor unit. The hand-written money-scan exclusion goes with
it, which is what FR-DATA-46 said may not grow."
```
...
| `top_levels` item shape | **Contract is right.** `("G1", 402)` gives a reader no way to know which element is the level, and the contract's third field (`exposure_years`) is the one an actuary needs — a top-20 by count is not a top-20 by exposure, and FR-DATA-25 asks for **both**. | A **model change with callers**. If it fits the slice, introduce a `LevelCount` shape and update `compare_profiles`, `psi_from_weights` and the frontend chip list. **If it does not**, append `FR-DATA-49` naming the obligation and an owner — do not edit the contract down to what was built. |
| `job_id`, `weight_column` on `Profile` | **Decide from the requirement, not the diff.** `weight_column` is what makes a one-way exposure-weighted and FR-DATA-26 implies it; `job_id` is provenance the Job model already records elsewhere. | Add `weight_column: str = "exposure_years"` to `Profile` if FR-DATA-26 reads that way; for `job_id`, either add it or remove it from the contract with a dated note saying where provenance lives instead. |
...
> A generated artifact matching its source proves neither is correct (`CLAUDE.md` §13 step 4). Check each field against **FR-DATA-25/26/27**, not only against the other file.
...
/** Bin labels from the edges. The last bin is closed (FR-DATA-48), hence the en dash. */
...
git commit -m "feat(w6a): the profile view renders FR-DATA-48's histogram
...
- Modify: `docs/roadmap.md` (slice record; W5 row; the divergence rows)
...
> *(2026-08-18)* The Profile view's histograms landed with FR-DATA-48. The **PSI comparison selector has not**: `compareProfiles()` is implemented and has no caller, and the dtype label that borrowed a PSI colour has been uncoloured rather than left reading as support the view lacks. FR-DATA-28's endpoint exists; the view that reads it is unowned. Owner: the slice that next opens `ProfileView.vue`, or W6b.

- [ ] **Step 2: Update the roadmap**

Add the slice record to W5's list (it becomes the seventeenth) in the established form: what landed, what it found, what it deliberately did not do. Update the "Two unresolved model/contract divergences" row — the `ColumnProfile` half **resolved** with the date, the `Dataset` half now `OQ-DATA-9` and open. Update the §5.3 audit row (~line 701): histograms delivered, PSI selector still absent with an owner.
...
FR-PLAT-54 makes the guide derived, not written — a failure means the derivation needs the renamed fields, never a hand edit to the guide.
...
gh pr create --title "feat(data): the profile contract — histogram, FR-DATA-46's rename, and a generated counterpart"
gh pr view <n> --json mergeStateStatus            # CLEAN means checks passed
```

- [ ] **Step 6: Update the working state**

Append one entry to `progress.md` (never rewrite it): what was done, the gate table with real numbers, the enforcement proven against broken input from Task 6 Step 5, and where the next session starts. Then **rewrite** `task_plan.md`'s "Next build step" so it no longer names this item, and add `OQ-DATA-9` to its decisions-waiting table.
...
| `01` §5.3 dataset list "status badge, last validated, owner" | **Not delivered** — genuinely open, raised as `OQ-DATA-9` (Task 1) rather than silently picked. |
| FR-DATA-46 (owner: "the next slice to change the profile contract") | 5 |
| FR-DATA-25's enumerated statistics | Unchanged. FR-DATA-48 is **appended**, not FR-DATA-25 edited — ids are permanent. |
| FR-DATA-27 (one answer regardless of engine) | 4 — the agreement test carries the histogram free |
| FR-OVR-6 (contract conformance) | 6 |
| FR-OVR-7 (money discipline) | 5 — the exclusion is deleted, not grown |
...
Plan saved to `.planning/2026-08-18-profile-contract.md` — git-ignored (`.gitignore` names `/.planning/`), matching the repo's stated policy that a committed plan file would be "a second, unaudited account of what the project is doing".
