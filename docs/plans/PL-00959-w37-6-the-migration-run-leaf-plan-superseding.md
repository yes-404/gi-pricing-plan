...
family: plan
kind: leaf
title: W37-6 — the migration run: leaf plan, superseding
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-09-02-w37-6-migration-run-leaf-plan-v2.md
---
...
> `status: active`.** It was filed `draft` with one blocking decision row open; RL-979
...
**Goal:** run RFC-937 §4's one-time scripted migration over the whole corpus, in one supervised
...
RL-987's acceptance item 2 was **withdrawn** by RL-970, not sharpened. Run as written it
returned a hit for every member and for ~950 non-members alike, because §4 step 6 rewrites
citations in every tracked file, so reverting *any* file produces an item-(d) hit. RL-970
...
document to produce."* RL-970 rejected that reason with a counter-example: the reverted H
...
**1.1 Its acceptance item 11 was amended in three respects.** RL-970 removed the
...
**1.3 Its §4 figures are pinned at `39ee30c`**, and RL-987 §3 requires the disclosure to
...
|---|---|---|---|---|
| DP-A | Which family and `kind:` the `Pending proposals` container takes, and therefore what `_discover_plan_reviews` emits besides `CR-` records | decision point → decision-maker, one `RL-` | **Yes** | **RL-979**, 2026-09-02 ([`../rulings/RL-00980-a-line-number-citation-into-a-split-file-is-re-derived-or-replaced-a-path-only-rewrite-is-forbidden.md`](../rulings/RL-00980-a-line-number-citation-into-a-split-file-is-re-derived-or-replaced-a-path-only-rewrite-is-forbidden.md)) — `RFC-`, `kind: process`, `status: closed`, `created: 2026-08-29`, `was:` `docs/closures/INDEX.md#plan-reviewsmd`, its own record. **`owner: maintainer`, against the derivation's recommendation of `planner`** |
...
**DP-A carried the freeze**, and RL-979 discharged it. Two things about that ruling bind
...
- **RL-979 amended RL-978's fourth constraint** — three citations of the container, not
...
dependency of this slice: RL-979 settled the container on the container's facts.
...
1. **Every discovery function is run against the real corpus before it is depended on**, and its
   ...
   declared exception, and the three sum to the total (RL-985). A count that merely looks
...
3. **W37-5b holds this**, not W37-6. The census runs before this slice, not inside it — RL-985
   §3 item 1, and RL-977 §2 for the parser fix, both on their own reasoning rather than this
   plan's.
...
Carried in full, as RL-987 §3 requires and as the maintainer asked: *"I want to read the run
...
|---|---|---|
| **rewrites a citation token in** | **954** | Every file matching RFC-937 §7 (d)'s pattern, measured through the shipped `LEGACY_FORM_PATTERNS` constant rather than a re-typed copy; 21 500 matching lines, 29 106 token hits. By tree: `docs/` 315 · `backend/` 217 · `frontend/` 143 · `packages/` 135 · `.claude/` 54 · `tests/` 53 · `scripts/` 19 · 7 root files · `examples/` 6 · `.github/` 3 · `deploy/` 2 |
...
- **2446 marker citations are rewritten** — not 1988. The 1988 figure is the
  ...
  governed by RL-989's six-class permitted-diff predicate. **Each is a citation the run
...
### 4.3 The enlargement RL-987 requires
...
§6, which this revision does not reopen. RL-997 has since ruled that RFC-937 §8's stage list
...
- **The commit is squash-merged and the branch auto-deletes.** RL-989 computed (g) as a
...
`docs/closures/INDEX.md#plan-reviewsmd` is 2816 lines and contains **exactly one** level-2 heading. Any
...
heading at its own level.** This is RL-985's structural invariant (its option (d)), and §5.2
...
as *"a known, reported gap"*, names RL-990 as having resolved it, and instructs: *"Apply the
...
*"nine of the 28 ruff-excluded skills carry an RFC-937 §5.4 change row, **two of which**
(`writing-plans`, `subagent-driven-development`) are creating instruments RL-987 places
inside W37-6's own migration commit"* (`scripts/_docid.py:323-325`). That was right when
written, against RL-987's floor of seven. The superseded plan's §6.2 then **derived** the
...
| RL-987 member | Skill | In the ruff exclude list |
|---|---|---|
| 1 | `writing-plans` | yes |
...
form.** RL-990 §2 part 3: *"The exemption reaches only the blanket passes… Exempt from the
...
RL-987 edit. RL-990 §4 acceptance item 3 is the proof: after this commit those
...
argued from this docstring rather than from RL-990 **twice in this Work, by two different
...
measured baseline's method and its command table), §6 (RL-987's derivation — thirteen members,
...
`docs/closures/INDEX.md#plan-reviewsmd` yields **twelve records**, not eleven, and the twelfth is not a
review.

| Output | Count | Fields |
|---|---|---|
| `CR-`, `kind: review` | 11 | one per `Plan review N` heading; `created:` each review's own date; `owner: planner`; `status: active` |
| `RFC-`, `kind: process` | 1 | the `Pending proposals` container, lines 1155-1232; `created: 2026-08-29`; **`owner: maintainer`**; `status: closed`; `was:` `docs/closures/INDEX.md#plan-reviewsmd` |
...
2. **The three `###` headings inside the container stay body.** RL-978 item 2 forbids
   ...
   [`PL-00962-w37-6-the-twelve-non-close-records-in-closure-records-md-a-family-derivation.md`](PL-00962-w37-6-the-twelve-non-close-records-in-closure-records-md-a-family-derivation.md),
...
RFC-937 §1.5 reads: *"A vendored skill (`planning-with-files`, `ui-ux-pro-max`, `graphify`,
...
**The divergence is therefore partial and one-directional.** RL-990 §1's verification table
...
list restricted to `.claude/skills/` — RL-990 §2 part 2, where the ruff list is *"not the
...
- **A second §7 entry was drafted and removed before filing**, because RL-990 §2 part 2
...
[`PL-00961-w37-6-everything-it-owns-before-the-run-one-list-with-each-item-s-state-and-what-discharges-it.md`](PL-00961-w37-6-everything-it-owns-before-the-run-one-list-with-each-item-s-state-and-what-discharges-it.md)
...
understating the frozen-plan load RL-989's predicate must classify. It does not — it is the
...
[`PL-00938-rfc-897-landing-package-the-reconciliation-update-the-work-row-draft-and-the-acceptance-batch-2026-09-01.md`](PL-00938-rfc-897-landing-package-the-reconciliation-update-the-work-row-draft-and-the-acceptance-batch-2026-09-01.md), filed one
...
own §7.1 Task 1**, under RL-981's licensing instrument.
...
**Now tracked as [`F92`](../findings/FD-01029-53-files-are-deferred-out-of-4-step-5-s-reference-stamp-set-and-the-deferral-lives-only-in-a-squash-commit-body.md)**, work item W37-6, phase 2 — a register row
