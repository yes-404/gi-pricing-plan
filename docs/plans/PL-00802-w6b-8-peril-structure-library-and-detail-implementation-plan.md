...
family: plan
kind: leaf
title: W6b-8 — Peril Structure Library and Detail Implementation Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-25-w6b-8-peril-structure-views.md
---
...
**Spec:** [`docs/specs/02-modelling.md`](../specs/02-modelling.md) — §3.9 (FR-188, FR-189, FR-190, FR-191,
...
- **`usage_count` must not appear on a peril row.** FR-167: the count is of Model Specs
...
- **FR-24 is not a licence to drop an item.** A view is an obligation because it has a row
  ...
  registered views have no FR"*). FR-24 decides whether a *listed affordance* binds, never
  whether the row does. Every cell item below carries a §13 verdict and an owner.
- **The contract is the floor** (OQ-587, decided 2026-08-21). Anything a view needs beyond
...
|---|---|
| **FR-188** | Per-peril `frequency × severity` or `burning_cost`, **model references pinned by version**; the risk premium is the sum over perils. |
| **FR-189** | Large-loss handling per peril — `none`, `capped`, `separate_model`, `flat_loading` — *"recorded with its calibration evidence."* |
| **FR-190** | Coherence: **every peril either modelled or explicitly excluded with a reason**; modelled burning cost reconciles to observed within a declared tolerance; the reconciliation is persisted. |
| **FR-191** | The structure is an approvable artifact in its own right, with its own lifecycle. |
| **FR-128** | The reconciliation **accounts for the treatment** — modelled cost is compared *after* restoration, and the treatment is stated alongside. |
| **FR-192** | `GET /peril-structures/{id}` returns the structure with its reconciliation or a 404 naming it; `POST /{id}/submit` moves `reconciled → review`. |
| **FR-167** | `GET /peril-structures` lists, cursor-paginated, filterable by `status` and `slug`. **No `usage_count` on the peril row.** |
...
states.** FR-167's 2026-08-24 amendment (third correction) does not merely omit the
...
FR-237 pins one per `model_call`), *"so the absent `/usage` route is a separate question
and not evidence for this one."* Do not reach for that to justify a count.

**3. Is `02`:2595/2596 among FR-24's seven carve-outs? No — neither cell is.** The seven
...
FR-24 applies in full and both cells are **prose binding nothing**.

**Where that precedent bears — and it bears here more than anywhere.** FR-24's closing
sentence: *"the contract-is-the-floor half is OQ-587's decided rule of 2026-08-21;
OQ-587 says nothing about a cell being prose, and that half rests on the `02` §5.3 Peril
structure library precedent alone."* **This slice builds the cell that is the sole precedent
for half of FR-24.** Two consequences an executor must not miss:

- The library cell's load-bearing content is a **negative** — "no usage count" — and OQ-549
  singled this row out as the one that *"enumerates its negative space."* Under FR-24 that
  negative binds nothing **as a cell**. It binds anyway, because **FR-167 independently
...
- Because this cell is the precedent, **an implementation that contradicts it weakens
  FR-24 itself.** If the built library renders a usage count, the sole support for
...
W6b-7 (`docs/plans/PL-00801-w6b-7-objective-library-objective-certificate-and-metric-library.md`, Task 4) creates
...
FR-153's objective/metric concept — which responses, which backends, whether an offset is
required — and `PerilStructure` carries no such field. `usageCount` is forbidden by
FR-167. Two of the six row fields are structurally absent, and the component was designed
...
column FR-167 rules *undefinable*, rendering an empty cell asserts that a usage count
...
| | `perils` | partly | FR-188 |
| | `excluded_perils` | **no** | **FR-190** — *"either modelled or explicitly excluded with a reason"* |
| | `reconciliation` | yes | FR-190 |
| | `status` | **no** | **FR-191** — the structure's own lifecycle |
| | `created_at` | no | Ordinary provenance |
| `PerilComponent` | `peril` | implied | FR-188 |
| | `method` | **no** | **FR-188's two routes** — which one this peril takes |
| | the three model refs | yes | FR-188 |
| | `large_loss` | yes | FR-189 |
| `LargeLossTreatment` | `kind` | yes | FR-189 |
| | `cap_minor`, `attachment_minor` | no | FR-189's parameters — see Finding 3 |
| | `restoration_loading` | **no** | **FR-128** — the reconciliation compares *after* restoration, so this is what makes the ratio mean what it means |
| | `excess_model`, `loading_factor` | no | FR-189's parameters for the other two kinds |
| | `evidence_blob` | no | FR-189's calibration evidence — deferred, §13 |
| `ExcludedPeril` | `peril`, `reason` | **no** | FR-190 |
| `Reconciliation` | `part` | **no** | **FR-190 reconciles *on the holdout*** — `part` is what says whether this verdict is one |
| | `tolerance` | no | FR-190's *"declared tolerance"* |
| | `ratio`, `status` | no | FR-190's verdict — `computed_field`s, Trap 6 |
...
*The contract.* OQ-587's decided rule of 2026-08-21 makes the generated contract the
...
**This slice is where that rule gets exercised for the first and only time.** FR-24
(`00-overview.md:227`) closes: *"the contract-is-the-floor half is OQ-587's decided rule
of 2026-08-21; OQ-587 says nothing about a cell being prose, and that half rests on the
...
they leave the sole support for half of FR-24 contradicted by the code written under it.

*The requirements.* Independently of any of that, five of the unnamed fields are required by
numbered requirements — `excluded_perils` and `part` by FR-190, `status` by FR-191,
`method` by FR-188, `restoration_loading` by FR-128. These bind whatever one
concludes about cells and contracts, and they would bind if FR-24 were repealed tomorrow.
...
### Finding 3 — the reconciliation panel has no route to a currency, and it is OQ-551's fourth view
...
**There is no route to one.** `Reconciliation` carries `dataset_version_id`, and OQ-551
...
exactly, and **the reconciliation panel is a fourth view in it.** OQ-551 is **open and
...
the derived `status` are **dimensionless**, and they are precisely what FR-190 makes the
requirement — *"reconciles to observed burning cost within a declared tolerance."* The
per-peril breakdown FR-128 asks for is preserved as each peril's **share of the modelled
...
when OQ-551 is decided; this plan proposes adding the panel to that question's view list.
...
- **They are unambiguously money.** A cap on a loss and an attachment point are amounts, not
  statistics, so OQ-538's classification question does not touch them. `validate.py:983-986`
...
- **Omitting them removes requirement content, where omitting the burning costs does not.**
  FR-190's verdict survives in `ratio`; FR-189's treatment does not survive in `kind`.
...
it is why OQ-551's fourth view is **two panels, only one of which can drop its amounts.**
Bring that to the manager when the question is costed; it is the sort of asymmetry that a view
count hides.

### Finding 4 — `OQ-538` is open on the very fields this view renders

`Reconciliation.observed_burning_cost_minor` and `ReconciledPeril.modelled_burning_cost_minor`
are named in OQ-538 as one side of a live divergence: burning cost is typed as strict
`MoneyMinor` here and as `float | None` without a suffix on `OneWayRow.mean_burning_cost`, and
FR-10's boundary now forbids both standings at once. The open recommendation is (b) — burning
...
This is not a defect to fix here — it is FR-207's staged-contract rule working as
...
not one of them. FR-24 makes these cells prose, which is why each item is dispositioned
rather than assumed.
...
| Library: `status`/`slug` **filtering** | **Delivered but untested at the UI** — Task 1 | The API module exposes both filters and they are unit-tested; no filter *control* is built. FR-167 requires the endpoint be filterable, not that the view expose controls. Owner: the slice that needs one |
| Detail: **per-peril model pins** | **Delivered** — Task 5 | `ArtifactRef` rendered as the canonical `model:slug@version`; FR-188's pinning is visible or it is not pinned |
| Detail: per-peril **method** (FR-188) | **Delivered** — Task 5 | Unnamed by any cell noun; Finding 2. Which of the two routes this peril takes decides which model refs are even meaningful |
| Detail: large-loss **parameters** — `cap_minor`, `attachment_minor`, `restoration_loading`, `loading_factor`, `excess_model` | **Delivered** — Task 5 | Unnamed by any noun. `restoration_loading` is FR-128's; the two `*_minor` are rendered currency-free per Finding 3 |
| Detail: reconciliation **`part`** (FR-190) | **Delivered** — Task 4 | FR-190 reconciles *on the holdout*; without `part` the reader cannot tell whether this verdict is one |
...
| Detail: large-loss **calibration evidence** (FR-189) | **Deferred, owner: the maintainer** | `LargeLossTreatment.evidence_blob` is an opaque blob with no contract-declared shape. Rendering it needs a declared shape first — a new requirement under OQ-587's floor rule, not a guess at a call site. Proposed to the roadmap |
| Detail: **reconciliation panel** — ratio, tolerance, derived status | **Delivered** — Task 4 | FR-190's verdict, read from the `computed_field`s |
| Detail: reconciliation **per-peril breakdown with treatment** (FR-128) | **Delivered** — Task 4 | As dimensionless share of modelled total, beside each peril's `large_loss_kind` |
| Detail: reconciliation **absolute burning-cost amounts** | **Deferred, owner: the maintainer** | Blocked on OQ-551 (no currency route) and touched by OQ-538. Finding 3 |
| Detail: **excluded perils with reason** (FR-190) | **Delivered** — Task 5 | Named by no cell noun; required by FR-190. Finding 2 |
| Detail: **status / lifecycle** (FR-191) | **Delivered** — Task 5 | Rendered; `reconciliation` is `None` in `draft` and that is a state, not an error (Trap 3) |
| Detail: **submit** affordance (FR-192) | **Not started, owner: WK-664 or its successor** | `POST /{id}/submit` exists and is unbuilt in the UI. A submit control needs the `reconciled → review` guard, an approval-request surface and an error path; it is a slice, not a button. Booked explicitly rather than left silent. Proposed to the roadmap |
...
4. **`draft → review` is not an edge in this lifecycle.** The module docstring: FR-191
   makes the structure approvable and FR-190 makes the reconciliation its evidence, so a
...
| Create `frontend/src/components/ReconciliationPanel.vue` | FR-190's verdict and FR-128's breakdown |
...
git commit -m "feat(w6b-8): read peril structures, with FR-167's two filters"
```
...
column from an unknown value, which is the whole of FR-167's third correction.
...
it("states the treatment's parameters, not just its kind (FR-189, Finding 3)", async () => {
  ...
  expect(wrapper.text()).toContain("1.08");     // FR-128's restoration
...
- [ ] **Step 2: Confirm the absence that FR-167 specifies**

```bash
grep -rn "usage_count\|usageCount" frontend/src/views/PerilStructureLibraryView.vue
```

Expected: **no output.** If this prints anything, the sole precedent supporting FR-24's
...
1. **OQ-551 gains a fourth view.** The peril reconciliation panel holds a
...
2. **`LargeLossTreatment.evidence_blob` has no declared shape.** FR-189 requires the
   ...
   treatment and not its evidence, which is half of FR-189.
3. **The submit and reconcile affordances are unbuilt and unowned.** FR-192 makes the
   ...
   approval surface, reconcile is a 202 needing `07`'s job progress. Owner: WK-664's successor.
...
**Spec coverage.** Every requirement in the scope table maps to a task: FR-188 → Task 5
test 1; FR-189 → Task 5 test 3, with its evidence half deferred and proposed; FR-190
→ Task 4 test 1 and Task 5 test 2, both limbs (tolerance verdict, and excluded-with-reason);
FR-191 → Task 5's status rendering, with the submit half booked not-started; FR-128 →
Task 4 test 2; FR-192's read half → Task 1, its submit half booked; FR-167 → Tasks 1,
