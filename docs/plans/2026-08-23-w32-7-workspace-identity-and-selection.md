...
**Architecture:** Three requirements in one dependency chain, so one plan. FR-PLAT-62 gives the
`workspaces` table its row and backfills one for every distinct `workspace_id` already stored
anywhere — the orphan case a foreign key must not discover after the fact. FR-PLAT-63's four
obligations then have something to name: `GET /me` lists the caller's memberships with their names,
and `require_caller` reads FR-PLAT-65's `Workspace-Id` header, checks it against
...
five-spec path-prefix edit FR-PLAT-65 refused on cost stays refused. The switch audit closes it:
`06` FR-GOV-24 chains per workspace, so leaving one and entering another writes two events, one in
each chain.

**Tech Stack:** Python 3.12, FastAPI, Pydantic v2, SQLAlchemy 2.x async, Alembic, PostgreSQL 16,
`pytest`, `uv`.

**Spec:** [`../specs/07-platform.md`](../specs/07-platform.md) §3.1 — FR-PLAT-62, FR-PLAT-63,
FR-PLAT-65 (the last decides OQ-PLAT-9). The audit obligation is
[`../specs/06-governance.md`](../specs/06-governance.md) FR-GOV-24; the identity endpoint is `06`
§5.1, built as `backend/src/app/api/me.py`.

**Proposed slice id:** `W32-7`. The W32 slice boundaries in
...
- No new requirement ids. FR-PLAT-62, FR-PLAT-63 and FR-PLAT-65 all exist in
...
- The header is spelled **`Workspace-Id`** — unprefixed, and deliberately not
  `x-dev-workspace-id`, which is the `local`/`dev` development header FR-PLAT-55 confines. Do not
...
FR-PLAT-62's backfill is the load-bearing half: *"a row for **every distinct `workspace_id`
...
Then give both existing rows the reference FR-PLAT-62 asks for. In `WorkspaceMemberRow` and
...
— FR-PLAT-62 names `workspace_members` and `workspace_settings` and no others, and putting the key
...
FR-PLAT-63 opens with it: *"The memberships are readable from the identity endpoint `06` §5.1
declares, each carrying FR-PLAT-62's name."* Without this the switcher can enforce a choice it
cannot offer.
...
so this list is empty for one. That is correct and not a bug: FR-PLAT-65 says a Service Account
...
**Where the header is declared, and why it is not on the routes.** FR-PLAT-65 says *"declared on
...
Create `backend/tests/test_workspace_selection.py`. Four tests, one per FR-PLAT-63 obligation:
...
Note what changed and what did not: the no-membership branch keeps `UNAUTHENTICATED` — FR-PLAT-4
owns it and FR-PLAT-63 does not touch it. Only the multi-membership branch gains a code of its own.
...
a different purpose, and FR-PLAT-65 says so in as many words. Add a one-line comment at its call
...
catalogue, and FR-PLAT-65 requires the refusal to be a typed platform error. Say so in a comment.

Add `Header` to the `fastapi` import.

- [ ] **Step 5: Update the module docstring and the stale comment**

`deps.py:12-23` currently says the capability is unbuilt: *"Until W32 builds that check, the multi-
...
them — W32 builds the header check; `_single_workspace` still refuses a multi-membership caller as
...
FR-PLAT-63's fourth obligation: *"a switch is audited into both chains — `06` FR-GOV-24 chains
...
explicitly in the ledger, and give FR-PLAT-63's fourth obligation the §13 verdict *deferred with an
...
- Create: `2026-08-23-w32-7-workspace-identity-and-selection-ledger.md`
...
and paste the relevant lines into the ledger — that artifact is the evidence FR-PLAT-65's
"declared… in the published contract" clause is satisfied.

- [ ] **Step 4: Write the ledger and the slice record**

Create `docs/plans/2026-08-23-w32-7-workspace-identity-and-selection-ledger.md` with the real gate
...
Give each of FR-PLAT-63's four obligations an explicit §13 verdict. Three should be *delivered and
