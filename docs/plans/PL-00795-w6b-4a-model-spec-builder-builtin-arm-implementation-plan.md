...
family: plan
kind: leaf
title: W6b-4a — Model Spec Builder, Builtin Arm — Implementation Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-25-w6b-4a-model-spec-builder-builtin.md
---
...
**Highest ids in use: `FR-168`, `OQ-604`, `OQ-552`. Next free: `OQ-605`,
taken by this PR.**

**`OQ-605` is filed with this plan although its finding belongs to `W6b-4b`.** The
...
- **No requirement renumbered**; this PR proposes no requirement and one open question,
  `OQ-605`, mirrored into `open-questions.md` and `02` §10 in the same commit.
...
| **`W6b-4b`** | The custom arm: the list endpoint, which statuses are offered, applicability filtering, the GLM/GBM asymmetry, the picker/library boundary, and `OQ-605`. |
...
**The Contents cell does not bind.** `02` §5.3:2584 names seven things. Under **FR-24** a
...
**The cell's citation is narrower than the phrase it attaches to** — the case FR-24 warns
of, where "a discharge is checked by reading the cited requirement's predicate, not by
confirming its id exists". The cell cites **FR-153** for "live spec validation";
FR-153 (`02`:217) is the *applicability* rule. The endpoint's own §5.1 row (`02`:1705)
cites **FR-153 and FR-185**, and the lead confirmed FR-185 is a distinct
...
| **`FR-120`** (`02`:174) | The builtin set: `count:poisson`, `reg:gamma`, `reg:tweedie` (with `tweedie_variance_power`), `binary:logistic`. |
| **`FR-185`** (`02`:253) | "Model complexity is a diagnostic **by default**, and a gate only where a workspace asks for one" (OQ-580). |
| **`FR-111`** | Family, link, offset and weight per response type — the GLM arm's correctness defaults (`CLAUDE.md` §7). |
...
| Tab | Shape |
|---|---|
| **GLM** | `family` — `poisson \| negative_binomial \| gamma \| inverse_gaussian \| tweedie \| binomial \| gaussian` (**7**) — **×** `link` — `log \| logit \| identity \| inverse` (**4**) — plus `family_params`. Governed by FR-111. |
...
the GLM pair into a single "objective" choice — losing the link, which FR-111 makes a
...
default and the decision — OQ-580 refused a platform-wide constant, so an unset limit means
...
Following from F2. FR-185: complexity is "a diagnostic **by default**, and a gate only
...
FR-185's *default* case is precisely the case such a builder discards — and the default
...
`family` × `link` pair **is** its distributional assumption, and FR-111 makes family, link,
...
| | Option | |
|---|---|---|
| **(a)** | Validate on every keystroke | Truest to "live". One request per character on a factor filter, and `SpecValidation` reads an unindexed JSONB column (FR-167's own note). |
...
1. **The custom objective arm is `W6b-4b`'s**, with the four contestable judgements that come
   with it.
2. **`GlmSpec.custom_objective_ref` is a governed gap, not a §0 disagreement.** FR-207
   ...
   lead is tracking that no WK-664 slice builds it.
...
+ `family_params`; GBM `GbmFunctionRef{kind: "builtin", name}` over FR-120's four with
...
default-workspace case FR-185 is actually about.
...
3. **If the builtin objective set for a *spec* is not FR-120's.** The GBM arm's four names
