...
family: plan
kind: leaf
title: Closure Audit Standard Implementation Plan — docs/audit, §14 rule, RFC-778 custody
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-27-closure-audit-standard.md
---

# Closure Audit Standard Implementation Plan — docs/audit, §14 rule, RFC-778 custody

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** File the closure-audit standard. It has three parts: a `docs/audit/` structure that records per-work-item and per-phase closures, a third rule in `CLAUDE.md` §14, and durable custody for the seven RFC-778 items.

**Architecture:** A hybrid complement. `close-workstream` and `phase-review` stay the binding procedures. `docs/audit/` already holds the archive (the closure records, plan reviews and retrofit list moved there by RFC-813). This plan adds the record layer: per-work-item and per-phase records, two checklists, and two registers. The checklists point at the skills. No new id family is minted. Work items and findings reuse existing ids.

**Tech Stack:** Markdown, `scripts/audit-docs.py`, `CLAUDE.md`, `docs/specs/00-overview.md`, `docs/open-questions.md`, `docs/roadmap.md`.

**Spec:** `RFC-812` (the structure) · `RFC-778` (the seven items). The maintainer's three rulings on RFC-812's acceptance points, 2026-08-27.

**Highest ids:** This plan mints one id. Next free: `OQ-554`.
...
**F1. `docs/audit/` exists as the archive, with no record structure.** RFC-813's slim (`#274`) created `README.md`, `closure-records.md`, `plan-reviews.md` and `retrofit-impossible.md`. The record layer — `checklists/`, `work/`, `phases/`, `register.md` — does not exist. The README's closing line says the RFC-812 structure stays open until the three acceptance points are answered. They were answered 2026-08-27, so the line is stale.

**F2. The seven RFC-778 items have no durable home, with two exceptions.** Item (c) expired: `W6b-1b` shipped as `#194` and `W6b-9` shipped its checks. Item (g) is placed: the roadmap §10 note of 2026-08-26 records that all twenty unplaced questions were placed or recorded, including the eight item (g) names. The remaining five — (a) the OQ next-free marker, (b) the §5.3↔§5.6 route check, (d) the two summary lines, (e) the §5.6 authoritative column, (f) the workflow-content OQ — are still unfiled.
...
| RFC-778 custody | per item | T6 |

---

## Tasks

### T1. `docs/findings/README.md` — the standard

**Files:**
- Modify: `docs/findings/README.md`

- [ ] Update the intro to name both roles: the archive (RFC-813) and the record layer (this plan).
...
### T2. `docs/process/checklists/work-item-close.md`

**Files:**
- Create: `docs/process/checklists/work-item-close.md`
...
### T3. `docs/process/checklists/phase-close.md`

**Files:**
- Create: `docs/process/checklists/phase-close.md`
...
**Files:**
- Create: `docs/findings/register.md`
- Create: `docs/audit/work/.gitkeep` or a first example record
- Create: `docs/audit/phases/.gitkeep` or a first example record

- [ ] Write `docs/findings/register.md`: the global list of open findings, one row per finding, keyed by the requirement or artifact id it concerns. Each row names the work item, the phase, and the decision.
- [ ] Create one example work-item record under `docs/audit/work/` as the fill-in-the-blank. Use a real, closed item as the exemplar, for example the WK-665 plan (`pr-265`).
...
### T6. RFC-778 custody — the seven items

**T6.1. Item (a): the OQ next-free marker.**

- [ ] Add a next-free marker block to the `docs/open-questions.md` header, mirroring the plans convention. One line per family, for example: `Highest ids in use: OQ-553, OQ-570, OQ-613, OQ-656. Next free: OQ-554.`
- [ ] Verify the marker names the current ceilings. Item (f) mints `OQ-554`, so the OVR line must say `Next free: OQ-554`.

**T6.2. Item (e): the §5.6 authoritative column.**

- [ ] Add a dated note to `docs/specs/00-overview.md` §5.6: the Route column is the canonical route. A module's §5.3 Route cell must agree with it. `00` §5.6 names the canonical views and routes (FR-25), so it governs.
...
- [ ] Next free: `OQ-554`. File the row in `docs/open-questions.md` under OVR: nothing checks that an `FR-` id cited by a workflow step contains what the step claims.
- [ ] Mirror the row in `docs/specs/00-overview.md` §10. `00` owns FR-19, the citation audit.
...
- All seven RFC-778 items have a home: five filed, two recorded as discharged.

---

## Sources

- `.claude/rfcs/RFC-00812-a-closure-audit-structure-for-work-items-and-phases.md`: the structure and the three acceptance points.
- `.claude/rfcs/RFC-00778-seven-deferred-items-with-no-durable-custody.md`: the seven items and their assessments.
