...
family: plan
kind: leaf
title: W37-5c — the slice decision, and gap 2 ruled
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-09-02-w37-5c-slice-decision.md
---
...
[`PL-00958-w37-6-the-migration-run-the-go-ahead-ask-with-condition-6-discharged.md`](PL-00958-w37-6-the-migration-run-the-go-ahead-ask-with-condition-6-discharged.md) §8 — *"Decision: not yet.
...
| 3 | **The three unparseable vendored manifests** | A stamping precondition on no list — RL-990 settled *stamping* a vendored file, not one whose front matter does not parse | §5 below |
...
33 raw READMEs rather than counting them: **17 become `CR-`** (16 work + 1 phase), `findings/README.md`
is deleted, `.claude/rfcs/README.md` goes with the stubs. **Verified by the lead** — 33 raw, 17
...
- **Exemption** — the three are named in an exempt-by-path list, as `docs/_templates/` already
  is under RFC-937 §1.4. Cheapest; leaves them unowned and invisible to checks 30–39; the
...
- **Does not amend RFC-937 §1** — §4.1's alternative and §4.2's exemption are both flagged to the
