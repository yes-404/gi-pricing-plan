# Paired Quantile Models (FR-MODEL-78) Implementation Plan
...
joins `spec_hash` (FR-MODEL-86, `v3 → v4`), which is what stops two bounds against different
central versions colliding. Prediction finds the pair by querying the link, decides one
verdict per model, and either returns bounds or FR-MODEL-77's typed absence.
...
- **Requirement IDs are permanent** (`CLAUDE.md` §5). Never renumber. Append, or mark
  superseded. The next free id is **FR-MODEL-100**; the next free open question is
  **OQ-MODEL-16**. Find the *maximum* id, not the last one you read — `02`'s requirement
  tables are not in numeric order.
- **`model-schema` is the single source of truth** for shapes crossing a boundary
  (`CLAUDE.md` §2, ADR-0002). Nobody hand-writes a shape that already exists there — not the
  backend, not the frontend, not a test fixture.
- **`pricing-core` stays importable standalone** with zero FastAPI/SQLAlchemy/Redis
  dependencies (ADR-0001, enforced by `.importlinter`).
- **Money is integer minor units or `Decimal`, never float**, in the rating path
  (`CLAUDE.md` §7). `DecimalStr` refuses a `float` as of PR #116 (OQ-OVR-8).
- **Artifacts are immutable.** A Model is never edited; a change is a new version
  ...
  `artifact_append_only` trigger (FR-DATA-42/47).
- **Any change to the set of fields entering the `spec_hash` payload increments
  `SPEC_HASH_VERSION` in the same commit as the field** (FR-MODEL-86).
- **A field is shown live only once a slice populates it** (FR-MODEL-87). Anything else is
...
**Task 1 raises `OQ-MODEL-16` and the plan cannot finish without an answer to it.** Tasks 2–5
...
FR-MODEL-98 (decided 2026-08-18, OQ-MODEL-13) says the platform offers **exactly one**
...
aggregate, and it arrives through a door FR-MODEL-98 did not name — an actuary who opted in
...
| Path | Change |
|---|---|
| `docs/specs/02-modelling.md` | FR-MODEL-100 appended to §3.10; FR-MODEL-77 and FR-MODEL-87 gain dated amendment notes; §4.4 gains the `interval_for` block; §4.8's staging note updated; §5.1's error-code paragraph gains one code. |
| `docs/open-questions.md` | OQ-MODEL-16 raised with options and a recommendation. |
...
| `docs/roadmap.md` | W5 slice record; W5's row gains this slice. |

### Deliberately not touched

- **The frontend.** No view renders a GBM interval, and `02` §5.3's model-detail view is
  W6b's. Building a screen here is building ahead of the row that owns it (`CLAUDE.md` §0).
...
## Task 1: The spec change — FR-MODEL-100, and the question the requirement leaves open
...
**Interfaces:**
- Consumes: nothing.
- Produces: `FR-MODEL-100` (the id every later task's `@pytest.mark.req` names) and
  `OQ-MODEL-16` (the gate above).
...
> not decided by FR-MODEL-77 and is decided by FR-MODEL-100 — a requirement rather than an
...
```markdown
> **`interval_for` is live from 2026-08-19 (W5, the paired-quantile slice)** — FR-MODEL-87's
> staging rule, and the last of the fields OQ-MODEL-8 listed as absent-entirely on this arm.
> `null` on every GBM that is not itself a bound, which is almost all of them: it is the
> declaration that *this* model is one side of another model's interval, and FR-MODEL-78
> makes that an opt-in the actuary pays 2–3× a fit for.
```

- [ ] **Step 5: Update §4.8's staging note and FR-MODEL-87's residual list**

In FR-MODEL-87's row, `interval_for` is listed under **absent entirely**. It is no longer
absent. Amend in place rather than deleting the mention:

```markdown
`interval_for` — **live from 2026-08-19** (FR-MODEL-100), on `GbmSpec` rather than on
`Model`;
```
...
- [ ] **Step 7: Raise OQ-MODEL-16 in `docs/open-questions.md`**
...
mirrored into `open-questions.md` and back — so if you added OQ-MODEL-16 to only one of them,
...
git commit -m "docs(model): FR-MODEL-100 — interval_for's home, and the two readings FR-MODEL-77 left open"
```

- [ ] **Step 10: Ask the maintainer for OQ-MODEL-16**
...
**Interfaces:**
- Consumes: `FR-MODEL-100` from Task 1.
...
def test_the_spec_hash_algorithm_version_moved_with_the_new_field() -> None:
    ...
    Without this the digest of every stored spec silently changes, FR-MODEL-66's dedup ends
...
```python
#: `interval_for` moved it `v3 → v4` (2026-08-19, FR-MODEL-100).
...
git commit -m "feat(model): FR-MODEL-100 — interval_for on GbmSpec, spec_hash v4"
```
...
FR-MODEL-78: *same Model Family, same dataset version, split and factor set*. Nothing enforces
...
```python
"""FR-MODEL-78/100 — paired quantile models, and the four ways a pair can be wrong.
...
async def _refuse_mismatched_interval_model(
    ...
    """FR-MODEL-78's "same family, dataset version, split and factor set", enforced.
    ...
    **Set comparison on `factors`, not tuple comparison.** Two specs listing the same factors
    ...
    if mismatches:
        raise PlatformError(
            ...
            f"specs disagree on {', '.join(mismatches)} (FR-MODEL-78). An interval fitted "
    ...
    for existing in await load_interval_models(
        ...
        if (existing_spec.interval_for.alpha < 0.5) == (spec.interval_for.alpha < 0.5):
            raise PlatformError(
                ...
                f"({existing.model_family_slug}@{existing.version}). FR-MODEL-100 allows one "
...
git commit -m "feat(model): FR-MODEL-78 — a bound must match the model it bounds"
```
...
async def load_interval_models(
    ...
    """Every bound fitted for `central_model_id`, lower first (FR-MODEL-78).
...
```python
def upgrade() -> None:
    """A functional index on the bound→central link (FR-MODEL-78).
...
git add backend/src/app/platform/modelling.py backend/migrations/versions \
        backend/tests/test_paired_quantile_models.py
git commit -m "feat(model): FR-MODEL-78 — find a model's bounds, lower first"
```
...
FR-MODEL-78: *crossing quantiles are detected, reported in the diagnostics, and never
silently reordered.* FR-MODEL-49 computes diagnostics once at fit time, so the detection
...
def test_it_never_reorders() -> None:
    ...
    downstream interval a fiction — the exact failure OQ-MODEL-2 was decided to avoid.
    """
...
def detect_quantile_crossing(
    ...
    OQ-MODEL-2 was decided to avoid.
...
class QuantileCrossing(BaseModel):
    ...
    FR-MODEL-49 computes diagnostics once at fit time. `counterpart_model_id` names the model
...
```python
    #: FR-MODEL-78. Set on the second bound of a pair; `None` on every other model.
...
```python
    # FR-MODEL-78. Only the *second* bound of a pair has a counterpart to cross; the first
...
FR-MODEL-78, not only against the model it was generated from.**
...
git commit -m "feat(model): FR-MODEL-78 — crossing quantiles are detected, never reordered"
```

---

## Task 6: Prediction consumes the pair — ⚠ needs OQ-MODEL-16 decided

**Do not start this task until the maintainer has written an acceptance line for OQ-MODEL-16
...
> requires FR-MODEL-98 to gain an addendum (never an edit — `CLAUDE.md` §14). Everything
> below is otherwise unchanged.

In `prediction.py`, add to `UncertaintyKind`:

```python
    #: A paired-quantile interval on `Y` itself (FR-MODEL-78, OQ-MODEL-16). **Not**
...
class IntervalModels(BaseModel):
    """Which two Models produced a quantile-pair interval (FR-MODEL-78).
...
async def test_a_gbm_with_only_one_bound_says_no_interval_models_fitted(client, ...):
    ...
    FR-MODEL-77's vocabulary is closed; a lone bound is the absence of a *pair*, which
...
async def test_an_approved_model_whose_bounds_are_only_fitted_says_not_approved(client, ...):
    """FR-MODEL-100(ii). The first time this reason has ever been reachable.
...
async def test_a_superseded_model_reports_its_bounds_stale(client, ...):
    ...
    silence FR-MODEL-77 exists to refuse.
    """
...
def test_every_unavailable_reason_is_now_reachable() -> None:
    ...
    FR-MODEL-87 required the unreachable members to be named in place. They are reachable
...
2. Central model `superseded` → `INTERVAL_MODELS_STALE` (FR-MODEL-100 iii).
3. Any bound less advanced than the central model → `INTERVAL_MODELS_NOT_APPROVED`
   (FR-MODEL-100 ii). Compare on an explicit ordering of `ModelStatus`, not on the enum's
...
git commit -m "feat(model): FR-MODEL-78 — a GBM can carry an interval, and all four reasons are reachable"
```
...
> **If the maintainer chose differently on OQ-MODEL-16:** the shape of this refusal does not
...
git commit -m "fix(model): FR-MODEL-78 — a crossing pair is refused, not reordered into a 500"
```
...
**Files:**
- Modify: `docs/roadmap.md` (W5's row; a new slice record in §6)
...
Expected: the specified count rises by **1** (FR-MODEL-100) and the marked count rises by
**1** or more. `FR-MODEL-78` must leave `--endpoints`' unevidenced list. If it has not, a test
...
FR-PLAT-54 makes it derived, not written, so there is nothing to update — but check that it
...
- **No frontend.** `02` §5.3's model-detail view is W6b's; nothing renders a GBM interval, so
  the capability is reachable only over the API. Owner: **W6b**.
- **`quantile_crossing` is computed and rendered nowhere.** Same owner, same reason.
- **OQ-MODEL-16's answer, with its date**, and — if the maintainer chose the third member —
  that FR-MODEL-98 was amended by **addendum**, never edited (`CLAUDE.md` §14).
- **The two interpretation choices** FR-MODEL-100 fixed, named as choices with alternatives,
  so a later reader can see they were decided rather than assumed.

- [ ] **Step 5: Add the slice to W5's row**

W5's row in the Phase 1b table currently ends "…the profile contract, and `top_levels`'
...
gh pr create --title "feat(model): FR-MODEL-78 — paired quantile models" --body "..."
```
...
**1. Spec coverage.** FR-MODEL-78's four clauses map to tasks: *"each bound is a Model in its
...
`IntervalModels` block on the response. FR-MODEL-77's three reasons → Task 6 Step 5.
FR-MODEL-86's bump → Task 2. FR-MODEL-87's staging rule → Task 1 Step 5 and Task 6 Step 3's
reachability test.

**Gap, stated rather than closed:** FR-MODEL-78 says the cost "is a choice the actuary makes
**and can see**". Over the API they can; on a screen they cannot, because no screen exists.
That is W6b's and Task 8 Step 4 records it as an owner rather than as done.
