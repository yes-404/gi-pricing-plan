...
**Goal:** Fit a GLM and a GBM on the freMTPL2 demo dataset, compare them, approve one, create a rating version that references it, and drive `wf-01` end to end. This is the Phase 1b exit criterion.

**Architecture:** The W7a seed already produces a validated freMTPL2 dataset version. This plan extends the seed to create factors, fit a GLM and a GBM through the real Job path (`model.fit`), run the model comparison, submit and approve the selected model, and create a rating version. The exit demo runs the journey over HTTP and verifies its postconditions. The full 03 rating surface stays Phase 2.
...
**Highest ids:** This plan takes one new id. Next free: `FR-PLAT-67`.
...
- The demo fit uses the full 678 013-row dataset. If the seeded state exceeds the NFR-PLAT-4 budget, sample with `--rows` and record the deviation.

---

## Scope

The manager's scope (2026-08-27): a fitted GLM on freMTPL2, a rating version, `wf-01` end to end. The five rulings (2026-08-27) set the breadth: the full journey, GLM and GBM, comparison, and one approved.
...
**F4. The carried-forward obligations hold.** FR-DATA-55 is delivered as to the never-inferred half and untested at the seam. FR-DATA-56 has no broken-input enforcement proof. FR-DATA-57 is spec-only and not started. FR-DATA-52 is a decided deferral with no owner. NFR-MODEL-7 is out of Phase 1 scope by the 2026-08-22 maintainer verdict. Each is detailed in the table below.
...
The W6b close record carries five findings that this plan folds in as obligations. Each has a named owner. The decision-maker's rulings set the owners on 2026-08-27.
...
| FR-DATA-55 | Test the reference-pin seam: the pinned path, the `None` path, and the pinned-but-unprofiled fallback. No Python test sets `reference_dataset_version_id` today. | W7 (Slice W7-4, T1) |
...
| FR-DATA-52 | Do the trigger-check: confirm the factor workbench does not ask for exposure-ordered levels. Keep the deferral unowned. Record the check. | W7 (Slice W7-4, T2) |
| NFR-MODEL-7 | Deliver the model round-trip test and reverse the 2026-08-22 out-of-scope verdict with a dated note. | W7 (Slice W7-4, T4) |
...
**OD2 — The NFR-MODEL-7 verdict. Ruled (a).** Reverse the 2026-08-22 verdict with a dated maintainer note in `02` §9. Deliver the round-trip test in W7. The demo models are the natural subjects.

**OD3 — The wf-01 demo breadth. Ruled (b).** The full journey: fit a GLM and a GBM, compare them, select one, approve it. The demo mirrors the roadmap's demo-able outcome.

**OD4 — The seed-time fit budget. Ruled (a).** Fit at seed time on the full dataset with a reduced factor set. Measure the total against NFR-PLAT-4. If the total exceeds the budget, sample with `--rows` and record the deviation.

**OD5 — The exit-demo mechanism. Ruled (a).** A scripted, HTTP-driven run that verifies `wf-01`'s postconditions. The UI shows the seeded results.
...
|---|---|---|
| W7-1 | The demo models and the FR-DATA-56 enforcement proof | none |
...
- [ ] Build the spec without an offset first. Verify the spec validation refuses it and names the acknowledgement, the FR-MODEL-27 refusal.
...
- [ ] Next free: `FR-PLAT-67`. Add the requirement row to `07` §3: the demo seed creates and approves a minimal Rating Version that pins an approved Model. The full `03` surface stays Phase 2.
...
**T4. NFR-MODEL-7: the round-trip test and the verdict reversal.**
...
- [ ] Add a dated maintainer note to `02` §9: the verdict is reversed, and the round-trip test in W7 delivers the requirement.
...
- [ ] Measure the total time and compare it to NFR-PLAT-4. If the total exceeds 5 minutes, sample with `--rows` and record the deviation.
...
- The FR-DATA-56 test fails on a broken mutation. The FR-DATA-55 test covers all three reference paths. The NFR-MODEL-7 test compares two scores.
- The exit demo runs end to end on freMTPL2 and the `wf-01` §4 postconditions hold.
- The seeded state stays within the NFR-PLAT-4 budget, or the plan records the sample and the reason.
