# W11 Slice 2 — the real-time scoring endpoint, its bundle slot, and the NFRs that need the HTTP path
...
default-live resolution W14 owns, and the three NFRs that can only be measured once the HTTP
path exists.
...
FR-RATE-40's already-enforced limb visible to `req-coverage.py`. **2D** measures. 2A → 2B is a
...
typed settings in `backend/src/app/config.py`. **No new dependency** — Ruling 17 is explicit
that `orjson` is not added.

**Spec:** [`../specs/03-rating-engine.md`](../specs/03-rating-engine.md) — §3.7 (FR-RATE-34,
35), §3.8 (FR-RATE-40), §5.1 (the `/score` row), §5.2 (`score_one`'s signature), §9
...
it, and note Ruling 12's obligation 4 means the generated side now exists too.

**Slice source:** [`2026-08-29-w11-scoring.md`](2026-08-29-w11-scoring.md), Slice 2. **That
...
[`2026-08-29-w11-slice2-rulings.md`](2026-08-29-w11-slice2-rulings.md) Rulings 14 and 15;
[`2026-08-29-w11-slices-2-4-rulings.md`](2026-08-29-w11-slices-2-4-rulings.md) Rulings 16, 17
and 18; [`2026-08-29-w11-prework-rulings.md`](2026-08-29-w11-prework-rulings.md) Rulings 4, 5,
8 and 10. **Register row [`../audit/register.md`](../audit/register.md) F32 withdraws one of
Ruling 16's two acceptance items** — see Correction C4.
...
error code is registered (`NO_LIVE_RATING_VERSION`, Ruling 14) and error codes are a separate
namespace from `FR-`/`NFR-`/`OQ-` ids, so the marker above does not cover it.

## Task identifiers — why letters

The frozen map names Tasks 2.1, 2.2 and 2.3. **Ruling 14 deletes 2.2** (*"Slice 2 loses that
task"*) and **Ruling 15 reduces 2.3 to a single test with no production code.** Renumbering
...
| **2B** — `POST /api/v1/score` | Task 2.1 | Gains the 409 branch; loses the `prod` restrictions (Ruling 14 clause 3) |
| **2C** — FR-RATE-40's one test | Task 2.3 | Reduced from a gate mechanism to one test, no production code (Ruling 15) |
| **2D** — NFR measurement | folded into map Task 2.1 | Split out; it has a scheduling constraint the others do not |
| *(deleted)* | Task 2.2 | Default-live resolution is W14's (Ruling 14) |
...
2. **The platform refuses to guess.** The same request with `options.rating_version_ref: null`,
   or with `options` omitted, returns **409 `NO_LIVE_RATING_VERSION`** — Ruling 14's own
...
3. **The response is not validated outbound.** A `ScoringResult` whose contents violate its own
   declared types is returned **verbatim**, not 500 — Ruling 17's acceptance test, which
...
4. **The slot serves a degraded read.** With the rating-version load patched to raise, a second
   ...
   while metadata storage is down** — Ruling 16 acceptance item 2.
5. **RBAC holds in three cases** (Ruling 18): a Service Account scoped to `uat` holding
...
one); default-live resolution, the Deployment entity and NFR-RATE-9's 99.95 % availability
target are W14's.
...
- **Money is integer minor units or `Decimal`, never `float`** — [`../../CLAUDE.md`](../../CLAUDE.md)
  §7, FR-RATE-29/56. The route does no ladder arithmetic; it must also not re-serialise money
  through anything that would float it.
- **NFR-RATE-13 as amended (Ruling 17): validate inbound, never outbound.** `QuoteContext` **is**
...
- **`load_bundle` stays pure** (Ruling 10, and `.importlinter:16-34` enforces the structural
...
W15 owns the scoring UI, so there is no frontend work here — but the scoring shapes are in
`GENERATED_SHAPES` as of Ruling 12's obligation 4, so `generate:api` and `type-check` must
still pass.
...
409.** Ruling 14's refusal is the route's, raised before `score_one` is reached; letting
...
**`pricing-core` cannot raise `PlatformError` — it cannot import it** (ADR-0001, enforced by
...
**A decline is not an error.** FR-RATE-39: `outcome: "declined"` with a populated ladder is a
...
| `QuotePurpose` | `"new_business"`, `"renewal"`, `"mid_term_adjustment"`, `"cancellation"`, `"what_if"` — five, Ruling 12 discharged |
| `ScoringOutcome` | `"quoted"`, `"declined"`, `"error"` |

`rating_version_ref` is **optional and nullable** inside `options`, and Ruling 14 clause 1
...
| `Permission.SCORE_EXECUTE` / `SCORE_BATCH` | Exist — `model_schema/permissions.py:58-59`. **Granted by no builtin role, deliberately** (FR-GOV-6), asserted by `backend/tests/test_rbac.py:101-107` |
| Any in-process cache in `backend/` | **None.** Ruling 16 swept `lru_cache`, `cached_property`, `@cache`, `functools`, module-level dicts and singletons — all absent. This slice ships the first |
...
|---|---|
| `Ruling 16's acceptance-test premise (F32)` | **Resolved, and it changes this plan.** Ruling 16's acceptance item 1 (the `load_bundle` purity test) is **withdrawn** — it was already discharged in Slice 1 Task 1.3. Item 2 stands. Task 2A must not re-implement it |
| `NFR-RATE-13/14 (F-W9-1)` | **This slice discharges the NFR-RATE-13 half** — it needs the HTTP path. The NFR-RATE-14 half was Slice 1's |
| `FR-RATE-61 (F-W9-2)` | W13's. Its text says the check *"specialises FR-RATE-40's general approval-evidence gate, which W11 builds"* — Ruling 15 rules what W11 actually builds of that gate, which is one test |
| `03 rating surface (F8)` | The phase-boundary row. W11 discharges its scoring quarter |
| `FR-RATE-25 (F-W9-3)` | W9/Task 1.2's, about compile-time clauses. Not this slice's |
| `03 rating shapes vs hand-authored contracts (F27)` | Owner decided (Ruling 29): the §14 review at W11's close. The `scoring` half was Ruling 12's obligation on Task 1.4 and has landed. Not this slice's |
| `FR-RATE-17 (F-W10-2)`, `FR-RATE-17/18/19/20 (F-W10-1)`, `FR-RATE-16 (F-W10-1-1)`, `DP3 diff cache (F-W10-2-1)`, `FR-PLAT-48 typed diff 200 (F-W10-2-2)`, `03 §5.1 POST /rate-tables/{slug}/versions (F-W10-3)` | W10's and W15's. Not this slice's |
...
**C1 — Task 2.2 does not exist any more.** Ruling 14: default-live resolution is W14's, and
*"Slice 2 loses that task and gains one refusal branch plus its test inside Task 2.1."*

**C2 — Task 2.3 is one test, not a gate mechanism.** Ruling 15 refuses option (a) as the map
...
tests red"* (`backend/tests/test_rating_versions.py:84` and `:259-264`). What W11 owes is *"one
test, and no production code."*

**C3 — the map's FR-RATE-35 line reads as two things to implement and is neither.** It says the
endpoint *"resolves an explicit `rating_version_ref` (FR-RATE-35 — `prod` restricts to
`approved` versions, records as `what_if`)"*. Ruling 14 clause 3: both restrictions sit inside
FR-RATE-35's own `prod` clause, W11 has no environments, so **neither applies**. Do not restrict
to `approved` and do not rewrite `purpose`.

**C4 — one of Ruling 16's two acceptance items is withdrawn**, per register row F32 and the
*Correction to Ruling 16* in
[`2026-08-29-w11-algorithm-pin-maturity.md`](2026-08-29-w11-algorithm-pin-maturity.md). The
`load_bundle` purity test shipped with Task 1.3. Item 2, the degraded read, stands and is
acceptance criterion 4 above.

**C5 — this plan's own predecessor was wrong about NFR-RATE-9, and the correction is load-bearing.**
[`2026-08-29-w11-slices-2-4-planning-readiness.md`](2026-08-29-w11-slices-2-4-planning-readiness.md)
argued option (b) *"satisfies NFR-RATE-9 by construction"*. Ruling 16 clause 5 refutes it: a slot
...
inside it ([`../audit/plan-reviews.md`](../audit/plan-reviews.md) review 8 Q4 found that
mechanism twice, and it has since fired a third time).

| Requirement | Where in `03` | Discharged by | How it is proven |
|---|---|---|---|
| FR-RATE-34 | §3.7 | 2B | 200 with a reconciling ladder over HTTP. **The default-live half is W14's** (Ruling 14) and owes a register row at the close |
| FR-RATE-35 | §3.7 | 2B | An explicit ref scores a `draft` version — the *"what-if and testing"* the requirement permits. Neither `prod` restriction is imposed (C3) |
| FR-RATE-38 | §3.7 | 2B | The error boundary: one test per code, each fired by a deliberately malformed input. **The per-quote *typing* was Slice 1's; the HTTP *mapping* is this slice's** |
| FR-RATE-39 | §3.7 | 2B | A decline is **200** with a populated ladder, never an HTTP error. Slice 1 proved it in-process; this proves it survives the boundary |
| FR-RATE-40 | §3.8 | 2C | A blank `change_summary` is refused **422**, from a test carrying `FR-RATE-40` |
| NFR-RATE-1 | §9 | 2D | p99 at the full HTTP path, plus the sustained-200 rps test [`../roadmap.md`](../roadmap.md) names separately |
| NFR-RATE-9 | §9 | 2A, 2B | The degraded read (acceptance 4). **The 99.95 % availability target is not discharged here** — Ruling 16 clause 5 — and owes a register row |
| NFR-RATE-11 | §9 | 2B | The three RBAC cases, plus a rate-limit test |
| NFR-RATE-13 | §9 | 2B, 2D | The malformed-`ScoringResult`-returned-verbatim test, and the re-measurement on the real path |

**Deliberately excluded, each with its owner:** FR-RATE-36, 37 → Slice 3. FR-RATE-38's batch
half → Slice 3. FR-RATE-42 → Slice 4. FR-RATE-43, 44, 45 → W12. FR-RATE-46–49 → W13. FR-RATE-50,
FR-PLAT-28 → W14. NFR-RATE-5 → Slice 3. NFR-RATE-12 → Slice 4.
...
| 2C — FR-RATE-40 test | — | — | Independent of everything; may run first or in parallel |
| 2D — measurement | 2B | **Task 1.5, in flight** | See below |

**2D's scheduling constraint is real and is not a dependency on 2B alone.** Ruling 6 rules that
...
unlike `orjson`, which Ruling 17 refuses.

**`PlatformError.__init__` raises `ValueError` on an unregistered code**, so 2A's registration
step physically cannot be skipped — which is the mechanism Ruling 14 relies on to stop itself
being half-applied.
...
**What this builds.** The error code Ruling 14 requires, and the per-worker in-process slot
Ruling 16 rules — *"the first in-process cache in this backend"*, which is why it must be
...
- Modify: `backend/src/app/config.py` — one typed setting, capacity as a **count**, default
  **1** (Ruling 16 clause 3). Follow the neighbouring fields' form; `env_prefix` is `GIP_`.
...
- `BundleSlot` — holds hydrated `CompiledBundle` objects, keyed by the source `Bundle`'s
  ...
  served, the hash it resolved to (Ruling 16 clause 5 — without it the degraded read cannot be
  reached, because ref → hash is itself a metadata read).
- Least-recently-used eviction, which at capacity 1 is replacement.
- **No refresh, no poll, no pub/sub, no environment pointer** — all four are W14's (Ruling 16
...
**Must NOT touch.** `load_bundle` — Ruling 10's purity property is already tested in the tree
...
1. Authenticate; the `Caller` carries `environments: frozenset[str]` and
   ...
   `Permission.SCORE_EXECUTE`. **Grant nothing** — Ruling 18: *"Task 2.1 grants nothing; it
   checks."*
...
7. Serialise the `ScoringResult` directly into a raw `Response`. **No `response_model=`, no
   Pydantic-model return annotation** (Ruling 17).
...
- [ ] **Step 3: Write the 409 test.** Post the same body with `"options": {"rating_version_ref":
      ...
      `NO_LIVE_RATING_VERSION`. Mark `@pytest.mark.req("FR-RATE-34")`.
...
- [ ] **Step 5: Write the outbound-validation test** (Ruling 17's acceptance test). Construct a
...
- [ ] **Step 5a: Write the four error-mapping tests, one per code.** `pricing-core` raises a
      ...
      **that** code. Mark `@pytest.mark.req("FR-RATE-38")`.
...
- [ ] **Step 5b: Write the decline test.** A constraint decline must be **200** with
      ...
      FR-RATE-39, never an HTTP error. Mark `@pytest.mark.req("FR-RATE-39")`.
...
- [ ] **Step 6: Write the three RBAC tests** (Ruling 18): scoped account with the permission
      ...
      (`backend/src/app/auth/service.py:212`). Mark `@pytest.mark.req("NFR-RATE-11")`.
- [ ] **Step 7: Write the degraded-read test** (Ruling 16 acceptance item 2). Patch the
...
- [ ] **Step 11: Commit.** `feat(rating): POST /api/v1/score on the shared evaluator (W11 Task 2B)`
...
## Task 2C — the one test FR-RATE-40 owes

**What this builds: one test, and no production code.** Ruling 15's words. One of FR-RATE-40's
four limbs — a change summary (FR-RATE-27) — is *already enforced on `main`*:
`backend/src/app/platform/approvals.py:170-177` raises
`PlatformError("VALIDATION_FAILED", "A change summary is required", 422, ...)` citing FR-GOV-10,
...
**attribution**: no test names FR-RATE-40, so `req-coverage.py` cannot see the control.
...
- [ ] **Step 1: Write the test.** Submit a rating version whose `change_summary` is `"   "`;
      assert **422**. Mark `@pytest.mark.req("FR-RATE-40")`.
...
- [ ] **Step 3: Run `uv run python scripts/req-coverage.py`** and confirm FR-RATE-40 now carries
      a marker.
- [ ] **Step 4: Commit.** `test(rating): attribute FR-RATE-40's change-summary limb (W11 Task 2C)`

**Must NOT touch.** `rating_versions.submit_for_review`. **Ruling 15's override condition is
explicit: if any W11 commit makes it call `policy.effective_evidence("rating_version")`, the
...
**Files**
- Reuse: Task 1.5's harness in `scripts/`. Extend rather than duplicate — Ruling 6 rules the
...
**Acceptance**
- **NFR-RATE-1 at the full HTTP path** — p99 against the **< 50 ms** bound for a ~200-step motor
...
- **NFR-RATE-13 re-measured on the real path**, against W8's synthetic p99 0.070 ms.
- **NFR-RATE-11's rate limit** exercised.
...
- NFR-RATE-8 is Slice 1's and is already proven over generated contexts. Adding a runtime raise
...
rather than in prose — FR-RATE-34's default-live half and NFR-RATE-9's availability target are
W14's, and each owes a register row the lead writes at the close.
...
Every design question this slice faced is ruled — DP1 by Ruling 14, DP2 by Ruling 15, D4 by
Ruling 16, F1 by Ruling 17, M4 by Ruling 18. The one open item this plan found is not Slice 2's:
...
NFR-RATE-9 claim. The readiness document reasoned that the slot satisfied it "by construction";
Ruling 16 clause 5 showed the reasoning skipped a metadata read, and the fix is a second index
