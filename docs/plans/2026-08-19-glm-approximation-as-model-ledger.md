# SDD ledger — plan: .planning/2026-08-19-glm-approximation-as-model.md

Spec: `docs/specs/02-modelling.md` — FR-MODEL-96 (§3.6), §4.4, §4.8, §4.9, §5.1, §5.2. Read, reachable.
...
|---|---|---|
| T1 → T5 | `MODEL_APPROXIMATION_INVALID` declared in `02` §5.1, registered in `backend/src/app/errors.py` | **DEFECT — see Ruling 1.** `backend/tests/test_errors.py::test_spec_error_codes_are_all_constructible` reads the spec's code list, so T1's commit reddens it and it stays red until T5. |
| T1 → T2..T5 | FR-MODEL-102, the id every later `@pytest.mark.req` names | Consistent. `req-coverage.py` fails on a marker naming a requirement that does not exist, so T1 must land first — it does. |
...
**FR-MODEL-96's own row** (`02` §3.6, line 172) still ends *"Until it is built,
...
makes the requirement's *text* permanent, not immune to amendment — FR-MODEL-50 and FR-MODEL-57 both
...
amendment to FR-MODEL-96's row; `audit-docs` exit 0 with the count **unchanged at 483** — an amendment
...
OQ-MODEL-8, OQ-GOV-7, FR-MODEL-87, FR-MODEL-88 and two notes, plus OQ-MODEL-10's own DECIDED summary
in `open-questions.md` and its `02` §10 mirror — the last two left under the convention OQ-MODEL-5
...
ruled points stated; `FR-MODEL-102` still bolded exactly once; `audit-docs` exit 0 at 483.

Task 1: **the spot-check earned its keep — Ruling: the OQ-MODEL-5 precedent does not cover
OQ-MODEL-10's two summaries, and both get a built clause.** OQ-MODEL-5's summary states only decision
*content*, so it cannot go stale; both of OQ-MODEL-10's assert "owned by Phase 1b" *inside the
...
invited to argue back if its reading of OQ-MODEL-5 survives a re-read. *Cost if wrong:* one redundant
clause in two decision records.

Task 1: fix round 2/5 (1 addressed pending re-review, 0 open; commit `e73256c`..`267ca0b`). Identical
`**Built 2026-08-19 (W5).**` clause appended beside the "owned by Phase 1b" claim in both rows,
...
literal `"__gbm_prediction__"` response column and no source model, which FR-MODEL-102's iff-validator
...
ruff 0, `lint-imports` 0 (ADR-0001 intact), model-schema 172 passed as the regression check. `mypy` exits
...
throughout (`CLAUDE.md` §13.4). Out of this slice's scope to fix: FR-MODEL-96 is about the approximating
...
Task 6: two open questions raised for the maintainer — **OQ-OVR-9** (nothing cross-checks a spec's
declared error codes against the `errors.py` registry; structural, every module) and **OQ-PLAT-7**
...
and `07-platform.md` §10. Roadmap: W5 "twenty" → "twenty-one" slices plus the FR-MODEL-96/102 entry; 15
...
`except PlatformError`, and `audit-docs.py`'s error-code check never opens `errors.py`. It **counted W5's
...
actually about `OQ-MODEL-10`/`approximating_model_id` all sit in dated slice records or struck-through
...
branch is unpushed, so an amend costs nothing and nothing was force-pushed). Caveat now in W5's row beside
...
Task 6: re-review round 1 — **ADDRESSED**, all five checks pass. Caveat inline in W5's row beside the
...
`approximation_spec`, **OQ-OVR-9** and **OQ-PLAT-7**.
...
contracts agree with what §4.4/§4.9 *require*, FR-MODEL-102 defined exactly once and nothing renumbered.
...
Final fix wave: commits **`e66e179`** (legacy-compatibility positive tests) and **`5c4927f`** (FR-MODEL-102's
slug convention, the three-vs-six field note, **OQ-MODEL-17**). Gate green: **pytest 1362 passed, zero
