...
**Slice:** `W32-7` · **Plan:**
[`2026-08-23-w32-7-workspace-identity-and-selection.md`](2026-08-23-w32-7-workspace-identity-and-selection.md),
...
FR-PLAT-63, FR-PLAT-65, and FR-PLAT-48 by collision (§4).
...
| 2 | branch commit 2 | `GET /api/v1/me` returns every membership with FR-PLAT-62's name, ordered by name. |
| 3 + 4 | branch commit 3 | The verified `Workspace-Id` header and `_select_workspace`; `platform.workspace_switch.record_switch`; OQ-PLAT-12 and OQ-PLAT-14; the injected-`422` strip. |
...
## 2. The four obligations of FR-PLAT-63, with verdicts

`CLAUDE.md` §13 admits four verdicts and silence is not one of them.

| # | Obligation (FR-PLAT-63) | Verdict | Evidence |
|---|---|---|---|
...
| 4 | **A switch is audited into both chains**; the first selection after login writes one event. | **Deferred with an owner — `W6b-11`** | Mechanism delivered and tested (`test_a_switch_is_recorded_in_both_chains`, `test_the_first_selection_after_login_writes_one_event`, plus §3's breakage proof). **The call site is not built**, and OQ-PLAT-12 says why. |
...
turning the FR-GOV-24 chain into a request log and burying the record of what was actually
done. OQ-PLAT-12 records all three options and recommends storing the previous selection,
reached through an explicit endpoint. **A green suite here must not be read as this obligation
being met.**

### FR-PLAT-65's own claims
...
## 4. FR-PLAT-48: declaring one header re-opened a defect a module exists to prevent
...
2. **Task 5 Step 1 recommends adding `workspace` to `ARTIFACT_TYPES`** so the switch event's
   ...
   appear in `entity_ref`". **OQ-PLAT-14** records the measurement and recommends declaring what
...
### FR-PLAT-65 in the generated client, checked rather than assumed
...
|---|---|---|
| **OQ-PLAT-12** | What tells the API that a workspace selection *changed*? | Store the previous selection, reached through an explicit endpoint; **refuse** auditing every selection. Consumed by `W6b-11`. |
| **OQ-PLAT-14** | 20 of 39 `entity_ref` spellings do not parse as an `ArtifactRef`, and nothing notices. | Declare and validate that `entity_ref` names the *subject* of the event; **never** widen `ARTIFACT_TYPES` to make an existing string parse. |
