# W6b — the revised slice map

> **This supersedes [`2026-08-22-w6b-slice-map.md`](2026-08-22-w6b-slice-map.md) for W6b, and
...
> was believed on 2026-08-22; this is the record of what is believed on 2026-08-24, after W32
> closed and after all eleven of its specification-backlog items were discharged.

**Written:** 2026-08-24, immediately after W32 closed (`e2ae7c6`, PR #165).

**Every `:NNNN` citation on this page is against `e2ae7c6`.** A line number without a tree is
not a locator: two sessions cited the W32 scope row correctly on 2026-08-24 and read each
...
W6b. W32's half of the 2026-08-22 map is closed and is not restated here.
...
**W32 closed, so W6b's dependency graph is now entirely internal.** All twelve W32 slices
merged. Every `Depends on W32-N` column in the frozen map is satisfied, and no W6b slice waits
on anything outside W6b.
...
W6b at the moment they cleared it.** A cleared blocker is not automatically a smaller slice.

**The frozen map's gate rows have aged, one of them dangerously.** Its **line 192** tells
`W6b-11` it *"waits only on W32 building the header half"*. W32 built it, and left a
...
`W6b-N` ids are cited **outside this directory**: 39 citations of six ids across
...
55 further citations of W6b slice ids inside [`this directory`](.) itself, in filed plans that
...
alternative silently dangles 55 frozen ones. It is also the form W32 already used for `W32-1b`.
...
true negative. This is the same defect the W32 closure gate was built around — `[a-z]?` is
...
set-level override. **`FR-DATA-54` answered: a new rule version, and a Rule Set entry "gains
...
the mechanism at `:564`: the W32 boundary was drawn by *subject matter*, but each later slice
...
**`W6b-5`'s owner clause is description-anchored, not id-anchored.** `02:232`'s `FR-MODEL-128`
reads *"Owner: the slice that builds the factor workbench's suggestion panel"* and contains
**zero** occurrences of `W6b`. It therefore re-resolves onto whatever slice builds that panel,
...
**Two cleared blockers deposited new W6b build scope.** `FR-PLAT-58` (`07:81`) ends *"Owned by
**W6b**"* — a local OIDC provider behind a compose profile with a checked-in realm, which is
not a Vue view and had no slice. `FR-DATA-54`'s tail declares a consequence *"unbuilt, owner
...
| **W6b-3** | **Dataset list Contents** — status badge, last validated, owner. `FR-DATA-50` landed **three** derived fields, not two: `last_validated_version` travels with `last_validated_at` and a validator refuses either alone | — | — |
| **W6b-4** | **Model spec builder** `/models/new` | W6b-1a | — |
| **W6b-5** | **Factor workbench remainder** — intent controls, interaction suggestions, inline one-ways. Carries `FR-MODEL-128`, which spans this view *and* the TreeSHAP holdout pass it needs | — | — |
...
| **W6b-8** | **Peril structure view** — and it carries **no usage count**: `FR-MODEL-127` defines that quantity as Model Specs referencing the artifact, and for a Peril Structure the reference runs the other way | — | — |
| **W6b-9** | **Tabular chart fallback** (`NFR-OVR-10`) | — | — |
| **W6b-10** | **Browser authentication** (`FR-PLAT-55`) — the PKCE flow in the SPA | W6b-14 | — |
| **W6b-11** | **Workspace selector, the shell** — `FR-PLAT-62`, `FR-PLAT-63` obligations 1–3 and `FR-PLAT-65` are delivered; the selector consumes them | W6b-10 | — |
| **W6b-11b** | **The switch audit** — `FR-PLAT-63` obligation 4 | W6b-11 | **OQ-PLAT-12**, undecided |
...
| **W6b-13b** | **The catalogue chain** — `FR-DATA-53`'s dropped `catalogue_id` and `FR-DATA-54`'s default thresholds in the catalogue entry, replacing the seed's empty `params` for all 38 rules | — | — |
| **W6b-14** | **The local OIDC provider** (`FR-PLAT-58`) — a compose profile and a checked-in realm | — | — |
...
- **`W6b-13`/`W6b-13b`** — a reviewer can accept the catalogue and seed change while rejecting
  the screen. The dependency runs one way and is real: `FR-DATA-54` says the seed's empty
...
- **`W6b-14`** — different artifact set, no Vue at all. A reviewer can accept the provider while
  rejecting the PKCE flow. `W6b-10` depends on it: `FR-PLAT-1` claims a local provider ships
...
`NFR-OVR-10` attaches a tabular equivalent to *each chart*: double-lift is not the same chart
...
roadmap's dated block *"2026-08-23 — the W6b slice map's specification backlog is resolved"*
carries the detail. Four became requirements (`07` `FR-PLAT-58`, `FR-PLAT-62`, `FR-PLAT-63`;
`01` `FR-DATA-54`; `02` `FR-MODEL-127`), four were the spec being wrong and are corrected on
the spec's side, two were shapes that escaped the contract, and three new questions came out of
the work (`OQ-MODEL-31`, `OQ-PLAT-8`, `OQ-PLAT-9`, all since decided).

**No W6b view gets a requirement, and this is settled rather than open.** `02` §5.3's amendment
...
Highest ids in use: FR-DATA-54, NFR-DATA-10. Next free: `FR-DATA-55`, `NFR-DATA-11`.
Highest ids in use: FR-MODEL-128, NFR-MODEL-14. Next free: `FR-MODEL-129`, `NFR-MODEL-15`.
Highest ids in use: FR-PLAT-65, NFR-PLAT-11. Next free: `FR-PLAT-66`, `NFR-PLAT-12`.
...
**Frontend requirement traceability still does not exist**, and W6b is the workstream it will
...
**`FR-PLAT-63`'s eventual test must assert the rule, not the symptom.** The obligation is *"a
...
convention adopted at W32's close, and `W6b-11b` is the slice it was written for.

**`NFR-MODEL-14` is booked forward as *delivered but untested*** and is not W6b's, but whoever
touches the partial-dependence path should know the budget rests on a W5 bench from 2026-08-22
