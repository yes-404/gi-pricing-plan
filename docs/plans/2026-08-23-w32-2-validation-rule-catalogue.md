...
- [`../specs/01-data-management.md`](../specs/01-data-management.md) — §4.4's four rule tables
  (lines 335–434) are the source of truth for the catalogue's contents; FR-DATA-15…24 govern
  rules and rule sets; FR-DATA-21 governs a rule's review; §4.5 (line 435) is the rule
  lifecycle; §5.1's endpoint table gains a row.
- [`../roadmap.md`](../roadmap.md) — line 1028, where `--catalogue VR` is recorded at 1 of 38.
- [`2026-08-22-w6b-slice-map.md`](2026-08-22-w6b-slice-map.md) — this slice's row, and line
  114 where W6b-13 is recorded as depending on it.
- [`../adr/0002-model-schema-single-source-of-truth.md`](../adr/0002-model-schema-single-source-of-truth.md)
...
- **`model-schema` is the single source of truth** (§2, ADR-0002). Nobody hand-writes a shape
...
- **`pricing-core` imports no FastAPI, SQLAlchemy or Redis** (ADR-0001). Task 1's second test
...
- **Requirement IDs are permanent** (§5): append, never renumber. This slice appends one
  requirement to `01` §3. Highest ids in use: FR-DATA-52, NFR-DATA-10.
  Next free: `FR-DATA-53` — and this plan takes it. No other plan of this date touches the DATA range.
...
BUILTIN_RULES: Final[Mapping[str, BuiltinRule]] = MappingProxyType(
    {
        ...
        for r in (
            ...
            _rule("VR-STR-9", "reject-rate", "reject_rate", _F, "Quarantined rows <= threshold (default 0.1 % of rows read) - FR-DATA-7"),
            _rule("VR-REF-1", "reference-resolve", "reference_lookup", _F, "Every value of a reference-backed column resolves in the pinned Reference Table Version, evaluated as at the declared date column (FR-DATA-31)"),
            ...
            _rule("VR-ACT-5", "claim-date-in-exposure", "claim_date_in_exposure", _F, "date_of_loss is in [exposure_start, exposure_end) for the linked row (FR-DATA-12)"),
...
place. Generating the file instead removes the class of defect, and is what FR-PLAT-48 exists
for.
...
1. **No requirement said built-in rules exist.** FR-DATA-15…24 govern rules and rule sets;
...
4. **The five DATA requirements with no evidence are unchanged by this slice.**
   `scope-audit.py DATA` reports FR-DATA-50, FR-DATA-51, FR-DATA-52, NFR-DATA-1 and
   NFR-DATA-2 without evidence. The first two are W32-3's; the last two are performance
...
```bash
grep -rn 'pytest.mark.req("FR-DATA-16")' packages/model-schema/tests/test_validation_catalogue.py packages/pricing-core/tests/test_builtin_rule_checks.py backend/tests/test_api_validation_rules.py
```

Leave any marker that genuinely belongs to FR-DATA-16 — none of these do; every one was
...
FR-DATA-50, FR-DATA-51, FR-DATA-52, NFR-DATA-1 and NFR-DATA-2 remain without evidence and are
