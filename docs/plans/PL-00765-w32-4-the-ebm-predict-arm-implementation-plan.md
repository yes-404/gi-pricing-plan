...
family: plan
kind: leaf
title: W32-4 — The EBM predict arm Implementation Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-23-w32-4-ebm-predict-arm.md
---
...
**Spec:**
- [`../specs/02-modelling.md`](../specs/02-modelling.md) — FR-140 (EBM is transparent
  by construction), FR-193 (the predict endpoint), FR-198 and FR-195
  (the typed-absence vocabulary this slice extends), FR-200 (what its members mean),
  §5.2's `predict.py` block at lines 2185–2205.
- [`../roadmap.md`](../roadmap.md) — lines 704–713 and 3311, where the refusal was recorded
  when it was written, and where ownership is still attributed to WK-664.
- [`PL-00753-wk-664-and-wk-692-the-slice-map.md`](PL-00753-wk-664-and-wk-692-the-slice-map.md) — where this slice sits.
- [`../adrs/`](../adrs/) — ADR-703 (`pricing-core` stays dependency-free, which is why the
  blob fetch lives in the backend) and ADR-705 (declarative artifacts, which is why an
...
- **`model-schema` is the single source of truth for shared shapes** (§2, ADR-704).
...
- **Requirement IDs are permanent** (§5): append, never renumber. This slice appends exactly
  one requirement to `02` §3. Highest ids in use: FR-179, NFR-488.
  Next free: `FR-180` — and this plan takes it; the W32-5 plan takes `FR-181`.
...
- **`pricing-core` imports no FastAPI, SQLAlchemy or Redis** (ADR-703). Nothing in this
...
| `docs/roadmap.md` | Modify `:711`, `:3311`, `:3331`, `:3340` | Ownership reconciliation — the roadmap has no WK-692 row and still names WK-664 |
...
specific and each is false of an EBM. `no_interval_models_fitted` (FR-198) says a
...
(FR-200) both presuppose that pair exists. `covariance_not_stored` (FR-195) says
...
```python
    #: An EBM. Not "no pair was fitted" (FR-198) — `interval_for` lives on `GbmSpec`
...
identity link, no link inversion, no `model_offset` (FR-116 refuses an EBM offset ref at
...
from the `@pytest.mark.req("FR-140")` line down to the
...
1. **No requirement said an EBM could be predicted, and none said it could not.** Across `02`'s
   EBM mentions, exactly two touch prediction: FR-116's refusal of an EBM offset ref,
...
3. **The roadmap names WK-664 as this arm's owner in four places and has no WK-692 row at all.**
   `grep -n "WK-692" docs/roadmap.md` returns nothing; the highest id is WK-691. The slice map
   ([`PL-00753-wk-664-and-wk-692-the-slice-map.md`](PL-00753-wk-664-and-wk-692-the-slice-map.md) §6) records that the split
   itself is accepted while the slice boundaries and sequencing are still pending, so this
   task reconciles the *ownership attribution* only and does **not** add a WK-692 workstream
...
EBM predict arm to WK-664 — replace the attribution with **W32-4** and append to each:
...
carries both `FR-140` and the new id. `pytest` runs with `--strict-markers` and
...
FR-140 is the only existing requirement the arm is written against and it is cited by
