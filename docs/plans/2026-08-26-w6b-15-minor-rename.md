...
**Goal:** Drop the `_minor` suffix from the five statistics the decision OQ-OVR-12 (b)
released — the two reconciliation burning-cost fields and the three severity fields — keep
every type as it stands, and regenerate every artifact the published names reach (FR-OVR-20).
...
follow from the source of truth (FR-PLAT-48). The severity trio lives in `pricing-core`'s
...
**Spec:** FR-OVR-20 ([`../specs/00-overview.md`](../specs/00-overview.md) `:226`), FR-OVR-7
(`:213`), OQ-OVR-11 (`:541`) and OQ-OVR-12 (`:542`) in the same file, their mirror rows in
...
rename serves is FR-OVR-20. The type rule is FR-OVR-7. The releases are the two OQ rows.

**Slice source:** [`2026-08-26-w6b-slice-map-revised-2.md`](2026-08-26-w6b-slice-map-revised-2.md)
§3, line 162 — *"The `_minor` rename — `OQ-OVR-12` decided (b): statistics mislabelled
`_minor` drop the suffix under `FR-OVR-20`. The integer type stays. ... The plan sweeps the
class."* Lines 172, 185 and 195 confirm the plan can land at any time and must sweep the
class.

**Highest ids in use, published at the revised-2 anchor (`b989097`):** FR-DATA-56,
NFR-DATA-10, OQ-DATA-15. Next free: `FR-DATA-57`, `NFR-DATA-11`, `OQ-DATA-16`.
This plan mints none of them. The rename is released mechanical work. It cites
requirements that already exist.

## Global Constraints

- **FR-OVR-20 reserves `_minor` for integer minor units.** The suffix is the defect, not
  the type. The five statistics drop the suffix and keep their types.
- **The integer type stays on the burning-cost pair.** FR-MODEL-60's sum-of-rounded-parts
...
- **The severity trio keeps its floats.** Bounds on, or measures of, a statistic are
  statistics (FR-OVR-7). The float is permitted, not a defect.
- **The fix inverts between the two groups.** `threshold_minor` and `largest_minor`
  denominate money. Their name is correct and their type is the defect (OQ-OVR-11). W6b-15
...
- **Nobody hand-writes a published shape.** The contracts regenerate from `model-schema`
  (FR-PLAT-48). The frontend types regenerate from the committed OpenAPI. A hand edit
...
**Retype — money, name correct, type the defect (released by OQ-OVR-11, NOT built here,
...
The OQ-OVR-12 register text names *"the two perils test suites, and the generated
...
The field it describes is a statistic. OQ-OVR-11 records the string as a surface neither
...
OQ-OVR-11's decision releases both corrections. W6b-15's slice row names only the rename
...
OQ-OVR-11, OQ-OVR-12 and FR-OVR-20 quote the pre-rename names as their record. After the
...
**Interfaces:**
- Consumes: FR-OVR-20's rule and OQ-OVR-12's decision (b).
...
**Interfaces:**
- Consumes: OQ-OVR-11's classification — the trio are bounds on, or measures of, a
  statistic.
...
- Modify: `docs/specs/00-overview.md:213` (FR-OVR-7), `:226` (FR-OVR-20), `:541` (OQ-OVR-11), `:542` (OQ-OVR-12)
- Modify: `docs/open-questions.md:34` (OQ-OVR-11), `:35` (OQ-OVR-12)
...
- [ ] **Step 2: Append a dated note to FR-OVR-20**

In `docs/specs/00-overview.md:226`, append a note to the FR-OVR-20 row:
...
`modelled_burning_cost`. The OQ-OVR-11 and OQ-OVR-12 register rows keep the pre-rename
names as their record.*

Do not rewrite the row's earlier clauses.

- [ ] **Step 3: Append a one-line pointer to FR-OVR-7**

In `docs/specs/00-overview.md:213`, append a note to the FR-OVR-7 row:

*Renamed 2026-08-26 (W6b-15): the `validate.py` names cited above are pre-rename names.
FR-OVR-20's row carries the rename note.*

- [ ] **Step 4: Append dated notes to both OQ-OVR-11 rows**
...
- [ ] **Step 5: Append dated notes to both OQ-OVR-12 rows**
...
rows and FR-OVR-20, amended in Task 6), the filed plans named in Finding 6, and this
...
**1. Spec coverage.** FR-OVR-20's rule and OQ-OVR-12's decision (b) → Tasks 1-3 and
6. FR-OVR-7's type rule → the types stay, stated per task and enforced by the unchanged
type annotations. OQ-OVR-11's two-group classification → Finding 1 and Task 3's explicit
exclusion of the retype pair. FR-MODEL-60's rounded-parts invariant → Task 1 Step 3 keeps
