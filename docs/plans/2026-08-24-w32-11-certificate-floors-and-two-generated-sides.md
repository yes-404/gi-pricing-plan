...
**Written 2026-08-24.** The slice [`2026-08-23-w32-closure-proposal.md`](2026-08-23-w32-closure-proposal.md)
...
slice before closure** (maintainer instruction, 2026-08-24): W32 does not close until it lands, and
does not stay open for anything discovered inside it — see Task 5.

## Goal

Give `ObjectiveCertificate` and `MetricCertificate` the check-count floors FR-MODEL-126 requires,
...
- **OQ-MODEL-30 is decided: option (a)**, on the maintainer's instruction, 2026-08-23. Leave
...
- **OQ-MODEL-30 is directive about atomicity**: the carve-out
...
than a §0 finding that FR-MODEL-126 asks for a shape the pipeline cannot express.
...
holds W32 open**; none is silently absorbed either.
...
Evidences FR-MODEL-126, and FR-MODEL-69 and FR-MODEL-105 for the two batteries.
...
1. Delete `min_length=1` from `CertificateResult.checks`
   ...
   FR-MODEL-126 requires. **Do not** add a floor to it, and do not subclass it.
...
3. Add the equivalent to `MetricCertificate` (`metrics.py`) for the **four** of FR-MODEL-105.
...
Evidences FR-PLAT-48, and closes the first half of Part C's third row.
...
4. **Every divergence is a `CLAUDE.md` §0 verdict, one at a time**: which side is wrong, with a
   ...
   W32-2 precedent ([ledger](2026-08-23-w32-2-validation-rule-catalogue-ledger.md)) resolved all
...
4. **File F2 and F3 as rows in [`../open-questions.md`](../open-questions.md) before the roadmap
   edit**, and give each row two things the closure record needs: **which W32 goal it bears on**,
   ...
   has been believed.
5. Append the slice record to [`../roadmap.md`](../roadmap.md) in the shape the other W32 records use.
...
- Task 1 is one commit and may not be split (OQ-MODEL-30). Tasks 2 and 3 are a commit each.

## Task 5 — close W32 — *not this slice's, as of 2026-08-24*
...
session holding the W32 docs the same day, and lands as **a second PR after W32-11 merges** — not
inside this slice. The reasoning is that session's and it is the right reasoning: it holds the W32
...
**A finding this slice cannot resolve does not hold W32 open.** It is booked forward — into a named
...
| # | Disposition | Why it does not hold W32 open |
|---|---|---|
| F1 | **Delivered.** FR-MODEL-126 asks for the floor to be enforced where each certificate is constructed; it never asked the generated client to carry it. Recorded as a stated limit — enforcement is server-side — not as a gap | The requirement is satisfied as written. The limit is published, not implied |
| F2 | **Booked forward** as an open question with an owner. It is the `gbm.quantile_crossing` shape — the gap that let a missing keyword sit green for months, which is the argument W32-10 was built on — and it is a guard-design change, not certificate work | Discovered inside this slice, outside every requirement W32 owns. Fixing it here would be new capability in a closing slice |
| F3 | **Booked forward.** `metric-certificate` has no authored contract at all, so giving it one is a new contract, not a comparison of an existing pair. Out of the scope written for this slice | Task 1 still enforces its four-check floor model-side, which is what FR-MODEL-105 asks for. Only the *comparison* is deferred |
...
requirement W32 owns, and neither is a §0 disagreement — both are reach, not correctness.
