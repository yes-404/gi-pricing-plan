# W5 Audit Remediation — execution ledger

Running record for `.planning/2026-08-22-w5-audit-remediation.md`. Branch
...
|---|---|---|
| **OQ-PLAT-7** — does `execute_job` preserve a handler's `PlatformError.code`? | **(a)** — a dedicated `except PlatformError` clause storing `JobError(code=exc.code, message=exc.detail or exc.title)`, `retryable` unchanged | Maintainer, 2026-08-22, accepting the recommendation already on file |
| **FR-MODEL-6** — reassignment to W30 | **Accepted**, dated 2026-08-22, recorded at `roadmap.md:2589` and `:2985` | Maintainer, 2026-08-22 |
| **FR-MODEL-80** — build Bühlmann–Straub or supersede it? | **Build.** The question dissolved on inspection: OQ-MODEL-5 was *already decided* 2026-08-15 — "both … so W5 builds two methods rather than choosing one" (`roadmap.md:112`). Superseding would mean overturning a recorded maintainer decision, which §14 does not permit an executing agent to do | Pre-existing decision, verified |
| **NFR-MODEL-1/2/10** — build a 5M-row fixture? | **No.** Measure at stated smaller scales and write the extrapolation down, which the plan explicitly permits. A 5M × 60 fixture is hours of compute for an extrapolation that a growth curve gives more honestly | Plan, §3d |
...
lineage. Resolved by tracing to `00` **FR-OVR-1** (*"every Artifact is immutable once it
leaves `draft`"*), which is the requirement R2 instantiates, alongside **FR-MODEL-65**,
...
### F4 — `FR-GOV-37`'s `peril_structure` justification rested on a false premise *(slice 1c, not in the plan)*

FR-GOV-37 and `EVIDENCE_FLOOR`'s docstring both justified `peril_structure`'s empty floor
...
cannot be queried — `model_comparison_if_predecessor`'s case exactly. **Owner: W17**
(FR-GOV-9..19, evidence enforcement).

### F5 — FR-OVR-18's audit was structurally unable to see this hole *(slice 5c)*
...
lossy form, which FR-OVR-7 forbids. That number decides a reconciliation
(`|ratio - 1| <= tolerance`).

**Why 2026-08-19 missed it:** OQ-OVR-8's audit swept fields that *were* `DecimalStr`. A field
...
**125** — the slice's own FR-MODEL-113, appended an hour earlier. Re-derived with
...
| 1b — FR-GOV-36 reference resolution | **done** — fan-out in the route (no registry), 6 of 20 types resolve, unresolvable fails closed with the new `06`-owned `ARTIFACT_TYPE_NOT_RESOLVABLE`; **5 FR-GOV-36 markers where there were 0**; enforcement proven (resolver removed → 8 failures). W17 owns the durable fix |
| 1c — `custom_metric` evidence floor | **done** — `06` §3.3 row, `EVIDENCE_FLOOR` entry, three negative tests, `metrics.py` docstring, FR-GOV-37 amended |
| 2 — FR-MODEL-23 + OQ-PLAT-7 | **done** — layer 1 `GLM_FIT_FAILED`; layer 2 the `except PlatformError` clause, marked **FR-PLAT-11** (platform job machinery, not FR-MODEL-23). OQ-PLAT-7 closed in both `open-questions.md` and `07`'s mirror |
...
| 6 — bookkeeping + closure record | 6a: **8 of 9 items applied** (count, counter, six verdict rows, FR-GOV-10, AST parser, requirement count, phase-review skill, custom-metrics entry). Outstanding: the two missing slice records for PRs #124/#125, drafted and verified but not yet inserted. FR-MODEL-6 acceptance **done** |
...
- Record OQ-PLAT-7's decision in `docs/open-questions.md` **after** layer 2 lands, so the row
...
mechanism by which `gbm.quantile_crossing` (FR-MODEL-78) and `gbm.tree_count` sat absent for
...
FR-MODEL-87 listed it declared-and-unbuilt, **owned by W5** — so W5 could not close while
owing it. FR-MODEL-96 settled the direction on 2026-08-19 (artifact → model), but the
...
The wrongly-marked error-code test could plausibly have taken `FR-OVR-19`, which is about
exactly this reconciliation. The agent rejected it: FR-OVR-19 specifies an `audit-docs.py`
...
It took `FR-PLAT-47` instead, which the sibling test already carries. This is the §13
...
2026-08-21, owner W5" that had **never been written**. Re-run today:

- **The one outstanding claim of that shape is the already-corrected one** at
  `roadmap.md:3012`. No other "dated note … owner W5" claim survives unverified.
- **I then ran it against the two slice records I had just written myself**, which claimed
  `02`'s FR-MODEL-22 amendment records *which side was wrong and why* about the
...
`scope-audit.py` reported **FR-MODEL-113 unevidenced** — the requirement I appended myself in
...
cases) carried only `FR-MODEL-80`.

That is **the same marker-misattribution defect slice 4 had just fixed**: the Bühlmann–Straub
tests were marked `FR-MODEL-14` while asserting FR-MODEL-80's content, so `scope-audit.py`
