# W11 Slices 2–4 — planning readiness, the signals that release each, and what a leaf plan can already take from here (2026-08-29)

**What this is.** [`2026-08-29-w11-scoring.md`](2026-08-29-w11-scoring.md) cuts W11 into four
slices. Slice 1 has a leaf plan
([`2026-08-29-w11-1-evaluator-core.md`](2026-08-29-w11-1-evaluator-core.md)); Slices 2, 3 and
...
[`../open-questions.md`](../open-questions.md):** FR-RATE-65, NFR-RATE-14, OQ-RATE-6.
Next free: `FR-RATE-66`, `NFR-RATE-15`, `OQ-RATE-7`.

**This document mints none of them.** The line is re-derived rather than carried forward from
[`2026-08-29-w11-1-evaluator-core.md`](2026-08-29-w11-1-evaluator-core.md), which verified it
at `7b8473a`: a stale allocation aid is what mints a colliding id, and that plan's own line
exists because [`2026-08-28-w10-rate-tables.md`](2026-08-28-w10-rate-tables.md) went stale the
same way. One `OQ-RATE-7` is *owed* by someone — see D5 — and this document does not take it.
...
rulings (D4, §9), a dependency NFR-RATE-13 needs that this workspace does not have (F1, §3.4), a
...
3. [`../../CLAUDE.md`](../../CLAUDE.md) §12 — a role decides what its charter names and nothing
   ...
   ([`2026-08-29-w11-scoring.md`](2026-08-29-w11-scoring.md) Task 2.2: *"Files and exit criteria
   depend on which option rules — do not guess."*).

A fourth consideration, from [`README.md`](README.md)'s own fifth convention and from
[`2026-08-29-w11-1-evaluator-core.md`](2026-08-29-w11-1-evaluator-core.md)'s self-review: **a
...
**five** — `cancellation` was added 2026-08-18 with FR-RATE-63.
[`../audit/register.md`](../audit/register.md) row `03 rating shapes vs hand-authored contracts
(F27)` records this divergence and carries it forward with an owner, and
[`2026-08-29-w11-slice1-rulings.md`](2026-08-29-w11-slice1-rulings.md) Ruling 12 assigns the fix
...
| `INPUT_CONTRACT_VIOLATION`, `REFERENCE_LOOKUP_MISS`, `LADDER_RECONCILIATION_FAILED` | Owned by `03` §5.1 and **absent from `backend/src/app/errors.py`** — the class Ruling 11's finding 1 describes; nothing in the gate reads `errors.py` |
...
**F1 — NFR-RATE-13 prescribes a response class whose library is not a dependency of this
workspace.** NFR-RATE-13 (`03` §9) requires the scoring endpoint to skip `response_model`
...
change and is not one. **Not resolved here** — whether NFR-RATE-13 is satisfied by adding
`orjson`, by a different encoder, or by re-reading the requirement in light of W8's measured
...
([`../audit/plan-reviews.md`](../audit/plan-reviews.md) review 8 Q4 found that mechanism twice).

| Requirement | Spec section | Slice | Note |
|---|---|---|---|
| FR-RATE-34 | §3.7 | 1 (core) + **2** (default-live resolution) | The split is DP1's subject |
| FR-RATE-35 | §3.7 | **2** | `prod` restricts to `approved`; recorded as `what_if` |
| FR-RATE-36 | §3.7 | **3** | `POST /api/v1/score/batch`, Job, parquet output |
| FR-RATE-37 | §3.7 | **3** | Identical bundle *and code path* as real-time |
| FR-RATE-38 | §3.7 | 1 (per-quote typing) + **3** (batch counts, samples, threshold) | The threshold's home is unruled — D3 |
| FR-RATE-39 | §3.7 | 1 | Amended 2026-08-29 by Ruling 9; nothing owed to 2–4 |
| FR-RATE-40 | §3.8 | **2** | DP2's subject |
| FR-RATE-41 | §3.7 | 1 (capture) | *"Traces are the same structure in real-time and batch"* binds Slice 3 |
| FR-RATE-42 | §3.8 | **4** | Sampling, ≥ 13-month retention |
| FR-RATE-64 | §3.7 | 1 | Both halves — the rung and the APR refusal |
| FR-RATE-65 | §3.4 | 1 | See §8 P3 |
| NFR-RATE-1 | §9 | 1 (component) + **2** (full path, sustained 200 rps) | |
| NFR-RATE-5 | §9 | **3** | ≥ 1 M risks/hour/worker |
| NFR-RATE-9 | §9 | **2** | *"degrading to the last-known-good cached bundle"* — see D4 |
| NFR-RATE-11 | §9 | **2** (auth, rate limits) + **4** (traces access-controlled) | |
| NFR-RATE-12 | §9 | **4** | 1 % of 50 M quotes under 200 GB/year |
| NFR-RATE-13 | §9 | **2** | Validate inbound, never outbound; `ORJSONResponse` |

**Excluded, each with its owner:** FR-RATE-43, 44, 45 → W12. FR-RATE-46, 47, 48, 49 → W13.
FR-RATE-50 and FR-PLAT-28 → W14. FR-RATE-61 → W13
([`../audit/register.md`](../audit/register.md) row `FR-RATE-61 (F-W9-2)`), and its own text
says the check *"specialises FR-RATE-40's general approval-evidence gate, which W11 builds"* —
so **DP2's ruling is a W13 input**, not only a W11 one.
...
Beyond that, **there is a hole between Ruling 10 and NFR-RATE-9 that no slice currently
...
| **S-K** — F1 settled: how NFR-RATE-13's `ORJSONResponse` is satisfied, and the `skills-map.md` and §8 rows it owes | 2.1 | Decision-maker; can be raised now |
...
- The register rows: `NFR-RATE-13/14 (F-W9-1)` — its NFR-RATE-13 half *"needs the HTTP path and
  is Slice 2's"* per [`2026-08-29-w11-1-evaluator-core.md`](2026-08-29-w11-1-evaluator-core.md);
  `FR-RATE-61 (F-W9-2)` — W13's, but see §4's note that DP2 is its input; `03 rating surface
  (F8)` — the phase-boundary row.
- Ruling 6 already settles Task 2.1's load driver: *"the sustained-load driver that Slice 2's
  ...
  `uv.lock`, CI workflow or setup instruction for this measurement, Ruling 6 has been overridden
  and needs a successor record.
- The two measurements Slice 2 owes and their sources: NFR-RATE-1 at the full HTTP path
...
- The evidence sweep the plan must re-run at **its own** pinned commit, not reuse from here:
  ...
  [`2026-08-29-w11-slice1-rulings.md`](2026-08-29-w11-slice1-rulings.md) between `9891be1` and
...
Worse for planning purposes, FR-RATE-37 does not merely require the same *bundle* — it requires
...
[`2026-08-29-w11-1-evaluator-core.md`](2026-08-29-w11-1-evaluator-core.md)'s Global Constraints
...
right call there, on Ruling 8's reasoning that naming a function before it is designed is how a
...
| **S-F** — D3 ruled: where FR-RATE-38's batch abort threshold is configured | the error-handling task | Decision-maker; reaches into `07`, so raise early |
...
- FR-RATE-36, 37, 38 (batch half) and NFR-RATE-5, with their spec sections (§4).
...
- FR-RATE-41's *"Traces are the same structure in real-time and batch"* is a Slice 3 constraint
...
- The recovered reasoning behind `chunk_rows: int = 100_000`, and the job-scoped-staging
  ...
  recommendation.
- FR-PLAT-9's *"a cancelled Job leaves no partially-visible artifact"* governs **cancellation,
  not crash-resume** — that distinction is the reason FR-RATE-37's "resumable" asks for something
...
a capacity projection proving NFR-RATE-12 *"against the actual serialised `Trace` size Task 1.4
...
| Signal | Releases | Depends on |
|---|---|---|
| **S-H** — Task 1.4 merged, and a real `Trace` serialised so its size can be measured | the NFR-RATE-12 projection | Serial behind Slice 1 |
| **S-I** — D2 ruled: thin Postgres row + blob body, GC-based retention | the persistence task | Decision-maker |
| **S-J** — the FR-RATE-41/42 batch-sampling silence resolved as an `OQ-RATE` or a spec change | the sampling task | Decision-maker; carries a [`../open-questions.md`](../open-questions.md) mirror row and a [`../roadmap.md`](../roadmap.md) §10 gate row with a recount |

S-I and S-J are dispatchable today. S-J is the longest-lead item in W11 that nobody is currently
...
- FR-RATE-42, NFR-RATE-12 and NFR-RATE-11's access-control half, with their spec sections (§4).
- NFR-OVR-6 is the ≥ 13-month retention authority FR-RATE-42 defers to; FR-PLAT-20's
...
- `07`'s content-addressed blob store is the persistence tier, and `DislocationRun`'s
  ...
  precedent and
  [`2026-08-29-w11-1-evaluator-core.md`](2026-08-29-w11-1-evaluator-core.md)'s correction C4
...
- `GET /api/v1/traces?rating_version=&from=&to=` is already in the §5.1 endpoint table, cited to
  FR-RATE-42 — the route path is specified, not invented.
...
against synthetic `Trace` objects. It does not hold for FR-RATE-42's own words, *"in production,
...
**P3 — [`../roadmap.md`](../roadmap.md)'s W11 row states its scope as the numeric range
`FR-RATE-34..42` plus FR-RATE-64, and FR-RATE-65 is inside W11 and outside that range.**
FR-RATE-65 (`03` §3.4) is what defines `CompiledBundle` as a distinct runtime type; Slice 1 Task
1.3 discharges it, and
[`2026-08-29-w11-1-evaluator-core.md`](2026-08-29-w11-1-evaluator-core.md)'s correction C5 added
it to that plan's coverage table. `git grep -n "FR-RATE-65" -- docs/` at `9891be1` returns no
workstream row in [`../roadmap.md`](../roadmap.md). This is the same mechanism
[`../audit/plan-reviews.md`](../audit/plan-reviews.md) already filed for FR-RATE-61 and
FR-RATE-63 — *"neither appears in any `W_` row's stated scope"* — whose maintainer acceptance is
still pending there. **Recommendation:** fold FR-RATE-65 into that pending item rather than
...
- Ruling 10 rules that the *refresh trigger* is W14's and gives Task 1.3 two properties so W14
...
- Ruling 10 also records that **Slice 2 has zero hits** for `redis`, `cache`, `warm`, `refresh`,
  `poll` and `slot`. So the frozen map schedules no work anywhere that builds the tier Ruling 10
  says must exist above `pricing_core`.
- Meanwhile NFR-RATE-9 is allocated to **Slice 2** and reads *"degrading to the last-known-good
  cached bundle if metadata storage is unavailable"* — a requirement that presupposes a cache in
  the scoring path — and NFR-RATE-1's 50 ms p99 has to absorb whatever the route does per
  request. Ruling 8 records that `predict_gbm` re-loads the booster on every call today, so
...
- *(a) None — resolve, fetch the `Bundle` from the blob store and `load_bundle` per request.*
  ...
  against both NFR-RATE-1 and the spirit of NFR-RATE-3, and leaves NFR-RATE-9's
...
- *(b) A per-worker in-process slot in `backend/`, keyed by `content_hash`, bounded, populated on
  first use via `load_bundle`, with no refresh trigger of any kind.* Satisfies NFR-RATE-9 by
  construction, keeps the seam on the `backend/` side of ADR-0001 where Ruling 10 puts it, and
  leaves every refresh mechanism — push, poll, pub/sub — open for W14, which is what Ruling 10
...
- *(c) A Redis tier holding serialised `Bundle` bytes, deserialised and loaded per request.*
  Matches `07` FR-PLAT-22's naming of Redis as the cache, but Ruling 4 already rules that
...
poll, no pub/sub, no "current hash for env X" pointer, all of which Ruling 10 keeps as W14's. An
acceptance test stated as the violation it must make expressible: **a test that fails if
`load_bundle` consults the slot, registers in a global, or starts a task** — Ruling 10's second
...
[`2026-08-29-w11-slice1-rulings.md`](2026-08-29-w11-slice1-rulings.md)'s own queued table or in
[`2026-08-29-w11-decision-points-recovery.md`](2026-08-29-w11-decision-points-recovery.md); this
adds the slice-release mapping, not new options.

| Ref | Item | Slice | Signal | Standing recommendation, and whose |
|---|---|---|---|---|
| **DP1** | Default-live resolution for `POST /api/v1/score` | 2 (task 2.2) | S-B | Map plan recommends (b) — defer to W14 as a named register deferral |
| **DP2** | FR-RATE-40's approval gate ahead of W12/W13 | 2 (task 2.3) | S-C | Map plan recommends (a) — build the mechanism now. **Also a W13 input**, per FR-RATE-61's own text |
| **D2** | Trace persistence: thin Postgres row + blob body, GC-based retention | 4 | S-I | Recovery item 2's option (b) |
| **D3** | Where FR-RATE-38's batch abort threshold is configured | 3 | S-F | Recovery item 4 — a workspace setting on FR-PLAT-45's precedent, with a per-request override. Reaches into `07` |
| **D5** | FR-RATE-41/42 state no **batch** sampling default | 3 and 4 | S-G, S-J | A spec silence, not a choice inside a requirement. Needs an `OQ-RATE` (next free `OQ-RATE-7`) or a spec change, with its [`../open-questions.md`](../open-questions.md) mirror and a [`../roadmap.md`](../roadmap.md) §10 gate row |

## 10. Corrections to the frozen map

The map is left standing; these are recorded so a reader of both is not misled.

**M1 — the map's Slice 2 requirement line omits NFR-RATE-11's second half.** It lists
"NFR-RATE-1 (full path + sustained load), 9, 11, 13" for Slice 2 and, separately, NFR-RATE-11 is
also what makes Slice 4's traces access-controlled — the map's Slice 4 line says so in prose
(*"access-control test (traces are access-controlled per NFR-RATE-11)"*) without listing the id
...
([`2026-08-29-w11-decision-points-recovery.md`](2026-08-29-w11-decision-points-recovery.md) item
2) and queued as unruled
([`2026-08-29-w11-slice1-rulings.md`](2026-08-29-w11-slice1-rulings.md)); D2 above is that item.
...
[`2026-08-29-w11-1-evaluator-core.md`](2026-08-29-w11-1-evaluator-core.md)'s correction C4 found
...
The marker is the point: the exclusion is **FR-GOV-6**'s — *"Service Accounts (for Consumer
...
here:** the grant path is a Service Account rather than a role, but FR-GOV-6 also scopes it *to
named environments*, and the Environment entity is FR-PLAT-28 — W14's, the same entity DP1
turns on. Whether Slice 2's RBAC test can express "a scoped Service Account may call it" before
W14 exists is a question for the Slice 2 plan, and it may prove to be DP1-shaped rather than
independent of it.
...
row-backed path stays synchronous. FR-RATE-36 specifies `POST /api/v1/score/batch` as a Job
...
[`2026-08-29-w11-slice1-rulings.md`](2026-08-29-w11-slice1-rulings.md)'s queued table that
...
by §4's requirement table, which lists NFR-RATE-11 against both slices, and **M2** by §9's D2
...
FR-GOV-6's scope, and both are the decision-maker's to read. P1, P2 and P3 are proposals and are
