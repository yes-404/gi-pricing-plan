...
forbids both, as Ruling 103 §5.4 already records.
...
git grep -n 'a hunk is permitted only where it is' origin/main -- docs/
  → docs/plans/2026-09-02-w37-migration-preconditions-rulings.md:256   (one hit, exactly)
```

### 1.1 Why the absence claim was produced, because the mechanism matters more than the correction

The searches that returned nothing were, per the commission, `'Ruling 68|six.class|permitted'`
...
same class as `RL-00993` and the `977` misattribution recorded against Ruling 103 — **a correct
...
**`docs/audit/plan-reviews.md:2854` does not say what it was read as saying.** It names *"Ruling
...
## 2. Question 1 — a widening, and Ruling 68 refutes the bare sentence before replacing it

**NT-0019 §7(g)'s own sentence:** *"the migration diff filtered to hunks that are neither header
nor citation-token is empty."*

**Ruling 68 §1's fourth verification row rules on exactly that sentence**, verbatim:
...
is precisely the list Ruling 68 gives as its grounds.
...
citation-token"*, which Ruling 68 rejected as the definition. **The row is red by construction,
...
### 2.2 The consequence for Ruling 102 §2 row 1, stated but not decided

Ruling 102 §2 puts (g) first in the sequence and names `NFR-RATE-13/14` as its broken-input
...
predicate Ruling 68 replaced. **Both can be true: the (g) *defect* is fixable and the (g) *row*
is not passable as implemented.**

**Whether that reopens Ruling 102 §2's ordering is the maintainer's**, and this record does not
touch it.
...
(`scripts/doc-id.py`) carries in its own comment that it is *"a further extension of Ruling 68's
...
Class 6 names **`INDEX.md`** unqualified. §1.4 puts one at `docs/INDEX.md`; Ruling 101 and
Ruling 102 §6 create one **per family** (`docs/closures/INDEX.md`, `docs/plans/INDEX.md`).
...
**That is `NT-0004` exactly** — a reference that resolves only for its writer — and it is a
...
**All three are the maintainer's.** Ruling 68 is a maintainer ruling and an amendment to what it
requires is the maintainer's alone (`CLAUDE.md` §12).

| option | what it costs | what it risks |
|---|---|---|
| **A — implement classes 3-6** as Ruling 68 obliges (*"implemented as code with the six classes named"*) | the largest build of the three; four classifiers, and class 4's *"concatenation reproduces the input's body lines in order"* is the expensive one | nothing about the ruling changes; the row becomes passable and the acceptance tests in Ruling 68 §4 become runnable. **This is the option the ruling already requires** |
| **B — amend §7(g) to the narrower sentence** and drop the enumeration | cheapest; `row_g` becomes correct as written | **reinstates the reading Ruling 68 refuted**, and its stated ground — *"left as worded, an executor invents a filter at the console"* — is not answered by making the row green |
| **C — implement 3-6 as a disclosed allow-list of paths** rather than classifiers | middle cost | Ruling 68 §2 **already rejected a path exclusion**, on the measured ground that thirteen §5.2 rows put script output and hand edits in the same file. Reintroducing it here would need that refutation answered, not repeated |
...
widening — which would require Ruling 68 §1's fourth row to say something other than
