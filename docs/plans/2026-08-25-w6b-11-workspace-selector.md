...
**Spec:** [`../specs/07-platform.md`](../specs/07-platform.md) — `FR-PLAT-62`, `FR-PLAT-63`, `FR-PLAT-65`, `FR-PLAT-66`; [`../specs/06-governance.md`](../specs/06-governance.md) — `FR-GOV-24`. The decision this plan executes is [`../open-questions.md`](../open-questions.md) `OQ-PLAT-12` (decided 2026-08-25, Reading 1, option (c)).
...
- **`FR-PLAT-4`** — access is explicit, never default: an authenticated user with no membership row can reach no workspace.
- **`FR-PLAT-63`** — the selection is checked against the principal's own memberships on every request, never trusted; a switch is audited into **both** chains (the workspace left and the workspace entered); the first selection after login has no chain to leave and writes one event. Four obligations; obligations 1–3 are delivered (W32).
- **`FR-PLAT-65`** — the chosen workspace travels as a verified `Workspace-Id` request header: declared on the route, optional in the published contract, required in the handler; unprefixed (`X-` is retired by RFC 6648); a principal with several memberships and no header is refused `WORKSPACE_SELECTION_REQUIRED`, never defaulted.
- **`OQ-PLAT-12`, decided: a switch is a human act.** Only the explicit `POST /api/v1/me/workspace` audits (`record_switch`); `require_caller` audits nothing; a client that changes its header without the act switches unrecorded — which under Reading 1 is not a gap, because such a caller has not switched.
...
- **`FR-PLAT-66`** (decides `OQ-PLAT-17`) — the browser learns the OIDC issuer and `client_id` from an unauthenticated `GET /api/v1/auth/config`. That endpoint is **`W6b-10`'s** build work, not this slice's.
...
4. The spec amendment these endpoints need (`FR-PLAT-63`, `FR-PLAT-65`, §5.1 rows).
...
async def test_the_list_is_readable_without_a_selection(client: AsyncClient, database):
    ...
    # each carrying its slug and name (FR-PLAT-62), ordered by name.
    ...
```
...
This route declares **no** `Workspace-Id` header parameter: it is the one surface that must work before a selection exists — the ruling's shape exactly (PR #237, `FR-PLAT-63`'s second amendment: unscoped, authenticated, list-only).
...
async def test_the_first_selection_writes_one_event(client: AsyncClient, database):
    ...
    # FR-PLAT-63: the first selection after login has no chain to leave.
...
async def switch_workspace(
    ...
    # login has no chain to leave (FR-PLAT-63) and record_switch takes left=None for it.
...
A malformed **body** `workspace_id` is a Pydantic `422` ProblemDetail — that is the platform's standard channel for invalid bodies (the header's str-then-parse exists only to keep *header* refusals in the error catalogue; `FR-PLAT-65` says nothing about bodies).
...
### Task 4: The one spec amendment the ruling left — the `FR-PLAT-65` carve-out

**Files:**
- Modify: `docs/specs/07-platform.md` (append a dated note to `FR-PLAT-65`, `:81`)

**The ruling already did the rest of the spec work (PR #237, merged):** `FR-PLAT-63` carries its second decision-maker amendment — the deadlock finding, the ruling, and the refused alternatives — and `06` §5.1 declares both routes. This task adds only the one sentence the ruling did not: `FR-PLAT-65`'s universal "required in the handler" now has a named exception, and a reader of that row alone would read the switch endpoint as violating it.

- [ ] **Step 1: Append the dated note**

After `FR-PLAT-65`'s existing amendment (`07-platform.md:81`), append — keeping every prior clause verbatim:
...
Expected: PASS — the amendment cites only ids already defined (`FR-PLAT-63`, `FR-PLAT-65`).
 ...
 *  selection can be made (07 FR-PLAT-63). */
export function listWorkspaces(): Promise<WorkspaceMembership[]> {
  return request<WorkspaceMembership[]>("/me/workspaces");
}

/** Choose the workspace to act in; the platform audits the switch (07 FR-PLAT-63). */
...
The header name is unprefixed, per `FR-PLAT-65` — deliberately not `x-dev-workspace-id`, which `FR-PLAT-55` confines to `local`/`dev`.
...
A multi-membership principal with nothing remembered starts with `current = null` and sends no header — the platform refuses scoped requests with `WORKSPACE_SELECTION_REQUIRED` until the user picks, which is the designed safety, never someone else's data (`FR-PLAT-65`).
...
A native `<select>` is deliberate: it is keyboard-navigable and screen-reader-announced for free, and the prompt option ("Choose a workspace…") is the honest state of a multi-membership principal before its first selection. A single membership shows the name and no control — no selection is needed (`FR-PLAT-63`).
...
- Modify: `docs/specs/07-platform.md` (dated note on the dev-header sentence in `FR-PLAT-55`)
...
- [ ] **Step 5: Amend the dev-header sentence in `FR-PLAT-55`**

In `07-platform.md`'s `FR-PLAT-55` row (the sentence naming *"the development identity headers (`x-dev-principal-id`, `x-dev-workspace-id`, injected by the frontend dev proxy)"*), append a dated note:
...
**1. Spec coverage.** `FR-PLAT-63` obligation 4 — Tasks 1, 3, 6, 7 (the endpoint, the call, the act); obligations 1–3 and `FR-PLAT-65` are delivered (W32) and consumed — Tasks 6, 8. `FR-PLAT-62`'s name — rendered by Task 7, carried by Task 2. `OQ-PLAT-12` option (c) — Task 3's route and Task 6's "a reload is not an act" comment. The pin disposition — Task 8, last. `FR-GOV-24`'s per-workspace chains — Task 3's `record_switch` call (mechanism delivered). `FR-PLAT-66` — recorded as W6b-10's, not built here. **Gaps:** none — Finding 1 was ruled and merged before filing (PR #237), and Tasks 2–4 build against the ruling's exact shape.
