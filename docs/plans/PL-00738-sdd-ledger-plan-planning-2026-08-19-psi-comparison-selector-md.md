...
family: plan
kind: leaf
title: SDD ledger — plan: .planning/PL-00739-the-psi-comparison-selector-01-5-3-fr-63-implementation-plan.md
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-19-psi-comparison-selector-ledger.md
---

# SDD ledger — plan: .planning/PL-00739-the-psi-comparison-selector-01-5-3-fr-63-implementation-plan.md

Spec: `docs/specs/01-data-management.md` §5.3 / FR-63 / `01` §4.4 VR-DST-1 — read, reachable.
...
|---|---|---|
| T1 → T6 | `docs/open-questions.md` OQ-556 row; `01` §10 mirror | Both edit the same two files. T1 writes §10 + the OQ row; T6 edits §5.3 and the OQ row's status. Different sections — no textual overlap. **But see RL-864**: T1's Step 6 asks the maintainer a question already answered. |
...
| T2 | `profiles.test.ts` — threshold test + a `@ts-expect-error` null test | **DEFECT — see RL-865.** The null test's runtime assertion (`expect(() => psiBand(null)).toBeDefined()`) is vacuous, and `.claude/skills/vue-frontend` states type assertions in a `*.test.ts` are tests that can never fail. |
...
| T6 | docs only, no tests | Agrees. Step 4 is conditional on T1's outcome — see RL-864. |

### Rulings made before execution

**RL-864 — T1 Step 6 records the decision rather than asking for it.** The maintainer
answered the decision gate (route query) when commissioning this execution, so the plan's
"ask the maintainer and stop" step is already satisfied. T1 writes the OQ-556 row as
...
**RL-865 — T2's type-level assertion moves to `profiles.test-d.ts`.** The plan puts a
...
**RL-866 — the plan's own frontend test-count estimates are not acceptance criteria.**
The plan states "the suite is 115 now" and similar. RL-865 changes the count. Implementers
...
That last line confirms RL-865's premise: `vitest run` executes the `*.test-d.ts` typecheck
...
Task 1: **Ruling: no new `FR-` id for OQ-556.** The reviewer noted every other
...
RL-865 vindicated: the `.test-d.ts` assertion produced a real TypeCheckError before the fix
...
disabled; (c) the "back/forward" claim in the OQ-556 row. The slice's entire subject is
