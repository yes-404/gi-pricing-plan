...
family: plan
kind: leaf
title: Artifact Library List Routes Implementation Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-23-w32-8-artifact-library-list-routes.md
---
...
half is `usage_count`: FR-167 makes "one aggregate per page, never one per row" part of the
...
**Spec:** [`../specs/02-modelling.md`](../specs/02-modelling.md) — FR-167 (`:231`) and the
three §5.1 rows it added (`:1697`, `:1705`, `:1712`).

**Proposed slice id:** `W32-8`. The WK-692 slice boundaries in
...
- FR-167 exists (appended 2026-08-23). No new requirement ids; no `Next free:` marker.
- **`usage_count` goes on objectives and metrics only.** §5.1's peril-structures row (`:1712`) asks
  ...
  explicitly. Task 5 records the discrepancy between that and FR-167's unqualified prose;
...
- Filters are `status` and `slug`. **`slug` is an exact match, not a prefix or substring** —
  FR-167 says the filter is what resolves §5.3's `slug@version` addresses against UUID-only
...
- Existing indexes cover the filters: every table carries
  ...
  slice.**
- Every new test carries `@pytest.mark.req("FR-167")`. `--strict-markers` is on.
...
async def usage_counts(
    ...
    calling it per row is the N+1 FR-167 names as part of the requirement, and it would
...
FR-167 makes the query count part of the requirement, so it gets a test, not a comment:
...
async def list_objectives(
    ...
    *,
    ...
    """One page of the workspace's objectives (FR-167).
...
async def list_custom_objectives(
    ...
    """The library `02` §5.3 renders (FR-167).
...
when written and are the observation FR-167 was raised from.

Rewrite both to say what is now true and keep the history: eight routes, the eighth added by
FR-167 on 2026-08-23, and the library screen `02` §5.3 specifies now has an endpoint. Do not
...
discrepancy with FR-167's unqualified prose. Add a comment on the route saying the omission
...
- Create: `PL-00772-w32-8-execution-ledger-the-artifact-library-list-routes.md`
...
FR-167 opens *"The three artifact libraries **§5.3 renders** are listable."* §5.3
...
Write the question with options: add the two missing §5.3 rows (making FR-167's sentence
true and giving `WK-664` two more screens to build); or amend FR-167's wording to say the three
...
FR-167's prose says *"`usage_count` is on the row, as §5.3 asks"* without qualifying which
...
Create `docs/plans/PL-00772-w32-8-execution-ledger-the-artifact-library-list-routes.md`: the real gate output,
...
Give FR-167 a §13 verdict: *delivered and tested* if all three routes and the budget test
