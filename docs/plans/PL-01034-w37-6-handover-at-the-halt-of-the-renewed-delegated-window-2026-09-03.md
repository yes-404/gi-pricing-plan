...
family: plan
kind: handover
title: W37-6 — handover at the halt of the renewed delegated window, 2026-09-03
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-09-03-w37-6-renewed-window-handover.md
---
...
[`../rulings/RL-01049-w37-6-the-maintainer-s-time-boxed-delegation-2026-09-03.md`](../rulings/RL-01049-w37-6-the-maintainer-s-time-boxed-delegation-2026-09-03.md) —
...
| **`none` (RFC-937 §7(a))** | **0** — buckets summing to 460 = `git ls-files docs/` |
...
| #665 | `d1cabe1` | **RL-1047** — maintainer prose rulings migrate as `RL-`, `owner: maintainer`, no `kind:` |
...
| #669 | `54a3ee5` | **RL-1048** — the three undeclared files reach destinations; no halt |
| #670 | `0042957` | RL-1047's own heading fix, plus **F96** |
...
RL-1040 §4 binds four figures and they must not be quoted apart:
...
by `was:`. That is RL-1039's own consequence, not a defect — but the ruling that makes
...
```
19  docs/rfcs/README.md      6  docs/adrs/README.md       5  docs/findings/README.md
...
**RFC-937 §5.2 says these READMEs are regenerated, and the code does not do it.** Line 317:
*"`adrs/000n-*.md` (6) + README … README **generated**"*. Line 318: *"`rfcs/00nn-*.md` (18)
...
2. **`F12` and the low-range ambiguity.** Three independent audit eras each minted their own
   low F-range — Track A `F1`–`F15`, the WK-661 ledger `F1`–`F12`, phase-1b `F1`–`F25`. `F12`
...
4. **F23/F24/F25 structural drift** — three rows of `docs/findings/register.md` split
...
| RL-1047's substantive rule | **unimplemented** — seven maintainer decisions still migrate as `owner: planner` |
...
| **RL-1047's implementation** | Seven documents to `RL-`/`owner: maintainer`; `_discover_plain_plans`' catch-all does not do it |
...
commit, not the writing: *"a handover written and not committed is the failure the WK-661 record
exists to prevent."*
