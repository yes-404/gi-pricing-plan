# W37-6 — Ruling 107: check 36 adopts (d)'s disclosed classes, check 32 adopts (e)'s conjuncts from shared code (2026-09-04)

## Ruling 107 — check 36 is "one rule at two times" with (d) and must carry (d)'s disclosed classes; check 32's padding/resolution clause adopts (e)'s conjuncts from `_docid`, not a private re-typing
...
<!-- Structural note: this heading exists so `_discover_multi_ruling_files`
     ...
     `PL- kind: leaf, owner: planner` catch-all — the same defect Ruling 106's structural
     note names (F96, `docs/audit/findings/F96.md`). -->
...
"Ruling 107"` against every open W37-6 remote branch returns hits on two, for two
*different* rulings:
...
- `origin/w37-6-h1-checks-31-32-36` (PR **#747**, `w37-6-h1-checks-31-32-36`) — 3 hits, all
  in `scripts/audit-docs.py:1740,1790,1792`, reading *"Contiguity (Ruling 107) reads the
...
No other open branch matches `Ruling 107` under `docs/`, `.claude/`, or `scripts/`.
...
> docstring, *"NT-0019 §7 acceptance item (d) and check 36's third clause are 'one rule at
> two times' (Ruling 67 §2), so both read this one function"* — same
...
> everything today's rulings disclosed at (d): the `F<nn>`/`F-W` alias classes (Ruling 105
...
`origin/main` in `docs/plans/2026-09-04-w37-6-migration-run-ledger.md`, one line). Nothing in
...
> prints **2043 "padded id outside a link target"**. Row (e) is Ruling 103's four conjuncts
...
>    fence, the path exclusion (`_MD_EMPHASIS_RE`-stripped, as Ruling 103 defect 3 required)
>    and the resolution test move from `_docverify` into `_docid` and both scripts read them
>    there; check 32 keeps its own `0*` breadth (NT-0019 §1.1 rule 2 is *"no exception"*, and
...
worktree `24fe074`; `to-lead.md:1257-1290`; `docs/plans/2026-09-02-w37-ruling-a-series-and-standalone-ruling-files.md`
§3 at `origin/main`.

> **"Ruling 107" is a label, not a record.** `git grep 'Ruling 107'` returns nothing [as a
...
> comments citing "Ruling 107". A citation that resolves in no document is NT-0004's case.
...
section) shows is imprecise taken word-for-word: `git grep 'Ruling 107'` *does* return hits,
...
code comments citing 'Ruling 107'" — already says this; the insertion makes the two
...
- **The ledger** (`docs/plans/2026-09-04-w37-6-migration-run-ledger.md`) — already carries
...
cited in PR #747's code comments as "Ruling 107" — the collision named in this record's
...
not name it as part of the "Ruling 107" label's content; it needs its own filed record and
...
1. PR #749, re-scoped to item 1 of the check-32 entry in full (the three conjuncts moved
   ...
   its code comments repointed from the bare string "Ruling 107" to this record's path.
