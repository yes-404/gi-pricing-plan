...
halt protocol of
[`2026-09-03-w37-6-time-boxed-delegation.md`](2026-09-03-w37-6-time-boxed-delegation.md) §4
...
Ruling 97 §4 binds four figures together and they must not be quoted apart:
...
struck by Ruling 96, and the one-discovery work merged as #657.
...
writes into the shared checkout's worktrees. That guard exists because two W10 incidents
...
| **Condition 3** | Fresh agent: `python3 scripts/register-owed.py W37-6` against a **committed** revision — the script refuses a dirty `register.md` under Ruling 52 — then a named disposition per row |
...
the commit, not the writing: *"a handover written and not committed is the failure the W5
