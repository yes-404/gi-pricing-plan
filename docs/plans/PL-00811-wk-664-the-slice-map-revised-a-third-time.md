...
family: plan
kind: leaf
title: WK-664 — the slice map, revised a third time
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-26-w6b-slice-map-revised-3.md
---

# WK-664 — the slice map, revised a third time

> **This supersedes [`PL-00810-wk-664-the-slice-map-revised-again.md`](PL-00810-wk-664-the-slice-map-revised-again.md) for
> WK-664, and nothing else.** [`README.md`](README.md) freezes that file at its date. This map does
...
assign to WK-664. This map does not restate the previous revision's scope statement. The earlier
revisions stay the record of their dates.

### Slice ids are not free to renumber

The constraint from the previous map is unchanged. It still binds. `WK-664-N` ids appear in
...
The items the manager did not attach stay in that queue: OQ-569's two halves, OQ-570's
report projection and badge, OQ-609's selection-time validation, OQ-654's coverage fix,
...
did not land with the filing: the new 02 FR for MODEL-43, the FR-82 sentence for OVR-15 (a),
00 §5.3's currency sentence for OVR-14, 01 §4.6's example for DATA-12, 01 §5.2 for DATA-13, and
the 07 texts for PLAT-10, PLAT-11 and PLAT-16. One amendment did land: OQ-570's FR-59
...
**The next-free line for the DATA family is spent.** The 08-26 map published FR-59 as next
...
```
grep -cE '^\| \*\*WK-664-[0-9]+[a-z]?\*\*' docs/plans/PL-00811-wk-664-the-slice-map-revised-a-third-time.md
```

No slice depends on anything outside WK-664. The `Depends on` column is internal only. It was
...
| **W6b-5a** | **The TreeSHAP holdout pass** — `FR-168`'s backend precondition | — | — | merged |
| **W6b-5b** | **The suggestion panel** — `FR-168`'s owner clause names this surface | W6b-5a | — | merged |
...
| **W6b-9** | **Tabular chart fallback** (`NFR-463`) | — | — | merged |
| **W6b-10** | **Browser authentication** — `FR-393`'s PKCE flow and `FR-394`'s `/api/v1/auth/config` channel | W6b-14 | — | merged (`c56ec75`, PR #250) |
...
| **W6b-11b** | **The switch audit** — `FR-396` obligation 4 | — | — | **vacated** — `OQ-652` decided (c). Obligation 4 folded into W6b-11 |
...
| **W6b-13b** | **The catalogue chain** — `FR-68`'s dropped `catalogue_id` and `FR-56`'s default thresholds | — | — | merged |
| **W6b-14** | **The local OIDC provider** (`FR-398`) — a compose profile and a checked-in realm, seeded `workspace_members` | — | — | plan merged (`dcb0823`). Build merged (`bc1d880`) |
| **W6b-15** | **The `_minor` rename** — `OQ-538` decided (b): statistics mislabelled `_minor` drop the suffix under `FR-23`. The integer type stays. Known members: `observed_burning_cost_minor`/`modelled_burning_cost_minor` (`model-schema/perils.py:290,306-307`, `pricing-core/modelling/perils.py:85,93-94`) and the `validate.py` names `FR-23` cites (`:1072-1073`, `:1077-1078`). The plan sweeps the class. `claim_amount_minor` (a column name) and `total_negative_minor` (`int(...)`-cast) conform and stay | — | — | plan merged (`47d1375`, PR #251). Build queued |
| **W6b-16** | **The surrogate slug derivation** — `OQ-604` ruled (c): `reserve_model` derives `source_family_slug + "-approx"` at reservation and overrides the caller's. The 64-char refusal moves with the derivation. `FR-141` amended. The version half raised as `OQ-613` | — | — | merged (`70934c1`) — the change set shipped as PR #246 (spec + code + tests) |
| **W6b-17** | **The GlmSpec companion address** — `OQ-613` decided (a): a `slug@version` pair on GlmSpec that follows IntervalFor (`FR-199`/`FR-200`) and joins `spec_hash` (`FR-206`). The id stays the lookup key. `SPEC_HASH_VERSION` bumps v10 to v11 with a lineage comment (`backend/src/app/platform/modelling.py:146`). The 02 FR text files with this slice | — | — | not started — queued after the #136 build |
| **W6b-18** | **The Offending Sample item shape** — `OQ-567` decided (b): the item is a keyed object, the item shape written out. `pricing-core` emits it. 01 §4.6's example, the tests and the contract regen land with it | — | — | not started — queued after the #136 build |
| **W6b-19** | **DatasetVersion catches up to its contract** — `OQ-568` decided (c): 14 flat fields adopt the contract's scalars, 3 structural fields adopt its object forms. A migration carries the shape change. 01 §5.2 updates with it. The decision sequences it with W6b-12. The executor verifies that premise at slice start (§2) | W6b-12 | — | not started — queued after the #136 build |
| **W6b-20** | **The views read the dataset's currency** — `OQ-551` decided (b): `getDataset(slug)` supplies the currency. The `?? "GBP"` fallbacks delete. 00 §5.3 records the rule | — | — | not started — queued after the #136 build |
| **W6b-21** | **The owner display, interim** — `OQ-552` decided (c) with (a) as the near-term step: the list endpoint resolves `owner_id` beside `latest_version_status`. FR-82 gains the resolved-name sentence. The (c) route defers with the shared trigger | — | — | not started — queued after the #136 build |
| **W6b-22** | **The one-sidedness registry** — `OQ-649` decided (b): a slug one-sided on purpose declares it in one registry. Anything one-sided and undeclared fails. The stale `peril-structure` comment fixes. F2 subsumes | — | — | not started — executor small fix, queued after the #136 build |
| **W6b-23** | **The revalidation sweep** — `OQ-650` decided (c) now: a script parses every stored artifact against today's models. The report names what no longer reads. 07 records the migration-event rule | — | — | not started — executor small fix, queued after the #136 build |
| **W6b-24** | **Two generated type sets** — `OQ-655` decided (c): `generate:api` runs twice over the same OpenAPI, once with `--default-non-nullable` and once without. Request bodies alias the permissive set, responses keep the strict set. The three `as unknown as ModelSpec` casts remove. 07 documents the flag | — | — | not started — queued after the #136 build |
...
- **`W6b-21`** — one list endpoint, one column, one FR-82 sentence. The (c) route is not
  part of it.
...
**`OQ-613` decided (a) — register row 107, struck at `7400846`.** GlmSpec gains a companion
...
**`OQ-567` decided (b) — register row 56, struck at `7400846`.** The Offending Sample item is
...
**`OQ-568` decided (c) — register row 57, struck at `7400846`.** 14 flat fields adopt the
...
**`OQ-551` decided (b) — register row 37, struck at `7400846`.** The views read the dataset's
...
**`OQ-552` decided (c), with (a) as the near-term step — register row 38, struck at
`7400846`.** The interim step is the owner display: the list endpoint resolves `owner_id` beside
`latest_version_status`. FR-82 gains the sentence that the owner column renders the
principal's resolved display name, never the raw id. The queued slice names a WK-664 slice for
(a) interim. The (c) route defers with the shared trigger. This map attaches (a) as
**`W6b-21`**.

**`OQ-649` decided (b) — register row 167, struck at `7400846`.** One-sidedness declares in a
...
**`OQ-650` decided (c) now — register row 168, struck at `7400846`.** A revalidation sweep
...
**`OQ-655` decided (c) — register row 173, struck at `7400846`.** `generate:api` runs twice
...
names a WK-664 slice: `frontend/package.json`, `frontend/src/api/generated` (VCS-ignored),
...
|---|---|---|---|
| **P1** | **Decide `OQ-652`** — **decided 2026-08-25 (c)**, not as recommended. `POST /api/v1/me/workspace` audits through `record_switch`. `require_caller` audits nothing. The stored-selection half is refused. Consequence: `W6b-11b` vacates. Obligation 4 goes to `W6b-11` (this map's §3/§4) | maintainer, then `W6b-11` | *decided* |
...
| **P4** | **The modelling PII guard is not WK-664's and must not be absorbed into it** — carried unchanged. No maintainer line has landed since 08-24. A column classified `direct_identifier` is still fittable. It sits on `FR-39`/`FR-90`. It needs a new id and a new unit | **unowned** — pending a maintainer line | *pending* |
| **P5** | **`W6b-13`'s title restated as rule versioning** — **adopted**. The plan is `PL-00792-w6b-13-rule-set-rule-versioning-screen-implementation-plan.md`. The slice's name and its plan agree | `W6b-13` | *adopted* |
...
| **P7** | **`WK-678` owns `FR-358`** — resolved by the record. The roadmap's own dated correction (2026-08-23) stands at `:2564`. The Phase 3 workstream table reads "Phase 3, WK-678" at `:2248`. The correction stands where the wrong owner stood. Nothing further to sign | `WK-678`, Phase 3 | *resolved* |
...
| **P10** | **The gap list folds into the close-time completeness audit, not into this revision** — carried unchanged. The auditor's findings arrive at the all-slices audit at close. They enter the slice inventory then | the close (WK-664 close slice), maintainer | *pending* |
...
| **P13** | **The unattached OQ build items stay queued per the filing** — OQ-569's halves, OQ-570's build halves, OQ-609, OQ-654, the shared-trigger group, the trigger-specific group. The manager's queue disposes them. The close reads them from the filing | maintainer (the queue) | *pending* |
...
Highest ids in use: FR-25, NFR-464, OQ-553. Next free: `FR-OVR-23`, `NFR-OVR-12`, `OQ-554`. *(unchanged.)*
Highest ids in use: FR-59, NFR-474, OQ-570. Next free: `FR-DATA-58`, `NFR-DATA-11`, `OQ-DATA-16`. *(08-26 said FR-59. The filing minted it with the OQ-570 strike at `01:121`.)*
Highest ids in use: FR-168, NFR-488, OQ-613. Next free: `FR-169`, `NFR-MODEL-15`, `OQ-MODEL-44`. *(Unchanged. `W6b-17` files the 02 FR when it plans. The OVR-15 (c) route files a 07 FR when the shared trigger fires. The number stays unallocated at the anchor.)*
Highest ids in use: FR-394, NFR-535, OQ-656. Next free: `FR-440`, `NFR-PLAT-12`, `OQ-PLAT-18`. *(unchanged.)*
Highest ids in use: FR-362, NFR-525, OQ-632. Next free: `FR-GOV-46`, `NFR-GOV-9`, `OQ-GOV-9`. *(unchanged.)*

---

## 6. What will bite at closure

**The roadmap's WK-664 rows still contradict merged builds.** The close's §13 audit derives scope
from the specification first. The roadmap is a specification artifact. Quoted against the anchor
SHA, with the correction:

- The accessibility row reads *"`NFR-463`'s tabular fallback for charts is **not** built"*,
  with owner WK-664 (`:1774`) — the W6b-9 build (chart-table retrofit) is merged. Correction:
  delivered. The 08-26 map first recorded this. The row still stands at the anchor.
- The browser authentication row reads *"**Not started, and correctly so.** OQ-644 was open
  when WK-663 closed"* (`:1770`) — the W6b-10 build merged as PR #250 (`c56ec75`). Correction:
...
**The OQ-613 version half stays open until `W6b-17` builds.** The W6b-16 change set
resolves the slug half of `OQ-604` only. `approximates_model_id` still has no companion
...
WK-664 is the workstream closure will judge it by. §13 rule 1 accepts a closure record that states
why a test is the wrong instrument. It does not accept silence. `FR-396`'s test must assert
...
anyone adds. `NFR-488` stays booked forward as *delivered but untested*. It is a WK-661 bench
from 2026-08-22, not WK-664's. `test_gbm.py:1879` mentions it in a docstring rather than a marker.
