...
**Spec:** [`../specs/07-platform.md`](../specs/07-platform.md) FR-PLAT-48 (the contract must not
drift) and [`../specs/00-overview.md`](../specs/00-overview.md) FR-OVR-6, which the affected tests
...
work. The W32 slice boundaries in [`2026-08-22-w6b-slice-map.md`](2026-08-22-w6b-slice-map.md) are
...
- No new requirement ids and no spec change. This is a test-infrastructure defect: the guard does
  not enforce what FR-PLAT-48 already requires. No `Next free:` marker.
- **The union in `_type_map` was added deliberately** (W5, 2026-08-22) and must not be reverted.
...
- **Out of scope, deliberately, and each for its own reason:**
  - `UNRESOLVED_CONSTRAINT_DISAGREEMENTS` (`:1109`) — FR-MODEL-126 decides it and
...
3. Key `found` by `(arm, path)`. The union at `found[key] = found.get(key, frozenset()) | types`
   **stays** — it now unions within an arm, which is the W5 behaviour the docstring defends, and no
...
carve-out (FR-MODEL-126's, not this one's) and the two schemas with no generated side.
