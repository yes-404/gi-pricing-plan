...
| `objective-certificate.result.checks` — `minItems` 8 against 1 | **Escalated as `OQ-MODEL-30`** — see below |

---

## `OQ-MODEL-30`, and why it was not a fix
...
`ObjectiveCertificate` and `MetricCertificate`, and `FR-MODEL-105` gives a metric certificate
...
- **`OQ-MODEL-30`** — the certificate check floor, maintainer's call.
