...
family: plan
kind: leaf
title: RFC-897 — file taxonomy, reference coding and custody: research, and the slice cut
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-31-nt-0016-investigation.md
---

# RFC-897 — file taxonomy, reference coding and custody: research, and the slice cut
...
**Goal:** Turn RFC-897 from a proposal into an executable cut — establish, by measurement
...
it. A decision gate sits after Slice 3 and gates only RFC-897's Stages 2–5, which this plan
...
(rev 2). **A note decides nothing** — `.claude/rfcs/README.md`. Everything this plan takes
from it is re-verified below or marked as unverified.

**Prior disposition this plan tests rather than inherits:**
[`PL-00900-rfc-895-rfc-898-the-joint-reconciliation-2026-08-30.md`](PL-00900-rfc-895-rfc-898-the-joint-reconciliation-2026-08-30.md) §4,
...
**"Tracked" means the output of `git ls-files`.** RFC-897 §9's own reproduction commands use
...
| Measure | RFC-897 §0, at `7db62ca` | Re-measured at `b551060` | Re-measured at `7db62ca` | Verdict |
|---|---|---|---|---|
...
**(c) RFC-898's `README.md` does not cite the pre-move path — it cites the notes not at all.**
Both RFC-897 §8 and the reconciliation §4 record a dependency in which the landed README
...
destinations — `docs/specs/`, `docs/adr/`, `docs/workflows/`, `docs/findings/register.md`,
...
project's design memory is currently unreachable from it by construction. RFC-897's boundary
question and RFC-898's public-face question are one question, and RFC-898 as landed has
already answered the half it could reach.

### 1a. The citation surface, by mutability class

This is the measurement the sequencing argument actually turns on, and RFC-897 does not take
...
`docs/closures/INDEX.md#plan-reviewsmd`, `docs/findings/register.md`, `docs/process/agent-settings.md`,
...
## 2. Which of RFC-897's seven questions can be ruled now
...
2. **Its premise is already ruled in the public face.** §1(c): RFC-898 landed a front door
   whose tour is `docs/`-only. Every day that stands, the repository's design memory is
   unreachable from the one file a visitor reads first. That is the exposure RFC-898 was
   written about, in the one directory RFC-898 did not reach.
...
| **4** | `.claude/notes/` → `docs/notes/`, living citations, tombstone, CI filter | 1, and Q5/Q6/Q7-notes ruled | Stage 3a; separable, mechanically acceptable, and the half RFC-898 could not reach |

**Decision gate, after Slice 3 and owned by the decision-maker:** rule Q1, Q2, Q3 and Q7's
general half against Slice 3's draft. **Stages 2–5 of RFC-897 are deliberately not scoped by
this plan**, and their trigger is that ruling — the same trigger the reconciliation names.

**What this cut is not.** RFC-897 §8 puts the move first, as `S0` of a five-slice Work,
...
5. **No frozen file is edited.** `git diff --stat <first-merge>..<close-tree> -- docs/plans/`
   ...
   *Corrections after filing* entries. RFC-897 C4 and [`README.md`](README.md).
...
**What this standard deliberately does not cover.** RFC-897 §11's own draft acceptance items
...
- **NFRs and enforcement are proven on deliberately broken input** — `CLAUDE.md` §13, third
  ...
  has not been tested.
- **No new id family is minted, and no `FR-`/`NFR-`/`OQ-`/`ADR-` id is defined** — RFC-897 C2,
  and this plan's own scope.
- **No retro-rename of a cited artifact** — RFC-897 C1. Slice 4 is the single exception the
...
- **Frozen means frozen.** No slice edits a file under `docs/plans/` other than by appending
  to this one's *Corrections after filing* — RFC-897 C4, [`README.md`](README.md).
...
- Create: `docs/audit/file-census-<tree>.csv` and a short `docs/research/RS-00952-file-census-rfc-897-stage-0.md` naming
...
def test_name_pattern_normalises_dates_then_digits() -> None:
    assert name_pattern("PL-00849-wk-671-slice-3-batch-scoring-the-pure-transform-the-checkpointing-handler-and-the-route.md") == "DATE-wN-N-batch-scoring.md"
...
**Files:**
- Create: `docs/research/RS-00953-file-taxonomy-draft-rfc-897-stage-1.md`.
...
- **Decompose the unreferenced plans into RFC-897 §6's verdict 2 versus verdict 4** — a
...
- Create: `.claude/rfcs/README.md` as a tombstone — one paragraph plus the old-to-new
  mapping.
...
- [ ] **Step 6: Write the tombstone at `.claude/rfcs/README.md`.**
...
1. `git grep -l "\.claude/notes" -- .` returns exactly `.claude/rfcs/README.md` plus files
   under `docs/plans/`, and nothing else.
2. `git log --follow docs/rfcs/RFC-00711-plan-review-at-each-phase-boundary.md | tail -1` shows the
...
**Coverage against RFC-897.** Stage 0 → Slice 2. Stage 1 → Slice 3. Stage 3a → Slice 4,
...
identifier; it moves no file. It does not decide whether RFC-897 is adopted — that is the
...
`scripts/audit-docs.py` check 28 landed in `26de823` (PR #510, RFC-895 adoption slice F) with
...
citation to one untouched. It is the same device `.claude/rfcs/0016-…` uses for its own §3a,
...
[`../rulings/RL-00948-q7-notes-half-only-cite-notes-by-nt-00nn-id-path-citations-are-rejected-for-this-one-category.md`](../rulings/RL-00948-q7-notes-half-only-cite-notes-by-nt-00nn-id-path-citations-are-rejected-for-this-one-category.md)
...
[`PL-00900-rfc-895-rfc-898-the-joint-reconciliation-2026-08-30.md`](PL-00900-rfc-895-rfc-898-the-joint-reconciliation-2026-08-30.md),
...
move are fixed. Nothing in §9 or RL-947 is superseded.**

§9's Step 6 and RL-947 (`../rulings/RL-00947-q6-tombstone-form-is-a-readme-mapping-symlinks-are-rejected.md`) both describe
the tombstone as one file — `.claude/rfcs/README.md`, one paragraph plus the old-to-new
...
individual note by its old path (e.g. `../../.claude/rfcs/0007-....md`) are markdown links
...
note** (18 files, `.claude/rfcs/NNNN-....md`, each pointing at its `docs/notes/` equivalent
...
too). This is not the symlink RL-947 rejects: the ruling's own reasoning is that
...
- `docs/rfcs/README.md` (moved, unedited in content) had three table rows (RFC-842,
  RFC-843, RFC-895) missing a trailing `|`, invisible to check 22's table-cell-count check
...
- Editing `docs/process/delivery-process.md`'s one literal `.claude/notes` reference (a
  ...
  right price for a forced re-read" cost RL-905 accepts by design for check 27. Reconciled
...
adds one execution detail (the stub files) RL-947 did not need to rule on because Q6 never
