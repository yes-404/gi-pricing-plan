...
**Goal:** Deliver FR-DATA-50 and FR-DATA-51 — the dataset list's status badge, last-validated
...
FR-DATA-50's own instruction and the reason it gives: a status on `Dataset` would be a second
...
**Spec:**
- [`../specs/01-data-management.md`](../specs/01-data-management.md) — FR-DATA-50 (line 117)
  defines both derived fields, their differing scopes and the "one further aggregate" budget;
  FR-DATA-51 (line 208) defines the owner and who may change it; §5.1 is the endpoint table;
...
- [`2026-08-22-w6b-slice-map.md`](2026-08-22-w6b-slice-map.md) — this slice's row. W6b-3
...
- **`model-schema` is the single source of truth** (§2, ADR-0002). The three new fields are
  defined on `Dataset` there and nowhere else; the frontend's types are generated.
- **Requirement IDs are permanent** (§5). **This slice allocates none** — FR-DATA-50 and
  FR-DATA-51 already exist, were appended 2026-08-19, and say "Not delivered". This plan
...
FR-DATA-50 and FR-DATA-51 are two of the five DATA requirements it reports with **no
...
| `docs/specs/01-data-management.md` | Modify FR-DATA-50, FR-DATA-51, §5.1 | Delivery recorded, the route published |
...
**Why three fields and not two.** FR-DATA-50 requires that "where the two refer to different
...
git commit -m "feat(w32-3): Dataset carries FR-DATA-50's three derived fields"
```
...
- Produces:
  ...
  - `_last_validated(session, dataset_ids) -> dict[UUID, tuple[int, datetime]]` — the one
    further aggregate FR-DATA-50 budgets for.
...
Only *currently* `validated` versions count. FR-DATA-23 and FR-DATA-43 move a version that
...
the capped count, and `_latest_versions`. FR-DATA-50 budgets "one further aggregate", so the
...
`FR-DATA-2` and is the test to build on; leave its marker alone and add three beside it. Use
...
**The invariant this creates, and the one refusal it needs.** FR-DATA-51 makes `owner_id`
...
**Why a route is needed at all.** FR-DATA-51 says the owner is "changeable only by Admin or
...
def test_the_reassignment_is_audited_with_both_owners(api_client) -> None:
    """`06` R2 and FR-DATA-51's "audited as a metadata change". `before` must carry the
...
async def patch_dataset_owner(...) -> Dataset:
    ...
    next metadata field FR-DATA-51's sibling requirements add has somewhere to go.
    """
```
...
**Files:**
- Modify: `docs/specs/01-data-management.md` — FR-DATA-50, FR-DATA-51
...
1. **FR-DATA-50 said "Not delivered. Phase 1b, owner W6b"** and predicted the shape of the work
...
2. **FR-DATA-51 said the same**, and is delivered including the change path it implies. Its
...
3. **FR-DATA-52 is unevidenced *by design*.** `scope-audit.py DATA` lists it beside the two
   ...
   NFR-DATA-1 and NFR-DATA-2 are budgets needing a measurement, not a marker; §13 rule 5 makes
   that a `bench-data.py` run, and it is **not this slice's**.

- [ ] **Step 1: Append delivery notes to both requirements**

To FR-DATA-50's row in `docs/specs/01-data-management.md`, append — as part of the same single
...
```
FR-DATA-50 and FR-DATA-51 delivered 2026-08-23 (W32-3). FR-DATA-50 landed as three fields
rather than two — the requirement's "states which" clause needs the version beside the date.

**Still open, and deliberately so:** FR-DATA-52 is a decided deferral with no owner, by its own
...
**Still open, and owned elsewhere:** NFR-DATA-1 and NFR-DATA-2 are budgets. §13 rule 5 makes
...
Expected: the first two PASS; `scope-audit.py DATA` no longer lists FR-DATA-50 or FR-DATA-51
as unevidenced, and still lists FR-DATA-52, NFR-DATA-1 and NFR-DATA-2 — which is correct and is
...
git commit -m "docs(w32-3): FR-DATA-50 and FR-DATA-51 delivered, with what changed"
```
...
- [ ] `scope-audit.py DATA` no longer reports FR-DATA-50 or FR-DATA-51 without evidence.
...
- [ ] FR-DATA-52's deliberate deferral is written down, so it is not re-audited as a gap.
- [ ] The branch is pushed and a PR is open. Do not force-push, do not merge, do not push to
      `main`.

## Self-Review

**1. Spec coverage.** FR-DATA-50's five clauses each map to a task: the two derived fields
...
no route that sets them). FR-DATA-51's three clauses map to Task 3 (non-null, set at ingestion)
