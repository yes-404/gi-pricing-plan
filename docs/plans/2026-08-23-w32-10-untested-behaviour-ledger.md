...
What executing
[`2026-08-23-w32-10-untested-behaviour.md`](2026-08-23-w32-10-untested-behaviour.md) actually
...
[closure proposal](2026-08-23-w32-closure-proposal.md) gives: it touches nothing the other four
...
`backend/tests/test_migration_dataset_owner.py`, 8 tests, all marked `FR-DATA-51`. **`8 passed
...
`test_an_ebm_scores_over_http` (`FR-MODEL-124`, `FR-MODEL-37`, `FR-MODEL-62`) and
`test_the_ebm_refusal_reaches_the_client_by_name` (`FR-MODEL-124`, `FR-MODEL-62`). **`18 passed,
...
EBM's fit result *is* its model (ADR-0003). There is no artifact fetch on this arm, so the two
...
backends, markers `FR-MODEL-118` + `FR-MODEL-125` stacked. **`FR-MODEL-52` was not added**, per
