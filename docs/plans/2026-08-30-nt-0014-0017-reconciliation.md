...
## 8. Appended 2026-09-01 — what landed under the maintainer's delegation, and the revised NT-0016 disposition
...
[`2026-08-31-nt-0016-investigation.md`](2026-08-31-nt-0016-investigation.md) uses for its own
...
| Slice 2 — Stage 0 | #537 | `4f95fb3` | `scripts/file-census.py` plus tests, the committed census `docs/audit/file-census-5ef559d.csv`, and its companion `docs/audit/file-census.md` |
| Slice 3 — Stage 1 | #545 | `9e70469` | `docs/audit/file-taxonomy-draft.md` — the taxonomy draft clustered from the census; rules nothing |
...
| Ruling 55 — Q4 | #535 | `c0d1712` | The ownership matrix lands as a living file in `docs/process/`; a frozen note appendix is rejected |
| Ruling 56 — Q5 | #535 | `c0d1712` | Notes destination is `docs/notes/`, keeping the family's name and README; folding into an existing `docs/` family is rejected |
| Ruling 57 — Q6 | #535 | `c0d1712` | Tombstone form is a README mapping; a symlink is rejected |
| Ruling 58 — Q7, notes half | #535 | `c0d1712` | Notes are cited by `NT-00NN` id, resolved via the notes index; new path citations are rejected for this category |
| Ruling 59 | #539 | `7f1e3c6` | The census CSV is outside FR-DATA-32's scope; the test gains a second carve-out bought by provable reproducibility — implemented in PR #541 (`15eb633`) |
| Ruling 60 | #542 | `df0a430` | Ruling 59's fetch path is broken against github.com; resolved by `fetch-depth: 0` — implemented in PR #543 (`4251501`) |
| Ruling 61 | #546 | `052afe3` | Ruling 57's tombstone gains 18 per-file stubs, watched by a new check 30 |
| Ruling 62 — Q1 | #547 | `43fd277` | The twelve-category set is ruled **amended**: closure/audit record gains its third home; the two filename-grammar splits go to Stage 2 as named items |
| Ruling 63 — Q2 | #547 | `43fd277` | Partial split: rulings and ledgers stay in `docs/plans/` under filename grammar; closure/audit records keep their three homes; register + findings keep their two |
| Ruling 64 — Q3 | #547 | `43fd277` | Verdict-4 status is **derived** from an existing closure record wherever one covers the file; an explicit declaration is required only for the residual |
| Ruling 65 — Q7, general half | #547 | `43fd277` | Mixed citation grammar: categories with an id family cite by id; the six id-less categories cite by dated filename — prospective only, no frozen retrofit |

### 8.2 The revised NT-0016 disposition

§4 above recommended **ADOPT STAGES 0–1 ONLY, and defer Stages 2–5**, with the trigger §6
names in its own words: *"NT-0016's Stages 2–5 are the only deferred item, and its trigger is
...
- **The named trigger is ruled.** Rulings 62–65
  ([`2026-09-01-nt-0016-q1-q2-q3-q7-general-rulings.md`](2026-09-01-nt-0016-q1-q2-q3-q7-general-rulings.md),
  ...
  trigger the plan names for NT-0016's Stages 2–5."* Combined with Rulings 55–58, all seven of
  NT-0016's §10 questions are now ruled — the gate record's *What this record's ruling now
...
- **The evidence the deferral was waiting for exists.** The census is committed and
  byte-reproducible (`docs/audit/file-census-5ef559d.csv`, corpus rule and row count stated in
  `docs/audit/file-census.md`), and the taxonomy draft (`docs/audit/file-taxonomy-draft.md`) is
  what Rulings 62–65 rule against.
- **Nothing remains to research.** Stages 2–5 are build work whose inputs are all fixed: the
  twelve-category set as amended (Ruling 62), the per-category home rule (Ruling 63), the
  verdict-4 derivation mechanism (Ruling 64), the mixed citation grammar (Ruling 65), and the
...
[`2026-09-01-nt-0016-landing-package.md`](2026-09-01-nt-0016-landing-package.md) §2 — kept out
...
filed. Their *state* has moved since `b749acb`: NT-0014's remainder (Slices E, F, G) and
NT-0015's P1–P5 have since landed — `docs/roadmap.md`'s pending-proposals table records both
...
new work. NT-0017's residue (impact rows 6 and 9) is unchanged: no Work row exists and the
...
rather than authorising it.* NT-0016 is now precisely that shape: the four slices and eleven
rulings above landed under the dated 2026-09-01 delegation, and §8.1 records them rather than
authorising them. That is the second instance of the exception's delegation limb (NT-0014's
Slices A–D being the first; NT-0017's S1/S2 the light-path limb), which is the strongest
...
**Maintainer acceptance: accepted as proposed, 2026-09-01 — all four dispositions (NT-0014
remainder E/F/G, NT-0015 P1–P5, NT-0016 Stages 2–5 as one Work row, NT-0017 residue) and §7's
clause-2 exception. NT-0016's Work row is placed in Phase 2 beside W12. Plan reviews 9, 10 and
...
rows in `docs/roadmap.md` §7 (clause 2) — NT-0016's row is the landing package's §2 draft at
its ruled placement (Phase 2, beside W12) — and applies the clause-2 exception as one clause
addition. NT-0017's impact-row-6 settings line remains the maintainer's to write. The four
