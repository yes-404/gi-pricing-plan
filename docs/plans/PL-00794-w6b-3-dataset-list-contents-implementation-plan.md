...
family: plan
kind: leaf
title: W6b-3 — Dataset List Contents Implementation Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-25-w6b-3-dataset-list-contents.md
---
...
**Map row:** [`PL-00786-wk-664-the-revised-slice-map.md`](PL-00786-wk-664-the-revised-slice-map.md):151 —
...
**Highest ids in use: `OQ-551`, `OQ-570`, `FR-58`. Next free: `OQ-552`.**
...
- **`docs/roadmap.md` is not edited by this plan** — not its WK-664 row, not §6. A slice close is
...
It is **not** the ownership *editor*. FR-82 delivered `PATCH /api/v1/datasets/{dataset_id}`
as the Admin-or-owner change path; no requirement asks the *list* to offer it, the rendering
half of FR-82 is the single clause "it is what §5.3's owner column displays", and putting
...
**`01` §5.3's Contents cell does not bind.** FR-24 (`00-overview.md`:227) made a §5.3
...
than assuming, because FR-24 warns that an id-matching sweep reports all seven green. So
...
which a cell-scoped sweep does not reach", and FR-24 says each discharge "falls due at
...
|---|---|
| **`FR-55`** (`01`:117) | Two columns — a status badge from `latest_version_status`, a last-validated date from `last_validated_at` — plus a **rendering rule**: "where the two refer to different versions the list states which, so the pair cannot be read as one fact." |
| **`FR-82`** (`01`:212) | One column: "it is what §5.3's owner column displays." |
| **`NFR-463`** (`00`) | The SPA meets WCAG 2.2 AA. No chart is added, so the tabular-equivalent half is not engaged; the badge and date must still be readable without colour. |
...
(FR-55)". So the view never has to render a date without its version or a version without
...
`owner_id` is a **uuid**, required and non-null. FR-82 says the column displays it, and
...
uuid; the gap is filed as OQ-552.** Decision 1 covers only the presentation of the uuid.
...
FR-55's "where the two refer to different versions the list states which" is a **view**
...
because it is the one clause in FR-55 a reviewer looking at three new columns would not
think to check.
...
- **`01`:826** — `GET /api/v1/datasets` is described as returning "`latest_version`,
  `latest_version_status` and `last_validated_at` (FR-55)". It omits
  **`last_validated_version`**, which `api/datasets.py`:339 passes, `platform/datasets.py`:394
  populates, the published contract declares (`schema.d.ts`:3383), and FR-55's own dated
...
**Which side is wrong is not in doubt** (§0 requires this be asked, not assumed): FR-55's
...
- **`title` alone fails WCAG 2.2 SC 1.4.13** (Content on Hover or Focus), which requires such
  ...
  those, and it is unreachable by keyboard and by touch entirely. `NFR-463` binds this SPA
...
FR-55 requires disclosure only "where the two refer to different versions". Two readings:
...
1. **OQ-551's currency defect does not reach this view.** `DatasetListView` renders
   `dataset.currency` from the Dataset object itself (`:135-137`), not a prop — correct here,
   and the positive control for OQ-551's claim that the value exists and is simply not
   threaded to `ProfileView`/`VersionDetailView`. Neither fixed nor worsened.
2. **No chart is added**, so NFR-463's tabular-equivalent half is not engaged. The list is
...
4. **F6's `put_dictionary` gap is reassigned to the backend — touched, not resolved.** The lead
   ...
   would be its own defect (FR-55)" — which is the detail route explaining why it passes
   ...
   **The tracking reference is unresolved and is deliberately not written here.** It was given
   ...
   resolves to the merged WK-661 pull request "feat(w5): the GLM spine". *Limit of my evidence:*
...
docs/
  ...
  open-questions.md                        OQ-552 (+ 00-overview.md §10 mirror)
```
...
### Task 2 — the badge and last-validated columns (FR-55, Decision 3)
...
This is the hazard `cellUnder` was built for in W6b-9 and filed as OQ-550 — a positional
...
- names the version when the last validated one is not the latest — FR-55's own
...
### Task 3 — the owner column (FR-82, Decision 1)
...
### Task 4 — the §5.1 amendment (F5) and OQ-552, both mirrors
...
rewrite (§0). Then OQ-552 in `docs/open-questions.md` **and** `00-overview.md` §10: how does
...
1. **If the maintainer reads FR-82's "displays" as requiring a name, not an id.** Then the
   uuid column is not a step toward the requirement but a failure to meet it, and the slice is
   blocked on the backend work OQ-552 proposes rather than shipping. The lead has ruled the
   other way; OQ-552 is where the maintainer can overturn it.
2. ~~**If `failed` is unreachable at the *latest* version.**~~ **Struck 2026-08-25, settled by
   ...
   to model on.
3. **If `01` §5.3's Contents cell is later declared exhaustive.** FR-24 lets a cell declare
...
the lead's, and both are pointed at OQ-552, which is where an overturn would land. A
