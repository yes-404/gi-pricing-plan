# W11 Task 4B — what `environment` on a sampled scoring trace means, and how it is derived (2026-08-30)
...
42–43 in
[`2026-08-30-w11-reopen-scope-and-batch-frame-contract-rulings.md`](2026-08-30-w11-reopen-scope-and-batch-frame-contract-rulings.md).

**Mints no `FR-`/`NFR-`/`OQ-` id.** FR-RATE-42 takes a dated clarification in place — the
shape Ruling 24 used for FR-RATE-38 and Ruling 25 for FR-RATE-42 itself. No other spec is
...
## Ruling 44 — the field is the environment the quote was served in, the spec already says so, and the branch does not merge until it says so too
...
environment whose live Rating Version FR-RATE-34 selects, and against which FR-PLAT-30
...
- [`../specs/00-overview.md`](../specs/00-overview.md)`:261` — the entity map reads
  ...
  is a Deployment.**
- FR-RATE-51 ([`../specs/03-rating-engine.md`](../specs/03-rating-engine.md)`:194`) —
...
- [`2026-08-29-w11-slices-3-4-rulings.md`](2026-08-29-w11-slices-3-4-rulings.md) Ruling 23
  — Deployment is W14's, so *"the row carries the parent it can resolve — the rating version
  reference, the bundle hash, and the environment as a plain string — gaining the Deployment
  reference in W14"*. The string **is** the deferred parent, held in the only form available
  before `Deployment` exists.

So the field's value must be something a W14 migration can reconcile to the Deployment that
...
FR-PLAT-30 ([`../specs/07-platform.md`](../specs/07-platform.md)`:141`) closes the mechanism
question the same way, from the credential end: *"A `uat` key can never score against
`prod`."* A **key** has one environment; FR-PLAT-3 (`07:73`) grants an **account** *"named
...
Ruling 14 ([`2026-08-29-w11-slice2-rulings.md`](2026-08-29-w11-slice2-rulings.md)`:49`) for
*"the target environment of a call [...] already derivable"*. Read to the sentence that
carries the claim: Ruling 14 is about resolving the **live Rating Version**, and that phrase
...
W14. It never reaches a one-element choice, because nothing in Slice 2 needed one. The
auditor's reading is the correct one; the executor's is not, and the ruling text is
unambiguous enough that no amendment to Ruling 14 is owed.
...
([`2026-08-29-w11-4-trace-sampling-persistence.md`](2026-08-29-w11-4-trace-sampling-persistence.md)`:104-114`)
...
- Migration `835988d1de4c:77-79` revokes `UPDATE` on `scoring_traces`, which is the design
  intent even though **F53** ([`../audit/register.md`](../audit/register.md)) correctly
...
retention-locked, uncorrectable entry in the record `05-monitoring.md` consumes and that W14
...
(`backend/src/app/platform/rbac.py:159`, FR-GOV-6), and there is no roles API in
...
two places: FR-PLAT-3 (`07:73`) scopes keys to *"named environments"*, plural, and FR-PLAT-30
...
granted environments are unreachable, and FR-PLAT-30's check cannot fire against any
...
- **`03-rating-engine.md` FR-RATE-42 takes a dated clarification in place**, in this
