...
**Plan:** [`2026-08-23-w32-9-transparency-exposure-share.md`](2026-08-23-w32-9-transparency-exposure-share.md),
...
every marker names one that already existed (FR-MODEL-36, FR-MODEL-79).

## What moved, per site

FR-MODEL-36's share appears in three places. **Two moved:**
...
FR-MODEL-79's `ShapInteraction.exposure_share` was **withdrawn, not computed**. It was the literal
`1.0` at its only construction site, so there was nothing to make correct; OQ-MODEL-31 decided on
...
**FR-MODEL-128 is left unbuilt.** The out-of-sample evidence that replaces the withdrawn field —
a holdout strength ratio — was appended by commit `b019070` with OQ-MODEL-31 as its origin and is
not this slice's scope. Between the withdrawal landing and FR-MODEL-128 being built, an interaction
...
deliberately differs from the FR-MODEL-125 precedent it otherwise mirrors: `02` §3.6 approximates
...
`b019070`, the same commit that appended FR-MODEL-128 at `:232` marked *(appended 2026-08-23,
OQ-MODEL-31)*. It is a dated, owned forward declaration of a function this slice is not building.
...
`docs/specs/02-modelling.md:194` — **FR-MODEL-79**. Its 2026-08-23 amendment already withdrew the
exposure-share clause and stated that *"Removing the constant field from `ShapInteraction` is
**W32**'s, and until it lands the artifact publishes a number that means nothing."* It has now
...
This slice makes the number behind FR-MODEL-36 mean what the requirement says rather than adding
a marker.
