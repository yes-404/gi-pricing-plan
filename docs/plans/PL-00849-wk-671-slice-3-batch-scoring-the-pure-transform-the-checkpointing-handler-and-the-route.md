...
family: plan
kind: leaf
title: WK-671 Slice 3 — batch scoring: the pure transform, the checkpointing handler, and the route
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-29-w11-3-batch-scoring.md
---

# WK-671 Slice 3 — batch scoring: the pure transform, the checkpointing handler, and the route
...
**Architecture:** Four tasks, split along the line RL-857 draws through the middle of this
...
the final parquet. **3C** is the route. **3D** measures NFR-493. The 3A/3B split is not a
convenience: ADR-703 makes it structural, and `lint-imports` enforces it.
...
storage outside it** for chunk parts (RL-857 §4).
...
shared-tail passage, `:676-690`), §9 (NFR-493). Platform: FR-400 (progress), FR-401
(cancellation), FR-410 (Job parameters retained), FR-420 (blob GC), FR-446/448
(settings), FR-414 (key release on failure).

**Slice source:** [`PL-00854-wk-671-scoring-sequenced-slice-plan.md`](PL-00854-wk-671-scoring-sequenced-slice-plan.md), Slice 3. **That file
...
[`../rulings/RL-00858-03-5-2-s-identical-step-evaluator-names-an-artifact-that-does-not-exist-the-spec-is-wrong-the-code-is-right.md`](../rulings/RL-00858-03-5-2-s-identical-step-evaluator-names-an-artifact-that-does-not-exist-the-spec-is-wrong-the-code-is-right.md)
Rulings **31** (D6) and **32** (§5.2's false shared-evaluator claim);
[`../rulings/RL-00890-d5-fr-258-259-are-not-silent-about-batch-and-no-open-question-is-raised.md`](../rulings/RL-00890-d5-fr-258-259-are-not-silent-about-batch-and-no-open-question-is-raised.md) Rulings **24**
(abort threshold) and **25** (no batch sampling);
[`../rulings/RL-00868-score-one-s-real-time-path-async-evaluate-not-evaluate-executor-offload-and-whether-5-2-s-sync-convention-is-itself-the-defect.md`](../rulings/RL-00868-score-one-s-real-time-path-async-evaluate-not-evaluate-executor-offload-and-whether-5-2-s-sync-convention-is-itself-the-defect.md) Ruling **5**
(`score_batch` stays `def`); [`../rulings/RL-00887-finding-3-the-framing-was-wrong-and-the-class-is-bigger-and-mostly-benign.md`](../rulings/RL-00887-finding-3-the-framing-was-wrong-and-the-class-is-bigger-and-mostly-benign.md)
Ruling **18**, whose answer this slice reuses — see C3; and
[`../rulings/RL-00871-no-8-stands-unamended-and-unexcepted-and-the-test-the-question-proposed-is-the-wrong-one.md`](../rulings/RL-00871-no-8-stands-unamended-and-unexcepted-and-the-test-the-question-proposed-is-the-wrong-one.md)
Ruling **33**, which governs when this slice may be built relative to Slices 2 and 4.

**One correction to a ruling was filed from here** — F-W11-3-1, against RL-857 §6's precedent
...
The source is the **addendum to RL-857** (`fb56dc6`, PR #424), filed in response to this
plan's F-W11-3-1. **RL-857's decision is unchanged; two of its citations were wrong, and so
...
**Error 2 — the guard was attributed one level too high, and this plan inherited it.** RL-857
...
directly — which *corroborates* RL-857 §1 rather than undermining it. It is now recorded in
*Verified facts* as scope: **Task 3B Step 1 writes a new test shape, it does not follow one.**

*Not carried into this section:* RL-863 (`fa2257c`) lands on Slice 4's design, not this one.
...
1. **FR-254's byte-identity, proven through the shared tail.** `score_batch` and `score_one`
...
3. **The checkpoint is keyed on content, not on the Job.** The manifest key is the compiled
   ...
   `JobRow.progress` as the ledger** — RL-857 §7.
4. **The abort threshold behaves in both directions** (RL-889): a batch request carrying a
...
5. **Errors are typed per category and do not abort individually** (FR-255): the run reports
   counts and samples per error type, and one bad row does not end the run below the threshold.
6. **No batch trace enters the production stream** (RL-890): `score_batch` takes no sampling
   parameter, and nothing this slice writes is returned by `GET /api/v1/traces`.
7. **NFR-493 is measured** — ≥ 1 M risks/hour/worker — and written into a dated note under
...
WK-671** — see the coverage table's FR-254 row, which is the single most important line in this
plan.
...
- **`score_batch` holds no durable state and stays plain `def`** (Rulings 31 §3 and 5). It takes
  ...
  resume point**, and `.importlinter`'s `core-has-no-infrastructure` (ADR-703/DEP-3) makes that
...
- **The handler owns everything durable**: manifest, scratch, skip, threshold, output.
  FR-400 already allocates this — *"`pricing-core` reports through the injected
  `ProgressCallback` (ADR-703); the worker translates it into Job state."*
- **Chunk parts are scratch, written outside the content-addressed store** (RL-857 §4), and
...
RL-858 corrected `03` §5.2 in place. The spec now reads: *"`score_one` and `score_batch`
...
**This is the whole of what FR-254's byte-identity means here.** `score_one` reaches the
...
One `bundle`. **FR-253 asks for *"one or more Rating Versions"*** — so multiple versions is
...
- **`ScoringResult` has no `quote_id`.** RL-857 §3 establishes it: the field lives only on the
  optional input `QuoteContext` and the optional `Trace` sub-object, in both
  `docs/contracts/schemas/scoring.schema.json` and `model_schema.scoring`. FR-253 requires
...
- **Nothing re-runs a crashed Job.** RL-857 §1's behaviour, with its attribution corrected by
...
| `Permission.SCORE_BATCH` | Exists — `model_schema/permissions.py:59`. **Granted by no builtin role, deliberately** (FR-347), asserted by `backend/tests/test_rbac.py:101-107` |
| `POST /api/v1/score/batch` | No route. Specified at `03` §5.1:517 as **202** → Job |
| `rating.batch_abort_failure_rate` | Does not exist. RL-889 rules the name and the shape |
...
| `JobProgress.update()` | **Silently drops writes inside a one-second throttle** (`backend/src/app/worker/progress.py`, `_MIN_WRITE_INTERVAL_S: Final = 1.0`), and nothing reads a Job's prior progress back. Forward-only, lossy by design — **excluded as a ledger candidate** (RL-857 §7) |
...
FR-401** — it is not an extra you have to invent.
...
| `NFR-502/501 (F-W9-1)` | Slice 1's and Slice 2's halves. Not this slice's |
| `Error codes across the spec/code boundary (F29)` | Open, owned. This slice registers no new code |
| `03 rating shapes vs hand-authored contracts (F27)` | Owner decided (RL-860): the §14 review at WK-671's close. **Bears indirectly** — if the batch output schema gains a published contract it joins that comparison |
| `RL-882's acceptance-test premise (F32)` | Resolved; Slice 2's. Not this slice's |
| `FR-224 (F-W9-2)`, `FR-240 (F-W9-3)` | WK-673's and Task 1.2's. Not this slice's |
| The six WK-670 rows, `F26`, `F28`, `F30`, `F31`, `F33` | WK-670's, WK-675's, or process rows. Not this slice's |
...
instruction"*), and RL-858 explicitly declines to edit it for the same reason.

**C1 — "calling Task 1.4's identical per-step evaluator" names something that does not exist.**
RL-858: the shared thing is `build_scoring_result`, the post-evaluation tail. The map carries
the same phrasing the spec did, at
[`PL-00854-wk-671-scoring-sequenced-slice-plan.md`](PL-00854-wk-671-scoring-sequenced-slice-plan.md):503; the spec was corrected and the map
...
**C3 — "grant `Permission.SCORE_BATCH`" is the same trap RL-884 dissolved for Slice 2, and it
would turn a passing test red.** `test_rbac.py:101-107`, marked `@pytest.mark.req("FR-347")`,
...
test that only checks the final output — which passes under full restart. RL-857 §6 supplies
...
[`PL-00848-wk-671-slice-3-still-held-on-one-unruled-decision-and-d6-the-decision-that-releases-it.md`](PL-00848-wk-671-slice-3-still-held-on-one-unruled-decision-and-d6-the-decision-that-releases-it.md)
offered (a) full-restart as the cheap baseline that *"matches the generic Job pattern exactly"*.
RL-857 §1: the generic pattern does not restart a crashed Job, it **strands** one — *"(a) is
not the cheap option; it is not an option at all."* And that document called (c)'s row key *"not
yet guaranteed unique"*; RL-857 §3 found it **does not exist at all**. Both are recorded
...
**F-W11-3-1 — RESOLVED 2026-08-29 by the addendum to RL-857 (`fb56dc6`, PR #424).** Accepted
...
**F-W11-3-1 as filed — RL-857 §6's precedent citation is inaccurate, and following it
...
|---|---|---|---|
| FR-253 | §3.7 | 3A, 3B, 3C | 202 → Job; a Dataset Version re-rated against **one or more** Rating Versions; content-addressed parquet carrying the quote key (from the input frame — it is not on `ScoringResult`), ladder and selected outputs |
| FR-254 — *chunked*, *progress-reporting*, *identical code path* | §3.7 | 3A, 3B | Byte-identity through `build_scoring_result`; chunking at `chunk_rows`; progress through the injected callback |
| FR-254 — *resumable* | §3.7 | 3B, **partially** | **Deferred with an owner at the WK-671 close — not delivered.** See below |
| FR-255 (batch half) | §3.7 | 3B | Counts and samples per error type; no individual abort below the threshold; threshold behaviour in both directions (RL-889) |
| NFR-493 | §9 | 3D | ≥ 1 M risks/hour/worker, measured with its shape recorded |

**FR-254's `resumable` clause is not fully discharged by this slice, and booking it as
delivered would put a guarantee in the roadmap that the repository does not have.** RL-857's
...
is `07` FR-403 and FR-414, owned by *"whichever workstream builds FR-413"*, and
...
The age-based sweep for orphaned scratch (RL-857 §4) belongs with the same work.

**Deliberately excluded, each with its owner:** FR-RATE-34, 35, 40 → Slice 2. FR-RATE-39, 41,
63, 64, 65 → Slice 1 (shipped). FR-259 → Slice 4. FR-RATE-43, 44, 45 → WK-672. FR-263–49 →
WK-673. FR-267, FR-428 → WK-674. FR-403, FR-413, FR-414 → whichever workstream
builds FR-413. NFR-RATE-1, 9, 13 → Slice 2. NFR-500 → Slice 4. NFR-RATE-2, 3, 4, 7, 8,
14 → Slice 1 (measured).
...
| 3D — NFR-493 | 3B | — | Measure the handler, not the route |
...
- **Slice 2 and Slice 4 have filed plans and may land in any order relative to this one — but
  not at the same time as it.** RL-871 (`../rulings/RL-00871-no-8-stands-unamended-and-unexcepted-and-the-test-the-question-proposed-is-the-wrong-one.md`) rules
  that *"two Slices of the same Work may not be built concurrently. §8 is not excepted for WK-671"*,
...
- Slice 4's Task 4C already writes the test that keeps batch traces out of the production stream
  (RL-890), so if Slice 4 lands first, acceptance criterion 6 is partly discharged there —
...
sampling rate (RL-890) or an abort threshold** — all of those are 3B's, and three of the five
...
- [ ] **Step 1: Write the byte-identity test first.** It is the slice's headline criterion, and
      ...
      serialised ladders are **byte-identical**. Mark `@pytest.mark.req("FR-254")`.
...
- [ ] **Step 4: Write the cancellation test** (FR-401): a callback signalling cancellation
...
- [ ] **Step 5: Write the purity test — the structural override condition, made behavioural.**
      ...
      import half; this covers the behavioural half. Mark `@pytest.mark.req("FR-254")`.
- [ ] **Step 6: Run all four, confirm each fails for its own cause, then implement.** Evaluate
      ...
      `build_scoring_result` — the identical tail (RL-858). Do not reimplement any of
...
- [ ] **Step 8: Commit.** `feat(rating): score_batch as a pure chunked transform (WK-671 Task 3A)`
...
`ProgressCallback` plumbing per FR-400.

**The manifest, exactly as ruled.** Key: **the compiled bundle's content hash (FR-239), the
...
the Job id** (RL-857 §2) — a fresh Job would not find a Job-keyed manifest and would re-score
everything, which is full restart wearing resume's name.

**Not decided by the ruling, and yours to choose** (RL-857 §7): the manifest's storage shape
...
**The threshold, exactly as ruled** (RL-889): `rating.batch_abort_failure_rate` follows the
...
- [ ] **Step 1: Write the resumability test first. Call the handler function itself.** Invoke it;
      ...
      **Do not route either call through `execute_job`**, whose `QUEUED` guard would refuse the
      second. This is RL-857 §6 as its addendum restates it, and it is **a new test shape for
...
- [ ] **Step 2: Run it and read the failure carefully.** Expected: the handler does not exist —
      ...
      handler, and both are the failure RL-857 §6 predicts: *"An executor who concludes the
...
- [ ] **Step 3: Write the two threshold tests.** A request whose argument is **above** the
      ...
      abort, counts still accrue. Mark `@pytest.mark.req("FR-255")`.
- [ ] **Step 4: Write the error-typing test** (FR-255): a frame containing rows that trigger
...
- [ ] **Step 5: Write the scratch-lifecycle test.** Chunk parts are written **outside** the
      ...
      per chunk. This is the test that keeps RL-857 §4 true.
- [ ] **Step 6: Run all of them, confirm the causes, then implement the handler.** Loop bundles
      for FR-253's *"one or more Rating Versions"*; carry the quote key through from the
...
trigger is a finding already filed for the lead (RL-857's closing finding) — **do not build a
...
- [ ] **Step 2: Write the RBAC test — three cases**, exactly as RL-884 established for
...
table with its section and task, listed individually, and FR-254 is **split across two rows**
...
error RL-857 §6 says makes an executor conclude the criterion is untestable; and 3C Step 3
...
FR-254 register row is the lead's to write at the close. **F-W11-3-1 corrected a ruling's
...
merely not-yet-unique; RL-857 found the first strands a crashed Job rather than restarting it,
