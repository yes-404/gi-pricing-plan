...
family: plan
kind: leaf
title: W37-6 row (h) — the named H rows, with the parser failure each one is named by (2026-09-03)
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-09-03-w37-6-row-h-the-named-h-rows.md
---

# W37-6 row (h) — the named H rows, with the parser failure each one is named by (2026-09-03)

**Filed** 2026-09-03 by the executor. **What this is.** RL-1043 §3 quotes the maintainer's
...
RL-1043 §3: *"which H rows those are is a list nobody has produced, and inventing it would
...
reading `audit-docs.py`'s own output on it** — not composed from `RFC-937` §5 by reading, and
...
(RL-1043 §1's instrument, `scripts/_docverify.py`), run by the executor in
...
a **denominator**, never an exit code (`RFC-789`).
...
### (1) `RFC-937` §5.5 — `audit-docs.py` · kind **H + M**
...
- **check 14's workflow-coverage** derived the module from `rid.split("-")[1]`, which
  ...
  substring membership test (`rid in wf_text`) also matched `FR-26` inside `FR-38`;
  with a `\b` boundary the control tree's coverage figures drop (DATA 39%→33%, PLAT 18%→13%
  and so on). **This is the same token-boundary class RL-1043 §2 row (g) names, present in
  `audit-docs.py` on the un-migrated tree today.**

### (2) `RFC-937` §5.5 — `req-coverage.py` · kind **H + M**
...
### (3) `RFC-937` §5.2 — `plans/2026-*.md (125) + README` · kind **M + H**, the **H** half
...
### (4) `RFC-937` §5.7 — the fixture tests · kind **H**
...
**Named here rather than silently dropped**, so the boundary is reviewable. RL-1043 §3
...
| §5.5 `register-lint.py` / `register-owed.py` | H + M | Check 29's dependency. Its parser is **not** broken by the migration: once root (1f) is fixed it runs and reports **11 real grammar violations** — rows merged in from `docs/findings/register.md`, which was never held to RL-193's Decision-cell grammar. A wider population, not a blind parser. The 11 belong to whoever merges the two registers. **W37-10.** |
...
source"*, `RFC-937` §1.4, RL-981), which had only ever been enforced for the case where
...
on the **parsed** `header.was` field, not on a substring — so RL-1043 §5's *"the exemption
currently keys on a substring test"* does not describe this site. What is wrong is upstream:
RL-1043 §5 measured that of 393 stamped documents carrying `was:`, **3** carry correct
provenance — 261 name the file's own new path and 129 name a path that never existed. **The
migration is writing the field wrongly**, and RL-1039's verbatim-migration exemption is then
honoured over a field that does not mean what it says.

**Owner: not W37-6's row (h).** It is the migration's `was:` write path plus RL-1043 §5's
...
naming RL-1043 §3 assigns and does not attempt.
...
commit — which RL-1043 §3 assigns to W37-10.
