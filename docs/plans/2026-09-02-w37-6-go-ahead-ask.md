...
### 5.3 Three figures in §4.2 were derived by argument, not measured

**`2446` marker citations rewritten, the `461` extra, and the `422` inside frozen dated plans
reproduce under no command that could be found — and the reason is now known.** This is sharper
than "unreproducible", and the difference matters to how much weight the figures should carry.

**They are not the superseded plan's figures.** That document says **1988** in all three places
the count appears, under a §5.5 that gives one command for it —
`git grep -c 'pytest.mark.req' <rev> -- backend packages` — which reproduces **exactly** at both
pins. It never claimed a repo-wide marker count, so there is no unrecorded command of its
author's to recover. Confirmed by its author, and by the figures being absent from that file
entirely.

**They enter with the superseding plan, and its own wording shows how.** §4.2 reasons: *"The 1988
figure is the `backend`+`packages` count, and step 6 rewrites over `git ls-files`, 'nothing
exempt'."* That is a correct observation about the earlier plan's scope — but the sentence
**derives 2446 from the argument that step 6 is unscoped, rather than from a measurement**. Which
is exactly consistent with no pattern reproducing it: it appears never to have been measured at
all. `git log -S'2446'` finds no commit defining it.

**So the defect is a class, not a bad number.** *A figure produced by reasoning, then presented in
a table of measured values.* Nothing marks it as different from the measured rows around it, so a
reader — including the maintainer this disclosure exists for — has no way to tell which figures
were counted. This is the same class as the superseded plan's own finding 11 against the map
plan's "1355 tracked files": a real count, of the wrong population.

**The closest reproducible pair is 2519 repo-wide and 445 within `docs/plans`**, on a fixed-substring
line count. **The direction of the error is worth stating plainly: 445 > 422, so the stated figure
understates the frozen-plan diff load** that Ruling 68's six-class permitted-diff predicate must
classify inside the commit that cannot be re-run. **The disclosure was conservative, not
optimistic** — which is the safe direction to be wrong in, and is why this is a defect in the
record's honesty about its own provenance rather than a risk to the run.

**Recommendation for these three: read them as an estimate, and treat 2519 / 445 as the
measurement.** They should not be struck — the argument behind 2446 is sound and step 6 genuinely
is unscoped — but they should not sit in a measured table unlabelled either.
