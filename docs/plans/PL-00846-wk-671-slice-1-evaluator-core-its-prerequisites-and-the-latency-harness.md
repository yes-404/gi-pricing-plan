...
family: plan
kind: leaf
title: WK-671 Slice 1 — Evaluator Core, its Prerequisites, and the Latency Harness
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-29-w11-1-evaluator-core.md
---

# WK-671 Slice 1 — Evaluator Core, its Prerequisites, and the Latency Harness
...
**Goal:** Build `score_one` — the single evaluator that FR-254 requires real-time and
...
**Slice source:** [`PL-00854-wk-671-scoring-sequenced-slice-plan.md`](PL-00854-wk-671-scoring-sequenced-slice-plan.md), the frozen WK-671
...
[`../rulings/RL-00868-score-one-s-real-time-path-async-evaluate-not-evaluate-executor-offload-and-whether-5-2-s-sync-convention-is-itself-the-defect.md`](../rulings/RL-00868-score-one-s-real-time-path-async-evaluate-not-evaluate-executor-offload-and-whether-5-2-s-sync-convention-is-itself-the-defect.md) Rulings 1–5.
...
**Rulings 6–11, filed in PR #368 (`docs/rulings/INDEX.md#2026-08-29-w11-slice1-rulingsmd`) while
this plan was being written.** They land after this plan's own evidence sweep and they
change five things in it; each is applied below and named where it applies. RL-872 rules
DP3 and **unblocks Task 1.5**. RL-873 fixes `load_bundle`'s signature and appends it to
`03` §5.2. RL-874 opens the loaded-booster seam. RL-875 rules the decline
representation. RL-876 gives Task 1.3 two properties it owes WK-674. RL-877 confirms
...
them: `git grep -n 'FR-256' docs/specs/03-rating-engine.md`. The requirement ids are
permanent; only the line numbers move.

**Highest ids in use, verified at `7b8473a` by scanning
[`../specs/03-rating-engine.md`](../specs/03-rating-engine.md):** FR-243,
NFR-501. Next free: `FR-RATE-66`, `NFR-RATE-15`. **This plan mints none of them.** It
cites ids that already exist and proposes no new one. The line is republished because
[`PL-00854-wk-671-scoring-sequenced-slice-plan.md`](PL-00854-wk-671-scoring-sequenced-slice-plan.md)'s own copy was verified at
...
owned by `03` §5.1, `MODEL_CALL_FAILED` as of RL-877 — and error codes are a separate
...
1. **A real Rating Version can be created, submitted and compiled entirely over HTTP**, and
   ...
   carrying the artifacts themselves rather than references to them** (RL-873).
2. **`score_one` prices that Bundle.** `await score_one(load_bundle(bundle), ctx)` returns a
   ...
   penny, on generated contexts rather than one example (NFR-496).
...
5. **NFR-489 (component), 2, 3, 4, 7, 8 and 14 are measured and written down** in a dated
...
- **Money is integer minor units or `Decimal`, never `float`** —
  [`../../CLAUDE.md`](../../CLAUDE.md) §7, FR-245/273.
  ...
  raises `TypeError` on a non-`Decimal` factor and `mode` has **no default** — FR-226
  requires rounding be declared per step. Do not reimplement either.
- **FR-273 — money crosses the engine boundary only as integer minor units.** The
...
- **FR-274 — division by zero returns `null` silently and raises only where the null is
...
There is no frontend work in this slice — WK-675 owns the scoring UI — but new `model-schema`
...
3. **`passThrough` decides whether intermediate values survive.** With the default, the same
   ...
   which passes a naive golden test and fails NFR-496.
4. **`options={"trace": True}` is the engine-native trace**, and it is exactly the shape
   FR-258 wants. The response gains a `trace` key holding one entry per node:
...
[`../findings/register.md`](../findings/register.md) before it is finalised.
...
|---|---|
| `NFR-502/501 (F-W9-1)` | **This slice's obligation.** Carried forward with an owner — "the WK-671 scoring workstream". Task 1.4's `nthread=1` measurement and Task 1.5's harness discharge the NFR-501 half; the NFR-502 half needs the HTTP path and is Slice 2's. The row already records WK-668's figures — NFR-502 p99 0.070 ms, NFR-501 p99 1.626 ms — so the measurement here is a re-measurement on the real path, not a first one |
| `03 rating surface (F8)` | The phase-boundary row that covers "compile, score, rate tables, deployment". WK-671 discharges its scoring quarter |
| `FR-224 (F-W9-2)` | Carried forward to **WK-673**, but its own decision text says the check "specialises FR-257's general approval-evidence gate, **which WK-671 builds**". FR-257 is Slice 2's, not this slice's — recorded so the dependency is visible when Slice 2 is planned |
| `FR-231 (F-W10-2)` | Carried forward with an owner (portfolio-dataset integration). **Not this slice's** |
| `FR-231/233/234/235 (F-W10-1)` | WK-670's, half resolved by PR #302. Not this slice's |
| `FR-230 (F-W10-1-1)` | Resolved 2026-08-28 (PR #302). Closed |
| `03 §5.1 POST /rate-tables/{slug}/versions (F-W10-3)` | Carried forward to the WK-675 rate-table editor slice. Not this slice's |
...
packages/pricing-core/src/` returns nothing. NFR-501 therefore requires a *change* to
...
**C5 — the map's Slice 1 requirement list omits FR-243.** FR-243
...
RULED — RL-878, and the ruling corrected this finding as it was reported.**
...
was added 2026-08-18 with FR-218: OQ-617's answer mounts the refund sub-graph on
...
**Ruled: the contract is the wrong side** (RL-878, a `CLAUDE.md` §0 code-versus-spec
...
OQ-617's decision row (`:807`). Amending the spec down to four would silently reverse a
...
OQ-617 — the contract was never disagreed with, it was never revisited.
...
FR-218's refusal test covering **both** members; and, added by the addendum in PR #373,
...
frozen map's own exit criterion (`PL-00854-wk-671-scoring-sequenced-slice-plan.md:401-404`). That is precisely the
...
**F-W11-1-2 — NFR-501's `nthread=1` has no implementation surface.**
...
**Superseded in scope by RL-874 (PR #368)**, which found the larger defect in the same
...
**F-W11-1-3 — FR-273's startup self-check exists as a function and is wired to
nothing.**
FR-273 (`03-rating-engine.md:220`) ends: *"A startup self-check asserts the round-trip;
...
blob bucket; it does not call it. The W9-2 audit record booked FR-273 "delivered" on
...
inside it — `../closures/INDEX.md#plan-reviewsmd` review 8 Q4 found that mechanism twice.

| Requirement | Where in `03` | Discharged by | How it is proven |
|---|---|---|---|
| FR-250 | §3.7 | Task 1.4 | Golden test: a known Bundle + QuoteContext gives an exact pre-computed ScoringResult. **Excludes** the default-live resolution, which is Slice 2's |
| FR-255 | §3.7 | Task 1.4 | One typed-error test per category, each fired by a deliberately malformed context |
| FR-256 | §3.7 | Task 1.4 | A constraint decline returns `outcome: "declined"` with a populated ladder and a non-empty `decline_reasons` — never an HTTP error |
| FR-258 | §3.7 | Task 1.4 | `trace=True` returns a `Trace` matching `scoring.schema.json`'s `$defs/Trace`; `trace=False` returns `None` |
| FR-218 | §3.1 (`:87`) | Task 1.4 | Refusal-guard test on broken input, **parametrised over both `mid_term_adjustment` and `cancellation`** (RL-878) |
| FR-252 | §3.7 | Task 1.4 | The `instalment_loading` rung appears in the ladder and reconciles |
| FR-243 | §3.4 (`:139`) | Task 1.3 | `CompiledBundle` exists as a distinct type from `Bundle` and refuses serialisation |
| FR-237 citation | §5.1 (`:512`) | Task 1.1 | `audit-docs.py` clean with the citation added |
| FR-239/240 | §3.4 | Task 1.2 | Round-trip: compile, fetch the persisted Bundle, confirm graph and payloads survive |
| FR-273 (runtime half) | §3.11 (`:220`) | Task 1.4 | See F-W11-1-3 — the startup call and its negative test |
| NFR-489 (component) | §9 | Tasks 1.4, 1.5 | Bare-metal p99 against the budget. The **full-path and sustained-load** halves are Slice 2's |
| NFR-490 | §9 | Task 1.5 | Traced vs untraced p99 delta against the budget |
| NFR-491 | §9 | Task 1.4 | DB and network blocked during `score_one`, and the test shown to fail with a deliberate DB call inserted |
| NFR-492 | §9 | Task 1.3 | Compile time and **serialised** Bundle size for a real large structure |
| NFR-495 | §9 | Task 1.4 | Same hash + same context, twice, in-process and in a subprocess, byte-identical |
| NFR-496 | §9 | Task 1.4 | `reconcile_ladder` over generated contexts, not one example |
| NFR-501 | §9 | Task 1.4 | `nthread=1` on the real `model_call` path, p99 over ≥ 1000 calls. See F-W11-1-2 |

**Deliberately excluded, each with its owner:** FR-RATE-35, 40 and the default-live half of
34 → Slice 2. FR-RATE-36, 37 → Slice 3. FR-259 → Slice 4. FR-RATE-43, 44, 45 → WK-672.
FR-RATE-46, 47, 48, 49 → WK-673. FR-267 and FR-428 → WK-674. NFR-493 → Slice 3.
NFR-RATE-9, 11, 13 → Slice 2. NFR-500 → Slice 4.
...
| 1.4 `score_one` | 1.3 | — (**F-W11-1-1 ruled**, RL-878) | The evaluator |
| 1.5 Latency harness | 1.4 | — (**DP3 ruled**, RL-872) | Measures 1.4 |
...
**F-W11-1-1 is ruled** — RL-878: five members including `cancellation`, fixed by Task 1.4
...
**DP3 (load-generation tooling) is ruled** — RL-872, PR #368: `scripts/bench-rating.py`
...
**F-W11-1-4 — FR-252 carries a second refusal that the map plan does not mention.**
The map's Task 1.4 takes FR-252 as "the `instalment_loading` ladder rung". Its full
...
RULED — RL-879 (PR #375): the code is right, the spec is stale. Not a blocker.**
...
`delivery-process.md` §3 makes theirs; **ruled as RL-879 (PR #375)** along the recommendation below.
...
**RL-864.** The defect: `create_rating_version` (`rating_versions.py:93`) and
...
- Modify: `docs/specs/03-rating-engine.md:513` — add the `(FR-237)` citation RL-864
  names. `docs/workflows/WF-00699-approved-models-to-approved-rating-version.md:56` already relies on it.
...
`F-W10-2-2` was raised against exactly that shape ("the ADR-704 divergence class, invisible
to the drift check") and was resolved by typing it. An untyped body publishes an empty
schema into the OpenAPI contract, which is what FR-451's drift check cannot see.
...
class RatingVersionCreate(BaseModel):
    """The body of `POST /api/v1/rating-versions` (03 §5.1, FR-237)."""
...
class RatingVersionSubmit(BaseModel):
    """The body of `POST /api/v1/rating-versions/{id}/submit` (03 §5.1, FR-257)."""
...
(FR-451). Then the frontend half: `pnpm --dir frontend generate:api && pnpm --dir
frontend type-check`.

- [ ] **Step 7: add the spec citation and re-run the docs audit**

Add `(FR-237)` to `docs/specs/03-rating-engine.md:513`. Run
...
git commit -m "feat(rating): wire the rating-version create and submit routes (FR-237)"
```
...
**RL-865 and its related finding, which the rulings record is explicit are one
...
§5.1:514 specifies **202**, and `WF-699:57` agrees. `JobKind.RATING_COMPILE` exists
...
**No spec change.** `03` §5.1 already says 202 and RL-865 already ruled the code wrong.
...
Seed a rate table with the WK-670 fixtures — read `backend/tests/` for the existing rate-table
fixture rather than constructing rows by hand; WK-670 shipped them and reusing one is the only
...
(`PIN_NOT_APPROVED`, FR-20). Return the row's real status, as the `model` branch already
...
itself, carried inside the payload, never a reference to it** (RL-873). `resolved_payloads`
...
it performs no I/O** (RL-873), so anything it cannot reach from inside the `Bundle` is
...
**Acceptance**
- A Rating Version pinning a real WK-670 rate table compiles — it does not today.
...
- **`resolved_payloads` carries each pinned artifact itself, JSON-round-trippable — never a
  blob reference** (RL-873). Proven by parsing the persisted JSON back and finding the
...
**Must NOT touch.** `compile_bundle()` itself (`compile.py:387`). RL-866 already settled
...
**RL-867** settles the type: `CompiledBundle` is a distinct *loaded* runtime wrapper,
never itself serialised. FR-243 (`03:139`) states it as a requirement. Neither settles
...
- Modify: `docs/specs/02-modelling.md` §5.2 — append the loaded-booster seam's signature in
  the same commit that adds it (RL-874's standing obligation, not optional).
...
- *Produces*, and Task 1.4 depends on these exact names:
  ...
  - `def load_bundle(bundle: Bundle) -> CompiledBundle` — **exactly this signature**, ruled
    and appended to `03` §5.2 by RL-873 (PR #368). It takes no resolver, no handler and no
...
exactly what happened to RL-878's list.

1. **RL-873 — a `model_call`'s payload travels *inside* the `Bundle`, never as a
...
2. **RL-874 — the loaded-booster seam does not exist: `predict_gbm` re-loads the booster
...
3. **RL-876 — two properties Slice 1 owes WK-674, so the deployment-switch mechanism still
   ...
   one, and a `CompiledBundle` that has forgotten its provenance makes FR-268's "either
   ...
   **pure with respect to the cache**: it consults no cache, registers itself in no global,
   ...
   trigger itself is **WK-674's**, not this slice's.
...
6. A `model_call` step becomes `{"type": "customNode"}`. The handler is supplied when the
   ...
   argument (RL-873), so whichever object owns the `ZenEngine` owns the handler.
...
boundary confusion RL-867's option (c) was rejected for.
...
- [ ] **Step 6: measure NFR-492** — compile a real large motor structure once 1.1 and 1.2
      make one compilable, and record the wall time against the **< 60 s** budget and the
      **serialised** `Bundle` size against **< 500 MB** (`03:780`). RL-873 is explicit that
      ...
      so a measurement landing near the cap is a finding against `NFR-492` rather than
      against the ruling.
...
- **Exactly one booster deserialisation across N scorings against one `CompiledBundle`, not
  N** — and the test **written to fail first** against today's per-call load (RL-874).
- **`CompiledBundle` exposes the `content_hash` of the `Bundle` it was loaded from**
  (RL-876) — without it FR-268's "either the old or the new bundle, never a mix"
  is unverifiable at runtime and WK-674 has no switch mechanism left.
- **`load_bundle` is pure with respect to the cache** (RL-876): consults no cache,
...
- **The loader's signature appended to `../specs/02-modelling.md` §5.2 in the same commit**
  (RL-874's standing obligation).
- NFR-492 measured and written down, both halves, the size on the **serialised** form
  (RL-873).
- `to_jdm`/`JdmGraph` docstrings no longer claim to produce the engine's shape.

**Must NOT touch.** `Bundle` itself. WK-669's DP1 settled what it is; this task builds the
...
**RL-868**, not open: the real-time path is built on `async_evaluate()`, never `evaluate()`
...
- Modify: `scripts/generate-contracts.py` — add the three new shapes to `GENERATED_SHAPES`;
  ...
  (RL-878's addendum, PR #373).
- Modify: `docs/contracts/schemas/scoring.schema.json:12` — the five-member `purpose` enum
  (RL-878, obligation 2), in the same commit as the `model-schema` enum.
...
- The example's ladder shows nine rungs and omits `instalment_loading`; the contract's
  `LadderRung.rung` enum has ten and includes it (FR-252 postdates the example). **The
...
- *Produces*, and Slice 2 and Slice 3 both depend on this exact shape:
  ...
  - a per-step evaluator `score_batch` can call directly (FR-254 — Slice 3 proves
...
**`purpose` is ruled — RL-878 — and it is five members, `cancellation` included.** The
...
1. The five-member enum defined **once**, in `model-schema` (ADR-704, `CLAUDE.md` §2 —
...
3. FR-218's refusal test covering both members (Step 9).
4. **Add the new shapes to `scripts/generate-contracts.py`'s `GENERATED_SHAPES`, and lift
   `"scoring"` from `backend/tests/test_contracts.py`'s exclusion dict** (RL-878's
...
`apply_factor(amount_minor, factor, mode)` per rung — `mode` has no default and FR-226
...
loader hydrates once — never to a per-call construction.** RL-874 makes that per-call load
...
**Then measure, in the shape the budget was measured in.** NFR-501's amended figure is
...
:70-79`; the row's 2026-08-27 WK-668 amendment — the original S2 figure of 1.09 ms is superseded
and citing it would be citing a struck number). Reproduce that shape: booster loaded once
outside the loop, DMatrix built inside it. RL-874's seam amortises the booster *load*;
DMatrix construction is genuinely per-quote and stays in the number.

**The failure mode is measuring a shape WK-668 never measured.** Load the booster inside the
loop and you produce load + DMatrix + predict — a fourth shape, absent from WK-668's table, whose
p99 lands far above 1.626 ms and reads as a failure of a budget the shipped path meets.
**There is no comparator ambiguity to resolve.** WK-668's three rows all preload the booster
...
- [ ] **Step 6: the typed per-quote errors (FR-255)**
...
- constraint decline → **not an error.** FR-256: `outcome: "declined"` with reason
  codes, a *successful* response, never an HTTP error.

**`MODEL_CALL_FAILED` is ruled and already in the spec** — RL-877 (PR #368) confirmed it
...
**Two corrections this plan owes to RL-877, both of which change what Step 6 does.**
...
by NFR-496's reconciliation work, and an earlier draft of this plan missed it. All four
...
**Ruled — RL-875 (PR #368), and FR-256 carries a dated amendment saying so.**
...
amendment appends no new `FR-` because it adds no obligation FR-256 did not already
...
`payable_premium_minor` under NFR-496's check. A single-decline test passes under
short-circuit and collect-all alike and would prove nothing.

- [ ] **Step 8: the trace (FR-258)**
...
`trace=False` must return `trace: None` and, per NFR-490 and R3, an **identical** premium.
...
- [ ] **Step 9: the FR-218 refusal guard, on broken input — parametrised over *both*
      members**
...
**Parametrise; do not write the `mid_term_adjustment` case alone.** RL-878 is explicit
...
- [ ] **Step 10: the FR-252 refusal guard (F-W11-1-4)**

The second half of FR-252: a Quote Context asking for a payment schedule, an APR figure
...
- [ ] **Step 11: NFR-491, proven on broken input**
...
`CompiledBundle`. The spike RL-868 rests on used a bare `asyncio.run` and a fresh handle,
not a long-lived handle serving concurrent calls the way a worker process will. This is the
first test of that shape.

- [ ] **Step 14: the RL-868 follow-up, resolved here**

RL-868 names a follow-up for "whichever slice builds `model_call`'s ADR-706 custom-node
...
- [ ] **Step 15: wire the FR-273 startup self-check (F-W11-1-3)**
...
prevents the service starting" since WK-669 and nothing has enforced it.
...
- **Two** constraint declines give `outcome: "declined"`, `len(decline_reasons) == 2`, and a
  ladder that still reconciles (RL-875 — a one-decline test passes under both readings).
- FR-218's refusal parametrised over **both** `mid_term_adjustment` **and**
  `cancellation` (RL-878).
- The five-member `purpose` enum in `model-schema` **and** `scoring.schema.json:12`
  corrected, in the same commit (RL-878).
- `QuoteContext`/`ScoringResult`/`Trace` in `GENERATED_SHAPES`, and `"scoring"` **lifted**
  from `test_contracts.py`'s exclusion dict (RL-878 addendum) — the drift guard must be
...
- **Four** codes in `RATING_ERROR_CODES`, and `score_one` raising a code-named `ValueError`
  ...
  (RL-877). `lint-imports` staying green is the mechanical proof.
- NFR-501 measured **including DMatrix construction, on an already-loaded booster** —
  WK-668's own shape, or the number is not comparable to the 1.626 ms it is checked against.

Plus: `NFR-489` measured *component-level only*.
`score_one`'s in-process latency is not a substitute for the sustained-load, ASGI-embedded
measurement `NFR-489` actually names (`03:777` — p99 < 50 ms at **200 rps per replica**).
That is Slice 2's Task 2.1, and `zen-evaluate-concurrency.md` is explicit that no such
measurement exists yet.

**Must NOT touch.** `score_batch`'s signature — it stays `def` per RL-868; batch has no
...
**Unblocked — RL-872 (PR #368) rules DP3.** `scripts/bench-rating.py` is stdlib-only,
...
**Discharges** `docs/roadmap.md:392`'s risk mitigation — "Build the latency harness in WK-671
...
**Files**
- Create: `scripts/bench-rating.py` — **stdlib-only** (RL-872), following
...
- Measures `score_one` directly — no HTTP, no FastAPI — against a ~200-step motor structure
  with one `exact` GBM call, reporting p99 against NFR-489's **< 50 ms**; and without a
  GBM call, against **< 15 ms** (`03:777`).
- Measures trace overhead (NFR-490): the same calls with `trace=True` and `trace=False`,
...
- **Not a CI gate**, and **no `locust`, `k6`, `hey` or `wrk` reaches any `pyproject.toml`,
  `uv.lock`, CI workflow or setup instruction** — RL-872's own acceptance test, stated as
...
silent. FR-243 was added after checking the map's own list against `03` §3.4 — it was
missing (C5).

**2. Placeholder scan.** No bare TBD, and — after RL-878 — **nothing in this slice is
...
naming, per RL-874's own reasoning that naming a function before it is designed is how a
...
conflict and is ruled as RL-879 (PR #375) — code right, spec stale; it blocked nothing, since Task 1.4
imports from the real path either way. **F-W11-1-1** is RL-878. The slice-granularity
...
enumerating them by their `## Ruling N` headings — and RL-878 had since gained a fourth
...
then ruled too**, as RL-878 in the same record, after this plan raised it — and the ruling
