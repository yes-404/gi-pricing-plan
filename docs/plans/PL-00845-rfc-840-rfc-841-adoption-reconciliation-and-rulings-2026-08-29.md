...
family: plan
kind: leaf
title: RFC-840 / RFC-841 adoption — reconciliation and rulings (2026-08-29)
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-29-nt-0010-0011-reconciliation-rulings.md
---

# RFC-840 / RFC-841 adoption — reconciliation and rulings (2026-08-29)

`workflow-design-proposal.md` (RFC-840) and its companion `agents-settings-proposal.md`
(RFC-841) are being adopted through the adoption procedure RFC-840 §15 itself specifies:
...
notes bind, `docs/README.md`, `docs/findings/register.md`, `TEAM-STRUCTURE.md` is explicitly
...
### A1. RFC-840 §9 — the single human checkpoint

**The conflict, precisely sourced.** RFC-840 §9 puts exactly one human approval in the whole
...
maintainer is well-evidenced **standing practice** (WK-664, WK-669, WK-670 all closed on the
maintainer's word, never the auditor's or the team's own verdict), not a §13 sentence a
literal read would catch. The gap is real either way: RFC-840 §9's single checkpoint would
...
a slice closes on a clean audit and the lead's merge, exactly as it does today. **RFC-840 §9
...
1. **It formalises what already happens rather than changing it.** The lead merges slice
   PRs on a clean audit today; WK-664, WK-669 and WK-670 each closed on the maintainer's word, never
...
2. **It survives the objection that would otherwise weaken it.** WK-669 closed on the
   maintainer's acceptance and still shipped incomplete: its own scope prose "names four
   sections totalling 26 requirements; it verdicted 24" (`docs/closures/CR-00825-plan-review-7-at-wk-669-s-close.md:25`,
   ...
   WK-669's close. Recording both halves matters because a reader who takes the checkpoint for
   a safety net will over-trust it exactly where WK-669 shows it does not act as one.
...
### A2. Role agents that decide and write to `docs/` — RFC-840 §2/§4 item 2, bundled with RFC-841's auditor tool-scope (§2 "auditor") and the `.claude/agents/README.md` scope question
...
RFC-841's own "auditor" tool-scope contradiction (owns "closure records, register deferral
...
- **(c) Split the difference by artifact type** — e.g., role sessions may write dated
  ...
  have blocked PR #308's actual §5.2 amendment and this record's own FR-237 citation fix
...
- **The auditor did not file `FR-218`'s register row.** Commit `71d8ca5` (#319), body:
  *"FR-218 was buildable inside WK-669's delivered scope and is a close gap, not an
  orphan. Its register row is deliberately held until plan review 7 rules its owner, since
  a row names an owner and filing one now would pre-empt the review."* `docs/findin
  gs/register.md` has no `FR-218` row today, confirmed directly — but that absence alone
...
- **The executor filed rather than absorbed an out-of-scope finding.** Commit `d4bc394`
  ...
  silently absorbed — OQ-615 carries the same stale figures in both its mirrors…"*
...
`RFC-778`), demonstrated on the adoption itself. Fix filed separately against
...
against RFC-840/841's proposal text, precisely because the person who benefits from a
...
*as well* would be a fourth place the same rule lives, which is the exact failure `RFC-756`
...
ruling — RFC-841 items duplicate two of RFC-840's, and are not re-litigated separately.

### B1. RFC-840 item 3 — "no parallelism at any layer" must not forbid delegation
...
### B2. RFC-840 item 4 + RFC-841 item 2 — "ultracode" is not "ultrathink"
...
needs the same correction (RFC-840 §4's paragraph and RFC-841 §1's summary table / §2
...
evidence sweep) and which no decision point in WK-670 ever needed. Maximum thinking on a rare,
binding, cheap-to-think-hard-about action is the reading that survives.

### B3. RFC-840 item 5 + RFC-841 item 3 — retry caps and cost tiers are guesses; adopt as instrumented defaults, not governance
...
RFC-840 §6 sets the Slice retry cap at ≤2. The auditor **independently reconfirmed** (not
...
threshold. RFC-841's separate cost-tier question (should the executor really get the
cheapest model, on the argument that the auditor "catches what slips through") has the same
shape: the auditor's own WK-670 record shows several caught defects that each cost a full
...
### B4. RFC-840 item 6 — the adoption procedure (§15) is adopted as written
...
nobody reconciled — and would breach RFC-756 (a process spec restating `CLAUDE.md` §§12–14
...
### B5. RFC-841 item 4 — model names, and the lead's unenforceable model row
...
- **Model names, pinned now rather than left to drift:** in this environment, `opus` →
  ...
  it by this ruling — the volume-vs-leverage logic in RFC-841 §1 already covers the four
...
- **The lead's model row is corrected, not merely restated:** the lead is the main
  ...
  those roles are spawned; the lead is not spawned, it is the session). RFC-841 §2's lead
...
**Ruled: RFC-840's suggested filename is rejected; the adopted document must not be named or
titled "workflow."** Placement and naming were already explicitly left open by RFC-840's own
...
the open placement/naming decision RFC-840 already deferred — except that "workflow" itself,
...
**Ruled: fix the four references in the same adoption pass that lands RFC-840, before or
...
`executing-plans`"), confirming what the stale file should say. RFC-840 §11 obligation 1
...
### B8. New — RFC-840 undercounts `.claude/agents/`: seven specialists, not eight

**Ruled: seven is authoritative. RFC-840's "already in force" section (`0010-…md:58-59`,
...
**Ruled: RFC-841's version is authoritative; RFC-840 §2's lead row is corrected to match
it.** RFC-841 matches confirmed practice (this session's own PR-only-plus-lead-merge
experience, `TEAM-STRUCTURE.md`'s table); RFC-840's is silently narrower in one direction and
...
Verified precisely: RFC-840 §2 (`0010-…md:239`), Lead's suggested tool scope — "Read-only
(Read, Grep, Glob) + write access to plan/map files only." No merge authority anywhere in
that row. RFC-841 §2 "lead" (`0011-…md:210`), Tools — "full read; git merge authority;
...
which RFC-841's own division of labour already gives to the planner role, matching this
session's own experience of what each role actually writes.

### B10. New — RFC-841's decision-maker incident citation: sound, but sourced wrong by the auditor's own check, and durable nowhere

**Ruled, in three parts, because the evidence trail here has three different actors each
partly right.**

1. **RFC-841's citation is factually accurate.** Re-verified directly against
   ...
   order defied, tracked files discarded — every element of RFC-841's sentence ("the
...
2. **The auditor's Priority-1 finding that the citation "doesn't match" is itself a
   ...
   was checking the wrong log, not evidence against the claim. RFC-841's sentence is
...
3. **The deeper point both the lead and the auditor made survives this correction
   ...
   all. This is `RFC-778`'s "deferred items with no durable custody" failure, committed by
   the very notes that elsewhere cite `RFC-778` as their own justification for writing the
   process down.

**Fix:** RFC-841's assessment prose (its "already in force" section, the sentence
...
— normative framing, not mere description — and RFC-840 §10 explicitly mandates placing
...
rule does not reach one. Reading 2 is correct about **what RFC-840 §10's adoption would
...
### RFC-840 — workflow proposal

| § | Proposal | Governing text (swept set) | Auditor found | **Ruled** |
|---|---|---|---|---|
| 1 Purpose | 4-layer hierarchy; one human checkpoint at Project | No "process" doc category exists in `docs/README.md`; process rules live only in `CLAUDE.md` prose | Silent (new category) | **Adopt** — closes the exact custody gap `RFC-778` named, one level up |
...
| 4 Per-layer flow | decision-maker owns the audit fix/accept/defer decision | Same §12/README conflict as §2; **also** superseded by RFC-841 §3 delta 1's own correction | Conflicts, self-corrected by the companion | **Amend** — adopt RFC-841 §3 delta 1's version: auditor proposes, lead adopts/amends/rejects and merges, decision-maker rules DPs/spec only |
...
| 8 Findings register | skeleton + decision taxonomy | `docs/findings/register.md` — verbatim match, including the literal "fix before close" wording (`register.md:14`) | Agrees, reconfirmed | **Adopt as-is** |
...
| 12 Audit record obligations | pin/scope/divergence/verdict/judgment/table/rfcs/sign-off | `close-workstream` skill and both existing closure checklists match closely, including the literal "silence is never a verdict" language | Agrees strongly | **Adopt as-is** |
...
### RFC-841 — agent settings companion
...
| 2 lead | opus/high; owns merges+verdicts; writes handover/status only | No governing document states lead merge authority anywhere; **disagrees with RFC-840 §2's own lead row** | Silent in the governing set; real inter-note conflict | **Adopt** (matches confirmed practice) — corrected against RFC-840 §2, Part B9; merge authority itself stays **silent** pending Part A2, not "agrees" |
| 2 planner | opus/high; writes `docs/` plan files only | Same `CLAUDE.md:216-219` question as §2/roles | Conflicts (same root as RFC-840 §2/§4) | **Amend together with Part A2** — not a second, separate ruling |
| 2 decision-maker | opus/ultrathink; no code-worktree write access; cites a WK-670 incident | The write-free rule is corroborated (one passing log line); the incident citation needed re-sourcing | Rule sound; citation needed a fix | **Adopt the boundary; fix the citation** — Part B10 |
...
| 2 watcher | script + haiku-on-anomaly; owns `roster-state.md` | Same silence as RFC-840 §14 | Silent in the governing set | **Adopt** — closes the same gap, Part D |
...
`docs/README.md`, not `docs/findings/register.md`. Their only source is `TEAM-STRUCTURE.md`
...
1. **Lead merge authority.** "Merge" appears in the swept set only as *mechanism*
   ...
   memory note.
2. **The whole RFC-840 §14 / RFC-841 watcher-reporter mechanism** (roster-state.md as the
...
3. **The decision-maker's write-free convention.** No governing document names a
   ...
   log line ("decision-maker-2 ruled write-free") in the WK-670 auditor's own state file —
...
- `.claude/rfcs/RFC-00841-per-agent-model-thinking-effort-and-skill-bindings-for-the-seven-roles.md`'s decision-maker incident
...
- Both notes' `Status` field stays `open`: Part A's two rows are not yet accepted, and
  `.claude/rfcs/README.md`'s own acceptance criteria are not met until they are.
  ...
  *(Corrected 2026-08-29, found by the §15 step 5 audit: both notes' `Status` field is now
  ...
  (`.claude/rfcs/RFC-00840-a-layered-slice-based-workflow-project-phase-work-slice-gated-at-every-layer.md:6`,
  `0011-per-agent-model-and-skill-settings.md:6`), mirrored the same way in
  `.claude/rfcs/README.md`'s own index rows. The sentence above was accurate when this
  ...
  the RFC-841 citation-correction note) — none introduces or is falsified by anything landed
  since.)*
