...
family: plan
kind: leaf
title: W6b-13 — Rule Set Rule-Versioning Screen Implementation Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-25-w6b-13-rule-versioning-screen.md
---
...
**Goal:** Make `FR-56`'s reviewed path reachable from the rule set editor — author the
...
**Spec:** [`../specs/01-data-management.md`](../specs/01-data-management.md) — `FR-56`
(`:118`), `FR-68` (`:168`), `FR-50`, `FR-51`, and §5.3's *Rule set editor* row
...
[`../specs/01-data-management.md`](../specs/01-data-management.md):** FR-58, NFR-474,
OQ-570. Next free: `FR-59`, `NFR-DATA-11`, `OQ-DATA-16`.
**This plan mints none of them** — it cites `FR-56`, which already states the rule, and
amends a §5.3 Contents cell, which is not a requirement. The line is published because the
revised slice map's equivalent (*"Next free: `FR-57`"*, verified 2026-08-24 at `e2ae7c6`)
...
- A threshold change **authors a new rule version** and moves only through `FR-50`'s
  ...
  `severity_override` and **gains no third override** (`FR-56`).
...
### Finding 1 — `01` §5.3 still specifies the operation `FR-56` forbids
...
`FR-56` (`:118`, appended 2026-08-23) says a Rule Set entry *"keeps exactly the two
...
the rule, and changing one authors a new rule version (FR-56)"*, and `:422` records the
...
since `FR-56` forbids the set-level editing the title names"*) and did not reach the spec
...
`catalogue_id` is the field `W6b-13b` added (`FR-68`'s tail: *"`RuleCreate` now carries
...
order to withdraw it), and `PL-00782-w6b-13b-the-catalogue-chain-implementation-plan.md:430` (that plan echoing its
...
[`PL-00782-w6b-13b-the-catalogue-chain-implementation-plan.md`](PL-00782-w6b-13b-the-catalogue-chain-implementation-plan.md) `:430` runs
...
[`PL-00782-w6b-13b-the-catalogue-chain-implementation-plan.md`](PL-00782-w6b-13b-the-catalogue-chain-implementation-plan.md) records that
**VR-DST-1's catalogue entry carries `warn_above` only** — `FR-56`'s tail says the same at
...
> (`FR-68`) returns `params` — fetch `warn_above`, zero literals. Beats (a), which sources
...
**Set-level threshold override.** Forbidden by `FR-56`. Not deferred — decided against.
...
| `docs/roadmap.md` | Modify `:1701`, `:1766`, `:3287` | Record the gap closed, under `FR-56`'s semantics |
 ...
 * backend change never reached the browser (`FR-68`).
 */
 ...
 * makes the platform allocate the next version (`FR-50` step 4).
 *
...
This task exists because of Finding 1: §5.3 specifies an operation `FR-56` forbids, and
...
Three rows carry "threshold editing" as outstanding WK-664 work: `:1701` (gap column), `:1766`
(the six §5.3 Contents items) and `:3287` (the remaining four). Append a dated note to each
recording that it is delivered under `FR-56`'s semantics — versioning, not set-level
...
```
FAILED (1):
  - FR-DATA-<probe> referenced but never defined (in ['plans/PL-00792-w6b-13-rule-set-rule-versioning-screen-implementation-plan.md'])
```
...
git commit -m "docs(w6b-13): §5.3 specified the threshold edit FR-56 forbids"
```
...
**Spec coverage.** `FR-56`'s reviewed path is reachable from the screen (Tasks 2–3);
`FR-68`'s `catalogue_id` lineage survives the browser (Tasks 1–2); §5.3's Contents cell
agrees with both (Task 4); §5.3's 2026-08-19 banding note stops being contradicted by the code
under it (Task 5). `FR-50` and `FR-51` are consumed unchanged — the builder already
