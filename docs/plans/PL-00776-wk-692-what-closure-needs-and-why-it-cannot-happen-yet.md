...
family: plan
kind: leaf
title: WK-692 — what closure needs, and why it cannot happen yet
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-23-w32-closure-proposal.md
---

# WK-692 — what closure needs, and why it cannot happen yet
...
[`PL-00753-wk-664-and-wk-692-the-slice-map.md`](PL-00753-wk-664-and-wk-692-the-slice-map.md) — a filed plan is frozen at its date.

## The verdict

**WK-692 cannot be closed.** Two independent reasons, and the second is the one that would otherwise
be missed:

1. **Unbuilt scope.** Four requirement groups WK-692 owns have no implementation, and three pieces of
   shipped WK-692 behaviour have no test that could fail if the behaviour were wrong.
2. **The workstream does not exist in the roadmap.** There is no `WK-692` row in Phase 1b's
   workstream table (`../roadmap.md:3773-3777` lists WK-661, WK-664 and WK-665 only). Closing a workstream
...
|---|---|---|
| **W32-7** | [workspace identity and selection](PL-00771-workspace-identity-and-selection-implementation-plan.md) | FR-395, FR-396, FR-397 |
| **W32-8** | [artifact library list routes](PL-00773-artifact-library-list-routes-implementation-plan.md) | FR-167 |
| **W32-9** | [transparency exposure share](PL-00775-transparency-exposure-share-implementation-plan.md) | FR-136, FR-135 |
| **W32-1b** | [drift guard arm attribution](PL-00759-drift-guard-arm-attribution-implementation-plan.md) | the constraint guard's arm blindness, and the `contract-guard` skill's stale counts |
| **W32-10** | [untested behaviour](PL-00758-untested-wk-692-behaviour-implementation-plan.md) | FR-82, FR-180, FR-181 — evidence, not new behaviour |
...
([slice map](PL-00753-wk-664-and-wk-692-the-slice-map.md):174-175), and the `contract-guard` skill uses it too.
...
### B1. Phase 1b has no WK-692 row

**Recommendation:** add one to the table at `../roadmap.md:3773`, after WK-664, since WK-692 is the
non-frontend half of the same split:

| # | Workstream | Depends on | Notes |
|---|---|---|---|
| **WK-692** | Everything in Phase 1b that is not a browser — contract guards, `model-schema` shapes, a migration, backend defects, endpoint tests, and one skill | WK-661 | Split from WK-664 2026-08-22 (`PL-00753-wk-664-and-wk-692-the-slice-map.md` §1). Ten slices; W6b-1, -3, -5 and -13 are blocked on it |
...
`docs/plans/` but no record in `../roadmap.md`, so the roadmap cannot answer *what did WK-692 deliver*
...
| **FR-158** — the escalated constraint disagreements | **Reassigned** | Named in `UNRESOLVED_CONSTRAINT_DISAGREEMENTS` and governed by `open-questions.md:83`, which is directive that the carve-out is removed in the same commit the two sides agree. W32-1b deliberately does not touch it |
...
| 3 | FR-167 says "the three artifact libraries §5.3 renders"; §5.3 renders **one** | **The spec, twice over** — recommendation is to add the two missing §5.3 rows |
| 4 | FR-167 says unqualified "`usage_count` is on the row"; §5.1's peril row omits it | Open — W32-8 follows §5.1 and asserts the asymmetry so it cannot close by accident |
...
| Item | Accepted |
|---|---|
| The verdict: WK-692 cannot be closed | **2026-08-24** — accepted, and it now rests on a second ground the proposal did not have: **W32-11**, allocated by Part C's decisions, is unbuilt |
| **B1** — the proposed Phase 1b `WK-692` row | **2026-08-24** — accepted as recommended; the row is at `../roadmap.md`, after WK-664. *(The proposal cited `:3773`; the table had moved to `:3926` by the time the row was written — the W32-9 and W32-10 records landed between.)* |
...
| Part C's three verdicts, and their owners | **2026-08-24** — the verdicts accepted, **two of the three owners amended**. FR-158 and the two uncompared schemas both go to a newly allocated **W32-11** — which had **no executor assigned** when this row was signed, leaving WK-692 un-closeable by design until the maintainer assigned one, and which **acquired one the same day**: the closure-execution session surfaced the inference and the maintainer confirmed it, adding that W32-11 is the **terminal** slice and that findings it cannot resolve are booked forward with an owner rather than held against the close. Both states are kept — the gap was real when signed: the proposal reassigned FR-158 without naming a successor, and OQ-600 already owns it to *WK-692*, so that was not a reassignment; and the schema row said *not started* without recording that it was also **unowned** after W32-1b declined it in writing. **FR-396's verdict is not accepted today**: it is *deferred with an owner* only once W32-7 has shipped the mechanism, and until then all four obligations are *not started*. Instantiated against fact at close |
...
| The recommended sequence | **2026-08-24** — accepted as amended, and already partly spent: W32-10 (`0e44db9`) and W32-9 (`faff060`) merged before this decision was written. The amendment is the tail — **W32-11 is appended and WK-692 does not close until it lands** |
