# A vacuously-true acceptance item does not satisfy itself — Ruling 84 §4's `slice:` clause, ruled (2026-09-02)

**What this is.** The executor implementing [Ruling 84](2026-09-02-w37-guard-arithmetic-and-ledger-family-rulings.md)
...
document — it can be a mutation of the writer**, which is the shape Ruling 70 item 2 already
...
out into separately-guarded branches *by `614c92c` itself*, the Ruling 84 implementation. §1(a)
has both. Acting on the quoted form would mean believing Ruling 84's positive obligation —
`work:` resolved to W5's `WK-` id — is unimplementable. It is implemented.
...
- **It lands as a new dated artifact, never an edit to Ruling 84** — the same rule Ruling 93
  applied to Ruling 88.
...
## Ruling 94 — the property stands; the instrument is amended, because a broken input need not be a document
...
optional (default `None`, **dropped exactly as before Ruling 84** — every caller but
`_write_document_drafts`'s `LG-` path leaves them unset)"*.

**So Ruling 84 §2's positive obligation is implemented, and §4's third acceptance item — a
...
**(c) And `slice:` is supposed to remain writable.** Ruling 84 §3 item 5: *"`slice:` stays
...
**But the property is not narrowed. The instrument is.** Ruling 84 §4's second item is
substituted:
...
> boundary sits, not that anything was verified ([`NT-0007`](../notes/0007-context-bound-measures-cap-not-discipline.md)).
...
emit. **Ruling 70 item 2 already established the other shape**, and Rulings 79 and 80 both use
...
1. **Ruling 84 §4's second item is read as substituted from this record's date.** Ruling 84 is
...
- **The `slice:` check reds under the `_stamp_header` mutation of §2**, and its output names the
  ...
  could not detect.
- **Ruling 84 §4's third item is exercised too**, since §1(a) shows it is live: an emitted `LG-`
...
Ruling 93 routed onward *"whether other filed acceptance items name unbuildable fixtures"*. **This
is a second instance arriving by a different route, and the two are different failure modes:**

| Shape | How it arises | Instance |
|---|---|---|
| **Invalidated** | The item was constructible when written; a later change removed the thing it named | Ruling 88 §4 item 2, after `#609` demoted the heading levels |
| **Vacuous at birth** | The item was never constructible; the code path it names has never existed | Ruling 84 §4 item 2, from the day it was written |
...
**Still a measurement rather than a decision, so still the lead's**, per Ruling 93. What this
...
|---|---|---|
| **The sweep over filed acceptance items** | A measurement. Ruling 93 routed it; this sharpens what it looks for | **The lead**, unchanged |
| **Whether `slice:` should gain a data source in this migration** | Ruling 84 §3 item 3 already settled that no `SL-` row is minted, and the refutation section of the template-parser record settled that the clause is vacuous rather than unsatisfiable | **Nobody — already decided.** Named here so it is not re-opened by this ruling's mention of the key |
...
implementing Ruling 84 and flagged rather than silently satisfied. The lead's reading of §13 is
adopted; its proposed remedy is not, for the reason in §2; and its code quotation is corrected in
§1(a) because the correction changes what a reader would conclude about Ruling 84's other items.
