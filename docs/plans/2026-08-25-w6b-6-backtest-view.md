...
**Spec:** [`../specs/02-modelling.md`](../specs/02-modelling.md) — FR-MODEL-57, FR-MODEL-92,
§4.12, §5.1, §5.3.
...
FR-MODEL-57's caption limb landed on `origin/main` between `8d0dcf4` and `7f671d5`, both on
...
FR-MODEL-57 now states the general rule, quoted from `../specs/02-modelling.md`:
...
**FR-MODEL-57 carries a split status and any closure record citing it must say which limb.**
...
FR-OVR-21 (`../specs/00-overview.md`) makes a §5.3 Contents cell prose that binds nothing,
...
- The floor is the **generated contract**, and OQ-MODEL-15's decided rule of 2026-08-21 makes
...
Three requirements, all backend-evidenced. The other six of the pre-split set — FR-MODEL-63,
...
|---|---|---|
| FR-MODEL-57 | Artifact limbs evidenced over HTTP 2026-08-23 (W32-6); **caption limb `not started`, owner W6b-6** | Discharges the caption limb (Task 3) |
| FR-MODEL-92 | Evidenced over HTTP — backtest readable by id | First frontend consumer of the read |
...
|---|---|---|---|
| `GET` | `/api/v1/models/backtests/{backtest_id}` | FR-MODEL-92 | **calls it** |
| `POST` | `/api/v1/models/{model_id}/backtest` | FR-MODEL-57 | does not call it — no launcher |
...
(FR-MODEL-92). So the `:slug` in the view's route is for the breadcrumb and the fit link, and
**not** for the fetch.
...
- The weighting label (FR-MODEL-55), and `computed_at`.
...
part of its split (FR-MODEL-57) — typically a later period."*
...
FR-MODEL-92 states the reason: *"By backtest id rather than by model, because unlike
...
- **A period-by-period breakdown.** The artifact has no period series — see FR-OVR-21 above.
...
**Spec coverage.** FR-MODEL-57's artifact limbs are backend-evidenced and untouched; its
caption limb is discharged by Task 3 step 2's second test, which is the assertion the
requirement's own row says nothing in the repository yet makes. FR-MODEL-92 is consumed by
