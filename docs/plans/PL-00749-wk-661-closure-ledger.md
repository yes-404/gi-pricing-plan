...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-22-w5-closure-ledger.md
---

# WK-661 Closure — ledger
...
**Maintainer decisions received 2026-08-22:** D1 accepted as proposed; D2 — NFR-482 out
...
| `fbec07a` | 2 | NFR-481 GLM two-fit determinism at 1e-10; raises OQ-599 |
| `585d490` | 1 | NFR-483 refusals carry `lineno`/`col_offset`/`end_col_offset` |
| `e9129e2` | 3 | FR-138 rebuild branch; also D1 + D2 |
| `7f96910` | 4 | FR-116 refusal reaches the caller by name |
| `2b3c992` | 6 | WK-661 closure record; WK-661's rows struck; CLAUDE.md §2 marks |
...
4. **Task 2 Step 4's grep was not empty.** The plan's conditional ("delete the parameter if
   ...
   tests**, and `02` §5.2 publishes the parameter. Recorded as **OQ-599**, open, with
...
6. **FR-138 cannot be met exactly as written.** Its `Diagnostics` clause says "loads";
...
- NFR-483: dropping `node=` fails both position tests; threading the child instead of the
  ancestor passes one and fails the other.
- NFR-481: refitting on one row fewer of 20 000 moves the intercept 5.8e-05, six orders of
  magnitude above the 1e-10 gate.
- FR-138: the call-count test fails on the pre-change handler; swapping the probe's read
...
- FR-116: narrowing the catch to `ModellingError` alone fails with
  `assert 'JOB_HANDLER_FAILED' == 'MODEL_OFFSET_MISSING'`.

## Open items raised, not closed

- **OQ-599** — `fit_glm`'s inert `seed`. Open, maintainer.
...
- **FR-94 / FR-166** evidenced only by "the route is published"; no test calls
  either endpoint. Owner: WK-664.
- **FR-178** carried by the module's only `xfail(strict=True)` — a pinned defect, not a
  refusal. A sparse cross still dies uncoded. Owner: WK-690.

## Not done, deliberately

FR-117(c) — building it would invert FR-117's own recorded ordering, and (a) is
Phase 1b and demand-gated. It stays refused by name, which this slice made true in the code as
well as in the sentence.

---

## Follow-on: OQ-599 decided, 2026-08-22 — option (b), remove the parameter

Maintainer decided at the recommendation's option. Implemented as **FR-179**.
...
`model_handlers.py:364`, the `fit_ebm` arm. Corrected in FR-179 and in the closure
record's defect row.
...
what NFR-481 exists to forbid and what `GlmCvSpec` had already refused on.
