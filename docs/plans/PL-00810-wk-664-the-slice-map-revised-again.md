...
family: plan
kind: leaf
title: WK-664 — the slice map, revised again
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-26-w6b-slice-map-revised-2.md
---

# WK-664 — the slice map, revised again

> **This supersedes [`PL-00786-wk-664-the-revised-slice-map.md`](PL-00786-wk-664-the-revised-slice-map.md) for
...
**Written:** 2026-08-26, after the `OQ-538`, `OQ-652` and `OQ-604` rulings, the
...
assign to WK-664. This map does not restate the previous revision's scope statement. The 2026-08-24
...
**The `_minor` rename became mechanical work.** `OQ-538` decision (b) landed 2026-08-25 via
...
**`OQ-652` was decided, and `W6b-11b` vacates exactly as the previous map predicted.** The
...
**`OQ-604` was ruled, and the change set needs a slice.** Ruling (c) landed 2026-08-26. The
server derives the surrogate slug at reservation. The 64-char refusal moves with the derivation.
`FR-141` gets an amendment. The version half becomes `OQ-613`. The change set merged
...
The 08-24 map's "Highest ids in use" block says `FR-57` is next free. `#197` minted
`FR-58` before that line. The W6b-12 plan header publishes the corrected line. Next free: `FR-59`, `NFR-DATA-11`, `OQ-DATA-16`. The OVR and PLAT families moved too. `FR-22` became
`FR-25`. `FR-397` became `FR-394`. Decisions minted `FR-23/24/25` and
`FR-394`. A DM sweep confirms that the map before 08-24 carried the same stale aid.
...
header (`FR-397`) already carries selection. Both readings refuse the stored-selection half.
A principal-level "current workspace" cannot represent two open tabs. `FR-397` made the
...
The 08-24 map found that `FR-168` "spans a view *and* a backend precondition". The holdout
...
panel"* (`02:232`, zero `WK-664` occurrences). The clause re-resolves onto `W6b-5b`. Both merged.

**`OQ-652`'s second amendment added routes to obligation 4's remainder.** The first-selection
...
```
grep -cE '^\| \*\*WK-664-[0-9]+[a-z]?\*\*' docs/plans/PL-00810-wk-664-the-slice-map-revised-again.md
```
...
| **W6b-10** | **Browser authentication** — `FR-393`'s PKCE flow and `FR-394`'s `/api/v1/auth/config` channel | W6b-14 | — | plan merged (`88a9c3e`, amended `845f298`). Build in progress |
...
mechanical. Its class definition (`FR-23`) is already in the spec.

---

## 4. The four decisions, each with its slice consequence

**`OQ-538` decided (b) — PR #235, squash `2010d50`.** Burning cost is a statistic. The integer
stays. The rounded-parts sum at `model-schema/perils.py:364-367` is the reason. The suffix is
`FR-23`'s defect, not a value defect. The released work is mechanical. This map attaches it as
**`W6b-15`**. §3's row carries the known members and the exceptions that conform.

**`OQ-652` decided (c) — PR #231 (squash `8d778ed`), amended by PR #237 (squash `d93cdf9`).**
...
**`OQ-604` ruled (c) — PR #246, squash `70934c1`.** The server derives the slug
...
worker's refusal becomes unreachable belt-and-braces. `FR-141` reads "at reservation". This
map attaches the change set as **`W6b-16`**. The version half is not part of it. It is
`OQ-613`, open.

**The ceiling correction, next free: `FR-59`.** The 08-24 map's `FR-57` next-free line predated
`#197`'s `FR-58`. §5 publishes the corrected lines for every family. The DATA line matches
...
| **P4** | **The modelling PII guard is not WK-664's and must not be absorbed into it** — carried unchanged. No maintainer line has landed since 08-24. Nothing in the roadmap names it. A column classified `direct_identifier` is still fittable. It sits on `FR-39`/`FR-90`. It needs a new id and a new unit | **unowned** — pending a maintainer line | *pending* |
...
| **P10** | **The gap list folds into the close-time completeness audit, not into this revision** — the auditor's findings arrive at the all-slices audit at close. They enter the slice inventory then. This map is filed from the four attachments of §4 | the close (WK-664 close slice), maintainer | *pending* |
...
Highest ids in use: FR-25, NFR-464, OQ-553. Next free: `FR-OVR-23`, `NFR-OVR-12`, `OQ-554`. *(08-24 said FR-22 — `FR-23/24/25` were minted by decided questions.)*
Highest ids in use: FR-58, NFR-474, OQ-570. Next free: `FR-59`, `NFR-DATA-11`, `OQ-DATA-16`. *(08-24 said FR-57. `#197` minted `FR-58`. The W6b-12 plan's header first published the corrected line.)*
Highest ids in use: FR-168, NFR-488, OQ-613. Next free: `FR-169`, `NFR-MODEL-15`, `OQ-MODEL-44`. *(OQ-613 minted with PR #246. The 08-24 block carried no OQ lines.)*
Highest ids in use: FR-394, NFR-535, OQ-656. Next free: `FR-440`, `NFR-PLAT-12`, `OQ-PLAT-18`. *(08-24 said FR-397 — `OQ-656` decided (a) as `FR-394`.)*
...
- The `/peril-structures/{id}` row reads *"Not started. Owner: WK-664, unchanged"* — the W6b-8
...
- The accessibility row reads *"`NFR-463`'s tabular fallback for charts is **not** built;
  owner WK-664"* — the W6b-9 build (chart-table retrofit) is merged. Correction: delivered.
- The W6b-11 gate text reads *"now waits only on WK-692 building the header half"*. That amendment
  is dated 2026-08-23, the day before WK-692 closed. W6b-11 does not wait on WK-692. It waits on
  `W6b-10`. `FR-396`'s amendment says so. Correction: dependency is internal.
...
assertion goes green on the first call site anyone adds. `NFR-488` stays booked forward as
*delivered but untested*. It is a WK-661 bench from 2026-08-22, not WK-664's. `test_gbm.py:1879` mentions
it in a docstring rather than a marker.

**`OQ-613` stays open.** The W6b-16 change set resolves the slug half of `OQ-604`
