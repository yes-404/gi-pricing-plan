...
family: plan
kind: leaf
title: RFC-897 landing package — the reconciliation update, the Work-row draft, and the acceptance batch (2026-09-01)
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-09-01-nt-0016-landing-package.md
---

# RFC-897 landing package — the reconciliation update, the Work-row draft, and the acceptance batch (2026-09-01)

| | |
|---|---|
| **What this is** | A **proposal package** collecting what the maintainer's next acceptance pass signs: the appended 2026-09-01 update to the frozen [`PL-00900-rfc-895-rfc-898-the-joint-reconciliation-2026-08-30.md`](PL-00900-rfc-895-rfc-898-the-joint-reconciliation-2026-08-30.md) (its new §8), the draft Work row for RFC-897's Stages 2–5 (§2 here), and the one-page acceptance-batch summary (§1 here). **Nothing here is in force**: a proposal binds only when the maintainer dates its acceptance line |
...
2. `git diff --stat 43fd277..<branch> -- docs/` names exactly two files:
   `docs/plans/PL-00900-rfc-895-rfc-898-the-joint-reconciliation-2026-08-30.md` and
   `docs/plans/PL-00938-rfc-897-landing-package-the-reconciliation-update-the-work-row-draft-and-the-acceptance-batch-2026-09-01.md`; and `git diff` on the reconciliation
...
4. No `FR-`/`NFR-`/`OQ-`/`ADR-` identifier is minted or claimed by this package. The ids it
   cites — `FR-72` and `NFR-500` — are already defined in
...
6. RL-948 is honoured by both changed files: notes are cited by `NT-00NN` id or by
...
which carries all four dispositions: RFC-895 — ADOPT the remainder (Slices E, F, G) as one
Work row; RFC-896 — ADOPT as one Work row (P1–P5); RFC-897 — **ADOPT Stages 2–5 as one Work
row**, the revision appended 2026-09-01 (§8.2 of that file); RFC-898 — ADOPT and close on its
residue (impact rows 6 and 9).

**Verified state at `43fd277`.** RFC-895 and RFC-896 both show *"Landed"* in the roadmap's
...
and none of the four dispositions binds until the signature. RFC-897's four investigation
...
`docs/roadmap.md` §7 (clause 2) — using the row drafted in §2 below for RFC-897 and the
reconciliation's own row shapes (§2–§5 of its frozen body) for the other three. RFC-898's
impact-row-6 line (the repository settings) is the maintainer's to write; nobody else can
supply it. RFC-897's row then opens on the maintainer's direction — the gate ruling's closing
...
[`../rfcs/RFC-00839-pending-proposals-for-the-14-review-at-wk-671-s-close.md`](../rfcs/RFC-00839-pending-proposals-for-the-14-review-at-wk-671-s-close.md) together, as review 11's proposal 11.1
...
release once RFC-897 has landed with a solid solution and implementation. The landings cited
...
maintainer's; `NFR-500`'s branch is the maintainer's to place; the F26, F58 and F61 owner
...
instances: RFC-895's Slices A–D under a dated delegation, RFC-898's S1/S2 under the note's
§7 light path, and now RFC-897's four slices and rulings under the 2026-09-01 acceptance.
...
clause addition, not a rewrite (the reconciliation's own §7 wording). RFC-897's §8.1 record
...
## 2. The draft Work row — RFC-897 Stages 2–5
...
[`RFC-897`](../rfcs/RFC-00897-file-taxonomy-reference-coding-and-custody-investigation-rev-2.md) §4–§7
...
**Placement — recommended, not decided.** Phase 2's workstream table, beside WK-672. Three
...
RFC-895 and RFC-896 conversion rows land beside this one on the same signature — one pass
...
process/documentation row of the same class as the RFC-895 and RFC-896 rows it lands beside.
...
lines in [`../rfcs/RFC-00839-pending-proposals-for-the-14-review-at-wk-671-s-close.md`](../rfcs/RFC-00839-pending-proposals-for-the-14-review-at-wk-671-s-close.md) are dated together under
