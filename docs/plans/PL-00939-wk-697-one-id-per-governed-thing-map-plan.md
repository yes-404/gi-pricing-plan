...
family: plan
kind: map
title: WK-697 — One id per governed thing: Map Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-09-01-nt-0019-id-standard-map-plan.md
---

# WK-697 — One id per governed thing: Map Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use subagent-driven-development (recommended) or executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Adopt RFC-937's document-id standard across the repository — one global integer
...
slice is dispatched, the same relationship `PL-00930-wk-672-testing-map-plan.md` has to its own child
plans. The cut follows RFC-937's four stages S1–S4 and says at every point where it departs
from them, and why.
...
1. `python3 scripts/doc-id.py check` exits 0 against `origin/main`: zero duplicate numbers,
   ...
   integer its filename pads. (RFC-937 §7 (b).)
...
8. Every **H** row in RFC-937 §5 is named by a merged commit, listed in a table in this
...
10. `python3 scripts/doc-index.py --phase P1b` prints a report containing every element
    RFC-937 §1.10 (c) names: Works closed and retired, slices planned versus delivered, plans
...
12. `docs/roadmap.md` carries a WK-697 row recording the close, and the maintainer has accepted
...
- **G1 — RFC-937 outranks current practice.** Maintainer ruling, 2026-09-01: *"RFC-937 is
  ...
  is **work in RFC-937's favour**, not a blocker and not a reason to narrow a slice. Two
...
- **G2 — the permanence rule yields, at two sites.** `CLAUDE.md` states *"Requirement IDs and
  ...
  edited in W37-9, each with a dated line naming RFC-937 as what it yielded to. An executor
...
- **G4 — the three gate scripts are standard-library only.** `.github/workflows/docs.yml`
  ...
  against the closed field set of RFC-937 §1.5. This is a verified fact about the repository,
...
**RFC-937 was written against `8f5d57d`, seven commits earlier.** Its §5 impact map and §10
...
(`docs/rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md`, `docs/plans/PL-00938-rfc-897-landing-package-the-reconciliation-update-the-work-row-draft-and-the-acceptance-batch-2026-09-01.md`)
...
| `docs/notes/` (18 notes + RFC-937 + README) | **20** | "(18) + README" |
...
**What this plan did not verify, and who owns it.** RFC-937 §5's per-row file lists and its
...
RFC-937's Sequencing row requires the migration to land *"now, at the next gap with no open
...
**The obligation.** [`../rfcs/README.md`](../rfcs/README.md) line 136-144's verdict table:
a note with status `accepted` obliges *"Named in a roadmap row before work starts."* RFC-937
is `accepted` (its own header, and its index row at `../rfcs/README.md` line 56). No roadmap
row names it: `git grep -n -E 'RFC-937|document-ids|doc-id|doc-index' -- docs/roadmap.md
...
**The acceptance line.** WK-693, WK-694 and WK-696 each rest on *"the reconciliation's dated acceptance
line"* — `docs/roadmap.md` lines 378, 379, 381. RFC-937 has no reconciliation, so the phrase
has no referent here. The question *"does the WK-697 row need a fresh dated maintainer acceptance
...
2. The `accepted` status is itself dated and the maintainer's: RFC-937's Status row reads
...
3. The reconciliation line exists in WK-693/WK-694/WK-696 because those three notes were **`open`**
   when the reconciliation ran; the line is what moved them to accepted. RFC-937 arrived
   accepted. The instrument differs because the situation differs, not because the standard
   is lower.
4. **WK-695 is the counter-example that proves the pattern rather than breaking it**: its row
...
Four stages, eleven slices. The stage column maps each slice back to RFC-937 §8; the two
...
the family table, the status vocabulary — directly out of RFC-937 §1.2, §1.2a and §1.5, which
...
**Files:**
- Create: `docs/process/document-ids.md` — RFC-937 §1.1 through §1.13, **lifted verbatim**.
...
**The parser is hand-rolled and stdlib-only** (G4, DP-5). RFC-937 §1.5's field set is closed
...
**Files:**
- Modify: `scripts/audit-docs.py` — add checks 30-39 per RFC-937 §1.11's table, extend the
...
tombstone check as **30**, and RFC-937 §1.11 also numbers its header check 30. RFC-937 §5.5
...
- [ ] The auditor's full-class sweep of RFC-937 §5 against `89dd2b1` has landed, and its
...
*"A `git mv` of that
directory for any reason -- not only RFC-897's planned move -- would leave the gate green
...
Each edit carries a dated line naming RFC-937 as what it yielded to, per G1's no-silent-change
...
**Recommended number: WK-697.** Derived, not chosen: `grep -oE '\bW[0-9]+' docs/roadmap.md |
grep -oE '[0-9]+' | sort -n | tail -1` returns **36** at `89dd2b1`, and WK-693-W36 were created
by #553 the same day. WK-697 is the next free integer, and no lettered variant is appropriate —
this is a new Work, not a split of an existing one.

**Placement:** immediately after the WK-696 row at line 381, keeping the four reconciliation-born
rows contiguous and ahead of WK-690's row at line 385.

Paste-ready. **Two mechanical notes for whoever pastes it.** First, the row deliberately
carries **no markdown link**. A roadmap-relative target — the form WK-695's row uses for
RFC-897, a relative path starting `../docs/notes/` — resolves from `docs/roadmap.md` but not
...
rather than theoretical. When pasting, wrap the row's first `RFC-937` mention in a markdown
link using the same relative-path form WK-695's row uses, with `0019-one-id-per-document.md` as
the filename. Second, the row is three cells, matching the table header at line 371.

```
| **WK-697** | **One id per governed thing — RFC-937, the whole standard** | Adopted 2026-09-01 from RFC-937 by the note's own dated `accepted` status, which the maintainer ruled that day is the acceptance line for this row — RFC-937 arrived accepted rather than being moved to accepted by a reconciliation, so the "reconciliation's dated acceptance line" the three rows above cite has no referent here; the derivation is in the map plan's Authority section. **Scope:** the note's §1 standard in full — one global integer sequence across every row and document family, the five-word status vocabulary, the family-per-directory layout, the YAML header with its closed field set, roles per family, and the generated index, ownership matrix and phase report. Its §4 one-time scripted migration; its §5 impact map — root governance, all of `docs/`, seven role charters, two agents, every skill (twenty-six substantively, a header on all forty-six), fourteen scripts, twelve tests, two CI workflows, and every code and test file citing a document or requirement (767 such files at the note's own tree `8f5d57d`, 770 at `89dd2b1`); and its §7 acceptance items (a)-(k). **Eleven slices** cut from the note's §8 stages S1-S4 by `docs/plans/PL-00939-wk-697-one-id-per-governed-thing-map-plan.md` — four building the instruments in parallel, one building the migration script against a fixture corpus, **one supervised run that moves the whole corpus and must land at a gap with no open branches**, four applying the conventions, and one proving acceptance (j) and (k). **Supersedes most of WK-695**: RFC-937 §9 replaces RFC-897 Stages 2 and 5 outright, lifts constraints C1 and C2, and Rulings 63 and 65 lapse by their own override clauses; Stages 3 and 4 survive as the two downstream Works the note's §8 names — the charter investigation and the create-read-retire audit. RL-943 is kept, as check 38. Rulings 55-58 are absorbed. **WK-695's row needs the lead's disposition in the same edit**, or the roadmap will carry two live rows planning the same corpus in incompatible directions — a bulk rename here against WK-695's "never a bulk rename". **Eight decision points recorded**: two disposed of in the plan with their reasoning, six open, four of those blocking — three block the migration run itself (two the maintainer's, one the decision-maker's) and one blocks the `CLAUDE.md` slice. **Acceptance:** the note's §7 (a)-(k) |
```

**Two things the lead should decide while pasting, which this plan cannot:**

1. **WK-695's disposition — the one that must not be skipped.** WK-695 was created 2026-09-01 by
   #553, and RFC-937, merged the same day as #555, supersedes its Stages 2 and 5, lifts the
   ...
   rename in WK-697 against WK-695's *"legacy migrates opportunistically-on-amendment only, never a
   bulk rename (C1)"*. The options are to mark WK-695 superseded by WK-697, carrying Stages 3 and 4
   into the two downstream Works, or to re-cut WK-695 down to those two stages. Either is the
...
Run against RFC-937 with fresh eyes, per `writing-plans`.
