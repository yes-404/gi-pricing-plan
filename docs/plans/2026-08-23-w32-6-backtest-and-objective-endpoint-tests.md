...
**Spec:**
- [`../specs/02-modelling.md`](../specs/02-modelling.md) — FR-MODEL-40 (backtest results),
  FR-MODEL-92 (a backtest is readable only inside its workspace, the refusal *naming* it),
  FR-MODEL-95 (the custom-objective routes), FR-MODEL-75 (who may derive an objective).
- [`../specs/06-governance.md`](../specs/06-governance.md) — the RBAC model the read and write
  paths check.
- [`2026-08-22-w6b-slice-map.md`](2026-08-22-w6b-slice-map.md) — this slice's row.
...
- **Requirement IDs are permanent** (§5). **This slice allocates none.** Every marker names an
  existing requirement: FR-MODEL-40, FR-MODEL-92, FR-MODEL-95, FR-MODEL-75. FR-MODEL-40 has no
...
evidence anywhere is `backend/tests/test_backtests.py:566-573`, marked FR-MODEL-92, which
...
```python
"""The backtest routes over HTTP (FR-MODEL-40, FR-MODEL-92).
...
def test_the_refusal_names_the_backtest_that_was_asked_for(api_client) -> None:
    """FR-MODEL-92's "naming it" clause, which `load_backtest:287-304` implements as
...
plus one OpenAPI-presence assertion at `:591-605` marked FR-MODEL-95. Seven routes, one of
...
FR-MODEL-75, which is about *who may derive an objective under what conditions*, and it proves
...
def test_deriving_from_an_objective_in_the_wrong_state_conflicts(...) -> None:
    ...
    this a test of FR-MODEL-75 rather than of RBAC."""
    assert response.status_code == 409, response.text
```
...
3. **FR-MODEL-40 had no marker anywhere.** Resolved: Task 1's four tests carry it.
...
To FR-MODEL-40's row, as part of the same single line:
...
To FR-MODEL-95's row, the same shape.

To FR-MODEL-75's row:
...
now have endpoint tests. FR-MODEL-40, FR-MODEL-92, FR-MODEL-95 and FR-MODEL-75 evidenced;
FR-MODEL-75's prior evidence was wrong and is corrected rather than added to.
...
Expected: the first passes; the second reports a higher marked count and lists FR-MODEL-40 as
...
- [ ] `uv run pytest backend/tests/test_api_backtests.py backend/tests/test_custom_objectives_api.py -q`
      ...
      designed to avoid.
- [ ] `req-coverage.py` shows FR-MODEL-40 covered, and the total moved.
...
behaviour rather than the document: FR-MODEL-40 (Task 1's 202/`Location`, read-back, and two
permission refusals), FR-MODEL-92 (the cross-workspace 404 and the "naming it" clause, both in
Task 1 Step 4), FR-MODEL-95 (Task 2's seven-route coverage: reads, the boundary, the three
conflicts, the expression-kind refusal with its permit, and certify's sampling default),
FR-MODEL-75 (Task 3's split). The three findings this slice does **not** fix — the uniqueness
