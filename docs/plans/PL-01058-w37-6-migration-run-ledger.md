...
family: plan
kind: leaf
title: W37-6 migration run — ledger
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-09-04-w37-6-migration-run-ledger.md
---
...
2026-09-03): every "record it in the ledger" line since RL-1046 D2 had no target file to
land in. This is that file. Pre-migration naming (`docs/plans/README.md`); it becomes an
`LG-` document at the run itself.

Held by the lead until the run's own executor exists (RFC-937 §1.6); executors append per
task and per PR from here forward.

## Acceptance Standard

This ledger's own tracked work is "done" per RL-1046's staged structure:
...
2. **Checkpoint 2**: the supervised migration run itself completes (RL-1046 D7's 8-hour
...
- **`wt-rowe`** (`fix/row-e-padded-id-prose-v2`, the stalled first row-(e) executor, no PR):
  ...
  **Disposition, delivered by `rowe2` 2026-09-04, committed `005bb69` on
  ...
  - **`_TOKEN_BOUNDARY_RE` — FOLDED IN, real bug.** `<`/`>` were hard boundaries, so the
    ...
    `.md` — a real filename citation (`PL-01240-<slug>.md`, RFC-937 §1.1 rule 3's own
  ...
  **Why neither latent bug showed in today's `--verify` runs**: the one on-disk example
  with this shape
  (`docs/rulings/RL-01044-a-padded-id-is-a-lint-error-where-it-names-a-real-governed-thing-f-is-measured-across-the-migration-a-split-source-s-index-section-lives-where-5-2-routes-it.md`'s
...
## `(h4)`'s measurement point — RL-1046 D2
...
The `## RL-1046 D2 — …` heading crash (above) was the census doing its job, not a code
...
| #717 | docs(plans): RL-1046 D7 — eight executors on the upgraded VM, disk-conditioned | `88ecfd0` | UNCHANGED: 14 (a dated append to a ruling record's prose, no predicate code touched) |
...
| #732 | docs(plans): ledger — #730/#733 merges, worktree incident, checkpoint-1 real figures | `e3739e1` | UNCHANGED: **13** fatal row(s), matching the recorded set of 13 — run `33888477735` on `docs-w37-6-ledger-cont7`'s `f2885ed` (**correction, deputy 2026-09-04 18:2xZ**: the lead's own entry here previously said 14, a remembered figure from before #725 re-recorded (e), not this run's own printed line — RFC-777 pasted-constant defect, fixed). This ledger's own continued history, no `docs/**` migration-scope path or predicate code touched. CLEAN, confirmed via the exact head SHA's own workflow runs (not a stale rollup) before merging. |
...
historical, and was resolved by the maintainer's RL-1043"): the checkpoint is cleared —
...
maintainer's RL-1043 (`4988dca`, 2026-09-03) already re-planned the slice — from a hand
...
see it. PRs #707 and #711 are Implement tasks of that re-planned work list (RL-1043 §2),
...
window gate fails after the run starts (RL-1046 D7: a second in-window failure is a halt
...
handover committed first; one replan (RL-1043, `4988dca`); work-list PRs merged since:
...
capacity the eight-executor cap (RL-1046 D7) was given on contention rather than on
...
4. The executor cap stays **eight** (RL-1046 D7 unchanged) — this budget shapes *when*
...
broken-input tests (cause3, cause4, negative control on `WK-663`). `ci-watcher-720` dispatched;
...
- **cause4** (`W3C/OpenTelemetry` → `WK-944C/OpenTelemetry`, 10 files) — `doc-id.py:5484`'s
  ...
  owed: `W3C/OpenTelemetry` fixed, `WK-663` (real lowercase slice suffix) untouched.
- **Placeholder id** (`OQ-556` → `OQ-8471`, no `REDIRECTS.csv` row) — RL-1041(iv): an
...
leading digits (digit→digit is never a `\b` transition). `OQ-540` (→`OQ-831`) swallowed
`OQ-539` (correctly excluded, ambiguous), leaving the orphan digit:
`OQ-539` → `OQ-831`+`1` = fabricated `OQ-8311`. Seven ids fabricated this way, collapsing
...
matches that happened to rewrite longer tokens (`RL-876` no longer eats `RL-1043`'s
...
W11-1-3
```
*Fenced 2026-09-04 under RL-1044 §5.1; value unchanged (row (d8), task #30) — this
...
exists, disclosed where none does (RL-1046 A's alias classes). **Never restored via
...
## 2026-09-04 — rowe2's own row-(e) fix moved PASS→FAIL, correctly: a second copy of the same bug on the write side, RL-1044 §1.8's own shape
...
an independent, unfixed copy of the identical text-matching bug — RL-1044 §1.8's "two
...
|---|---|---|
| cause3(a) Work/slice/task keys, 85 files (62% of code-tree `other`) — `W6b-11`→`WK-947b-11`, bare map has no compound | classified-table ruling (`:455-470`): slice/task keys are the disclosed alias class, never rewritten — bare `WK-664`→`WK-949`, compound `W6b-11` left whole and listed | executor-30-2, #30 |
...
| Lockfiles in the sweep's corpus (`uv.lock`, `pnpm-lock.yaml`, `frontend/pnpm-lock.yaml`) | new scope defect: a lockfile carries data, never a citation (RL-988 Part 2's class) — excluded from the sweep and (d) rows by declared entry, alongside `REDIRECTS.csv` | executor-30-2, same PR |
...
- **New executor (`exec-excl`), dispatched now**, one small PR: the three exclusions,
  ...
  constant (RL-988 §2), plus the `.pyc`/`PYTHONDONTWRITEBYTECODE` corpus fix.
...
prose (`FR-208` citing `**OQ-571**`) becomes an independent draft alongside its
...
legitimate per-directory bare-basename repeats aren't caught). Verified: `OQ-571`/`-24`
...
- **OQ-556**: ran `doc-id.py migrate --verify --keep` against a fresh `origin/main`
  (`85a352f`, post-#726) snapshot myself and read the actual output. `docs/REDIRECTS.csv`
  now carries `OQ-556,OQ-849,docs/specs/01-data-management.md,...` — a real row, no
...
Confirmed OQ-556 independently (matches: one `REDIRECTS.csv` row, no conflict) and
...
multi-ruling split file (`docs/rulings/INDEX.md#2026-08-30-nt-0014-q1-q3-q4-rulingsmd`, splitting
...
skill manifest are stamped by `migrate` with **no `id:` line at all**, by design (RFC-937
...
Also reproduced the lockfile risk for real: this corpus's own `RFC-711 -> RFC-1`
...
| d9 | FAIL | `docs/plans/2026-`-prefixed citation form, un-migrated — real structurally-unfixable-here citations remain, per PR #740's own assessment; stays FAIL, not further reducible in this PR | exec-paths | #740 open, not yet merged (§ below) |
| d10 | FAIL | `docs/audit/`-prefixed citation form, un-migrated — same as d9, stays FAIL | exec-paths | #740 open, not yet merged |
| d11 | FAIL | the old notes directory, un-migrated — PR #740's wrap-preserving rewrite + trailing-slash directory tokens drop the real fatal population from ~135 to ~5 lines (measured on #740's own branch, not yet re-verified post-corrections) | exec-paths | #740 open, corrections required before merge (see dated section) |
| d12 | FAIL | `docs/adr/`-prefixed citation form, un-migrated — same mechanism as d11, fatal population ~38→~3 lines on #740's branch | exec-paths | #740 open, corrections required before merge |
| d13 | FAIL | the old `.claude` notes root — INERT, see its unanchored companion. **#740 as shipped claims FAIL→DISCLOSE at ~90 disclosed, but this is a false green** (deputy ruling, 2026-09-04): the disclosed-count predicate was evaluated before the tombstone-row-emission mechanism (an earlier 18:xxZ ruling) was actually shipped, so "no successor in REDIRECTS.csv" is true only because the thing that would make it false wasn't built. Real successor exists via the tombstone README's table. **Stays FAIL until #740 ships the tombstone rows** and d13 is recomputed — expected DISCLOSE with a small count (bare-directory mentions only), not ~90 | exec-paths | #740 open, does not merge until this ships |
| g | FAIL | the token-boundary defect, RL-1043 §2 row 1 — residue down (none=320→190 on `6c92d55`); the +15 mangled-companion rise is **class 3c** (deputy ruling, 18:3xZ) — the instrument's own test modules exhibiting the shapes they detect, same class as (d)'s ruling — **discharged by exec-ids' `_docid.sweep_exclusion_reason` tuple landing, not by explanation**; **predicted drop: 21** (`tests/test_doc_id_migrate.py` 11 + `tests/test_doc_id_verify.py` 10, exact `MANGLED_CITATION_RE`-pattern grep, every occurrence line-numbered). **Measured on #737's docs run**: migrated −34 (472→438, control 38→21), split control −17 (the tuple's own effect on the fixture files) + migrated −17 (from #737's respellings/fences beyond the tuple). **Predicted 21 ≠ measured 17 on the tuple itself — a 4-line gap, reconciliation dispatched to executor-30-2**, task #42 not yet closable until the four lines are named | multiple (partial) | #720, #723, #727, #729, #737 (open) merged/pending; reconciliation in progress |
| h1 | FAIL | `audit-docs.py` non-disclosed classes still non-zero. **Full class breakdown, tree `5b6c903` (post-#735), classified via `sed -n '/^FAILED/,$p' <log> \| grep '^  - ' \| sed -E 's/^  - //; s/^(check [0-9]+):.*/\1/; s/^broken link in .*/check 1/' \| sort \| uniq -c`, sum reconciles exactly to 8368**: check 36=5657 (legacy pre-migration form survives), check 32=2286, check 1=250 (broken links), check 35=79 (disclosed, W37-10), check 30=78 (disclosed, W37-10), check 29=11 (disclosed, W37-10), plus 7 singleton failures, **named by check number now that #741's classifier fix can attribute them** (deputy, 2026-09-04 21:5xZ): check 2 ×1 (undefined FR reference — never-allocated predicate, disclosed), check 5 ×3 (undefined ADR-1/2/3 references — class 3a fence with the note), check 27 ×1 (digest mismatch, reconcile miss — investigating), check 28 ×1 (missing Acceptance Standard heading — **fixed directly by the lead**, `6a6a64a`, to avoid a collision on this actively-edited file), check 31 ×1. All seven already owned by exec-h1 (task #47) before #741 could name them by class — #741 unmasked no new work, only made the existing seven attributable. Zero `unclassified` on the migrated tree is task #43's acceptance line. **8200 outside the 168 disclosed (29/30/35)** — check 36 and check 32 dominate. **Ruled (deputy, 2026-09-04 20:1xZ)**: check 36 and (d) are "one rule at two times" (RL-988 §2, same `_docid.LEGACY_FORM_PATTERNS`) but currently disagree — check 36 has no fence exclusion, no disclosed class, no test-module tuple, so it fails on everything (d) has already disclosed. Fix: check 36 adopts (d)'s exclusions/disclosed classes via the same shared predicates; acceptance is check 36's fatal count reconciling exactly with the sum of (d)'s fatal rows on one snapshot. **Check 32's kinds** (python categorization against the same log, sum=2286 exact): padded id outside a link target (RFC-216 §1.1 rule 2)=2043; id does not resolve in `docs/INDEX.md`=240; link-text/target id mismatch=3. No fix dispatched — kinds are tabled per the ruling, most expected to be class-1's disclosed population wearing a different message. **Check 1**=250 broken links, the link subset of d9-d13 — discharged by exec-paths' landing, remeasured then. | exec-h1 (check 36 queued after task #43, task #46; the seven singletons, task #47); exec-paths (check 1, via d9-d13) | #735 merged `5b6c903`; follow-ups pending |
...
- **`exec-ids`** (d1, d6, d7) — 74 lines in ≤35 files. First deliverable is investigation,
  not code: the 74 lines by directory and by kind (citation vs. specification, RFC-777's
...
- **`exec-h1`** (h1) — checks 32 (downstream of A/B, not yet), 36/1/31/27 (now): each
  ...
  with owner) before any fix. 29/30/35 stay disclosed (RL-1046 §B), untouched.
...
migrated under a separate, earlier effort (RFC-181 Slice 4, RL-951 — its own id fenced
...
in `.claude/rfcs/README.md`'s own table, which `migrate()` never reads. d13's 90 lines:
...
building a second redirect-consuming path reaching outside RFC-937's domain, or rewriting
...
- **Check 27** — a self-correction: exec-h1's own first automated pass read this as zero,
  ...
  step per RL-905, not something `migrate()` should auto-fix).
...
## 2026-09-04 — d13 ruled in scope after all: RFC-937 §5 already schedules its own retirement; d9-d12 get a precise three-part fix
...
RFC-937 §5 itself schedules the `.claude/notes/` stubs' deletion (step 4, `:281`/`:342`)
and that exemption test's own removal (`:409`, replaced by `test_audit_docs_redirects.py`).
RL-951 is recorded **INVALIDATED**, its own horizon explicitly at W37-6
(`docs/research/RS-01003-ruling-acceptance-item-sweep-audit-record.md:133-145`, quoting `audit-docs.py`'s docstring:
...
zero, in scope.** Fix: `migrate()` reads `.claude/rfcs/README.md`'s tombstone table once
...
|---|---|---|---|
| 3a — Markdown/frozen-prose exhibit | `.claude/skills/docs-audit/SKILL.md`, `docs/plans/PL-00282-…md`, `docs/plans/PL-00066-…md` (borderline, ruled in) | 5 | Fenced, RL-1044 §5.1's note verbatim, same as (e)'s exhibits |
...
own test modules" — its subject, `scripts/register-owed.py`, is itself an RFC-937 `:382`
...
decided here. `exec-ids` proceeding on its two unambiguous fixes (`RFC-895-896`'s
...
on a real file diff (`docs/rulings/RL-01049-w37-6-the-maintainer-s-time-boxed-delegation-2026-09-03.md` 1 occurrence →
`docs/rulings/RL-00295-...` 2), with a negative control confirming the mechanism is
conditional on title text, not universal (`docs/plans/PL-00939-wk-697-one-id-per-governed-thing-map-plan.md`
...
verbatim) RL-1045 §2 already names, occurring at two levels — family/global index
...
- **g**: residue down (none=320→190) but **the mangled companion rose by 15** (457→472)
  — a real regression signal, exactly what RL-1043 §2 row 1 exists to catch. `#733`
...
documents whose content is fully correct — only the filename slug still carries `WF-698`.
...
separate, genuine un-migrated-citation issue; the growth (RL-949 +4, RL-950 +2,
RL-951 +2) is fully explained: all three are single-H1 historical files never split
...
2-occurrence exception" is refused on the merits regardless: RFC-937 makes id==filename
...
`a2c0afa`): task keys join d8's disclosed component per the deputy's Ruling #26 (RFC-937
