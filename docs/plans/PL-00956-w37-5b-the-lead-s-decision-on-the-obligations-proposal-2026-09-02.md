...
family: plan
kind: leaf
title: W37-5b — the lead's decision on the obligations proposal, 2026-09-02
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-09-02-w37-5b-slice-decision.md
---
...
[`../rulings/RL-00969-w37-6-s-go-ahead-is-withheld-the-maintainer-s-decision-of-2026-09-02-and-what-yet-costs.md`](../rulings/RL-00969-w37-6-s-go-ahead-is-withheld-the-maintainer-s-decision-of-2026-09-02-and-what-yet-costs.md).
...
1. **`W37-5b` is named in `docs/roadmap.md`'s WK-697 row**, in its `Progress` clause, with its
   ...
   `W6b-5` all appear only inside a Work row's prose, and WK-697's own `W37-1`…`W37-6` do the
...
3. **A new dated leaf plan for W37-6 is filed**, superseding
   `2026-09-02-w37-6-migration-run.md` by name and date, carrying RL-970's amendment
   to acceptance item 11 in the plan's own text rather than by reference.
4. **This record is cited where W37-5b's close is recorded** — the WK-697 row's `Progress`
   clause, and the Work-level closure record if WK-697 later files one — so the reasoning for
...
`is_vendored`, which still keys on `LICENSE` presence — the criterion **RL-990
...
- **RL-977 §2** — the parser fix *"lands as its own pull request, merged on its own,
...
- **RL-985 §3 item 1** — *"the census runs before W37-6, not during it"*, with §2
...
`_discover_roadmap` faces semantics carried in decoration (`~~**WK-660**~~ ✔`), non-work rows
sharing the work column, free-prose status cells, and several status tables per phase.
`_discover_plan_reviews` needed **RL-978 to decide what the unit is** before any pattern
...
mis-nested under a `##` headed *"Pending proposals"*. RL-978 §3 item 4 files it as a
structural correction to the document, and RL-978's own reason for raising it is that
...
charters and eleven skills to **S3**; RL-987 pulls three charters and six skills into
...
- **It does not reopen W37-5.** W37-5b is a successor slice. **W37-5 has no closure record
  ...
  merge"*, reserving a filed record to a Work, Phase or Project; `docs/closures/INDEX.md#closure-recordsmd`
  contains zero occurrences of `WK-697` at `b648c22`, and the roadmap states only *"W37-5
