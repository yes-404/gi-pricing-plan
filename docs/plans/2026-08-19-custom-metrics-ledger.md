# SDD ledger — plan: .planning/2026-08-19-custom-metrics.md

Spec: `docs/specs/02-modelling.md` — FR-MODEL-45 (§3.7), §4.4, §4.5, §4.7, §5.1. Read, reachable.
...
|---|---|---|
| T1 → T2..T6 | FR-MODEL-103..108, the ids every later `@pytest.mark.req` names | Consistent. `req-coverage.py` fails on a marker naming a requirement that does not exist, so T1 must land first — it does. |
| T1 → T5 | `METRIC_*` codes: T5 declares them in `02` §5.1 **and** registers them in `errors.py` in one commit | Consistent, and **unlike the FR-MODEL-96 slice there is no deliberately-red window**: `test_spec_error_codes_are_all_constructible` iterates `PLATFORM_ERROR_CODES` only (OQ-OVR-9 records exactly this gap), so a modelling code declared in the spec reddens nothing. Noted so nobody expects PR #120's red-test pattern here. |
...
| T3 | 6 tests vs 2 functions | Agrees **after Ruling 2** — the plan's `SamplingSpec(...)` call is wrong as written. |
...
implementer added condensed rows there in the existing OQ-MODEL-16/17 style rather than inventing one,
...
FR-MODEL-106's premise at the mechanism rather than trusting it: `grep -rn 'eval_metrics' --include=*.py .`
...
pre-existing OQ-MODEL-16 pattern the brief told them to follow. Noted for whoever owns the convention.
...
Task 2: **Ruling 5 — `MetricDirection` is a duplicate, not a namesake. Reuse the existing enum.**
...
declared with the metric rather than assumed by the reader"* — is FR-MODEL-104's concept word for word.
The metric enum's two members are a strict subset. **Reuse it, and refuse the two non-early-stopping
members on `CustomMetric` with a validator naming FR-MODEL-104:** early stopping compares successive
...
Task 2: **Ruling 6 — the spec example is wrong and the code is right: `"cap": 250000.0` → `250000`.**
The `money_minor` template parameter's own `check` refuses a float, so the JSON in `02` §4.13 could not
be constructed. Money is integer minor units (`CLAUDE.md` §7, and OQ-OVR-8 decided `DecimalStr` refuses a
...
OQ-OVR-9-shaped work, not this slice's. *Cost if wrong:* none — an example that cannot be constructed is
...
Task 2: **Ruling 7 — the same defect class one line further down, and it gets the same answer.** The
...
Task 2: **Ruling 8 — the Important finding is real and is fixed now, not deferred.** `MetricCertificate`
...
Task 2: **Ruling 9 — a Minor upgraded to Important, on the sibling's own reasoning.** The reviewer noted
...
would pass Task 6 and then evaluate a Gamma loss on counts. FR-MODEL-105's `finiteness` check would not
...
Task 2: **Ruling 10 — `MetricCertificate` must be a published contract, because its sibling is.** The
...
registers `"objective-certificate": "ObjectiveCertificate"` at line 87, and **FR-MODEL-108 declares
`GET /custom-metrics/{id}/certificate` returning a `MetricCertificate`** — so it crosses the API boundary
exactly as its sibling does. ADR-0002 and FR-PLAT-48 make a boundary-crossing shape a published contract;
...
Task 3: **Ruling 11 — the plan's Task 3 test helper is now refused by Task 2's own validator, and the
...
Task 3: **Ruling 12 — no `# type: ignore[arg-type]` for the `template` narrowing.** The plan's
...
discriminates. ADR-0001 verified at the imports (`metrics.py` pulls only numpy and model-schema — no clock,
...
Task 4: **Ruling 13 — the implementer found a real bug in Task 2's output, and it is load-bearing.**
...
Task 4: **Ruling 14 — the three omitted constraints get added, and the implementer was right to ask.**
...
`direction` CHECK — the type refuses it and so does the table. **Ordered after Ruling 13**, so the
...
Task 2 (reopened): Ruling 13 fixed — commit `30b6388`, 13 tests. The implementer confirmed the new
...
`draft` and `deprecated` — so Ruling 13's fix did not simply move the bug one layer down. Migration
...
Task 5: **Ruling 15 — a declared lifecycle edge is unreachable again, and it is the same defect class as
Ruling 13.** `backend/src/app/api/approvals.py:320,326` dispatches an approval decision to
...
declares `{APPROVED, CERTIFIED}` and FR-MODEL-45 requires the same lifecycle as objectives. Load-bearing:
...
Task 5: **Ruling 16 — a live bug in shipped code, found by building the parallel path. Fixing it, out of
...
defaults). That is OQ-GOV-7-shaped governance policy rather than a code defect, and this slice should not
decide it.

Task 5: fix round 1/5 (2 addressed, 0 open; commits 49bc16d..040e6e8). `deb49e7` — FR-MODEL-45's
...
Task 5: **Ruling 17 — the `DEFAULT_POLICY` entry is accepted on precedent, not as a fresh governance
...
was blocked two layers deep. I had deferred `EVIDENCE_FLOOR` as OQ-GOV-7-shaped and did not want this slice
...
Task 5: **Ruling 18 — and the spec owes the matching note, which Task 7 must write.** `06` §4.2's JSON
block enumerates these entries by artifact type and carries the exact precedent in a dated note:
*"`peril_structure` added 2026-08-18 (W5, the peril-structure slice)."* The code now has a `custom_metric`
...
Task 6: **Ruling 19 — Task 6 extends the derived error-code test to the GBM arm, because it is about to
...
evidenced rather than asserted (`CLAUDE.md` §13.4). Distinct from OQ-OVR-9, which is spec-vs-registry; this
...
Task 6: **Ruling 19 discharged, and it came back clean.** The AST-walk extension to
...
Task 6: **Ruling 20 — the Important finding is real and gets fixed, not pinned.** `_fit_lightgbm`'s
...
reasons to fix rather than document: `gbm.py`'s own module docstring commits to FR-MODEL-25's *"one
contract, two backends"*, and nothing in FR-MODEL-106/107 records a backend-specific reduction — so the
...
Task 7: **Ruling 21 — failure 1 is a real omission and is fixed.** `JobKind` gained `metric.certify` in
...
Task 7: **Ruling 22 — failure 2 is a real omission, and I misinformed the Task 5 dispatch.** The three
...
cross-checks those two lists (OQ-OVR-9), so this is discipline rather than an enforced check." That was too
broad and it is worth correcting:** OQ-OVR-9 is about spec-vs-`errors.py` in general, but this repository
...
Task 7: **Ruling 23 — failure 3 is a stale test assumption, and the reason it went stale is a milestone.**
...
FR-MODEL-104's stated failure verbatim — *"stops the fit at the wrong round in exactly half of cases, and
...
**Important:** I3 LightGBM drops declared *builtin* eval metrics (Ruling 20's defect, second instance);
I4 `EVIDENCE_FLOOR` has no `custom_metric` entry **and `metrics.py:643-647` claims a control the code does
not have**; I5 `MetricUsage` defined in `backend/` rather than `model-schema` (Ruling 10's argument not
extended to the usage pair); I6 the roadmap's own `W5 — outstanding work` table still says 34/35 and lists
...
*Ruling 20 was incomplete and its own fallback was not applied:* it generalised from the custom-vs-custom
...
covers"* — two survived. *Ruling 17 was over-extended:* the `perils.py` precedent covers `DEFAULT_POLICY`
and establishes nothing about `EVIDENCE_FLOOR`, and FR-GOV-37's empty-floor rule is scoped to artifacts
...
entirely. **Ruling 16 (the out-of-scope money fix) was judged right**, on the grounds that the rule is
...
than stopped on, where XGBoost reports both. That is Ruling 20's fallback applied properly this time —
...
and its `EVIDENCE_FLOOR` entry, owner W5, in that order. This is the gap Ruling 17 mis-deferred; it is in
...
**Ruling 24 — the residual is parked, and raised as an open question rather than decided.** LightGBM
...
so. That is a narrower instance of the exact defect FR-MODEL-106 was raised against. It is **pre-existing
on this branch** (`params["metric"]="None"` predates the fix wave and appears as unchanged context), and it
is now tested and named in `02` FR-MODEL-107's 2026-08-20 amendment — so it is a *documented* asymmetry,
not a silent one. But whether a documented silent drop satisfies FR-MODEL-106's "honoured", or whether the
...
labelled `rmse` that is not LightGBM's `rmse`, which is worse and violates FR-MODEL-106's "passed to the
