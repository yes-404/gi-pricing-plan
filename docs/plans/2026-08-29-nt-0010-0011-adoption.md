# NT-0010 / NT-0011 Adoption — Implementation Plan
...
`docs/plans/2026-08-29-nt-0010-0011-reconciliation-rulings.md` ("the rulings record") rules
...
product code** — this is documentation and role definitions only, per NT-0010's own
Deliverable line (`.claude/notes/0010-layered-slice-based-workflow.md:7`).
...
**Spec:** `docs/plans/2026-08-29-nt-0010-0011-reconciliation-rulings.md` Parts A–D (the
governing record); `.claude/notes/0010-layered-slice-based-workflow.md` and
`.claude/notes/0011-per-agent-model-and-skill-settings.md` (the source proposals, cited only
...
**Slice source:** This is a single Work item (the NT-0010/0011 adoption) sliced directly
into Tasks below — no parent map file exists yet because Part C row 3 rules "Project" needs
only the label, not a new artifact (`docs/plans/2026-08-29-nt-0010-0011-reconciliation-
rulings.md:417`). Team-lead's dispatch (`task_assignment` id 5, 2026-08-29T11:29:31Z) is the
slicing authority for the ordering used below.

**Highest ids in use at the anchor (`3b66ede`):** N/A — this plan mints no `FR-`/`NFR-`/
`OQ-`/`NT-` id. `.claude/notes/` currently ends at NT-0011; this plan creates no new note.
...
- **PR-only; lead-only merge; CI verified on the exact head** (NT-0011 §3 delta 3, adopted
  as-is — rulings record Part C, NT-0011 row 3).
...
- **A dated correction is recorded beside the fact it corrects, never by silent
  replacement** (the rulings record's own convention, Part B8; the notes' `NT-0005` custody
...
- **Also not in scope, recorded for completeness:** `.claude/notes/0011-…md` lines 35 and
...
`adr/`, `skills-map.md`, `open-questions.md`, `phase-0-status.md`, `roadmap.md`. It has no
...
Part B8 rules NT-0010's claim ("`.claude/agents/` holds eight delegable specialists")
corrected to seven, and PR #320 already applied it in place with a dated note
(`.claude/notes/0010-…md:58-61`, "*(corrected 2026-08-29 — a directory listing that
...
**`.claude/notes/0011-per-agent-model-and-skill-settings.md:58` carries the identical
miscount — "`.claude/agents/` holds eight delegable specialists and no role definitions" —
and has not been corrected.** Part B8's own text names only NT-0010; it does not claim to
have swept NT-0011 for the same sentence, so this is not a case of re-opening B8 — it is
the sibling instance a full-class sweep is supposed to catch and B8's own scope never
claimed to reach. Task 2 fixes it, in the identical dated-correction style NT-0010 already
uses, so the two notes read consistently.

### Finding 4 — the existing closure checklists already substantially satisfy NT-0010's audit obligations; no new checklist file is needed for "Project"

`docs/audit/checklists/phase-close.md` and `work-item-close.md`, read in full: both already
use "phase" / "work item" (workstream, slice, or PR) / "slice" terminology identical to
NT-0010's Project→Phase→Work→Slice hierarchy, already point at `docs/audit/phases/<phase>/`
and `docs/audit/work/<id>/` record conventions, already carry the §13 four-verdict /
`docs/audit/register.md` carry-forward discipline, and already state "a finding with no
verdict is silence, which §13 forbids" — matching NT-0010 §12's obligations closely enough
...
|---|---|---|
| `docs/process/delivery-process.md`, the human-checkpoint section (from NT-0010 §9) | A1 | (a) checkpoint stays at Work/workstream close, escalation-vs-acceptance distinction adopted; (b) checkpoint moves to Project close only, §13/§14 gain an explicit no-per-workstream-checkpoint rule; (c) hybrid — Work close auto except a register-bound defer |
...
| `.claude/notes/0010-…md` and `0011-…md`'s `Status` field | Both A1 and A2 | Stays `open` until both close (rulings record Verification section, `...rulings.md:497-498`) |
...
**Interfaces:**
- Consumes: rulings record Part C rows (NT-0010 §§1, 3, 5, 6, 7, 8, 10, 11, 12, 14, 15 —
  all `Adopt`; NT-0011 §§1, 2 [non-A2 fields], 3 — all `Adopt`); Part B1 (parallelism
...
Content, adapted from `.claude/notes/0010-…md`'s "Original wording" section (lines
...
`docs/workflows/wf-01…05`, the cross-module *domain* journeys (`CLAUDE.md` §4).

## 1. Purpose

[NT-0010 §1 content — the four-layer hierarchy, gated at every layer, one human checkpoint.
...
[NT-0010 §2's role table verbatim — Lead/Planner/Decision-maker/Auditor/Executor/Watcher/
...
[NT-0010 §3 — Project (the whole-repository scope `CLAUDE.md` §1 already names informally;
no new artifact, per rulings record Part C row 3) → Phase (`CLAUDE.md` §9) → Work
(workstream) → Slice.]

## 5. Per-layer flow

[NT-0010 §4, amended per rulings record Part C row 4 / NT-0011 §3 delta 1: auditor
...
[NT-0010 §5's TDD cycle, confirmed against `test-driven-development` / `python-test`
skills — Adopt. Step 6's "who decides fix/accept/defer" text uses §5's wording, not the
original's — same correction as §5 above. The enforcement *hook* NT-0010 §5 step 4 assumes
...
[NT-0010 §6's caps (≤1 Project/Phase/Work, ≤2 Slice) adopted **as instrumented defaults**
per Part B3: log every replan/audit-fix loop iteration and re-audit count from the first
slice under this process (the pilot — see Task 5's note on W11); revisit the numbers after
a workstream's worth of data. The ≤2 Slice cap is not a settled ceiling — W10-3A's own
history sits exactly on it.]

## 8. Parallelism

[NT-0010 §7 adopted **with the Part B1 carve-out**: sequential processing of a layer's
...
[NT-0010 §8 — Adopt as-is; this *is* `docs/audit/register.md`, verbatim-matching per the
rulings record's own verification. No new file.]

## 10. Required artifacts

[NT-0010 §10's list, with names corrected: process spec = this document; agent settings =
...
NT-0010 §11's own words." Do not restate the eight obligations here; one source, not two.]
...
[NT-0010 §14 verbatim-adapted — events-over-polling, the watcher script + agent split,
...
[NT-0010 §15 — Adopt as-is; this document, the rulings record, and this plan are steps 1–3
of it, in progress.]
```

- [ ] **Step 2: Write `docs/process/agent-settings.md`**

Content from `.claude/notes/0011-…md`'s "Original wording" §§1–3 (lines 171–332), with
...
- The lead's row (Part B9): **"Tools: full read; git merge authority (sole merge
  authority); write to handover/status files only."** — NT-0011's version, not NT-0010's;
  state the correction inline: *"NT-0010 §2's lead row disagreed (read-only + plan/map
...
- **Do not write a final `docs/`-write tools column for planner, decision-maker or
  auditor.** State per role: *"docs/-write scope: pending `docs/plans/2026-08-29-nt-0010-
  0011-reconciliation-rulings.md` Part A2 — see `.claude/roles/<role>.md` for the current
...
git commit -m "docs(process): adopt NT-0010/0011 — delivery process, agent settings, CLAUDE.md pointer"
```

## Task 2: Fix the two drift instances this plan's own sweep found

**Files:**
- Modify: `.claude/notes/0011-per-agent-model-and-skill-settings.md:58`
...
- Consumes: Finding 1 and Finding 3 above; the exact dated-correction style already applied
  to `.claude/notes/0010-…md:58-61` (mirror it, do not invent a new style).
...
- [ ] **Step 1: Correct NT-0011's specialist count in place**

In `.claude/notes/0011-per-agent-model-and-skill-settings.md`, line 58, change:
...
Run: `grep -n 'seven delegable specialists' .claude/notes/0011-per-agent-model-and-skill-settings.md`
...
```bash
git add .claude/notes/0011-per-agent-model-and-skill-settings.md .claude/skills/writing-plans/SKILL.md
git commit -m "docs(notes): correct NT-0011's specialist count and writing-plans' stale superpowers: refs"
```
...
**Files:**
- Modify (small, or none — see Step 1): `docs/audit/checklists/phase-close.md`
- Modify (small, or none — see Step 1): `docs/audit/checklists/work-item-close.md`
...
**Do not start this task until both `docs/plans/2026-08-29-nt-0010-0011-reconciliation-
rulings.md` Part A1 and Part A2 carry a dated maintainer acceptance line.** Nothing in
...
- `.claude/notes/0010-layered-slice-based-workflow.md` and `0011-…md` — flip `Status` from
  `open` to whatever `.claude/notes/README.md`'s convention calls the accepted state, once
...
mechanism NT-0010 §14 and NT-0011 specify. The scripts themselves are not written by this
...
**is W11's first slice, not a synthetic exercise** — the mechanical scripts, if built at
...
before the slice runs. Building Task 6 now would be guessing the same way NT-0010's own
...
concrete artifact is covered: NT-0010 §§1, 3, 5, 6, 7, 8, 10, 11, 12, 14, 15 and NT-0011
§§1, 2 (non-gated fields), 3 → Task 1 (process docs) and Task 3 (role files). NT-0010 §2 /
§4 and NT-0011 §2 (gated fields) / §1 (auditor's tool-scope contradiction) → Task 5,
blocked, per Part A2. NT-0010 §9 → Task 5, blocked, per Part A1. NT-0010 §13 (self-
