...
family: plan
kind: leaf
title: Phase 1b Pre-decision Implementation Plan — findings, dispositions, UAT-readiness
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-27-phase1b-predecision.md
---
...
**Architecture:** Two halves. Half 1 enumerates every open finding from the WK-665 close residue, plan review 6, `docs/findings/register.md`, and the deferred items, and gives each a disposition. Half 2 defines what UAT-ready requires for the exit demo and lands the fix-before-close findings. The sequence ends at the auditor's close-confirmation.
...
**Spec:** Plan review 6 (P1: the core `WF-698` journey is the Phase 1b exit criterion) · the WK-665 close record (`docs/closures/INDEX.md#closure-recordsmd`) · the WK-664 close record carry-forward · `docs/findings/register.md` and `docs/findings/register.md`.
...
The sources: the WK-665 close residue, plan review 6, the audit register, and the deferred items. Each row names the finding, its state, and the proposed disposition. A ruling on a decision point can move a disposition.
...
| F5 | NFR-488 has no marker | measured 0.0480 fits per pass against the 0.06 budget. The marker is missing | fix before close, add the marker (MD5) |
| F6 | FR-59 (`unrun_layers`) | Phase 2 handoff, not built in Phase 1b | defer with a strong reason: the phase boundary. Owner, the Phase 2 validation-report successor |
| F7 | FR-67 (exposure-ordered top-20) | trigger-checked 2026-08-27. The trigger has not fired. Unowned by design | defer with a strong reason: the trigger has not fired. No consumer has asked for the work |
| F8 | The full `03` rating surface (compile, score, rate tables, deployment) | Phase 2 | defer with a strong reason: the phase boundary |
| F9 | The `WF-698` §4 surfaces the demo does not seed (bandings, Peril Structure, reconciliation) | Phase 2, plan review 6 P1 | defer with a strong reason: the phase boundary |
| F10 | The `listRules` client gap (backlog #8) | **RESOLVED** — `frontend/src/api/rules.ts:114` exports `listRules` with tests | discharged |
| F11 | FR-194/196 (prediction) delivered-but-untested | markers exist in the prediction path | discharged, verify the markers at close |
...
**MD1 — F1, the `validate.py` strings.** The strings assert minor units on a statistic. The WK-664 close record says they need their own line item. Options: fix before close (rewrite the strings to not assert minor units), or carry forward to the next money-correctness pass. **Recommendation:** fix before close. The strings are user-visible and small.

**MD2 — the demo UAT scope.** UAT-ready covers the core `WF-698` journey over HTTP: validated dataset, split, factors, GLM and GBM fits, comparison, approval, rating version. The surfaces plan review 6 records as Phase 2 — bandings, Peril Structure, reconciliation — stay out of the UAT. **Recommendation:** accept the core-journey scope.
...
**MD5 — F5, the NFR-488 marker.** The measurement exists. The marker is missing. Options: add the marker to the measured test, or accept the recorded measurement as the evidence. **Recommendation:** fix before close. A marker makes the measurement visible to `req-coverage.py`.
...
UAT-ready means the exit demo runs the core `WF-698` journey end to end over HTTP, the postconditions verify, and the UI shows the journey results. The acceptance mechanism is the scripted HTTP run, with the UI available for hands-on driving.
...
- The total seed time stays within the NFR-529 budget, or the deviation is recorded.
...
1. The findings and dispositions are filed in `docs/findings/register.md` and `docs/findings/register.md`.
...
**Files:**
- Modify: `docs/findings/register.md`
- Modify: `docs/findings/register.md`
...
### T4. F5 — add the NFR-488 marker (MD5, fix before close).

**Files:**
- Modify: the backend test that holds the NFR-488 measurement

- [ ] Add `@pytest.mark.req("NFR-488")` to the measured test.
- [ ] Verify `req-coverage.py` now reports NFR-488 as evidenced.
...
- [ ] Record the total seed time against NFR-529. If it exceeds the budget, sample with `--rows` and record the deviation.
...
- `req-coverage.py` reports NFR-488 as evidenced.

---

## Sources

- `docs/closures/INDEX.md#closure-recordsmd`: the WK-665 close record §7 residue, the WK-664 close record §7 carry-forward.
- `docs/closures/INDEX.md#plan-reviewsmd`: plan review 6, P1 and P2.
- `docs/findings/register.md` and `docs/findings/register.md`: the registers the findings file into.
- `docs/roadmap.md` §6: the Phase 1b status table and the Exit demo row.
- `W6B-AUDITOR-HANDOVER.md` §24-§27: the WK-665 audit records and the close position.
