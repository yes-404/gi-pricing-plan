...
coefficients inline (FR-MODEL-96, OQ-MODEL-10 decided 2026-08-18).
...
**Spec:** `docs/specs/02-modelling.md` — FR-MODEL-96 (§3.6), §4.4 `ModelSpec`, §4.8 `Model`,
§4.9 `TransparencyArtifact`, §5.1 error codes, §5.2 `pricing-core` interfaces.
Decision record: `docs/open-questions.md` OQ-MODEL-10.
...
- **`pricing-core` reaches no database and stores no blob** (ADR-0001). It is handed
  resolved artifacts and returns data.
- **Artifacts are immutable.** `transparency_artifacts` is insert-only at the privilege
  layer (FR-DATA-42); a Model is immutable once fitted (`02` R2).
...
| | Option | Consequence |
|---|---|---|
| **A** *(recommended)* | Keep `coefficients`/`relativities` on `GlmApproximation` as **legacy-only**, with a validator making the two eras exclusive: `approximating_model_id is None` **iff** inline coefficients are present. New writes always set the id and never the inline fields. | Old evidence stays readable and stays labelled as pre-FR-MODEL-96. The published contract carries two eras, with a dated note saying which is which. This is the shape `covariance_blob` already has — optional, with the absence given a stated meaning rather than pretended away. |
...
fitted before this date has none", FR-MODEL-93) and FR-MODEL-87's staging rule: an absence
...
**Ruling 1 — no new field on `Diagnostics` for "these are surrogate diagnostics".**
FR-MODEL-96(iii) requires the surrogate's diagnostics to be *recorded as* diagnostics
...
**Ruling 2 — `approximates_model_id` is a bare `UUID`, not an `IntervalFor`-shaped block.**
FR-MODEL-100 gave `interval_for` a block with the central model's id *and* version because
`motor-ad-frequency@7` is what a human reads in a review. FR-MODEL-96 names the field
...
**Ruling 3 — the surrogate Model's `covariance_blob` is stripped, not stored.**
...
None})`: the surrogate then reports FR-MODEL-93's typed absence at predict time instead of
...
wrong:* the surrogate offers no prediction interval; OQ-MODEL-11 is where that question
belongs anyway.

**Ruling 4 — the surrogate is reserved and fitted inside the transparency Job's final
...
**Ruling 5 — the plan's test counts are not acceptance criteria.** Report the number the run
...
| `docs/open-questions.md` | OQ-MODEL-10's outcome column records the build date |
| `docs/roadmap.md` | W5 slice record |
...
- Modify: `docs/open-questions.md` — OQ-MODEL-10's outcome column
- Test: none — docs only. `python3 scripts/audit-docs.py` is the check.

**Interfaces:**
- Consumes: nothing.
- Produces: **FR-MODEL-102** (the id every later task's `@pytest.mark.req` marker cites for
...
Expected: `FR-MODEL-101` is the maximum, so the new requirement is **FR-MODEL-102**. If it
is not, use `max + 1` and use that id everywhere this plan says FR-MODEL-102.

- [ ] **Step 2: Append FR-MODEL-102 to §3.6's requirement table**

Add as the last row of the §3.6 table (after the FR-MODEL-37 row):
...
- [ ] **Step 8: Record the outcome on OQ-MODEL-10**

In `docs/open-questions.md`, append to OQ-MODEL-10's outcome column (the row is already
struck through and marked decided — **do not** restate the decision, only its build):

```markdown
**Built 2026-08-19** (W5): `approximates_model_id` on `GlmSpec` joining `spec_hash` at
`v5`, the surrogate reserved and fitted inside the `model.transparency` Job with
diagnostics against the source model's predictions, and FR-MODEL-102 appended for the
...
still splits a table row** (write `E[Y\|x]`), and **a bolded `**FR-MODEL-102**` used as a
...
git commit -m "docs(model): FR-MODEL-102 — a surrogate is identifiable from its spec (FR-MODEL-96)"
```
...
**Interfaces:**
- Consumes: FR-MODEL-102 from Task 1 (the marker id).
- Produces:
...
is "**not** populated" and cites OQ-MODEL-10 as open. Replace it with what is now true,
...
git commit -m "feat(schema): approximates_model_id and spec_hash v5 (FR-MODEL-96, FR-MODEL-102)"
```
...
git commit -m "feat(model): pricing-core returns the fitted surrogate (FR-MODEL-96)"
```
...
git commit -m "feat(model): the transparency Job persists the approximating Model (FR-MODEL-96)"
```
...
git commit -m "feat(model): refuse a surrogate that does not match its source (FR-MODEL-96)"
```
...
**Files:**
- Modify: `docs/roadmap.md` (W5's slice record and its slice count)
...
should need an edit — the model-detail view that would read these fields is W6b's.
...
maintainer, because FR-MODEL-51 makes type-III tests part of what GLM diagnostics *are*.

- [ ] **Step 4: Write the roadmap slice record**

In `docs/roadmap.md`, W5's row lists its slices and a count. Add this one, and correct the
count in the same edit — the paired-quantile slice had to correct it once already because a
previous slice was omitted:

```markdown
the GLM approximation as a Model (FR-MODEL-96, FR-MODEL-102)
```

Then re-read the whole W5 row for claims this slice falsified. **Search the file for every
sentence saying the approximation is carried inline or that OQ-MODEL-10 is unbuilt** —
`grep -n "approximating_model_id\|OQ-MODEL-10\|inline" docs/roadmap.md` — and fix each. The
...
git commit -m "docs(roadmap): the GLM approximation as a Model — W5 slice record"
...
**Spec coverage.** FR-MODEL-96's three obligations map to tasks: (i) `approximates_model_id`
