...
**Goal:** Make `GET /api/v1/dataset-versions/{id}/lineage` answer FR-DATA-35 in the shape
...
every other response (FR-PLAT-48); the DATA service's `lineage_of` returns the typed data
...
equivalent (NFR-OVR-10), in the Dataset detail view.
...
`DatasetLineage` (`:771-807`, its definition and invariants), FR-DATA-35 (`:186`),
FR-DATA-36 (`:187`), FR-DATA-23 (`:115`), the §5.1 lineage row (`:846`, whose P3 amendment
...
[`../open-questions.md`](../open-questions.md):** FR-DATA-56, NFR-DATA-10, OQ-DATA-15.
Next free: `FR-DATA-57`, `NFR-DATA-11`, `OQ-DATA-16`.
**This plan mints none of them** — it cites FR-DATA-35, which already states the rule, and
...
(*"Next free: `FR-DATA-55`"*, `beba1ae` #166) is now two ids stale, and a stale allocation
...
- **`01` §4.9's invariants are the contract.** Every field is present in every response;
  ...
  empty (W9's and W27's); the response is assembled where the modules meet, not inside the
  DATA service.
- **DEP-1 (ADR-0001) forbids a module importing from its right.** `app.platform.datasets`
...
- **A chart is a canvas plus a table** (NFR-OVR-10, `00-overview.md:520`): every chart the
...
### Finding 3 — the only FR-DATA-35 evidence marker reads the old shape (defect 3)

`backend/tests/test_lineage.py:305-328` is the only `@pytest.mark.req("FR-DATA-35")` marker
...
deletes FR-DATA-35's evidence. Task 2 rewrites it in the same commit as the service change
that breaks it.
...
FR-DATA-35 (`01:186`) says "what depends on this?" spans Models, Rating Versions and
Monitoring baselines, "used to compute the blast radius of FR-DATA-23" (`01:115` — a
re-validation that flags *every* Model fitted on the version). Neither FR-DATA-23 nor §4.9's
...
FR-MODEL-65's `parent_model_id` refit lineage. That is a different lineage, on a different
...
§4.9; W9 and W27 own them. This slice types them `list[Any]` and never populates them.

**FR-DATA-37's access scoping.** The handler already refuses callers without read access
...
| `backend/tests/test_lineage.py` | Modify `:305-328`, add tests | The FR-DATA-35 rewrite; models arm; direction semantics |
...
- Produces — the wire vocabulary every later task types against (names fixed here, used
  ...
  always empty (§4.9: W9's and W27's).
...
- Modify: `backend/tests/test_lineage.py:305-328` (the FR-DATA-35 rewrite, same commit)
...
- Produces: `lineage_of(session, *, workspace_id, version_id) -> DatasetLineage` — the
  ...
  The router (Task 4) fills `models`; the last two are never filled by anyone (W9, W27).

- [ ] **Step 1: Rewrite the FR-DATA-35 test to the typed shape — before the service changes**
...
`str(...)` comparisons go away with the dict. Keep `@pytest.mark.req("FR-DATA-35")` and the
test's docstring.
...
Expected: PASS. The rewritten marker still carries `@pytest.mark.req("FR-DATA-35")`, so
the coverage script's single FR-DATA-35 evidence marker survives the commit that breaks
...
Run: `uv run pytest backend/tests/test_lineage.py -q`
Expected: PASS (the new test and the rewritten FR-DATA-35 test). The ordering assertion
...
The table is `ChartFigure`'s always-in-DOM tabular equivalent (NFR-OVR-10); the chart
...
histograms (W5), PSI selector (W5), lineage graph (W6b-12), plus threshold editing
...
Expected: PASS, and FR-DATA-35 reported with evidence (the rewritten marker still carries
`@pytest.mark.req("FR-DATA-35")` — verified in Task 2).
...
always empty → Task 1 (typed `list[Any]`, documented W9/W27) and Task 2 (never populated).
...
response). FR-DATA-35's two directions → Tasks 2-4 and their tests; FR-DATA-36's
derivation recording → the `derived_versions` arm reading `derived_from` (Task 2).
FR-DATA-23's blast radius → Task 3's any-status models arm. §5.3's Dataset detail
