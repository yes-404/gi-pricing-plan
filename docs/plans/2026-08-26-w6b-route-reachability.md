# W6b Route Reachability Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make every route the frontend registers reachable from the application entry by links alone, enforced by a route-graph test (FR-OVR-22).
...
**Spec:** `docs/specs/00-overview.md` §5.1 — FR-OVR-22 (line 228): "A route the frontend registers is reachable from the application entry by following links alone."

**Slice source:** Backlog #136 (HANDOVER §5, close-record skeleton §5): FR-OVR-22 specified, the four-item connection scope agreed, nothing built. Phase-exit blocker. No slice-map row exists. FR-OVR-22's owner clause names "the slice that carries the connection work".

**Highest ids:** No requirement id is minted by this slice. The slice changes no spec. FR-OVR-22 governs. FR-PLAT-53 and FR-OVR-21 bound it (Global Constraints).

## Global Constraints

- FR-OVR-22: every registered route is reachable from the entry by links alone.
- The graph counts registered routes and view-source links only. Component-sourced links do not count.
- FR-PLAT-53: the demo path is gated behind `dev_auth_enabled`. The demo entrance does not discharge reachability.
- FR-OVR-21: the jobs view belongs to no Phase 1b slice. No slice builds one.
...
**F1. The route count moved.** FR-OVR-22's measured "3 of 18" is a dated record from 2026-08-25. The router now registers 23 routes. The graph test re-measures. The plan's expected named set is 16.
...
**F4. The wf-01 A9 profile route is orphaned.** `/data/:slug/v/:version/profile` has no inbound link. VersionDetailView.vue:114-131 links the validation report and the factor workbench. It omits the profile. ProfileView is built.
...
**F6. One route cannot be wired.** `/models/:slug/backtests/:backtestId` is job-result-addressed: frontend/src/api/backtests.ts:10-12 carries the reason verbatim, the backend exposes only GET /models/backtests/{id}, and the Model type carries no backtest ids. The jobs view is a later phase (FR-OVR-21). **Ruling (manager, 2026-08-26):** the graph test whitelists this route with its reason. The exception lifts when the jobs UI lands.

**F7. The demo entrance stays out of the graph.** DemoView.vue:161 binds `:to="view.route"`, a computed value with no literal. The extractor counts literals only, so the demo adds no edges. This matches FR-PLAT-53's gate.
...
git commit -m "test(frontend): route-graph reachability test for FR-OVR-22
...
The link class copies ModelDetailView's inline-link style (`text-sky-700 hover:underline`). The slug span stays untouched: FR-DATA-50's badge semantics still hold, and the graph counts the RouterLink only.
...
FR-OVR-22. The dataset list severed the graph: the whole /data/:slug
...
FR-OVR-22. The model subtree had six routes and no list entrance. The new
...
FR-OVR-22. The version detail links its wf-01 A9 profile, the model detail
...
Expected: PASS. The plan is the only `docs/` change. Every requirement id it cites (FR-OVR-22, FR-PLAT-53, FR-OVR-21) exists.
...
docs/plans/2026-08-26-w6b-route-reachability.md
```
...
**1. Spec coverage.** FR-OVR-22's enforcement is Task 1 (the graph test plus its positive control). The dataset-list finding is Task 2. The absent `/models` route is Task 3. The profile and predict orphans plus the three nav-less libraries are Task 4. The two recorded findings and the closure table cover every route: 23 registered, 4 reached by the entry, 16 wired by this slice, 3 whitelisted with reasons. FR-PLAT-53's gated demo and FR-OVR-21's jobs phase bound the whitelist. Neither is violated. No spec change, so no requirement id is minted.
