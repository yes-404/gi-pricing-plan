...
**Spec:** [`../specs/02-modelling.md`](../specs/02-modelling.md) — FR-MODEL-49 through -55, FR-MODEL-81, FR-MODEL-96, FR-MODEL-102, FR-MODEL-109, FR-MODEL-118; [`../specs/00-overview.md`](../specs/00-overview.md) — FR-OVR-7, FR-OVR-21, NFR-OVR-10. Slice context: [`2026-08-24-w6b-slice-map-revised.md`](2026-08-24-w6b-slice-map-revised.md).
...
- **Money is integer minor units or an exact decimal string, never float** (FR-OVR-7). No diagnostic field in this slice is money; `exposure_years` is an exact decimal **string** and `Number()` on it is permitted for a chart coordinate only, where nothing computes with the result — `OneWayChart.vue:31-35` is the precedent and states the reasoning.
- **Train and holdout are rendered side by side wherever the contract expresses them, and nowhere else** (FR-MODEL-54 as amended 2026-08-24). See "Two partitioned surfaces" below — this is the single most misread rule in the slice.
- **Every chart has an accessible tabular equivalent** (NFR-OVR-10), delivered through one shared wrapper rather than nine bespoke tables. See Task 2.
...
|---|---|---|---|
| A/E by factor | `universal.{train,holdout}.ae_by_factor` | chart | FR-MODEL-50 |
| Lift by decile | `universal.{train,holdout}.lift` | chart | FR-MODEL-50 |
| Calibration by decile | `universal.{train,holdout}.calibration` | chart | FR-MODEL-50 |
| Headline metrics | `universal.{train,holdout}` scalars | table | FR-MODEL-50, -55 |
| Residual summary | `universal.{train,holdout}.residual_summary` | table | FR-MODEL-50 |
| Complexity | `complexity` | table | FR-MODEL-81 |
| GLM fit statistics | `glm` scalars | table | FR-MODEL-51 |
| Type III tests | `glm.type_iii_tests` | table | FR-MODEL-51 |
| VIF and aliasing | `glm.vif`, `glm.aliasing` | table | FR-MODEL-51, -109 |
| GBM eval curve | `gbm.eval_curve` | chart | FR-MODEL-52 |
| GBM importances | `gbm.importances` | chart | FR-MODEL-52 |
| Permutation importance | `gbm.permutation_importances` | chart | FR-MODEL-52 |
| Partial dependence | `gbm.partial_dependence` | chart | FR-MODEL-52, -118 |
| Monotonicity | `gbm.monotonicity` | table | FR-MODEL-52 |
| Tree summary | `gbm` scalars | table | FR-MODEL-52 |
| Quantile crossing | `gbm.quantile_crossing` | table | FR-MODEL-52 |
| CV alpha path | `cross_validation.path` | chart | FR-MODEL-53 |
| CV fold dispersion | `cross_validation.fold_metrics` | chart | FR-MODEL-53 |
| CV method header | `cross_validation` scalars | table | FR-MODEL-53 |
...
### NFR-OVR-10, and why the wrapper is built here

No chart component in this repository renders an accessible table. `OneWayChart.vue:107-122` and `HistogramChart.vue:74-80` render a bare `<VChart>`; `DoubleLiftChart.vue:77-99` answers NFR-OVR-10 with redundant non-colour encoding (distinct `lineStyle.type` per series) rather than a table. The nearest tabular precedent is `EbmShapePanel.vue:127-158`, which is not a chart at all.
...
| `frontend/src/components/ChartFigure.vue` | One chart plus its accessible tabular equivalent (NFR-OVR-10) |
| `frontend/src/components/SurrogateNotice.vue` | The A/E-denominator label for a GLM approximation (FR-MODEL-96, -102) |
...
| `frontend/src/components/ComplexityTable.vue` | Counts and ratios beside whatever thresholds are in force (FR-MODEL-81) |
...
git commit -m "feat(w6b-1b): a chart and the table that says the same thing (NFR-OVR-10)"
```
...
**Why residuals are a table and not a chart.** FR-MODEL-50 names "residual summaries", and `ResidualSummary` (`diagnostics.py:124-131`) is four scalars — mean, std, minimum, maximum. There is no per-row residual series in this artifact to plot; see F2.
...
```vue
      ...
      >
        ...
        <!-- FR-MODEL-52. Present only on the second bound of a quantile pair, and a scalar
...
git commit -m "feat(w6b-1b): the cross-validation arm, with the dispersion FR-MODEL-53 persists"
```
...
| Requirement | Task |
|---|---|
| FR-MODEL-49 — read, never recomputed | 1, 4 |
| FR-MODEL-50 — A/E, lift, Gini, calibration, residual summaries | 4, 5, 6 |
| FR-MODEL-51 — GLM deviance, AIC/BIC, type III, VIF/aliasing | 7 |
| FR-MODEL-51 — residual and leverage plots | **not started**, F2 — the contract carries blob refs and no read resolves one |
| FR-MODEL-52 — eval curve, importances, permutation, PD, monotonicity, trees | 8, 9, 10 |
| FR-MODEL-53 — fold construction, seed, per-fold metrics, dispersion | 11 |
| FR-MODEL-54 — train and holdout side by side | 4, 5, 8 |
| FR-MODEL-55 — weighting labelled | 1, 4 |
| FR-MODEL-81 — complexity beside the thresholds in force | 6 |
| FR-MODEL-96, -102 — the surrogate A/E denominator | 3 |
| FR-MODEL-109 — aliasing entries are bare names | 7 |
| FR-MODEL-118 — partial-dependence omission reasons | 10 |
| NFR-OVR-10 — a tabular equivalent for every chart | 2, and every chart task |

**FR-MODEL-51's residual and leverage plots are the one requirement this slice does not deliver**, and it is booked *not started* rather than left silent: `residual_blob` and `leverage_blob` are references, `git grep` over `backend/src` finds no reader, and `02:2932` assigns the owner to the slice that first stores a per-row residual series. Task 7 renders the state instead of an empty frame.
