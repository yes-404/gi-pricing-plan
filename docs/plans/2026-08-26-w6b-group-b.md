...
- `OQ-OVR-14` — **DECIDED 2026-08-26: (b)** — views read the dataset's currency via `getDataset(slug)`; the `?? "GBP"` fallbacks are deleted.
- `OQ-OVR-15` — **DECIDED 2026-08-26: (c), with (a) as the near-term step** — the list endpoint resolves `owner_id` to a name; FR-DATA-51 gains the resolved-name sentence. The (c) route defers with the shared trigger.
- `OQ-PLAT-16` — **DECIDED 2026-08-26: (c)** — two generated type sets: strict for responses, permissive for request bodies.

**Slice source:** `docs/plans/2026-08-26-w6b-slice-map-revised-3.md` §3 rows for W6b-20, W6b-21, W6b-24; the manager's batching direction (Group B = W6b-20 + W6b-21 + W6b-24, one branch, one PR).

**Highest ids:** No new requirement id is filed. W6b-20 records a dated rule on the existing FR-OVR-7 row; W6b-21 amends the existing FR-DATA-51 row; W6b-24 records a dated note in `07` §5.3. The decisions are already recorded in each spec's §10 row.

## Global Constraints

- `CLAUDE.md` §2's money rule binds W6b-20: the currency a view formats with must be the dataset's own, never a hardcoded default. `FR-OVR-7` governs values; a euro amount under a pound sign is the defect this slice removes.
- W6b-21's `owner_name` is a derived, read-only field on `Dataset`, like `latest_version_status` — it is computed per request from `users` / `service_accounts` and stored on no row. A `None` name is an honest answer (the id did not resolve); the frontend falls back to the raw id, never to a fabricated name (OQ-OVR-15's rejection of (d)-as-end-state does not forbid the raw-id fallback; it is the correct interim).
...
**T1. 00 spec: record the rule on FR-OVR-7.**
- [ ] Add a dated amendment to the FR-OVR-7 row (`00-overview.md:213`): a view that formats a monetary amount reads the dataset's currency via `getDataset(slug)` (OQ-OVR-14, decided 2026-08-26 (b)); a hardcoded currency default is forbidden because a wrong symbol renders a right-looking wrong number. Note the drift: the slice map's "00 §5.3" is the Error model, so the rule lives here.
...
- [ ] Add `owner_name: str | None = None` to `Dataset` (`packages/model-schema/src/model_schema/datasets.py:156`), beside `owner_id`, with a comment: derived per request (OQ-OVR-15 (a)), stored on no row, null when the id does not resolve.
...
- [ ] Backend tests: the list carries `owner_name` for a resolvable owner and `null` for an unresolvable one; a service-account owner resolves to its slug; the four single-row routes agree with the list on the same dataset (FR-DATA-50's disagreement defect does not recur).

**T4. frontend: the owner column renders the name.**
- [ ] `DatasetListView.vue` owner cell (`:227-231`): render `dataset.owner_name ?? dataset.owner_id`; keep the font-mono style for the id fallback and a non-mono style for a name; update the comment (`:217-226`) to record OQ-OVR-15 (a) — the list endpoint now resolves the name, and the raw id remains the honest fallback when the id does not resolve.
...
**W6b-20:** `docs/specs/00-overview.md` (FR-OVR-7 amendment) · `frontend/src/views/ProfileView.vue` · `frontend/src/views/VersionDetailView.vue` · `frontend/src/views/__tests__/ProfileView.test.ts` · `frontend/src/views/__tests__/VersionDetailView.test.ts`.

**W6b-21:** `docs/specs/01-data-management.md` (FR-DATA-51 amendment) · `packages/model-schema/src/model_schema/datasets.py` · `packages/model-schema/tests/` · `backend/src/app/platform/datasets.py` · `backend/src/app/api/datasets.py` · `backend/src/app/data/ingestion.py` · `backend/tests/test_api_datasets.py` · `frontend/src/views/DatasetListView.vue` · `frontend/src/views/__tests__/DatasetListView.test.ts` · `docs/contracts/schemas/generated/` · `frontend/src/api/generated/` (ignored).
...
1. **The slice map's "00 §5.3" is the Error model (W6b-20).** The map's citation does not match the section at the anchor (`7400846`) or at `8b0977f`. The currency rule is a money-correctness fact and is recorded on the FR-OVR-7 row instead (F2).

2. **W6b-21 needs backend work despite the "frontend" grouping.** The owner display's mechanism is the list endpoint resolving `owner_id`; Group B's PR therefore spans `model-schema`, `backend/`, `frontend/` and `docs/`. This is the decision's own mechanism (OQ-OVR-15 (a)), not a scope extension.

3. **`owner_name` is populated by every `Dataset`-producing route, not only the list.** The decision names the list endpoint; FR-DATA-50's recorded shape-consistency finding requires the other four routes to agree (F4). The `(c)` route — a batch principal-name endpoint — is deferred with the shared trigger and is not built here.
