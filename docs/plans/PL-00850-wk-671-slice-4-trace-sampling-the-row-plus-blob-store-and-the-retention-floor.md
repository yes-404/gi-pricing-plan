...
family: plan
kind: leaf
title: WK-671 Slice 4 — trace sampling, the row-plus-blob store, and the retention floor
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-29-w11-4-trace-sampling-persistence.md
---

# WK-671 Slice 4 — trace sampling, the row-plus-blob store, and the retention floor
...
control. **4D** is the NFR-500 capacity projection, which is a measurement and not a
...
**Spec:** [`../specs/03-rating-engine.md`](../specs/03-rating-engine.md) — §3.7 (FR-258),
§3.8 (FR-259), §4.5 (the `Trace` shape), §5.1 (the `/traces` row), §9 (NFR-499,
NFR-500). Retention floor: [`../specs/00-overview.md`](../specs/00-overview.md) NFR-459.
Blob store: [`../specs/07-platform.md`](../specs/07-platform.md) FR-PLAT-18, 19, 20.

**Slice source:** [`PL-00854-wk-671-scoring-sequenced-slice-plan.md`](PL-00854-wk-671-scoring-sequenced-slice-plan.md), Slice 4. **That
file is frozen and is not edited by this one.**

**Rulings this plan rests on, cited by number and not re-argued:**
[`../rulings/RL-00890-d5-fr-258-259-are-not-silent-about-batch-and-no-open-question-is-raised.md`](../rulings/RL-00890-d5-fr-258-259-are-not-silent-about-batch-and-no-open-question-is-raised.md) Rulings 23 and
25. RL-888 rules the storage shape *and corrects the retention reasoning the recovery
document proposed*; RL-890 rules that batch contributes nothing to this stream.
...
RL-890 examined the batch-sampling question this slice was expected to raise it for and
...
[`PL-00849-wk-671-slice-3-batch-scoring-the-pure-transform-the-checkpointing-handler-and-the-route.md`](PL-00849-wk-671-slice-3-batch-scoring-the-pure-transform-the-checkpointing-handler-and-the-route.md)'s own corrections
...
[`PL-00901-wk-671-slice-4-the-always-capture-design-pins-the-traced-fraction-at-1-and-nfr-489-fails-on-all-real-time-traffic-2026-08-30.md`](PL-00901-wk-671-slice-4-the-always-capture-design-pins-the-traced-fraction-at-1-and-nfr-489-fails-on-all-real-time-traffic-2026-08-30.md),
resolved the same day by RL-862 — was filed as a sibling document instead. **Both are live
...
`app.platform.traces.delete_trace` (`traces.py:126`) when a delete falls inside NFR-459's
≥ 13-month floor.
...
reference. The signal RL-890's exclusion must be written against is a **null `environment`**:
...
- **C2's owed register row.** The `Deployment` parent is still WK-674's, Task 4A stored the
...
[`PL-00849-wk-671-slice-3-batch-scoring-the-pure-transform-the-checkpointing-handler-and-the-route.md`](PL-00849-wk-671-slice-3-batch-scoring-the-pure-transform-the-checkpointing-handler-and-the-route.md) carries the same
...
due at WK-671's close, not edited into that file.
...
3. **The row and the body cannot diverge.** A test proves they are produced from **one**
   serialisation of one object — RL-888's second constraint. The discriminating assertion is
...
4. **The retention floor is enforced on deliberately broken input.** Deleting a trace row
   younger than NFR-459's ≥ 13-month floor is **refused**. **Overridden if any build reclaims
   a trace blob whose row still exists, or ships an expiry job that deletes rows on a schedule
   shorter than the floor** — RL-888's acceptance test.
5. **Traces are access-controlled** (NFR-499), and **no batch-produced trace appears in the
   production stream** (RL-890): `GET /api/v1/traces` returns nothing attributable to a
   `score.batch` Job.
6. **NFR-500 is projected from a measurement**, not an estimate: the projection uses the
...
the `Deployment` parent of a `ScoringTrace` is WK-674's (see C2); the 99.95 % availability target
is WK-674's.
...
- **The row is a projection of the blob, written from the same object** (RL-888). Serialise
  once, write both in one operation.
- **No expiry job.** RL-888's correction: FR-420's GC is a *deletion* mechanism and
  NFR-459 is a *preservation floor*; they do not compose into "GC-based retention". A
...
- **Deduplication will not help and the budget must not assume it** (RL-888). `TraceStep`
  carries `elapsed_us`, so no two traces are byte-identical even for identical inputs.
  NFR-500 must be met by the schema and the sample rate alone.
...
**Three of the four fields RL-888's row needs are already in the body** — `quote_id`,
...
| Any trace persistence | **None.** `git grep -ln "ScoringTrace\|scoring_trace\|trace_sampling\|sample_reason" -- packages backend` returns nothing (RL-888 ran this at `d614f24`; re-run it at your own commit) |
| `GET /api/v1/traces` | No route. Specified at `03` §5.1 as *"Sampled **production** traces (FR-259)"* |
...
| `03 rating shapes vs hand-authored contracts (F27)` | Owner decided (RL-860): the §14 review at WK-671's close. **Bears indirectly** — if 4A gives the trace *row* a published contract, it joins that comparison; RL-888 says the row needs no published contract unless the slice finds it does |
...
| The six WK-670 rows | WK-670's and WK-675's. Not this slice's |
...
recommendation until RL-888.

**C2 — a `ScoringTrace`'s parent is a Deployment, and Deployment is WK-674's.** RL-888 names a
...
a plain string — gaining the Deployment reference in WK-674. Booked the same way as RL-880's
...
would have breached the floor it was meant to honour.** RL-888's correction, quoted because
an executor who reads only the recovery document will build the wrong thing: *"FR-420's GC
is not a retention mechanism; it is a deletion mechanism, and NFR-459 is a preservation
floor."* There is **no expiry job to build**.

**C4 — the map's exit criteria say "1% of 50M annual quotes"; the budget is over quotes, not
batch rows**, and RL-890 makes that load-bearing rather than incidental: batch contributes
...
listed signal S-J — *"the FR-258/259 batch-sampling silence resolved as an `OQ-RATE` or a
spec change"* — as the longest-lead item nobody was working. RL-890 dissolved it: the suite
...
|---|---|---|---|
| FR-258 | `03` §3.7 | 4A | The persisted body is a `Trace` equal to the one `score_one` returned. **Capture** was Slice 1's; this is persistence |
| FR-259 | `03` §3.8 | 4A, 4B | 1 % default configurable, plus 100 % of declines and errors; persisted with the ≥ 13-month floor enforced |
| NFR-459 | `00` §9 | 4A | Deleting a trace row inside the floor is refused, proven on broken input |
| NFR-499 | `03` §9 | 4C | Traces are access-controlled; an unauthorised caller is refused |
| NFR-500 | `03` §9 | 4D | A projection from the measured serialised size, over quotes only |

**Deliberately excluded, each with its owner:** FR-RATE-34, 35, 40 → Slice 2. FR-RATE-36, 37,
38's batch half → Slice 3. FR-RATE-43, 44, 45 → WK-672. FR-263–49 → WK-673. FR-267,
FR-428 → WK-674. NFR-RATE-1, 9, 13 → Slice 2. NFR-493 → Slice 3.
...
| 4D — NFR-500 projection | 4A | — | Needs real serialised traces to measure |

**4B's dependency on Slice 2 is real and the frozen map does not carry it.** RL-890 scopes
...
- Modify: `backend/src/app/db/models.py` — one row type. Fields: the three projected from the
  ...
  the environment as a **plain string** (C2 — not a Deployment FK, which is WK-674's), the blob
...
- [ ] **Step 4: Write the retention-floor test on broken input.** Attempt to delete a trace row
      whose age is inside NFR-459's floor; assert refusal. Then attempt one outside it and
      assert it is permitted, so the guard is shown to have an edge rather than refusing
      everything. Mark `@pytest.mark.req("NFR-459")`.
...
- [ ] **Step 7: Confirm the GC interaction rather than assuming it.** Write a trace, run the
      blob GC, assert the body survives. RL-888 says a referenced blob is not a candidate
...
- [ ] **Step 8: Run the gate. Commit.** `feat(rating): trace rows with blob bodies (WK-671 Task 4A)`
...
- Modify: `backend/src/app/config.py` or the workspace settings — the rate. FR-448's
...
always-capturing breaches NFR-489, that is a finding for the decision-maker, not a licence
to drop the 100 % rule** — the rule is FR-259's text.
...
- [ ] **Step 2: Write the access-control test** (NFR-499). An unauthorised caller is
      refused. Mirror the RBAC fixtures the neighbouring API tests already use; do not build new
      ones.
- [ ] **Step 3: Write the RL-890 exclusion test.** A trace row written with a `score.batch`
      Job as its parent must **not** be returned. Mark `@pytest.mark.req("FR-259")`.
...
## Task 4D — the NFR-500 projection
...
- **Project over quotes only** — NFR-500 budgets *"1 % sampling of 50 M annual **quotes**"*.
  Batch rows are excluded by RL-890, and including them would be projecting a budget the
  requirement does not set.
- **Assume no deduplication benefit** (RL-888): `elapsed_us` on every step makes two traces
...
coverage table with its source and its task, listed individually, plus NFR-459, which the map
...
WK-674 with a register row owed (C2), and the always-capture cost is escalated to the
decision-maker if it breaches NFR-489 rather than resolved by dropping a requirement's clause.

**7. One correction to this plan's own predecessor is recorded rather than folded in** — C5. The
readiness document called the batch-sampling gap the longest-lead item in WK-671; RL-890 found
