...
family: plan
kind: leaf
title: The `Pending proposals` container: its family and `kind:` — a derivation for ruling (2026-09-02)
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-09-02-w37-pending-proposals-container-family-derivation.md
---
...
**Goal:** answer RL-978 §3 item 3 — *"The planner derives the container's family and
`kind:`, under RFC-937 §5.2 and §1.2"* — so the unit at
`docs/rfcs/RFC-00839-pending-proposals-for-the-14-review-at-wk-671-s-close.md:17-94` has a destination and RL-985's census can close.
...
1. **RL-978's four constraints are each honoured explicitly**, by name, and any one found
...
2. **Both surviving placements are tested against RL-989 acceptance (g) class 4**, and the
...
- **The decision is the decision-maker's.** RL-978 fixed the exclusion; this derives the
  positive assignment and returns it. That is RL-975's shape, one day older.
- **No filed plan or frozen record is edited.** `docs/closures/INDEX.md#plan-reviewsmd` is not touched.
...
RL-978 fixes the boundary as the whole `##` section, lines **1155-1232**. Confirmed: 1155
...
## 2. RL-978 says two citations. There are three, and the third is from another record

RL-978 §3 item 3's fourth constraint: *"Whichever is chosen, **Plan review 9's two
...
**Reported rather than folded in silently**, per RL-978's own framing of what a derivation
...
## 3. §5.2 has no row for it, which is the RL-974 situation
...
That is precisely the position RL-974 resolved for the two orphan `docs/audit/` files: no
...
|---|---|---|
| `CR` Closure | *"one work, phase or review close"*; `work · phase · review`; status subset is the single value `active` | **Excluded by RL-978**, on those grounds and the section's own first sentence |
...
RL-989 acceptance (g) class 4 permits *"a split, where the concatenation of the outputs
...
**Class 4 does not discriminate**, exactly as RL-978 said when it left two placements
standing. The discriminator is what the three citations of §2 say afterwards.

| Citation | Under **P1** | Under **P2** |
|---|---|---|
| 1994 (Plan review 9) | *"from `RFC-<n>`"* — a cross-record reference, resolvable | becomes self-referential: the record cites its own preamble. RL-978 named this cost |
...
(the date in its own heading), `owner: planner`, `was:` `docs/closures/INDEX.md#plan-reviewsmd`.**
...
1. **`_discover_plan_reviews` gains a non-review record**, so the file's split is *"one `CR-`
   per review **and** one `RFC-` for the container"*. RL-978 item 2 forbids widening
   `_REVIEW_HEADING_RE` to reach it; the container is found as a section, not as a fifteenth
   pattern match — which is what RL-978 meant by *"whatever handles the container section
   ...
   **record** heading (`### Plan review 9`, line 1233), not at the next heading of its own
   level. This is the same mis-nesting RL-978 §3 item 4 recorded as the lead's finding,
   showing up as an implementation trap rather than as a rendering defect, and it is why the
   boundary RL-978 fixed by line number cannot be re-derived from heading level alone.
2. **§5.2's row gains this destination**, or the migration carries it as a bespoke derivation
   the way RL-974's two files are carried. Either is consistent; the second needs no
   standard edit.
...
4. **The phrase *"both above in this document"* at 2121 stops being true** under either
   placement and needs replacing, not just re-pointing. RL-978's constraint says the
...
|---|---|---|
| `docs/closures/INDEX.md#plan-reviewsmd` | 11 | `.claude/skills/close-workstream/SKILL.md:646`, `docs/findings/register.md` (×4), two filed plans, and the file itself (×2) |
| `docs/findings/register.md` | 3 | — |

A citation of the form `docs/closures/CR-00925-plan-review-9-at-wk-671-s-close.md:775-787` **is** caught by the legacy-form
...
One of the fourteen is inside an instrument (`close-workstream`), which puts it in RL-987's
territory as well.
...
- It does not rule. RL-978 fixed the exclusion; this returns the positive assignment.
- It does not edit `docs/closures/INDEX.md#plan-reviewsmd`, the standard, or any filed plan.
- It does not fix the heading mis-nesting RL-978 §3 item 4 recorded as the lead's, and it
