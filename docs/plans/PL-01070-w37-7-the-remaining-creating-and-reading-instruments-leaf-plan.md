...
### Task 16: close the reserved-identifier gap ruled by DP-7-6

**Added 2026-09-19 (planner, on the deputy's ruling of the same date).** Numbered 16 because
task numbers here are append-only, and **sequenced after DP-7-6 is ruled and before Task 14**,
which is the slice sweep, ledger and gate and must remain last. Order: 13 → 15 → 16 → 14.
**Do not start this task until DP-7-6 carries a resolver id.** The three options change
different files in incompatible ways; starting on a guess means building one and reverting it.

**Provenance:** this row traces to the deputy's ruling of 2026-09-19, not to `RFC-937` §5.4,
`PL-939:740-751`, `CR-1064` or `CR-1065`. See the annotation at `:318` and its extension.

**Files:** (the ruled option decides which)
- Modify: `scripts/doc-id.py` — `compute_next` and its scanners
- Modify: `docs/rfcs/RFC-00937-…md` §1.7 — the sentence naming the allocator's sources
- Modify: `docs/REDIRECTS.csv` — only under option (b)
- Modify: `tests/` — the allocator's tests
- Modify: this slice's `LG-` ledger

**Interfaces:**
- Consumes: DP-7-6's ruling. Nothing from other tasks.
- Produces: an allocator whose sources match what `RFC-937` §1.7 says they are.

- [ ] **Step 1: Re-derive both predicates at the head you branch from — never quote a figure**

```bash
TREE=$(/usr/bin/git rev-parse HEAD)
/usr/bin/git show "$TREE":docs/REDIRECTS.csv | awk -F, '$6 ~ /^"?title:/ {print $2}' | grep -oE '[0-9]{4}' | sort -u > /tmp/block.txt
/usr/bin/git show "$TREE":docs/INDEX.md | grep -oE '^\| [A-Z]+-[0-9]{4}' | grep -oE '[0-9]{4}' | sort -u > /tmp/idx.txt
comm -12 /tmp/block.txt /tmp/idx.txt | wc -l      # (i) any family
/usr/bin/git show "$TREE":docs/INDEX.md | grep -oE '^\| FD-[0-9]{4}' | grep -oE '[0-9]{4}' | sort -u > /tmp/idxfd.txt
comm -12 /tmp/block.txt /tmp/idxfd.txt | wc -l    # (ii) FD- only
```
Record both numbers **with `$TREE` beside them** in the ledger. They are larger than any figure
written in this plan, because each document minted since grows (i) by one. That growth is the
measurement this task exists to stop, so capture it before changing anything.

- [ ] **Step 2: Read the standard and the tool together before editing either**

```bash
/usr/bin/git show HEAD:scripts/doc-id.py | grep -n 'def compute_next'
/usr/bin/git show HEAD:docs/rfcs/RFC-00937-*.md | grep -n '1\.7'
```
`RFC-937` §1.7 names four sources; `compute_next`'s docstring and body implement four. They agree
with each other and both omit reservations. **Whichever option was ruled, the standard's sentence
and the tool's implementation change in the same commit** (`CLAUDE.md` §2). Landing one without
the other reproduces the defect in the opposite direction and the audit reports a consistency the
repository does not have.

- [ ] **Step 3: Implement the ruled option, and only it**

Under **(a)** add the reservation scanner beside the four existing ones and amend §1.7 to name
five sources. Under **(b)** re-point the legacy rows so the reserved block no longer overlaps
live numbers, **without breaking resolution of genuinely retired references** — every id that
resolved before must still resolve, which Step 4 proves. Under **(c)** move the allocator's
floor above the block's top and record in §1.7 why a deliberate hole exists in a sequence the
standard requires to be one. If (b) and (a) were both ruled, land them in one commit: the
re-point resolves the existing collisions, the scanner stops the class recurring.

- [ ] **Step 4: Prove it on deliberately broken input**

`CLAUDE.md` §13: a check that has never printed a failure has not been tested. Construct a
reservation that the fixed allocator must refuse to mint over, run the allocator, and record that
it does not return that number:

```bash
python3 scripts/doc-id.py next 2>/dev/null     # stdout is the integer alone; the diagnostic is stderr
python3 scripts/doc-id.py check; echo EXIT=$?
python3 scripts/audit-docs.py; echo EXIT=$?
```
Expected on the clean tree: `EXIT=0` from both, and `next` returning a number outside the
reserved block. On the broken-input copy: a non-zero exit naming the offending reservation. Both
runs and both outputs go in the ledger. Under option (b), additionally re-resolve every id the
re-pointed rows previously served and record that each still resolves.

- [ ] **Step 5: Re-run Step 1 and show the intersection did not grow**

Re-derive (i) and (ii) at the post-fix tree and record both beside that tree's SHA. Under (b) the
intersection is empty; under (a) or (c) it is unchanged from Step 1 and cannot grow thereafter.
**An unchanged (i) under (a) or (c) is the expected result, not a failure** — those options stop
the bleeding without healing the wound, which is what DP-7-6's recommendation says of them.

- [ ] **Step 6: Regenerate `docs/INDEX.md` per DP-7-5 option (a)** — by regenerating, never by
...
- [ ] **Step 7: Commit** — `fix(ids): close the reserved-identifier gap ruled by DP-7-6`, spec and
code in the one commit.

**ETA basis:** 3 h under (a) or (c); 4 h under (b), whose re-pointing needs every previously
