...
family: plan
kind: leaf
title: WK-661 Audit Remediation — Sequenced Slice Roadmap
...
**Spec:** `docs/specs/02-modelling.md` (the module), `docs/specs/06-governance.md` (slices 1 and 6), `docs/specs/07-platform.md` (slice 2's error-code registration), `docs/roadmap.md` §6 and the WK-661 slice records. The audit this plan answers: https://claude.ai/code/artifact/45b1666c-1d29-4654-b916-e16ae5659cb3
...
- **Do not build ahead of the phase.** `02` §5.3's model spec builder, diagnostics, comparison and peril-structure views are **WK-664's**. No slice here adds a frontend view.
...
| 2 | FR-115 + OQ-646 | — | **OQ-646 (a)/(b)/(c)** | small |
| 3 | NFR evidence | — | NFR-475/476/485 fixture; NFR-477 fix-or-restate | medium |
| 4 | Bühlmann–Straub | — | **build vs supersede FR-106** | medium |
| 5 | The contract half, the interfaces, and four strays | — | none | **large** |
| 6 | Bookkeeping + closure record | 1–5 | FR-95 acceptance | medium |

Slices 1–5 are mutually independent and may run in parallel in separate worktrees. Slice 6 must run last. **Take the three decisions before starting**, so slices 2, 3 and 4 are not blocked mid-flight: OQ-646's option, whether FR-106 is built or superseded, and whether a 5M-row fixture is built.
...
**Requirements:** `02` R2 (artifact immutability), `06` FR-386, `06` FR-363/364.
...
**Exit criteria**
- A negative test proving a repoint on a fitted model raises, marked `@pytest.mark.req("FR-203")` or whichever requirement `02` R2 carries — check `docs/specs/02-modelling.md` §4.8 for the R2 requirement id rather than inventing one.
...
| `custom_metric` | `metrics.resolve_ref(...)` `:296-321` | Shape is right but raises **`METRIC_REF_UNRESOLVED`**, deliberately per its docstring at `:297-301`. FR-386 says `NOT_FOUND` — **reconcile explicitly, do not paper over it** |
...
**Exit criteria**
- A reference naming a non-existent version is refused with `NOT_FOUND`, marked `@pytest.mark.req("FR-386")`.
...
**Files**
- Modify: `docs/specs/06-governance.md` §3.3's FR-363 evidence table — a Custom Metric row citing `02` FR-154/157/162.
...
## Slice 2 — FR-115's remainder, and OQ-646
...
**Requirements:** `02` FR-115; `07`'s error registry; OQ-646 (open, maintainer-owned).
...
- Modify (if OQ-646 (a)): `backend/src/app/worker/tasks.py:175-195`.
- Modify: `docs/open-questions.md:132` — record the decision with its date.
- Test: `packages/pricing-core/tests/test_glm.py` beside the existing FR-115 markers at `:134` and `:429`; and `backend/tests/test_model_jobs.py` for the job-boundary assertion.
...
- `retryable` behaviour unchanged — OQ-646's recommendation is explicit that `JobError.retryable` still defaults to `False`.
...
**Requirements:** NFR-475…12 (`docs/specs/02-modelling.md:2332-2343`).

### The roadmap's own classification is wrong, and this slice corrects it

`docs/roadmap.md:2579` groups NFR-482/483/484 as *"Testable today, no fixture needed."* Two of the three are not:

- **NFR-482 has nothing to test.** There is **no Model export or import path anywhere in the repository** — not a route, not a CLI, not a bundle schema. The only `export` in the HTTP surface is the audit log (`backend/src/app/api/audit.py:238`). `FR-5`, its parent, carries **zero markers**.
- **NFR-483's "position-accurate error" is not met.** `ExpressionError` (`packages/pricing-core/src/pricing_core/data/expressions.py:65`) attaches no `lineno`/`col_offset`; `_check` at `:69-86` refuses without position. `test_prepare.py:95` catches `(ExpressionError, SyntaxError)` interchangeably, so the two are not distinguished today.
...
| NFR | Work | Anchor |
|---|---|---|
| **NFR-481, GLM half** | Mirror the GBM determinism test: two `fit_glm` calls on one spec, `abs(a - b) <= 1e-10` pairwise over `GlmFitResult.coefficients`. **Note `GlmSpec` has no `seed`** — the GLM half is `spec_hash` determinism plus IRLS determinism, not seeded sampling. Say so in the test docstring. | Mirror `packages/pricing-core/tests/test_gbm.py:269-283`; use `test_glm.py`'s `_frequency_data()` `:39`, `_spec()` `:60`, `_factor()` `:32`. Assertion style: `test_ebm.py:428` |
| **NFR-484** | Assert an Audit Event with `before`/`after` for each named act. **The four status transitions already carry `before`** (`modelling.py:1093, 1282, 1344, 1380`). | Mirror the `01` twin exactly: `backend/tests/test_data_nfrs.py:147`, which queries `AuditEventRow` by `workspace_id` + `action` |

**NFR-484's real gap, and the verdict it needs.** `factor.created` (`modelling.py:248`), `banding.created` (`transformations.py:286`), `grouping.created` (`:344`), `model.reserved` (`modelling.py:438`) and `model.fitted` (`:827`) pass **`after=` only, no `before=`**. The requirement says "creation and edit … with before/after state". For a versioned artifact where an "edit" *is* a new version, create-only events arguably have no before by construction — **write that verdict down rather than adding an empty `before={}` to satisfy a grep.** Objective *derivation* is Phase 2 (`objectives.py:243` refuses `expression`), so it is out of scope and should be named as such.

### 3b — Measurement only

Record each in `02` §9 as a blockquote, following the NFR-477 precedent at `docs/specs/02-modelling.md:2345-2366` **exactly**: a bolded verdict sentence with requirement id, date and `(WK-661)`; the data scale, machine and budget; a `| Thing | Measured | Verdict |` table with real numbers and bolded **met**/**not met**; the cause of any shortfall in numbers with the miss as a percentage; and routes out with a bolded **Owner:**. Then mirror it as a short pointer from `docs/roadmap.md` — spec first, roadmap points at it, never the reverse.

| NFR | Budget | How |
|---|---|---|
| **NFR-479** | diagnostics ≤ 30% of fit wall-clock | `fit_seconds` already exists on all three fit results (`glm.py:652`, `gbm.py:690`, `ebm.py:282`). **There is no diagnostics-side timer** — wrap the three `compute_*_diagnostics` calls at `backend/src/app/worker/model_handlers.py:395-420` with `time.perf_counter()` and divide. A ratio needs no data scale, which is why it is measurable today |
| **NFR-480** | certification < 3 min | The smoke fit already times itself (`pricing_core/modelling/objectives.py:1280`, reported in the check detail at `:1294`). `ObjectiveCertificate` has **no duration field**, so time `record_certificate` (`backend/src/app/platform/objectives.py:425`) or the pricing-core `certify` call |
| **NFR-486** | diagnostics artifact < 50 MB | One JSONB document — `DiagnosticsRow.payload` (`db/models.py:1445`). Measure `len(json.dumps(d.model_dump(mode="json")).encode())`. The blob-spill the requirement names partly exists: `residual_blob`, `leverage_blob` (`model_schema/diagnostics.py:233-234`) |
...
**Requirements:** `02` FR-106 (`docs/specs/02-modelling.md:115`), OQ-579 (decided 2026-08-15).

### The blocking decision, and why it is a decision rather than a task

WK-661's scope was set by a written decision: *"both, limited fluctuation as the default, recorded per grouping (FR-106) — **so WK-661 builds two methods rather than choosing one**"* (`docs/roadmap.md:112`). One shipped. The refusal test's own docstring says it plainly: *"FR-106 specifies it and this build does not implement it."*

So there are exactly two honest outcomes, and §0 forbids a third:
1. **Build it** — this slice.
2. **Supersede FR-106's second method** with a dated amendment saying which side was wrong and why, and re-open OQ-579 or raise a successor.

What is not allowed is closing WK-661 with FR-106 counted among the 108 evidenced while the method it names is refused at runtime. **Take this decision before starting.**
...
**It is assigned nowhere.** `grouping_evidence`'s constructor (`groupings.py:497-505`) omits the field entirely, so it is always `None` — declared-and-unbuilt in FR-207's sense.
...
**If superseded:** a dated amendment on FR-106 stating which side was wrong; a successor open question or an explicit "one method is the whole capability" verdict; the refusal test kept and re-marked `FR-106`; and `credibility_components` given a verdict rather than left declared-and-unbuilt forever.

### Must NOT touch
`bandings.py`'s `min_claims_per_band`, which is a separate credibility path that does not share `_full_credibility_claims`. `reference_hierarchy`, whose refusal is architecturally justified — a Reference Table needs a database, which ADR-703 keeps out of pricing-core — and which therefore needs a backend-side implementation and its own owner.

---

## Slice 5 — The contract half

The largest single piece of work on the list, and the only slice whose **first task is a failing guard rather than a feature**. `docs/contracts/` is a published specification artifact external consumers read (FR-451), so this is a wrong public contract, not an internal inconsistency.

**Requirements:** `00` FR-9, `07` FR-451, `02` FR-206/207/161.
...
| `model-spec` | The nested `regularisation` object that FR-207 says was "corrected in place" to flat `GlmSpec` fields. No `tweedie` property, so FR-114 is inexpressible. Missing `loss_treatment`, `approximates_model_id`, `interval_for` — three of the ten fields that moved `spec_hash`. **`ebm` is in the `model_type` enum with no `if`/`then` branch**, so a malformed EBM spec validates with zero field-level checking. Conversely `filter` and `custom_objective_ref` are declared and do not exist on the types. |
| `diagnostics` | The `gbm` block differs in name, cardinality or shape on nearly every field: `importances` plus a separate `permutation_importances`; `partial_dependence` is structured objects not a blob string; `monotonicity` is a tuple of checks not a boolean; three depth fields not a `tree_summary`; `quantile_crossing` (FR-199) has no property at all; `best_iteration` is declared here but lives on the fit by explicit design. |
...
|---|---|---|---|
| `fit_glm`, `linear_predictor`, `predict_glm`, `predict_glm_interval`, `compute_diagnostics`, `backtest_model` (`02` `:1918-1933`) | no `model_offset` | all take `model_offset: np.ndarray \| None = None` (`glm.py:517`, `predict.py:198/230/260`, `diagnostics.py:562/695/706`) | FR-116 is documented at length in §3.4 and unreachable from §5.2 |
...
**§5.3's views.** Six of eight are unbuilt and only two carry an in-spec "not built" note (`:2225-2236`). The other four — spec builder, diagnostics, comparison, peril structure — are recoverable as WK-664's only from `CLAUDE.md`'s layout table. **This is a sequencing gap, not a defect**: add the notes, and do not build the views.
...
| **`ReconcileRequest.tolerance` is a bare `Decimal`** | `backend/src/app/api/peril_structures.py:96`. A JSON number coerces there and is stringified into the job parameters — the same float hole `DecimalStr` closed one layer up under FR-21. Recorded when found, never fixed, no owner |
| **`02` §4.8 has never carried a GBM `fit_result` example** | Examples exist for GLM (`:1017`) and EBM (`:1144`) only, so `dropped_eval_metrics` had none to join and FR-161's amendment points readers at the generated contract instead |
| **FR-208's three unbuilt factor arms** | `spline`, `polynomial`, `offset` are refused by name; each needs its own contract field and resolver argument. **No owner named.** Give it one or a dated deferral — do not leave it counted among the evidenced |
...
- The four unbuilt §5.3 views carry an in-spec "not built, owner WK-664" note.
...
| `roadmap.md:1240` | FR-352 "not started, WK-660/WK-661" | **Delivered** — `backend/tests/test_model_lifecycle.py:246` proves `EVIDENCE_INCOMPLETE` fails closed. A WK-659-era verdict nobody struck |
| `roadmap.md:65` | AST parser "Phase 1, WK-661" | Contradicts OQ-573 (expressions → Phase 2) and `:2981` (WK-690 owns it) |
...
| FR-189 excess layer | "the slice that fits an excess-layer model" |
| FR-364 remainder (`model_comparison_if_predecessor`) | "the slice that gives model_comparisons a queryable model reference" |
...
The roadmap already caught this once, on FR-185, 2026-08-16: *"the requirement counted as evidenced because a test marked it."* The lesson was recorded against that one requirement and never applied to the headline. The closure record must not repeat it.
...
The 2026-08-22 slice ran `git log -S` and found that a "dated note 2026-08-21, owner WK-661" the EBM slice claimed to have written **had never been written** — an obligation recorded as discharged against evidence that did not exist. Before closing, verify every remaining "dated note, owner WK-661" claim in a slice record against `02-modelling.md` rather than trusting it. `git log -S'<phrase>' -- docs/specs/02-modelling.md` is the instrument.

### 6e — The blocking decision

**FR-95's reassignment to WK-690 is "maintainer acceptance pending"** at `roadmap.md:2589` and `:2985`, dated 2026-08-19. §14 requires an explicit dated acceptance line. Until it is accepted or rejected, a requirement inside WK-661's own module has formally ambiguous ownership and WK-661 cannot honestly disown it.
...
- `uv run pytest backend/tests/test_demo_guide.py` passes — FR-409's guide is derived, so the check is that it still derives.
- No open PRs for WK-661; branch deleted after merge, **verified by content** (`git diff --stat main <branch>`), because squash-merge rewrites history.
