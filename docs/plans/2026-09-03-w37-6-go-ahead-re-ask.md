...
and not extended. It superseded [`…-go-ahead-ask.md`](2026-09-02-w37-6-go-ahead-ask.md), whose
...
([`…-time-boxed-delegation.md`](2026-09-03-w37-6-time-boxed-delegation.md) §6).
...
#661), Ruling 98 (`d1cabe1` #665), the renewed delegation and its two rulings (`e56d038` #664,
quoted at §7), the register reconciliation (`735c828` #666), F95 (`c039729` #668), Ruling 99
...
`0042957` (Ruling 98/99 heading fix) and `2f0467e` (`FD`/`WF` implementation, `#671`).
...
(`F95.md`, `F96.md` under `docs/audit/`) + 1 (Ruling 99's own file) = +3 real corpus growth,
plus 5 test-fixture-only additions under `tests/` that do not touch any `docs/` count. Ruling
headings +2/+2: Ruling 98's own file gained its heading (F96's fix) and Ruling 99 is new — **not
the seven documents Ruling 98's rule names (§5)**, which carry no `## Ruling N` heading by
...
**Ruling 98: one half moved, one did not — stated separately because a reader needs to know
which.** `FD`/`WF` are **implemented and verified above**. **Ruling 98's own substantive rule
...
files). Only Ruling 98's *own* document now classifies correctly — F96's heading-fix (self-
referential: a ruling missing its own `## Ruling N` heading) is a different defect that
happens to share this window, not the rule Ruling 98 states. **Consequence, stated plainly: a
run at this tree stamps those seven `owner: planner`, the exact misattribution Ruling 98 was
...
**The four figures are not quotable apart — Ruling 97 §4, and the maintainer's own ruling on
...
(§6.3's own **531**/**292** are Ruling 97's `15ed00d` figures; this ask's larger **588**/**349**
...
outcome, not a defect: **the evidence is the control, not the zero** (Ruling 97 §3, proofs 1/2;
...
[`2026-09-03-w37-6-d1-d2-rulings.md`](2026-09-03-w37-6-d1-d2-rulings.md), **Ruling 96** (D1 —
`check_shape` does not govern a verbatim-migrated body; `was:` marks it) and **Ruling 97** (D2
...
2. **On the gap — §6 is the accepted condition restated; `[SLOT-4]` stays unfilled until the
   branch is cut.**
3. **On Ruling 98 — disclosed, not a condition** (§5): the run will misattribute seven
...
[`…-time-boxed-delegation.md`](2026-09-03-w37-6-time-boxed-delegation.md) §2 — *"If all six
