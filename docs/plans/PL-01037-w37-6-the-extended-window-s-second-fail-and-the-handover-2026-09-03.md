...
family: plan
kind: handover
title: W37-6 — the extended window's second fail, and the handover (2026-09-03)
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-09-03-w37-6-second-fail-handover.md
---
...
**Six of the nine RFC-937 §7 rows fail at `0de529e`, and one of them is content corruption
...
The mechanism: a compound citation like `NFR-502/501` names **two** requirements in
...
```
-  close the hand-compiled owed list **lost NFR-502/501** (F41) …
...
[`RFC-789`](../rfcs/RFC-00789-zero-calls-above-200k-tokens-measures-the-compaction-cap-not-discipline.md) at scale. **And check
...
*"every §5.2 H row is named by a commit"* — **which is §7(i) verbatim.** RFC-937 §8 agrees by
...
**And the S2-scoped reading of (i) fails anyway**: one of RFC-937 §8's named S2 H rows —
...
`was: docs/ledgers/LG-00030-w5-wf-01-driven-end-to-end.md` — a real 233 KB WK-661 execution
...
maintainer with a disclosure under RL-1039 — is granted on a field that is right 3 times in
...
| **3 — semantic** (RL-1042 §101.3) | **SURVIVES**, as the maintainer classified it. A link resolving to the wrong index section still resolves |
...
| #679 | `0de529e` | **RL-1047's implementation** — a maintainer's dated decision migrates `RL-`, plus the RUF001 escape and a by-symbol predicate |
| #680 | `8c66f67` | **RL-1042** — the undetermined citation resolves to the split's index entry; bucket (iv) 0 by construction |
...
- **The OQ collision population is 8 ids, not 1.** `OQ-539` is claimed by **three** records
  (`OQ-539`, `OQ-545`, `OQ-551`), not two, and seven more ids are multiply claimed
  (`OQ-538`, `OQ-556`, `OQ-577/-11/-23/-24`, `OQ-632`). **The guard warns rather
...
- **A premise of RL-1042 does not hold.** Clause 1 says `docs/<family>/INDEX.md` as though a
  ...
  meant is decided** — which preserves RL-1042's own grounds. **It needs the maintainer's
  ruling and has not had it.** `_MIGRATION_DIFF_FAMILY_INDEXES` was added explicitly rather
  than silently extending RL-989's six-class enumeration.
...
| **RL-1042's cross-family placement** | **maintainer's** (§10) |
...
| **"RL-989 class-6 ratification" and "the lettered-ruling nested-span floor"** | **resolve to nothing in `docs/`** — dropped from any load count until cited |
...
3. **The ASCII-apostrophe ruff fix** — would have silently deleted typographic-apostrophe
   ...
   application.**
4. **"CI is green on the RL-1047 branch"** — CI had **never run** on it; no PR, no run, absent
...
7. **"137 in the disputed class"** and **"`OQ-539` claimed by `OQ-812` and `OQ-818`"** — both
   unsourced and both wrong; the executor refused to reconcile to either.
8. **The 167 and RL-1042's 171 relayed as one class** — they are different populations, and
   **141 of the 167 are single-target moves RL-1041 §2.3 leaves untouched.** Had that stood,
