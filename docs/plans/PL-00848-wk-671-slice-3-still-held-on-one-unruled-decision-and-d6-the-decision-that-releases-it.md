...
family: plan
kind: leaf
title: WK-671 Slice 3 — still held, on one unruled decision; and D6, the decision that releases it
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-29-w11-3-batch-readiness-and-d6.md
---
...
([`PL-00847-wk-671-slice-2-the-real-time-scoring-endpoint-its-bundle-slot-and-the-nfrs-that-need-the-http-path.md`](PL-00847-wk-671-slice-2-the-real-time-scoring-endpoint-its-bundle-slot-and-the-nfrs-that-need-the-http-path.md),
...
**Highest ids in use, re-derived at `d6505e9`:** FR-243, NFR-501, OQ-620.
...
| **S-F** — D3, where FR-255's batch abort threshold is configured | **Fired.** RL-889 |
| **S-G** — the chunk/resume mechanism, and whether batch traces at all | **Half fired.** RL-890 settles batch traces. **Nothing has ruled chunk/resume** |

**The half that did not fire is the slice's spine.** FR-254 requires batch scoring to be
...
`chunk` across the three WK-671 ruling records returns nothing at all.
...
- **There is no per-step evaluator in `pricing-core` to share.** Step-by-step evaluation happens
  ...
  FR-254's *"identical code path"* actually means here is the **post-evaluation tail** —
...
- **`score_batch` stays plain `def`** (RL-868, restated in the module docstring), against
...
**Why the platform does not already answer it.** FR-401 gives *"a cancelled Job leaves no
partially-visible artifact"* — and that clause governs **cancellation, not crash-resume**. The
distinction is the recovery document's own and it is the crux: FR-254's *"resumable"* is
asking for something the generic Job contract does not supply. An implementer who reads
FR-401 as covering it will build nothing and believe the requirement is met.
...
- RL-889 settles the abort threshold: a workspace setting `rating.batch_abort_failure_rate`,
  unset by default, plus a per-run Job argument that **may only lower** the effective threshold.
- RL-890 settles traces: `score_batch` takes **no sampling policy**.

### The options

- **(a) Full restart on failure.** Simplest; matches the generic Job pattern exactly; adds
  nothing to build. But it gives FR-254's *"resumable"* no meaning beyond what every other
  Job already has, and at NFR-493's 1 M risks/hour a late failure on a multi-million-row run
...
- **(b) Chunk-checkpointed resume.** Each chunk writes its part to job-scoped staging on
  ...
  FR-401's *"no partially-visible artifact"* true for callers while satisfying resumability
...
run leaves orphaned partial parquet nobody deletes. FR-420's reference-counted blob GC is
the obvious candidate and it is *not* automatic here, because RL-888 established for traces
...
That is a different situation from Slice 2, where RL-881 shrank one task and the rest was
...
- **Requirements**, each listed individually: FR-253 (`03` §3.7, the endpoint, Job and
  content-addressed parquet output), FR-254 (§3.7, chunked/resumable/progress and the
  identical code path), FR-255's batch half (§3.7, counts and samples per error type, abort
  only past the declared threshold), NFR-493 (§9, ≥ 1 M risks/hour/worker, linear in workers).
- **The FR-254 proof, stated exactly**: `score_batch` and `score_one` scoring the identical
...
- **RL-889's shape**: `rating.batch_abort_failure_rate` follows the `<module>.<name>` form of
...
- **RL-890's exclusion**: `score_batch` takes no sampling policy; a batch run may produce
...
- **The evidence sweep the plan must re-run at its own commit**, not reuse from here:
  ...
  `git diff` of the WK-671 ruling records between `d6505e9` and that commit — **by diff, not by
...
- **The absence claim behind the hold** was made with the full marker class, not one spelling:
  ...
  sweep of all three WK-671 ruling records (no hits).
