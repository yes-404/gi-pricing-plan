...
family: plan
kind: leaf
title: WK-671 Slice 4 — the always-capture design pins the traced fraction at 1, and NFR-489 fails on all real-time traffic (2026-08-30)
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-30-w11-4-always-capture-correction.md
---

# WK-671 Slice 4 — the always-capture design pins the traced fraction at 1, and NFR-489 fails on all real-time traffic (2026-08-30)

**What this is.** A correction record against
[`PL-00850-wk-671-slice-4-trace-sampling-the-row-plus-blob-store-and-the-retention-floor.md`](PL-00850-wk-671-slice-4-trace-sampling-the-row-plus-blob-store-and-the-retention-floor.md),
...
that, and what the measurement shows that RL-863 could not.
...
is its sibling, the same treatment the addendum to RL-857 gave that ruling.
...
> **RESOLVED, same day, by RL-862** (`fdbcb90`, PR #427 —
> [`../rulings/RL-00862-serve-untraced-produce-the-trace-off-the-request-path-by-deterministic-re-score.md`](../rulings/RL-00862-serve-untraced-produce-the-trace-off-the-request-path-by-deterministic-re-score.md)).
...
**Highest ids in use, re-derived at `2e2da7c`:** FR-243, NFR-501, OQ-620.
...
([`:317-331`](PL-00850-wk-671-slice-4-trace-sampling-the-row-plus-blob-store-and-the-retention-floor.md), read at `2e2da7c`) chose the
...
> **If measurement in 4D shows always-capturing breaches NFR-489, that is a finding for the
> decision-maker, not a licence to drop the 100 % rule** — the rule is FR-259's text.
...
NFR-489's budget is **p99 < 50 ms**. A traced request is roughly **2x the budget**.
...
matching `task-1-5` or `bench-rating` returns nothing). RL-863 already rests on the same
...
RL-863 ([`../rulings/RL-00863-sampling-cannot-remedy-either-requirement-and-the-reasons-differ.md`](../rulings/RL-00863-sampling-cannot-remedy-either-requirement-and-the-reasons-differ.md))
...
1. FR-259 requires **100 % of declines and errors** to be traced.
...
**So *p* = 1, and NFR-489 fails on 100 % of real-time traffic** — at a traced p99 of
103.227 ms against a 50 ms budget — not on a 1 % sample and not on the decline population.

**Why this is stronger than RL-863, and in a way that ruling could not see.** RL-863 left
...
*p* is still 1. **The breach is unconditional in *d*.** RL-863's algebra is correct about the
...
violates RL-863's own compliance condition at every value of *d*; and that a traced request as
measured is about twice NFR-489's budget.
...
- **That FR-259's 100 %-of-declines rule is wrong.** The frozen plan's own clause forbids
...
[`PL-00851-wk-671-five-decision-points-recovered.md`](PL-00851-wk-671-five-decision-points-recovered.md) gave
...
carries it: RL-863's *p* and *d* algebra against the ruling itself, Task 4B Step 5's wording
...
- **A planner who has proved a defect does not thereby acquire the choice of its fix.** That is
  the exact overreach the addendum to RL-857 corrected in
  [`PL-00849-wk-671-slice-3-batch-scoring-the-pure-transform-the-checkpointing-handler-and-the-route.md`](PL-00849-wk-671-slice-3-batch-scoring-the-pure-transform-the-checkpointing-handler-and-the-route.md) hours earlier: this
...
`score_one`'s trace mechanism, a change to what FR-259 requires be captured versus persisted,
an amendment to NFR-489's population, or something not listed — with the options and a
recommendation, from whoever's charter owns it.

> **Added after RL-862, because the outcome is evidence about this section rather than only
...
- **Slice 4's other three tasks are untouched.** 4A (the row-plus-blob store), 4C
  (`GET /api/v1/traces` and its access control) and 4D (the NFR-500 projection) do not depend
...
- **RL-890 is untouched.** Batch contributes nothing to the sampled production stream, and
  `score_batch` takes no sampling policy. The finding above is about the real-time path only.
- **RL-888 is untouched.** The storage shape, the one-serialisation constraint and the
...
- **Slice 2's plan is untouched but is where this lands.** Its Task 2B builds the route that would
  do the always-capturing, and its Task 2D measures NFR-489 at the full HTTP path. Whichever
  remedy is ruled, Slice 2 is the slice that implements it.

## 8. Resolved by RL-862 — and it corrects a premise of §3

**Ruled** (`fdbcb90`, PR #427): *"trace **production** is decoupled from the serving request."*
The quoting path scores **untraced** and responds; where FR-259 requires a trace — every
decline, every error, the sampled fraction — it is produced by re-scoring the same Quote Context
against the same **pinned** bundle, off the request path. **FR-259's 100 % rule is not
...
*"re-scoring is not acceptable on a 50 ms budget."* RL-862 §2:
...
- **(b) The re-score verifies it reproduced the served quote, and says so when it did not.** The
  ...
  not swallowed**; FR-260's *"exact for money"* is the available standard. *"A trace that
  ...
  because NFR-495's *"across machines and platform versions"* limb is only partly evidenced.

### 8.3 What the ruling does not fix — do not book NFR-490 as satisfied

**NFR-490 remains failing.** It is a per-request ratio, so it is violated wherever a trace is
produced at all, on-path or off. Moving the cost off the request path removes it from
NFR-489's population; **it does not make tracing cheaper.** Roughly *d* + 1 % of requests
still pay a ~8.2× traced cost, off-path.

So the remedy **discharges the NFR-489 limb and leaves the NFR-490 limb open**, still gated
on PR #416's owner question. RL-863 §4's remaining candidates apply to that limb alone. This
...
fixing 'the interaction' would leave NFR-490 booked as delivered."*

### 8.4 Binding constraint on the carrier — the Quote Context must not travel in `JobRow.parameters`

RL-862 routed a governance question to the lead and it is answered. **This is the clause an
...
job metadata. **NFR-499 forbids exactly this:** *"quote inputs are never logged in full
...
- **Slice 4 Task 4A** — the row gains a pending state and the fields above. Its
  one-serialisation constraint (RL-888) is unaffected: the pending row is written at serve
...
- **A capacity fact the ruling records and nobody had costed:** always-capture is ~8.2× CPU on
  all traffic, so sustaining NFR-489's 200 rps per replica would have needed roughly **eight
...
- **RL-863's algebra was read at source, not from the relay that reported it.** §2's *"Let
  **p** be the fraction of real-time requests traced"* and §3's *"**p** = **d** + 0.01(1 − **d**)"*
  were quoted from
  `docs/rulings/RL-00863-sampling-cannot-remedy-either-requirement-and-the-reasons-differ.md` — the claim that the ruling
...
- **The measurement's non-merged status** was established by listing `docs/research/` at
  ...
  commit log.
- **RL-862's three code claims behind the carrier constraint (§8.4) were re-read at
