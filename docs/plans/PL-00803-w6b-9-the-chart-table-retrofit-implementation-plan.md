...
family: plan
kind: leaf
title: W6b-9 — The Chart/Table Retrofit Implementation Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-25-w6b-9-chart-table-retrofit.md
---
...
- **The slice map is frozen.** `docs/plans/PL-00786-wk-664-the-revised-slice-map.md` is not
...
|---|---|---|---|---|
| `OneWayChart.vue` | `17e35f7` (#53, WK-663) | `ProfileView` | A table in the **view**, not the component — anonymous, and **missing the frequency CI** | **None.** No component test, and its only consumer stubs it |
...
`NFR-463` (`00` §9): *"Accessibility: the SPA meets WCAG 2.2 AA; all charts have an
...
> The exact Poisson interval (FR-61), drawn as a whisker per level. A frequency without
...
### 1. Moving the one-way table relocates code governed by the open OQ-538
...
position on **exactly** the question `OQ-538` leaves open — whether burning cost is an
...
`/ 100`, same "—" for null. This plan notes the relocation so that whoever decides OQ-538
...
Also pin `Number(row.exposure_years)`: the value is an exact decimal **string** (FR-10)
...
is the FR-10 boundary this component sits on, and it is currently asserted nowhere.
...
Take the next free `OQ-OVR-` id from the file rather than assuming one — `OQ-538` is the
...
the histogram carries it (`FR-65`); the column is omitted rather than filled with `—`
...
well as hue for NFR-463's non-colour channel; the table is the second half of the same
...
3. **The `mean_severity` / `mean_burning_cost` comment moves verbatim.** It states a position
   on the open `OQ-538`; relocating is not the place to re-argue or resolve it.
