...
family: plan
kind: leaf
title: W32-5 — the two partial-dependence defects: execution ledger
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-23-w32-5-partial-dependence-exposure-ledger.md
---

# W32-5 — the two partial-dependence defects: execution ledger

What executing
[`PL-00767-w32-5-the-two-partial-dependence-defects-implementation-plan.md`](PL-00767-w32-5-the-two-partial-dependence-defects-implementation-plan.md)
...
| The ranking FR-175's cap applies | row count | summed exposure |
...
| `FR-181` | unallocated | defined, evidenced by two tests |
| NFR-488 | 0.0480 fits/pass (2026-08-22) | **0.0356** fits/pass, budget 0.06 |
...
- Next free: `FR-180` — the W32-4 branch's, allocated on 2026-08-23 and not yet landed.
...
plan takes `FR-181`, and — explicitly — *"do not renumber if the two land out of
order"*.
...
concurrent slices.** The id is left at `FR-181` as the plan directs — renumbering to 124
...
```
NFR-476 gbm fit, 500 trees      16.25 s wall
NFR-488 gbm diagnostics       360.87 s wall
NFR-488 — 360.87 s over 623 scoring passes (60 permutation, 560 partial-dependence points)
...
1. **The default scale is nine times the comparison scale.** `--only gbm` with no flags is
   678 013 rows x 60 factors x 500 trees. NFR-476's own budget for the *fit* alone at that
...
| Closing checklist: *"the branch is pushed and a PR is open"* | Not done, by instruction: the main thread lands all WK-692 branches |
...
This branch adds **no** migration, so FR-417's single-head invariant is untouched:
...
in the suite was red, and the WK-690 `xfail` was counted as `xfailed` rather than turning green.
...
- **Cross factors still grid over their first source column.** A representative value for a
  ...
  [`../roadmap.md`](../roadmap.md). **Owner: WK-664**, with the frontend that first plots a cross
  factor's curve.
- `_permutation_importances` shares the row-count defect at the same module's `:870`. Left
  alone: its FR-176/177/178 block is **WK-690's**.
- The WK-690 `xfail(strict=True)` at `test_a_gbm_with_a_sparse_interaction_can_produce_diagnostics`
