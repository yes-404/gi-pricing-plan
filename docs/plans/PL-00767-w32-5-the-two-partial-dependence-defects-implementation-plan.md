...
family: plan
kind: leaf
title: W32-5 — The two partial-dependence defects Implementation Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-23-w32-5-partial-dependence-exposure.md
---
...
`scripts/bench-model.py` measures NFR-488.

**Spec:**
- [`../specs/02-modelling.md`](../specs/02-modelling.md) — FR-174 (partial dependence),
  FR-175 (the cap, and its explicit deferral of the second defect), FR-176,
  FR-177 and FR-178 (the neighbouring importance requirements, owned by WK-690 and
  **not** this slice), NFR-488 (the diagnostics budget), §5.2's fenced block at
  lines 2148–2310.
- [`../roadmap.md`](../roadmap.md) — lines 3604–3614 (the first defect, recorded as two
  sub-defects) and 3615–3621 (the second), plus 704–713 where WK-661's closure recorded both.
- [`PL-00753-wk-664-and-wk-692-the-slice-map.md`](PL-00753-wk-664-and-wk-692-the-slice-map.md) — line 95, this slice's row.
- [`PL-00750-wk-661-closure-the-five-items-wk-661-still-owns-and-the-closure-record.md`](PL-00750-wk-661-closure-the-five-items-wk-661-still-owns-and-the-closure-record.md) — lines 76 and 77, where both
...
- **`pricing-core` imports no FastAPI, SQLAlchemy or Redis** (ADR-703). This slice is
...
- **Requirement IDs are permanent** (§5): append, never renumber. This slice appends one
  requirement to `02` §3. Highest ids in use: FR-179, NFR-488.
  Next free: `FR-180` is taken by the W32-4 plan of the same date, so this plan takes `FR-181`.
...
- **NFRs are measured, not asserted** (§13 rule 5). Task 4 re-measures NFR-488 and
...
with "the frontend that first plots the curve"; that frontend is WK-664's and is not this slice.
...
`:846`. That function shares this defect at `:870`, but its FR-176/177/178 block
(`:853-867`) is owned by WK-690 and is **out of scope here**; do not fix it in passing.
...
owned by WK-690** — if this task turns it green, `strict=True` fails the run, and the correct
...
model was fitted on. FR-175 records this deferral explicitly — it is a known gap being
...
note. The neighbouring xfail at `:1843-1889` is WK-690's and stays red.
...
Run: `uv run pytest packages/pricing-core/tests/test_gbm.py -q`
Expected: PASS, with the WK-690 xfail at `:1843-1889` still red.
...
Expected: a passes-per-fit figure at or below NFR-488's budget of **0.06**. The last
...
cap" and cites OQ-596 as an open question. The cap has existed since FR-175 and
OQ-596 is decided (`docs/open-questions.md:79`). Rewrite those lines to describe what the
...
2. **§5.2's `compute_gbm_diagnostics` signature omits `max_partial_dependence_levels`**
   (`:2255-2263`). The parameter exists, is what FR-175's cap is spelled as, and is
...
> (W32-5).** It is how FR-175's cap is spelled, it has existed since the cap did, and
...
representative value for a cross level is a tuple across several columns. Owner: WK-664, with the
...
```
NFR-488 re-measured after the fix: <figure> passes/fit against the 0.06 budget, at load
average <loadavg>.
```
...
both of Task 1's tests, so each carries `FR-174` and the new id. `pytest` runs with
...
- [ ] The WK-690 xfail at `packages/pricing-core/tests/test_gbm.py:1843-1889` is still red.
