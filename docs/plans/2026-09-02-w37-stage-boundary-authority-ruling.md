...
charters, the eleven primary skills' to **S3**, after the migration. Ruling 66 pulls three
...
document's own account: `docs/process/document-ids.md` is titled *"NT-0019 §1, lifted verbatim"*
and lifts §1.1 through §1.13 — not §4, §5, §7 or §8. And §8's own construction does not have a
boundary to move: **S2 is a criterion with an illustrative list, S3 is a residue.** Ruling 66
...
in NT-0019 — in §8 itself — with no support anywhere else in the note. **§8's enumeration was
...
- **This record amends no filed plan and edits no note.** W37-6's leaf plan is frozen; NT-0019 is
  the maintainer's.
...
carried forward with NT-0019's migration as its trigger. Honoured here; the check is not patched
from this branch.
...
## Ruling 85 — §8 is sequencing, not standard; S2 is a criterion and S3 is a residue, so Ruling 66 moved nothing
...
...
Lifted verbatim from NT-0019 §1.1 through §1.13,
```
...
**(c) NT-0019's own amendment mechanism is about §1's structures, not §8's stages.** §1.12
...
**(d) The Work's own scope clause omits §8.** `docs/roadmap.md`'s W37 row:
...
[`2026-09-01-nt-0019-id-standard-map-plan.md`](2026-09-01-nt-0019-id-standard-map-plan.md) cut
the Work into eleven slices *from* §8's stages. A map plan is the planner's, revisable by a
replan (`CLAUDE.md` §12, NT-0019 §1.6's `PL-` row). So even on the reading most hostile to
Ruling 66, re-cutting §8 is at worst **the planner's**, never the maintainer's.
...
Ruling 66 held that the instrument set is a criterion rather than a list and derived thirteen
...
2. **The leaf plan's closing rule**: *"On any disagreement between this plan and
   [`NT-0019`](../notes/0019-one-id-per-document.md), the note wins and the disagreement is a
...
**(g) The measurement that answers (f)(1), and it is decisive.** `"eleven"` appears in NT-0019
exactly once:

```
$ grep -n 'eleven' docs/notes/0019-one-id-per-document.md
...
```
$ sed -n '<§5.4>,<§5.5>p' docs/notes/0019-one-id-per-document.md | grep -c 'primary'
6
```
...
**(h) The answer to (f)(2).** The closing rule's precedence is *plan* below *note*. **Ruling 66
is neither.** The chain runs ruling → plan: §6.2 derives the instrument set *that Ruling 66
requires*, and the leaf plan cites Ruling 66 as an obligation it discharges. The closing rule
...
stages is not an amendment; Ruling 66 stands as issued and needs nothing further.**

Row 36's two dispositions were *"either an amendment to §8, or a ruling that Ruling 66's
...
resizes S3 by construction. Ruling 66 did not extend into S3's population; it computed S2's
...
1. **W37-6's commit contains the thirteen instruments Ruling 66's criterion derives**, including
...
4. **Row 36 is closed by this record**, not by an amendment. The obligations list's *"NEEDS A
   RULING"* state resolves to `Ruling 85`, and the register row's disposition column should name
   it.
5. **Nothing here touches Ruling 66's own acceptance.** Ruling 73 already withdrew that ruling's
...
- **A check that every instrument in Ruling 66's derived set is modified by the migration
...
[`2026-09-02-w37-template-parser-conflicts-rulings.md`](2026-09-02-w37-template-parser-conflicts-rulings.md)
...
| **Whether Ruling 66's thirteen is still the right set** | Ruling 66 settled the criterion; the leaf plan §6.2 derived the members and Ruling 73 already withdrew the acceptance item that would have re-derived them. Re-opening membership is a new question, not this one | **The lead**, if anyone raises it |
