...
family: plan
kind: leaf
title: W6b-5b — The Suggestion Panel and the Workbench Remainder — Implementation Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-25-w6b-5b-suggestion-panel.md
---
...
**Highest ids in use: `FR-168`, `OQ-608`, `OQ-552`, `OQ-655`.**
This plan proposes **`OQ-612`**, **`OQ-611`** and **`OQ-610`**, and no
requirement. Next free after them: `OQ-609`.

**This slice carries FR-168's owner clause** — description-anchored to "the slice that
...
- **A suggestion is never an addition** (FR-135). Adding an interaction stays an
  authored decision carrying an intent and a rationale.
- **No threshold anywhere** (FR-168): ranked evidence, never an admission test.
...
FR-24 makes a §5.3 cell non-binding unless it declares itself exhaustive, and the factor
workbench is not among the seven carve-outs. Scope is the requirements; the cell indexes them.

| Cell item | State |
|---|---|
| Column list with profile one-ways (`01` FR-61) | **missing** — this slice |
...
| Interaction suggestions ranked by SHAP strength, with FR-168's ratio beside each | **missing** — this slice |

**Building the intent control discharges an already-recorded §14 q4 finding.** FR-84
...
editors must show an edit's consequence *before* it is saved. FR-24's rule is
...
**Filed as `OQ-611`, cross-referencing `OQ-605` as its sibling.** These are the
same shape — a list route whose filters omit the axis a view needs — and they are the only
two: a sweep of `docs/` found `OQ-565` and `OQ-552` to be missing *fields* rather than
a missing filter *axis*, `02`:1747 a parameter-name mismatch already fixed, and
`OQ-607` a missing machine-readable set. An earlier draft of this plan called F1 the
...
The generated enum is `risk | control | offset | diagnostic`. FR-84 supersedes
`offset` and FR-86 supersedes `diagnostic`, both **keeping the arm in the published
...
with an existing slug allocates version N+1 (`platform/modelling.py`:229-235, **FR-96**).
...
**`OQ-607`(c)'s prerequisite is already shipped**, recorded as a dated append in both
...
**The interim mirrors `OQ-605`'s rather than inventing one.** The same cursor pagination
...
1. **`OQ-608`** — the ratio's null is below `1` because the denominator is a selected
   maximum. The panel must not imply `1` is the neutral point; that interpretive sentence is
   FR-168's and the maintainer's.
2. **`OQ-610`** files the intent surface as **one** question with two named surfaces:
...
3. **FR-168's rebuild clause** stays unsatisfied with an owner, recorded in 5a.
4. **The row cap is untested** — "delivered but untested" for the closure record.
5. **§5.3's Interaction requirement paragraph cites FR-107, which does not carry it —
   but a numbered requirement does.** FR-107's predicate is what a Grouping *stores*
   ...
   either needs a number or should be struck; **checked, and neither is so**. **FR-102**
   carries it explicitly — "§5.3's interaction requirement — that an edit's consequence is
   visible *before* it is saved — is otherwise unmeetable" — and was added 2026-08-15 (WK-661)
   for that purpose, giving Bandings and Groupings an evaluate-without-persisting route. The
   paragraph's FR-107 citation is narrow and correct as far as it goes: FR-107 is
...
1. **Inline one-ways** on the column list, read from the Profile the view already loads and
   never computed in the browser. The tighter authority is **FR-61**'s "are computed
   once, here"; **FR-62** is cited alongside it but its predicate is recomputing *a
...
3. **The model selector** (Decisions 3 and 4).
   ...
   **What is reused is the pattern, not the function.** `listObjectives`
   ...
   OQ-605's applicability problem. What transfers is the **discipline** — page to a
...
4. **The suggestion panel** — ranked by strength, ratio beside each, no threshold, and
   ...
   (FR-135). Decision 5's wording for the all-`null` case.
5. **`OQ-612`, `OQ-611`, `OQ-610`** in both mirrors, plus the dated append to
   `OQ-607` recording that (c)'s CI prerequisite is shipped.
...
1. **If §5.3's cell is later declared exhaustive.** It is prose under FR-24 today.
2. **If `GET /models` at scale makes Decision 3 untenable.** The bound makes truncation
   ...
   selector honestly incomplete on every visit — which is `OQ-611`'s point.
