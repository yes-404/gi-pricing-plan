...
family: plan
kind: leaf
title: Transparency Exposure Share Implementation Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-23-w32-9-transparency-exposure-share.md
---
...
closed one module away in `diagnostics.py`. FR-136's worst-region share is computed as
...
and becomes "of exposure". FR-135's `ShapInteraction.exposure_share` is the literal `1.0`
...
**Spec:** [`../specs/02-modelling.md`](../specs/02-modelling.md) — FR-136 (the fidelity
statement and its worst regions), FR-135 (interaction candidates), with FR-181 as the
precedent this mirrors and OQ-601 as the decision that withdrew the interaction share.

**Proposed slice id:** `W32-9`. The WK-692 slice boundaries in
...
FR-181, and `transparency.py:206-208` already states the reason: `02` §3.6 approximates the
...
def _worst_regions(
    ...
    *,
    ...
    The share is a share of **exposure** (FR-136), the same quantity FR-181 names one
...
Extend the existing FR-136 statement test rather than adding a third. **Insert two
...
FR-135 withdraws the field rather than fixing it. The disanalogy with a partial-dependence
point or a worst region is real: those are per level and their shares partition exposure, while a
candidate is one object spanning the entire frame. OQ-601 decided on 2026-08-23 that what
stands beside a candidate is its holdout strength ratio (FR-168), which is **not** this
...
```python
    ...
    `1.0` by construction and was withdrawn by OQ-601 on 2026-08-23, and the out-of-sample
    evidence that replaces it is FR-168's holdout strength ratio, which is not built here.
```
...
- Create: `PL-00774-w32-9-execution-ledger-the-transparency-exposure-share.md`
...
Create `docs/plans/PL-00774-w32-9-execution-ledger-the-transparency-exposure-share.md` recording, task by task,
...
same commit that appended **FR-168** (`:232`, *(appended 2026-08-23, OQ-601)*). The
...
What this slice does record is FR-135's withdrawn field: `top_interactions[].exposure_share`
...
three FR-136 sites moved, that FR-135's field was withdrawn rather than computed, and
that FR-168 is left unbuilt with OQ-601 as its origin.

- [ ] **Step 6: Commit**

```bash
git add docs/plans/PL-00774-w32-9-execution-ledger-the-transparency-exposure-share.md docs/roadmap.md
