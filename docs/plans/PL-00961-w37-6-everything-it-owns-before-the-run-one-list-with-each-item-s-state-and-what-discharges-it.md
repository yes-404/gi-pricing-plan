...
family: plan
kind: leaf
title: W37-6 — everything it owns before the run: one list, with each item's state and what discharges it
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-09-02-w37-6-outstanding-obligations.md
---
...
with the frozen
[`PL-00960-w37-6-the-migration-run-leaf-plan.md`](PL-00960-w37-6-the-migration-run-leaf-plan.md)
...
5. **The frozen leaf plan is not edited.** *Violation: any diff in the pull request
   ...
   `docs/plans/PL-00960-w37-6-the-migration-run-leaf-plan.md` (`CLAUDE.md` §2).*
...
- **Every count carries its tree and the corpus it counted over**
  ([`RFC-777`](../rfcs/RFC-00777-a-reference-that-resolves-only-in-the-writer-s-context.md)).
...
|---|---|---|---|---|---|
| 1 | A | `_discover_plan_reviews` under-discovers silently: 14 `###` headings, regex matches 10, function returns 10; the four unmatched fold into `Plan review 1` (2026-08-15), which absorbs lines 1061-2155 including the whole of `Plan review 9` (2026-08-30). RL-978 §1(b) | IN FLIGHT | agent `exec-plan-reviews`, branch `worktree-plan-reviews-fix`, zero commits at 08:47Z | Trailing-text capture group in the shape `#585` used for `_CLOSURE_HEADING_RE`, plus the coverage assertion of RL-985; positive control = the four headings named, red before, green after |
| 2 | A | `_discover_roadmap` converts 0 of 41 works and `migrate` reports success. Guard added in `#584` raises; the pattern is untouched. RL-998/999 record's *Correction* section | FILED, NOT FIXED | unassigned | A roadmap transform designed against the real shape — semantics in decoration (`~~**WK-660**~~ ✔`), non-work rows in the work column, free-prose status cells, several status tables per phase — not a widened regex |
| 3 | A | `_discover_register` matches 0 of 73 data rows: `_REGISTER_FINDING_RE.fullmatch` wants a bare `F<n>`, real cells are compound. Guard added in `#584` raises; the pattern is untouched | FILED, NOT FIXED | unassigned | A cell pattern derived from the register's declared row grammar, plus RL-985's census applied to the row unit (RL-985 *Not ruled* routes the measurement to W37-6's executor) |
| 4 | A | Ten `WK-661 —` closure records carry no family; `_discover_closure_records` raises on the **first** and stops, so clearing them is ten sequential resolve-and-re-run cycles, not one | RULED, UNMERGED | RL-986, branch `docs/w37-rulings-83-84-…` at `2694fc3`, no pull request at 08:47Z | Merge RL-986; the "not closed" branch emits `LG-` with `work:` and no `slice:`; the test asserting 21 drafts as 8 `CR- work` + 1 `CR- phase` + 2 `RS- audit` + 10 `LG-` |
| 5 | A | The guard class fails on zero, not on undercount. Both shipped guards test only "zero drafts from a non-blank file", which is why item 1 passes ungoverned at 10 of 14 | RULED, UNMERGED | RL-985, same branch | Merge RL-985; implement the level-independent census with buckets 1-3 balancing and unit-naming failure text. **RL-985 §3 item 1 puts the census before W37-6, not inside it** |
| 6 | A | What `RL-902`, `A2`, `A3` are — a family and id-form question the census cannot proceed past. RL-985 *Not ruled*: *"the census cannot be cleared while three units are unclassified"* | OWED BY THE PLANNER | planner, then the decision-maker | A derivation enumerating the option set under the id standard §1.2 and §1.7, returned to be ruled |
| 7 | A | The `## Pending proposals` container's family and `kind:`. RL-978 §3 item 3 fixes four constraints and hands the positive assignment over | OWED BY THE PLANNER | planner, then the decision-maker | A derivation testing both surviving placements — own record, or `Plan review 9`'s preamble — against RL-989 acceptance (g) class 4, saying what `Plan review 9`'s two self-citations rewrite to |
| 8 | A | Rulings 79 and 80: `_ROW_FIELDS` is a transcribed field policy wrong in both directions; `scan_phase_sections` requires a fence the templates do not use and reads with unbounded lookahead | IN FLIGHT, zero commits | agent `exec-rulings-79-80`, branch `fix/doc-index-row-phase-fields`, byte-identical to `main` at 08:47Z | RL-977: one pull request, merged on its own, before W37-6, carrying reader and writer together, plus two fixtures' phase sections rewritten unfenced. RL-977 §3 item 3: if it does not land first the work reverts to W37-6 |
| 9 | A | `_VENDORED_SKILLS` does not exist. `is_vendored` still keys on `LICENSE` presence — the criterion RL-990 rejected — exempting 2 of 46 skills where the repository treats 28 as vendored, under-exempting 240 tracked files | FILED, NOT FIXED | W37-6 executor, per RL-973 | A declared constant reconciled against the ruff exclude list, drift loud. **This blocks leaf-plan acceptance item 13** — see §5.4 |
| 10 | A | `docs/_templates/REFERENCE.md` lines 39-49 still teach the rejected criterion in the words RL-990 rejected: *"not a hand-kept list"*. It is inside `_ID_SCOPE_ROOTS`, so it is governed today | FILED, NOT FIXED | W37-6 executor, per RL-973 | Rewrite to the declared-constant rule. Member 2 of RL-973's three-site class |
| 11 | A | Five tests pin the rejected criterion: `test_is_vendored_*` at `tests/test_doc_id.py:387,398,410,427,438` | FILED, NOT FIXED | W37-6 executor, per RL-973 | Re-point all five and add the reconciliation broken-input proof. Member 3 of the class, and on none of the known sources |
| 12 | A | F76 — check 39 calls `build_corpus` unguarded at `scripts/audit-docs.py:2207`; it is the last call in `check_ids_30_39()`, which `main()` runs before six further checks. **Check 28 is one of the six** | FILED, NOT FIXED | W37-6 per RL-998 | Guard it, with a broken-input proof that a malformed header does not silence the six. See §5.5 |
| 13 | A | No check feeds a template's own example block to the parser that consumes it. RL-998 §4 first acceptance bullet | FILED, NOT FIXED | W37-6 executor | A check that fails today with `unknown row field 'tree'` — the positive control the corpus already supplies |
| 14 | A | `plan-reviews.md`'s heading mis-nesting: reviews 9 to 11 sit under a `##` titled *"Pending proposals"*. RL-978 §3 item 4 | FILED, NOT FIXED | the lead, as a finding | A structural correction to the document. Blocks nothing; RL-978: *"the migration should not be the first thing to discover it"* |
| 15 | A | Whether the multi-ruling splitter tries to split the three h1 ruling files. RL-985 *Not ruled* | NEEDS A DECISION — as a measurement | W37-6 executor, inside the census | Run the splitter over them. If it does try, that is a new finding |
| 16 | B | RL-987's thirteen creating instruments, with `Verified` dates refreshed in the same commit. The thirteen live in leaf plan §6.2, not in RL-987, which states seven as a floor | FILED, NOT FIXED | W37-6 | The single commit, plus the nine explicit exclusions each with its slice |
...
| 18 | B | RL-981 and RL-982 both oblige the register's header prose to declare the row field set and the ownership shapes in one place | FILED, NOT FIXED | W37-6 | The rewrite, with RL-981 acceptance item 1's fixture: a register row with no `decision:` must fail |
...
| 25 | B | RL-970 amends leaf-plan acceptance item 11 in three respects: `brainstorming`'s exemption removed, limb 2b (the exclusion list) added, limb 2c (neither list) added | FILED, NOT FIXED | W37-6 | Running 2a, 2b and 2c. **The frozen plan cannot carry the amendment**, so a run against the frozen text alone applies a weaker standard — see §6.3 |
| 26 | B | RL-971's negative obligation: no W37-6 artifact may assert that the identifier standard's PR-title clause is enforced, satisfied or discharged | FILED, NOT FIXED | W37-6 | Running limb 2b over `git-hygiene` and recording *"no document to produce"* as evidence rather than a silent pass |
| 27 | B | RL-972's charter edits: `decision-maker.md` still reads *"recorded as dated sibling records"*; `auditor.md` names **six** dissolving paths, not the four the task list states; `planner.md` names `plan-reviews.md` throughout | FILED, NOT FIXED | W37-6 | The three edits, verified by `git grep -n 'docs/audit/' -- .claude/roles/` returning nothing at the merge tree |
| 28 | B | RL-974's two orphan files need `REDIRECTS.csv` rows **by explicit path** — a glob-driven generator misses both | FILED, NOT FIXED | W37-6 | Both rows, plus the prospective-only findings clause. The routing question itself is settled |
...
| 34 | C | The technique RL-985 ratifies already exists in this repository — `scripts/register-lint.py` asserts `classified == seen` for a different file and was never applied to `migrate`'s discovery functions | FILED HERE | W37-6 executor | Reuse it rather than inventing a second form. On none of the known sources |
...
| 36 | A | The identifier standard's §8 assigns *"seven charters, the eleven primary skills"* to **S3**, after the migration. RL-987 pulls three charters and several skills into **S2**, which is this commit. §8 is unamended, and the leaf plan's own closing rule makes the note win | NEEDS A RULING | decision-maker | Either an amendment to §8, or a ruling that RL-987's criterion reaches across the stage boundary without one. §5.6 |
...
them:** the two `docs/audit/` files with no destination row (RL-974, settled
unconditionally); `git-hygiene`'s exclusion from the instrument set (RL-971, confirmed
on two independent grounds); and the three role charters' membership (RL-972, confirmed,
...
copy of the pattern, per RL-988 acceptance item 2 applied to measurement as well as to
the control.
...
Every file `docs/plans/` gained is WK-697's own governance output, and three of the four
...
RL-989's six-class permitted-diff predicate. Each is a citation the run rewrites and the
...
### 5.6 The standard's own sequencing and RL-987 disagree, and the standard is authoritative
...
RL-987 makes the instrument set a criterion rather than a list, and leaf plan §6.2
derives thirteen members from it — including three role charters and six skills. **Those
are §8's S3 population, moved into S2.**

The move is disclosed and reasoned in leaf plan §4.3, and RL-987's ground is sound: a
...
RL-970 withdrew RL-987's acceptance item 2 and replaced it with a three-limb
...
The frozen plan's item 11 still states two exemptions, one of which RL-970 rejects by
...
This is not my inference. **RL-977 §2** holds that the parser fix *"lands as its own
...
variable in the run that can least afford one."* **RL-985 §3 item 1** holds that *"the
...
species: RL-978 had to decide what the *unit* is before any regex could be written.
...
1. **Insert `W37-5b` between W37-5 and W37-6**, scoped to group A. It is the second half of
   ...
   Its acceptance standard is one sentence and is already written, in RL-985:
...
2. **File a new dated leaf plan for W37-6, superseding the frozen one.** Not because the
   ...
   edited to agree with them: its acceptance item 11 is amended by RL-970 (§6.3); its
   ...
   its §4 figures are pinned at a tree that is 15 commits behind, while RL-987 §3
   requires the disclosure to arrive **with** the ask.

**What I do not propose:** no re-cut of WK-697 into different workstreams, no change to the
...
- It does not rule the guard property. RL-985 does, on a branch with no pull request.
- It does not perform the two derivations rows 6 and 7 name. They are the planner's, they
  are sized in §7.5, and they are on the critical path: RL-985's census cannot be
