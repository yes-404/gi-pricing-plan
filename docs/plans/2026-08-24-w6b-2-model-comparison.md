...
**Spec:** [`../specs/02-modelling.md`](../specs/02-modelling.md) — §4.11 `ModelComparison` (the artifact and its invariants), §5.1 (the two endpoints), §5.3 (the view row), FR-MODEL-56.
...
- **Never hand-write an API type in the frontend; generate it from OpenAPI.** (`CLAUDE.md` §3.) Every domain type in this plan is `components["schemas"][...]` re-exported from a hand-written module in `frontend/src/api/`, per [`../adr/0002-model-schema-single-source-of-truth.md`](../adr/0002-model-schema-single-source-of-truth.md). `frontend/src/api/generated/` is VCS-ignored and may not be edited or cited as evidence.
...
- **FR-OVR-7** — money is integer minor units in "the rating path and all persisted rate tables"; "Floats are permitted only inside model fitting and diagnostics, never in a quoted premium." A comparison artifact is a diagnostic read, so its floats are in scope of that permission. `exposure_years` is nevertheless a `DecimalStr` (a **string**) on the wire and needs `Number()` at the chart boundary with a comment saying why, matching `OneWayChart.vue`.
...
**Requirement ids cited by this plan, all of which already exist:** FR-MODEL-50, FR-MODEL-56, FR-MODEL-84, FR-MODEL-92, FR-MODEL-127, FR-OVR-7, **FR-OVR-21**, FR-DATA-36, FR-DATA-42, FR-PLAT-8, NFR-PLAT-3, NFR-OVR-10. **No new ids; no `Next free:` marker.**

**Frozen against `f838662`** — the commit that landed FR-OVR-21. Every line number and every quoted literal in this plan was read at that revision; a line number in a document someone is appending rows to goes stale without moving, so the revision travels with them.

---

## Where this view's contents come from, and why the §5.3 cell is not the source

**FR-OVR-21** (`00-overview.md:227`, landed 2026-08-24 as the ruling on OQ-OVR-10): *"A Contents cell in a module spec's §5.3 Frontend views table is prose and binds nothing … What a view must show is stated in a numbered requirement or in the generated contract."* It is **prospective, and not in force** until seven §5.3 obligations carried nowhere else are discharged; `02` Model comparison is not among the seven, and this plan adds no obligation to it.

This plan therefore derives the three panels from **a numbered requirement and the generated contract**, which is the route FR-OVR-21 names, and the cell corroborates rather than supplies:

| Source | Standing under FR-OVR-21 | What it names |
|---|---|---|
| `02` FR-MODEL-56 | Numbered requirement — binding | "compared on aligned metrics, double-lift, and factor-by-factor relativity differences" |
...
`JobResult.ref` for this job is **`model_comparison:{uuid}`**. FR-MODEL-92 records the backtest job's result as `backtest:{id}`, and the analogy invites writing `comparison:{id}`. It is `model_comparison:`.
...
- **A tabular fallback for the double-lift chart.** NFR-OVR-10 requires one for every chart; the revised slice map gives it to **W6b-9** ("Tabular chart fallback (`NFR-OVR-10`)") as a single cross-cutting slice. Building a bespoke one here would be the shape W6b-9 then has to unpick.
- **A progress bar.** `Progress` (FR-PLAT-8) carries `fraction`, `stage` and `counters`, and `Job.stalled` is derived on read (NFR-PLAT-3). Task 6 surfaces the **stage label and elapsed attempts** only. A fraction the worker may never populate renders as a bar stuck at zero, which reads worse than no bar.
...
5. **The shared holdout is stored, not promised** (FR-DATA-36). `split_ref` and `holdout_rows` are on the summary so the claim is checkable by a reader; the view shows both.
6. **Double lift is not a diagnostic.** FR-MODEL-50's 2026-08-17 amendment removed it from the universal diagnostics list — *"double lift is pairwise, the comparison model is unknown at fit time"* — and it lives only here. Nothing in this view reads from `Diagnostics`.
 ...
 * diff, which is what FR-MODEL-56 asks for.
 */
...
Expected: audit passes. `req-coverage.py` **cannot see the frontend** — it will not mark FR-MODEL-56 as evidenced by anything in this slice, and that is the script's known blind spot rather than a gap in the work. Do not add a marker to make it look otherwise.
...
**1. Spec coverage.** `02` §5.3's Contents cell has three items and each has a task: aligned metric table (Task 3), double-lift chart (Task 4), factor-by-factor relativity diff (Task 5). FR-MODEL-56's four clauses — two or more models, a shared holdout, the three comparisons, a persisted artifact — are covered by Task 6's minimum-two guard, the rendered `split_ref`/`holdout_rows`, Tasks 3-5, and the `GET` by comparison id. §4.11's six rules each have a named test. NFR-OVR-10 is partly covered (the non-colour channel in Task 4) and partly **deferred to W6b-9 by the slice map**, stated in the file structure.
