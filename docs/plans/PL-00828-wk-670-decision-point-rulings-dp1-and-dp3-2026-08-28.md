...
family: plan
kind: leaf
title: WK-670 decision-point rulings — DP1 and DP3 (2026-08-28)
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-28-w10-rate-tables-rulings.md
---

# WK-670 decision-point rulings — DP1 and DP3 (2026-08-28)

Decision-maker rulings on the W10-2 decision points, recorded per the WK-669 decision-point
precedent (2026-08-27). The plan itself is frozen at its date
(`PL-00829-wk-670-implementation-plan-rate-tables-seeding-diffs-bulk-operations-import-export.md`) and is not amended; this record is the dated home of
the rulings.
...
- **FR-231 names the portfolio as the source.** "the exposure weight behind each
...
- **A diff compares any prior version** (FR-231: "against any prior version"). Under
...
- **Versions are immutable** (FR-232: storage mode fixed at write; the version is
...
- **FR-231 diffs against any prior version.** Eager materialisation at creation can
...
- **The diff is a review-time tool, not a quote-path read.** FR-231: "so an actuary
...
- **FR-232 points the same way.** Above the storage threshold, the diff "becomes a
...
- **FR-232 already decides this.** The requirement's own text: "The threshold is a
...
- **"New versions only" is not a loss.** The threshold's purpose is to bound storage for
  ...
  it was reviewed against, and its reader (the FR-231 diff / FR-232 Job path)
...
`seeded_from` unchanged, so FR-230's question stays answerable along the whole chain;
...
- **FR-230's guarantee is a chain, not a point.** "Subsequent manual edits are diffed
...
- **Inheritance is the only shape a save-time check can state.** "Never re-seed, never
  ...
  FR-234's validation can enforce it at save time, and the audit record can re-derive
  it after.
...
FR-234's save-time validation resolves the baseline from `BulkOperation.applied_to` /
...
- **The hash and the name answer different audit questions.** The hash answers "which
  ...
  offline artifact and the online record in FR-235's export→edit→import workflow.
