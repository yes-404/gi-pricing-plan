...
| 2 | **The discovery-and-stamp path for `.claude/skills/`, `.claude/agents/`, `.claude/roles/` and the README population** | Stamp targets with no discovery coverage; the README population is 32 files against zero §1.6 rows | Handover §6; §3 row 5 below |
...
| **README** | **not ruled** | Deliberately not folded in. The planner brings a candidate derivation under the same *"cite the cell"* constraint |
