...
| Task | Outcome |
|---|---|
| 1 | `CertificateResult.checks` unbound; `battery_is_exactly` enforces the nine-name battery (`ObjectiveCertificate`, `02` §4.7) and the four-name one (`MetricCertificate`, FR-MODEL-105) **by name, not by count**. Authored `objective-certificate.schema.json` `minItems` 8 → 9. `UNRESOLVED_CONSTRAINT_DISAGREEMENTS` emptied. FR-MODEL-126 / OQ-MODEL-30 |
...
| V3 | `results.[].offending_sample.[]` is `string` on the model, `object` in the contract | **Neither side is obviously right, so no side was picked.** §0 forbids a silent choice; recorded as `OQ-DATA-12` with a recommendation, and the type comparison pinned at that one path with a companion test. Reasoning below |
| V4 | `DatasetVersion` and its contract diverge on 22 of 48 paths, all one-sided | **The contract is ahead of the model on the flat fields; the three structural fields are a real shape question.** Recorded as `OQ-DATA-13`. Trimming the contract to match the code was rejected outright — that is deleting the specification to make the tooling agree |
...
FR-DATA-20 and the `01` §2 glossary both say "primary keys of rows" without choosing; §4.6's only
...
`diagnostics.aliasing` was until `OQ-MODEL-15` was decided and its pin was deleted rather than
...
|---|---|---|---|
| F1 | With the shared type unbounded, the floor is published on the authored side only; the generated side and the OpenAPI component carry none | **Delivered.** FR-MODEL-126 asks for enforcement where each certificate is constructed and never asked the generated client to carry a floor. The limit — enforcement is server-side — is published, not implied | The requirement, satisfied as written |
| F2 | No authored-keyword completeness check exists: a keyword declared on the authored side alone is compared against nothing | **Deferred with an owner.** This is the keyword case of a more general finding and is **subsumed by `OQ-PLAT-10`**, named there so it is not filed twice | `OQ-PLAT-10` |
| F3 | `metric-certificate` has no authored contract at all | **Deferred with an owner.** Its four-check floor is enforced model-side, which is what FR-MODEL-105 asks; only the comparison and the publication are outstanding | `OQ-PLAT-13` |

### The five execution found

| # | Finding | §13 verdict | Where it lives |
|---|---|---|---|
| F4 | Nothing revalidates artifacts already stored under a looser shape when a shape is tightened; the failure surfaces on the read path, to a user who did nothing | **Deferred with an owner.** Two real instances: `OQ-OVR-8`'s decimal narrowing and this slice's own battery change | `OQ-PLAT-11` |
...
| F6 | Every layer of the contract guard is scoped to the **intersection** of its two sides, including the completeness check, which defines an eligible schema as one having both — so it is defined over the complement of the problem | **Deferred with an owner.** Nothing is wrong today and the residual is bounded; what is missing is any way to keep knowing that | `OQ-PLAT-10` |
...
| F8 | `DatasetVersion` diverges from its contract on 22 of 48 paths with every comparison green | **Deferred with an owner** | `OQ-DATA-13` |
...
| Requirement | Evidence |
|---|---|
| FR-MODEL-126 | `battery_is_exactly` called from both certificate constructors; tests for missing, unexpected and duplicated names |
| FR-MODEL-105 | `MetricCertificate`'s four-name battery enforced by name |
| FR-PLAT-48 | `generate-contracts.py --check` exit 0, 26 generated contracts match the models; `COMPARED_SLUGS` 15, equal to the both-sided set |
| FR-DATA-20 | Unresolved as to encoding — `OQ-DATA-12`, dispositioned not delivered |
