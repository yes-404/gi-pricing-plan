...
**Final state: `done`** — PR #122 merged-ready — CI CLEAN, 1449 tests, delivering FR-MODEL-45 custom eval metrics and closing MODEL's endpoint axis at 40/40; five defects found by building rather than reading, two of my own rulings overturned by the final review, and three items left explicitly for your decision.
...
last commits:
5ef8b94 docs(model): record OQ-MODEL-21 — LightGBM's silent builtin-metric drop
9d33539 docs(model): a docstring claiming a control the code lacks, and a self-contradicting roadmap
c59a283 fix(model): FR-MODEL-103/106/107 — resolve template defaults, bind stopping by name
f3689dd docs(roadmap): the custom-metrics slice record, and three gate defects fixed
d8859a2 fix(model): FR-MODEL-107 — LightGBM reports every declared custom eval metric
```
...
- `2026-08-20T01:47:47.711Z` **done** — PR #122 open (19 commits, 1449 tests green, MODEL at 40/40 endpoints) delivering FR-MODEL-45 custom eval metri
...
- `2026-08-20T01:52:15.397Z` **done** — PR #122 merged-ready — CI CLEAN, 1449 tests, delivering FR-MODEL-45 custom eval metrics and closing MODEL's en
...
**The `06` §3.3 evidence row** is the one genuine gap this slice created and did not close. The `DEFAULT_POLICY` entry went in on the peril-structure precedent, but the evidence-floor half is a governance change I deliberately didn't make alone. It's owned by W5 in the roadmap with the ordering constraint stated.

**OQ-MODEL-21** needs your decision eventually — whether a documented silent metric drop satisfies "honoured", or whether the combination should refuse. Recommendation on file is to record the drop on the fit result rather than refuse a spec that's valid on the other backend.
...
result: PR #122 merged-ready — CI CLEAN, 1449 tests, delivering FR-MODEL-45 custom eval metrics and closing MODEL's endpoint axis at 40/40; five defects found by building rather than reading, two of my own rulings overturned by the final review, and three items left explicitly for your decision.
