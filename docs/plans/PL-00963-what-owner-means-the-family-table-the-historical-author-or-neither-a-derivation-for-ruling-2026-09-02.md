...
family: plan
kind: leaf
title: What `owner:` means: the family table, the historical author, or neither — a derivation for ruling (2026-09-02)
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-09-02-w37-owner-field-derivation.md
---
...
**Goal:** answer the general question RL-979 routed here — *"does `owner:` follow §1.6's
...
1. **The `was:` question is settled from the schema, not from a ruling's assumption about it.**
   *Violation: an answer to question 3 that cites RL-979 rather than the field's definition
   and its assignments.*
...
**What the standard says `was:` is for.** RFC-937 §4 step 1: *"`was:` and `REDIRECTS.csv` keep
every old **id and path**."* Ids and paths. Its own worked examples are the same shape —
`was: RFC-937`, `was: RL-941`, `was: F27`, `was: PL-00731-the-profile-contract-histogram-rename-and-the-generated-counterpart.md`.
...
`:1152`, `:1186`; the literal `"docs/closures/INDEX.md#closure-recordsmd"` at `:1268`; `None` at `:2024`.
...
`was: PL-00731-the-profile-contract-histogram-rename-and-the-generated-counterpart.md`, a filename. The other **forty** are test fixtures, and
...
RL-979 §2 ground 1: *"The planner's authorship is not erased by this; it is in the body and
in `was:`."* **The `was:` half is false.** `was:` will carry
`docs/closures/INDEX.md#plan-reviewsmd` — a path, from which no author is recoverable without already
...
**RL-979's outcome survives, and this is not a challenge to it.** Grounds 2 and 3 are
...
**RL-995 is the one that matters, because it obliges an action on the same premise.** Its §2
...
|---|---|
| `was:` preserves it | `was:` will carry `docs/plans/INDEX.md#2026-08-30-nt-0012-0013-0014-adoptionmd` — a path. No role, no grant |
| the record's own body says who ruled it and under what grant | **No subsection says either.** A1 (`:67-80`) and A2 (`:81-95`) contain no mention of the delegation, the lead or the maintainer at all. A3 (`:96-108`) mentions the lead **once** — *"the lead was reading carefully each time"* — which is RFC-843's evidence about reading behaviour, not a statement of who ruled A3 or under what authority. The delegation is at **§1.1 of the same file**, which the split leaves behind |

**And on that premise RL-995 §3 item 2 obliges** *"`owner:` is not `decision-maker` for these
...
decision-maker; RL-995 forbids him for these three. This derivation does not resolve that —
...
`docs/plans/INDEX.md#2026-08-30-nt-0012-0013-0014-adoptionmd,81,96`. **No subsection states who ruled it or under what
...
RFC-843's evidence. The delegation lives at **§1.1 of the same file**, under a different
...
**Named here, not decided — it is a consequence of RL-995's split, and it goes back with
this.**
...
passes check 35 — the hole RL-979 named, confirmed here independently.
...
| **B — one role after a rule the cell implies but never states** | `RFC` (*"mints and **owns**"* — the verb disambiguates), `CR` (*"auditor (`work`, `phase`); lead (`review`)"* — `kind:` disambiguates), `RL` (*"decision-maker; the maintainer **may**"* — primary vs conditional), `PL map/leaf` and `RS audit` and Reference—`process/`/charters (the semicolon introduces a duty or a route, not a second owner) | **Yes, but the reader must supply the rule.** RL-979 had to infer *"owns wins"* for `RFC`; §1.6 never states it |
...
- **Delegation transfers drafting only.** `owner:` stays `decision-maker` for A1–A3. The field
  keeps meaning *who may amend*, consistently with §1.6 and with RL-979's own logic applied
...
deliberately narrow: it covers *"the landing of RFC-842, RFC-843 and RFC-895, and nothing
...
derivation does not overturn it.** RL-995 §3 item 2 obliges an owner that is *not*
`decision-maker`, on the premise §1.1 measures as false. So the decision-maker has a question
this derivation can frame but not answer: **does RL-995 item 2 survive its premise
failing?**
...
- **If it does not survive** — the reasoning being that `owner:` means *who may amend* and the
  ...
  A1–A3 take `decision-maker` and RL-995 item 2 is amended.

**PR #603's `_ruling_file_owner` implements RL-995 correctly and is not wrong.** An earlier
...
follows RL-995, and it is §1.6 and RL-995 that disagree.**

**Gap 1 — the disambiguating rule in bucket B is never stated.** RL-979 supplied it correctly
...
- **It does not challenge RL-979's outcome.** §1.1 records that one supporting sentence is
...
- **It does not decide the severance remedy** of §1.2, which is a consequence of RL-995's
