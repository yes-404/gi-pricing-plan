...
**Spec.** `docs/specs/02-modelling.md` FR-MODEL-24 (line 162), amended by Task 1 of this plan (dated 2026-08-21). The resolved design, which the amendment records:
...
- `GlmFitResult.offset_model_ref: ModelRef | None = None` — the resolved pinned ref, "what was actually constructed is recorded on the fit result" (FR-MODEL-71's rule, applied to GLM).
- Named refusals: `GbmSpec` with `kind="model"`; a ref naming a non-fitted, non-GLM, or link-mismatched model (`MODEL_OFFSET_REF_INVALID`); the peril-reconciliation scoring path fails named (`MODEL_OFFSET_MISSING`) until W5 wires the resolver there.
...
- `spec_hash` covers the ref automatically (the payload is the whole canonicalised spec dump); `SPEC_HASH_VERSION` moves 7 → 8 in the same commit (FR-MODEL-86).
...
- **Scope honesty — what this slice does NOT do** (each with an owner): GBM/EBM referenced models and GbmSpec offsets (refused by name; OQ-MODEL-22 records the widening options); the peril-reconciliation scoring path (declared-and-refused, owner W5); EBM as a model type (FR-MODEL-37, separate slice); FR-MODEL-23's fit-error surfacing (unbuilt, owner W5); the frontend model-spec builder (W6b). `wf-02…05` owe FR-OVR-17(ii) regardless.
...
- **Every new test** carries `@pytest.mark.req("FR-MODEL-24")`; **a negative test precedes every positive one** for each invariant.
- **Spec-change discipline (§0):** any change to what a model *is* bumps `SPEC_HASH_VERSION` in the same commit (7 → 8, FR-MODEL-86); any new error code is registered in `backend/src/app/errors.py` **and** the §5.1 backtick catalogue (`02-modelling.md:1619-1639`) **in the same commit as its first raise** — the catalogue entry lands with the implementation, not the amendment (the Tweedie slice's correction). New codes also get a dated blockquote note (`MODEL_APPROXIMATION_INVALID` at :1649-1660 is the format).
- **Contracts:** run `scripts/generate-contracts.py` after every model-schema change and commit the result (FR-PLAT-48); `--check` in the gate. The hand-authored `docs/contracts/schemas/model-spec.schema.json` is synced by hand in the same commit (the script never compares the two — verified 2026-08-21). Read `.claude/skills/contract-schema` before the sync step.
...
- **Conventional Commits** (`feat(model-schema):`, `feat(pricing-core):`, `feat(backend):`, `chore(contracts):`, `docs(spec):`, `docs(roadmap):`), each tagged with FR-MODEL-24, with the `Co-Authored-By: Claude <noreply@anthropic.com>` trailer on every commit.
...
## Task 1 — Spec amendment: FR-MODEL-24 defined, §4.4 block, OQ-MODEL-22

**Files:**
- Modify: `docs/specs/02-modelling.md` (FR-MODEL-24 row at :162; §4.4 offset example at :498)
- Modify: `docs/open-questions.md` (new OQ-MODEL-22)

**Interfaces:**
- Consumes: the FR-MODEL-87 staged-contract rule (:325), the §4.4 amendment-block style of `GlmCvSpec`/`TweediePowerSpec` (02-modelling.md:559-594).
- Produces: the amended FR-MODEL-24 text every later task argues from; OQ-MODEL-22.
...
- [ ] **Step 2: Amend the FR-MODEL-24 row (02-modelling.md:162)**

Replace the row's single sentence with the definition plus a dated amendment (append inside the same table cell, matching the FR-MODEL-20/22 amendment style):
...
- [ ] **Step 4: Record the widening question as OQ-MODEL-22 in docs/open-questions.md**

Follow the file's OQ format; options and recommendation:

```
### OQ-MODEL-22 — which offsets-from-model come after the GLM-to-GLM slice? (open)

FR-MODEL-24's 2026-08-21 amendment builds offset-from-another-model for GLM specs
...
(d) only if residual modelling stays GLM-shaped in practice. Gate: W5, no date.
```
...
git commit -m "docs(spec): FR-MODEL-24 defined — offset from another model, GLM-to-GLM v1 (OQ-MODEL-22)"
```
...
git commit -m "feat(model-schema): FR-MODEL-24 — offset_model_ref replaces the dead model_ref scaffold (SPEC_HASH_VERSION 8)"
```
...
git commit -m "feat(pricing-core): FR-MODEL-24 — fit_glm honours a supplied model offset, named refusal otherwise"
```
...
(Adjust `BacktestSummary`'s attribute name to whatever the existing `test_backtests.py` asserts — grep `summary\.` there; the `ref` strings follow FR-MODEL-57's invariant that the backtest target is not the fitted-on version.)
...
git commit -m "feat(pricing-core): FR-MODEL-24 — prediction, diagnostics and backtest take the resolved model offset"
```
...
And append a dated addendum to the existing `MODEL_OFFSET_MISSING` note (grep `` `MODEL_OFFSET_MISSING` `` in `02-modelling.md` to find it): "…FR-MODEL-24 also raises it when a `kind="model"` spec reaches fit or scoring without the resolved offset array."
...
git commit -m "feat(backend): FR-MODEL-24 — the fit job resolves offset_model_ref and fits on the referenced linear predictor"
```
...
git commit -m "feat(backend): FR-MODEL-24 — prediction resolves the offset model per request"
```
...
- [ ] **Step 1: Write the failing test** in `test_backtests.py`, mirroring its existing backtest-job test: fit the Task 5 pair, run the backtest job for the residual model against v2's sibling version (or v2 itself, following FR-MODEL-57's target rule as the file's existing tests do), assert `JobStatus.SUCCEEDED` and a sensible summary (`rows_scored == frame height`).
...
git commit -m "feat(backend): FR-MODEL-24 — backtests honour the resolved model offset"
```
...
- Produces: validation reports the ref's problems before a Job is queued (wf-01 D2's rule applied to offsets-from-model).
...
git commit -m "feat(backend): FR-MODEL-24 — spec validation resolves offset_model_ref before a job is queued"
```
...
**Files:**
- Modify: `docs/specs/02-modelling.md` (FR-MODEL-87's list at :325 gains the ninth live entry; §4.4's Task 1 block gains the "live" wording)
- Modify: `docs/roadmap.md` (the "W5 — outstanding work" table row 4 at :2569 struck DELIVERED; the count line :2560-2562 corrected; a new `#### W5 slice —` record after :2598's)
- No code changes.

- [ ] **Step 1: FR-MODEL-87's ninth live entry** — append to the list at :325, matching the eighth entry's wording:
...
git commit -m "docs(roadmap): FR-MODEL-24 — the offset-from-another-model slice record, live on the staged contract"
...
4. The plan's literal backtest ref block passed `dataset_version_ref="dataset_version:book@2"` **and** `fitted_on_ref="dataset_version:book@2"` — identical, which `BacktestSummary`'s own validator refuses ("fitted on", FR-MODEL-57). The plan's parenthetical says the refs "follow FR-MODEL-57's invariant that the backtest target is not the fitted-on version", so the target is `dataset_version:book@2` and the fitted-on is `dataset_version:book@1` — the parenthetical's rule, not the sketch's literal strings.
...
**Commits:** the Task 4 commit (`feat(pricing-core): FR-MODEL-24 — prediction, diagnostics and backtest take the resolved model offset`).
...
**Commits:** the Task 5 commit (`feat(backend): FR-MODEL-24 — the fit job resolves offset_model_ref and fits on the referenced linear predictor`).
