...
before applying** — several agents write this tree, and the FR-MODEL-6 acceptance hunk
...
| 3 | 1444-1450 | six requirements "Not started" | **All stale — and the audit under-counted.** They occupy **four** rows, not six. **FR-MODEL-64 reads "Partial.", not "Not started".** And **FR-MODEL-52 at :1444 is a seventh stale row the audit missed** — delivered 2026-08-17, six markers |
| 4 | 1240 | FR-GOV-10 "not started" | **Delivered in two of three clauses.** Evidence Bundle + change summary are built and fail closed. **The third clause — "a completed checklist" — has no implementation**: `grep -rn "checklist" backend/src` is empty. Do not write a flat "delivered". Cite the test *name*, not its line: it is `:246` at HEAD and `:372` in the tree |
| 5 | 65 | AST parser "Phase 1, W5" | **Stale, and the contradiction is a three-way split.** The *parser* was built in **W4** for `01` FR-DATA-10; `02` §4.6's *grammar* is **Phase 2, W30** by OQ-MODEL-1 (2026-08-15). Line 65 hands the same spec section to a different phase than W30's own row at :2981 |
...
| 9 | 2704-2718 | custom-metrics "Not delivered: `custom_metric` evidence floor, owner W5" | **Delivered today**, in the order the entry itself specified. Strike the verdict clauses; keep the diagnosis |
