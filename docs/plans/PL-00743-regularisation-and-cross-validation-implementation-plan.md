...
family: plan
kind: leaf
title: Regularisation and Cross-Validation Implementation Plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-08-21-regularisation-and-cv.md
---
...
**Goal:** Give `GlmSpec` a documented elastic-net penalty path (FR-112) and a
cross-validated alternative to fixed-alpha fitting (FR-182) — declared fold
...
**Spec:** `docs/specs/02-modelling.md` — FR-112 and FR-182 (§3, the GLM
...
- **FR-112**: "A GLM fit MAY apply elastic-net regularisation via `alpha` (overall
...
- **FR-182**: "Where regularisation strength is chosen by cross-validation
  (`select_by: cv`), the fold construction MUST be declared (`random`, `temporal` or
  `grouped_by_key`, `01` FR-73's three methods) with a persisted seed, and the
...
`CLAUDE.md` §0):** neither FR-182 nor FR-73 defines what a `temporal` *K*-fold
split means — FR-73 only defines a two-part cutoff split (`train`/`test`). This plan
...
existing `uncertainty_basis` property (FR-197) reads `self.alpha` to decide whether a
...
report a CV-selected, near-certainly-penalised fit as unpenalised. FR-197 predates
...
into FR-197 itself. Task 10 carries this into the spec as the amendment.
...
- `model-schema` is the single source of truth for shared data shapes; never define a
  shared shape anywhere else (`CLAUDE.md` §2, ADR-704).
...
- Model and rating definitions are declarative JSON artifacts, never pickled objects
  ...
  fitted estimator.
- Diagnostics are computed once at fit time and read thereafter (FR-170); nothing
...
- `docs/contracts/` is generated from `model-schema` and committed; CI fails on drift
  (FR-451) — `uv run python scripts/generate-contracts.py --check` must pass after
  Task 4.
...
git commit -m "feat(pricing-core): assign_folds — K-fold generalisation of the split methods (FR-182)"
```
...
```
    `cross_validation` is still declared and always `None`: FR-182 computes it **at fit
...
git commit -m "feat(model-schema): CrossValidationDiagnostics — the CV path and fold dispersion shape (FR-112, FR-182)"
```
...
git commit -m "feat(model-schema): GlmCvSpec and GlmSpec.select_by/cv — the documented penalty path (FR-112, FR-182)"
```
...
git commit -m "feat(pricing-core): _fit_cv_path — the elastic-net penalty path reaching glum (FR-112, FR-182)"
```
...
git commit -m "feat(backend): thread GlmFit.cv into the persisted Diagnostics artifact (FR-112, FR-182)"
```
...
- Produces: `SPEC_HASH_VERSION == 6`; every digest computed from this commit onward is
  `v6:sha256:...`; every prior `v5:...` digest is stale (FR-206).
...
def test_the_algorithm_version_moved_with_the_new_field() -> None:
    ...
    digest stops matching its own spec and FR-204's dedup ends with no error to see.
    """
    assert SPEC_HASH_VERSION == 6, (
        "GlmSpec.select_by/cv joined the payload (FR-112, FR-182); the tag "
        "moves with it"
    )
...
git commit -m "feat(backend): bump SPEC_HASH_VERSION to 6 for GlmSpec.select_by/cv (FR-206)"
```
...
git commit -m "test(backend): a CV-selected model fits through the real Job and records fold dispersion (FR-112, FR-182)"
```
...
must be against FR-112/182's actual shape, not only "the file changed".
...
by workstream WK-664 per `02-modelling.md` §5.3's Contents column) — do **not** run
...
Immediately after FR-182's requirement text (§3), add:
...
- [ ] **Step 3: Add the dated amendment note resolving the FR-197 interaction**

Find FR-197's requirement text (`uncertainty_basis`). Immediately after it, add:
...
already documents `01` FR-73's vocabulary for the dataset-split feature; cross-
...
```markdown
| `GLM_CV_FOLD_EMPTY` | 409 | A fold has no held-out (or no training) rows at some alpha on the CV path. Added 2026-08-21 (WK-661, the regularisation-and-CV slice). |
```
...
git commit -m "docs(spec): amend 02-modelling for the regularisation-and-CV slice (FR-112, FR-182)"
```
...
- [ ] **Step 1: Mark the WK-661 slice delivered**

Find the `#### WK-661 — outstanding work, derived 2026-08-19` section's entry for
...
**DELIVERED 2026-08-21.** `GlmSpec.select_by`/`GlmSpec.cv` (FR-112), `GlmCvSpec`'s
three fold-construction methods via `pricing_core.data.splits.assign_folds` (FR-182),
...
view's CV screen remains WK-664's). Two spec interactions found and resolved by dated
amendment in `02-modelling.md`: K-fold `temporal` semantics (undefined by FR-73/
FR-182; resolved as contiguous time-ordered blocks) and FR-197's
...
git commit -m "docs(roadmap): WK-661's regularisation-and-CV slice, delivered"
```
...
**1. Spec coverage.** FR-112 (elastic-net `alpha`/`l1_ratio`, scanned path,
full-path persistence): Tasks 2, 3, 5, 9 — `CvPathPoint`, `GlmCvSpec.alphas`,
`_fit_cv_path`'s path construction, the regenerated contract. FR-182 (`select_by:
