...
FR-DATA-54's "no parameter metadata exists from which a threshold allowlist could be derived"
...
**Spec:** [`../specs/01-data-management.md`](../specs/01-data-management.md) — FR-DATA-53 and
FR-DATA-54, and §4.4's catalogue.
...
- **Money is integer pence/cents** (FR-OVR-7). `VR-ACT-12`'s two bounds are minor-unit money
...
1. **The slice map's cell over-scopes Leg B.**
   [`2026-08-24-w6b-slice-map-revised.md`](2026-08-24-w6b-slice-map-revised.md) §3 describes
...
2. **FR-DATA-53's and FR-DATA-54's tails name owner `W6b-13`**, which the revised map re-cut
...
| `docs/specs/01-data-management.md` | Modify FR-DATA-53 (`:166`), FR-DATA-54 (`:118`) | Retire the two "declared and unfixed/unbuilt" tails |

---

### Task 1: `catalogue_id` reaches the create path

FR-DATA-53's tail records the defect: *"the create handler drops `catalogue_id`, so that
...
- Modify: `docs/specs/01-data-management.md:166` (FR-DATA-53's tail)
...
```python
    #: The `01` §4.4 entry this rule derives from (FR-DATA-53). A workspace authoring its own
...
- [ ] **Step 9: Amend FR-DATA-53's tail**
...
it that "thresholds are deliberately absent". FR-DATA-54's amendment reverses exactly that
...
```
    Thresholds live here (FR-DATA-54). `01` §4.4's 2026-08-23 correction struck the claim that
...
- **`VR-ACT-12` is the one row that is not a byte-for-byte copy**, and the exception is
  ...
  `1e12`, but both are **minor-unit money**, and FR-OVR-7 makes money integer — `audit-docs.py`
...
`"params": {}` at `:140` for all thirty-eight. This is the line FR-DATA-54's tail is about, and
the only line that makes the defaults reach a caller.

**Files:**
- Modify: `backend/src/app/platform/validation_rules.py:140`
- Modify: `docs/specs/01-data-management.md:118` (FR-DATA-54's tail)
- Test: `backend/tests/test_api_validation_rules.py`
...
- [ ] **Step 5: Amend FR-DATA-54's tail**
...
**Spec coverage.** FR-DATA-53's tail → Task 1. FR-DATA-54's tail → Tasks 2, 3, 4. §4.4's
...
3. Nothing validates a printed spec example against the type it claims to be an example of
   (raised by the W6b executor, 2026-08-24) — its own unit, not folded in here.
4. **`validate.py:1071-1072` writes two minor-unit money fallbacks as floats** (`0.0`, `1e12`).
   FR-OVR-7 makes money integer, and `audit-docs.py` enforces that on documents but reaches no
