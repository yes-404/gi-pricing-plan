...
| Commit | Scope |
|---|---|
| `c59a283` | `fix(model): FR-MODEL-103/106/107 — resolve template defaults, bind stopping by name` (C1 + C2) |
...
- All marked `@pytest.mark.req("FR-MODEL-103")`.
...
- `test_a_second_declared_builtin_does_not_also_drive_early_stopping` — FR-MODEL-106, both
...
- `test_a_declared_custom_metric_does_not_capture_stopping_from_a_named_builtin` —
  FR-MODEL-107, both backends. A `quantile` metric at `alpha=0.9` declaring
...
- `test_lightgbm_drops_a_builtin_eval_metric_rather_than_stop_on_it` — FR-MODEL-107, the
  pinned limitation.
...
`docs/specs/02-modelling.md` §3 — dated amendments appended to FR-MODEL-106's and
FR-MODEL-107's requirement rows, recording the binding rule, the two backends' opposite
...
`docs/roadmap.md`, `#### W5 — outstanding work, derived 2026-08-19`. Not deleted; superseded
...
six (FR-MODEL-103…108, all evidenced); the unevidenced 19 and their verdicts are unchanged.
...
deferral's own reasoning turned out to be the argument for building it inside W5 —
FR-MODEL-107 made a Custom Metric the only way to early-stop under a callable objective.

The evidence-floor gap was added to the slice record's **Not delivered** list with **W5** as
...
- `06` §3.3's `custom_metric` evidence row and the matching `EVIDENCE_FLOOR` entry — owner
  W5, recorded in the slice record. Deliberately out of this fix wave.
