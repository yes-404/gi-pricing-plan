...
family: plan
kind: leaf
title: W37-6 — The migration run: leaf plan
...
corrected_by: []
relates: []                     # ids only
was: docs/plans/2026-09-02-w37-6-migration-run-leaf-plan.md
---
...
**Goal:** Run RFC-937's migration once, as one supervised pull request at a gap — renumber
...
[`PL-00939-wk-697-one-id-per-governed-thing-map-plan.md`](PL-00939-wk-697-one-id-per-governed-thing-map-plan.md)'s
...
([`../research/RS-01002-rfc-937-verification-and-impact-sweep-audit-record.md`](../research/RS-01002-rfc-937-verification-and-impact-sweep-audit-record.md),
pinned at `89dd2b1`), not from the map plan's evidence table — the map plan says so in its own
W37-6 section, and the sweep corrected the two largest rows of RFC-937 §5.6 by more than a
...
W37-5), **hand edits the script cannot know** (RFC-937 §5's **H** rows scoped to stage S2), and
**instruments** (RL-987's derived set — the files that tell an author how to produce a
...
**Spec:** [`../rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md`](../rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md) — §4
...
4. **(d) Legacy-form sweep.** The sweep runs from the **shipped constant** in
   `scripts/audit-docs.py` (RL-988: one constant, pattern and exclusion list defined once,
   ...
   violation.* RL-988 acceptance item 2 fixes the control's ten members. Every exclusion
...
6. **(f) No product identifier moved.** `git grep -c 'VR-DST-1'`, summed over all files, equals
   ...
   historical figure RFC-937 §7 (f) names, and pinning it to that figure guarantees a false red.
7. **(g) The script touched only headers and reference tokens.** The `migrate` run's own diff
   on a clean tree, filtered through the six-class closed enumeration RL-989 fixes,
...
10. **The widened scope is the stamped set, exactly.** The post-flip value of
    `_ID_SCOPE_ROOTS` selects the same file set RFC-937 §4 step 5 stamps, and
...
11. **Every derived instrument is load-bearing — tested on its H content, not on its file.**
    RL-987 acceptance item 2 says reverting a member to its merge-base content must produce
...
12. **Requirement-facing, and it is the one that matters.** W37-7's own leaf plan and its
    ...
    *Violation: any of checks 30, 31, 33 or 36 firing on either of those two files.* If one
    ...
    quiet edit (RL-987 acceptance item 1).
13. **Vendored exemption reaches only the blanket passes.** After the run, a file beneath a
    ...
    a rewritten citation; either of the second two is unchanged* (RL-990 acceptance items 2
...
- [ ] **DP-1, DP-2 and DP-3 each carry a resolver id.** Satisfied at filing:
      [`../rulings/RL-00990-rfc-937-1-5-s-vendored-parenthesis-is-a-gloss-not-a-detector-the-set-is-declared-and-reconciled-and-the-exemption-reaches-only-the-blanket-passes.md`](../rulings/RL-00990-rfc-937-1-5-s-vendored-parenthesis-is-a-gloss-not-a-detector-the-set-is-declared-and-reconciled-and-the-exemption-reaches-only-the-blanket-passes.md)
      — RL-987 (DP-1), RL-988 (DP-2), RL-989 (DP-3). RL-990 in the same record fixes
      RFC-937 §1.5's vendored criterion, which this slice applies. Rulings 70, 71 and 72
      ([`../rulings/RL-00983-the-map-plan-roll-up-runs-through-the-slices-and-has-no-catch-all.md`](../rulings/RL-00983-the-map-plan-roll-up-runs-through-the-slices-and-has-no-catch-all.md))
...
- [ ] **The maintainer has given a dated go-ahead for this specific run, with the enlargement
      disclosed.** RL-987 §3: the go-ahead *"covers the enlarged commit only if the
...
| 3.5 | The fixture corpus layout under `tests/fixtures/docs-migration/` | That the mutation fixtures RL-989 acceptance items 1 and 2 require exist there **before** the real run |
...
to two paths" to "scoped to everything", and — because of RL-987 — also rewrites the
...
|---|---|---|
| **rewrites a citation token in** | **930** | Every file matching RFC-937 §7 (d)'s own pattern; 20 507 matching lines. By tree: `docs/` 297 · `backend/` 217 · `frontend/` 144 · `packages/` 135 · `.claude/` 55 · `tests/` 45 · `scripts/` 19 · `examples/` 6 · `.github/` 3 · `deploy/` 2 · 7 root files |
...
the whole corpus, and **13 instruments** are rewritten under RL-987 (§6.2).
...
| `04ec6bf` — RL-987's evidence tree | **1360** | **881** |
...
- **25 under `docs/`** — 13 `_templates/`, 7 `audit/` (finding records and the auditor's sweep),
  ...
  self-referential part: RFC-937's own slices produce documents RFC-937 must migrate.
- **65 under `tests/`** — almost all the `tests/fixtures/docs-ids/` corpus W37-4 built.
- **3 under `scripts/`** — `doc-id.py`, `doc-index.py`, `_docid.py`.

**The dominant term is fixtures, not governed documents, and that cuts the other way.** RL-988
...
RL-988 acceptance item 1 requires **every entry to be load-bearing** — removing it must make
...
### 4.3 The enlargement RL-987 requires, stated plainly

The map plan's W37-6 said this commit carries the migration's output plus a list of scripts,
tests, CI and process edits. **RL-987 adds the creating instruments**, because a document
...
RL-987 makes the set **a criterion, not a list**: *"every instrument whose output is checked
...
excludes one of the two candidates RL-987 itself named** — both with reasons, as RL-987
requires.
...
- **The commit is squash-merged and the branch auto-deletes.** RL-989 chose option (a) — (g)
...
- **No body line of a frozen file changes.** Splits preserve every line; stamps add lines;
  rewrites change reference tokens only. Acceptance item (g) is the check, and RL-989 makes
  it a closed enumeration with **no pass-through** for a hunk the filter cannot classify.
- **Nothing in RFC-937 §1 is edited.** Rulings 69, 70, 71 and 72 each say so explicitly; §1
...
RFC-937 §7's items (i), (j) and (k) belong to later slices — (i) to the Work's closure record,
...
| Files matching RFC-937 §7 (d)'s legacy-form pattern | 876 | **930** (20 507 lines) |
...
**43 files** at `39ee30c`. RFC-937 §5.2's rows account for **41** of them. The two with no
...
| `register.md`, `register.md`, `phases/1b/README.md` | 3 | one `docs/findings/register.md`; the phase register merges in with `phase: P1b`; the phase README becomes a `CR- kind: phase` |
...
| **`FD-00894-rfc-840-841-adoption-pilot.md`** | 1 | **No §5.2 row.** §5.2's row names `work/*/README.md` only, and this is not a README |
...
| Row | Map plan / RFC-937 §5.6 | Sweep at `89dd2b1`, re-confirmed at `39ee30c` | Effect |
|---|---|---|---|
...
RFC-937 §5.5's *"fourteen scripts"* (no reading — 17 raw, 19 named, 16 citation-bearing —
...
RFC-937 §7 (f) reads *"`git grep -c 'VR-DST-1'` is unchanged from `8f5d57d`"*. The map plan's
...
|---|---|---|
| `8f5d57d` — RFC-937 §7 (f)'s baseline | 104 | 25 |
| `89dd2b1` — map plan item 6's baseline | 107 | 26 |
| `39ee30c` — `origin/main` at filing | 109 | 28 |

The drift is ordinary prose: new plans, `document-ids.md` and RFC-937 itself all mention
...
`\b(FR|NFR|OQ|DEP|VR)-[A-Z]+-[0-9]+\b`; `SWEEP` is RFC-937 §7 (d)'s own alternation.
...
## 6. RL-987's derivation — the instrument set

RL-987 rules DP-1 as option (a) and makes the set a **criterion**:
...
|---|---|---|
| **30** Header present and parseable; no unknown field; required fields per family read from `docs/_templates/` | The YAML front-matter block of every produced document | **Yes** — every instrument that mints a document of a family. The field policy itself is read from `docs/_templates/` (RL-981), already migrated in W37-1, so the templates are not an enlargement |
...
| **32** Every `<PREFIX>-<n>` in prose resolves in `docs/INDEX.md`; prefix matches the number's family; no padded id outside a link target | Citations written into a produced document | **No new member.** The citation convention is RFC-937 §1.7, carried by `docs/process/document-ids.md`, already migrated. A creating skill holding a stale citation is caught by §4 step 6's mechanical rewrite, which exempts nothing |
...
| **38** Loop signal | **Nothing.** `check_loop_signal` *"never fails the gate, only notes"*; its body appends a note and returns | **No.** An instrument whose output is touched only by check 38 cannot cause a failure, and admitting one would fail RL-987's own acceptance item 2 |
...
| 4 | `.claude/skills/phase-review/` | `CR- kind: review` | 30, 31, 33, 37 | ``lands in `docs/closures/INDEX.md#plan-reviewsmd` as a dated `### Plan review N` section`` (line 22) — a path into the dissolving tree **and** a heading grammar |
| 5 | `.claude/skills/adr-write/` | `ADR-` | 30, 31, **34**, 37 | ``File as `docs/adrs/NNNN-kebab-title.md` `` (line 16) — the retired directory and padding; and the addendum-versus-edit rule, which is check 34's subject exactly |
...
| 8 | `.claude/roles/planner.md` | `PL-`; **`CR- kind: review`** | 30, 31, 37 | ``filed to `docs/closures/INDEX.md#plan-reviewsmd` as a dated `### Plan review N` section`` (line 22) — the charter files the §14 review itself, at a path this commit deletes, in a heading form no template declares |
| 9 | `.claude/roles/auditor.md` | `CR- kind: work`/`phase`; `FD-`; `RS- kind: audit` | 30, 31 | Four filing paths into the dissolving tree in one bullet (lines 16-25): ``Closure records at `docs/audit/work/<id>/README.md` ``, ``register deferral rows … at `docs/findings/register.md` ``, ``checked against `docs/audit/checklists/…` ``, ``Evidence essays live at `docs/audit/findings/<F-id>.md` `` |
...
document, which RFC-937 §1.6 states outright (*"the generated ownership matrix shows two
...
under the same criterion rather than filing a document and correcting it afterwards — RL-987
...
Derived from the auditor's sweep (§5) and RFC-937 §5's **H** rows scoped to stage S2 — not from
...
- RFC-937 §4 step 5 stamps `docs/**`, `.claude/roles/**`, **`.claude/skills/*/SKILL.md`** and
...
- Adding `.claude/skills` as a **directory** root therefore makes check 30 demand a header on
  237 files step 5 does not stamp and RL-990 §3 explicitly exempts from the blanket pass. The
...
- [ ] Make the scope **print its own coverage** — files selected, per root. A check that has
      never seen a file cannot be enforcing anything (RL-981 acceptance item 3, which caught
...
firing on a file RFC-937 §4 step 5 does not stamp.*
...
- [ ] Declare the skill and agent keys in `docs/_templates/REFERENCE.md`'s front matter, marked
      conditional in the template's own comment. RL-981 makes the template the licensing
      instrument — *"the permitted set for a family is the set of keys in that family's template
      front matter"* — so this is the sanctioned route and not a widening of RFC-937 §1.5's
      closed set. **Nothing in §1 is edited.**
- [ ] Prove it the way RL-981 acceptance item 2 specifies: add a key to the template and a
...
**Files:** `scripts/audit-docs.py`, `tests/test_audit_docs_*.py`. Per RFC-937 §5.5's
...
- [ ] `check_notes_tombstone` is already renamed to `check_redirects` (slot 30 to 36, merged in
      ...
      by RFC-937 §5.5's own resolution is where that check's protective job ends.
...
- [ ] **Check 35's second clause is inert corpus-wide.** `grep -rn "Permitted owners" docs/
      ...
      green is exactly the boundary metric RFC-789 warns about.
...
- [ ] **Correct `docs/_templates/REFERENCE.md`'s vendored-detection comment, which contradicts
      RL-990 in the words RL-990 rejected.** The template's closing comment block states:
      *"`doc-id.py`'s detection rule (`grep`-able: any directory holding a `LICENSE` that is not
      the repository's own) is what decides this, **not a hand-kept list**."* RL-990 rejected
      ...
      The template landed in W37-1, before RL-990; RL-990 §3 obliges W37-2, W37-4 and
...
- [ ] Run `migrate`; commit **the script's output alone** as the branch's first commit. This is
      not a required commit boundary — RL-989 settles what (g) *means*, not how many commits
...
- §4 step 2 says *"Multi-ruling files → one per `## Ruling N`"*. There are **72** ruling headings
  ...
  `../rulings/RL-00949-rfc-897-slice-2-s-census-csv-and-fr-72-the-test-is-overbroad-the.md` (Rulings 60, 61 and 59). A splitter matching
...
- **Two of the 29 are not named `*ruling*`** —
  `../rulings/INDEX.md#2026-08-29-w11-algorithm-pin-maturitymd` and `../rulings/RL-00861-wk-671-s-and-it-is-forced-rather-than-chosen.md`. A
...
- **The `closure-records.md` half is not caught by anything.** Three of that file's 21 `###`
  records are titled *"WK-661 — … (in progress, not closed)"* and one is *"Phase 1a — exit demo
...
- [ ] `docs/findings/register.md` — header prose rewritten so the **row's** field set is declared
      in one place. **RL-981 obliges this on W37-6**, because a register row is not a
      header-bearing record and §1.5's template mechanism does not reach it; **RL-982 obliges
...
RL-990 fixes the mechanism: `_VENDORED_SKILLS` is a **declared constant** of 28 skill
...
- [ ] Dissolve `docs/audit/`'s **43** files into `findings/`, `closures/`, `research/` and
      ...
      §5.4's bespoke-audit rule) and `FD-00894-rfc-840-841-adoption-pilot.md`.
...
### 7.12 Task 12 — The derived instruments (RL-987)
...
- [ ] 1 `writing-plans` · [ ] 2 `subagent-driven-development` (+ `scripts/task-brief`) ·
      ...
      [ ] 7 `library-spike`.
- [ ] 8 `.claude/roles/planner.md` — replace the `docs/closures/INDEX.md#plan-reviewsmd` filing path and the
...
- [ ] 13 `.claude/skills/README.md` — the *"creates"* column, plus RL-990's class-level
...
- [ ] Record the merge-base SHA **and** the exact command that computed (g), so the result is
      re-derivable by checking that SHA out and re-running `migrate` (RL-989's obligation).
...
record or handover file."* The register carries this as [`F68`](../findings/FD-01006-audit-docs-py-check-28-has-no-plan-kind-test-for-a-ruling-record.md), with
the disposition **carry forward with a trigger — RFC-937's S2 migration PR**, which is this
slice.
...
1. **Acceptance (f)'s baseline is not stable, and the two documents that name one name
   different ones.** RFC-937 §7 (f) pins to `8f5d57d` (104); the map plan's acceptance item 6
...
5. **Check 39's PR-title clause is noted, not checked — and that decides `git-hygiene`.** Ruling
   ...
   grammar"*. The merged `check_index_stable` explicitly does not check it. Under RL-987's own
   criterion and its own acceptance item 2, `git-hygiene` is **not** a member. §6.3.
6. **RL-987's floor of seven is missing three families, all carried by charters.** No skill
   ...
   carries four filing paths into the dissolving `docs/audit/` tree in a single bullet. RFC-937
...
7. **RFC-937 §4 step 5 and §5.4 disagree about `.claude/skills/README.md`.** Step 5's stamp
...
10. **Two files under `docs/audit/` have no destination row in RFC-937 §5.2** — the auditor's own
    sweep record (this plan's evidence base) and
    `FD-00894-rfc-840-841-adoption-pilot.md`. §5.2's rows cover 41 of the directory's 43
    files. §7.11.
11. **The two filed size figures are both correct and both stale, and the corpus grows while the
    ...
    *"rewrites citations in 1355 tracked files"*; RL-987's evidence tree states 1360 at
    ...
    pin; and RFC-937 §10's own figure for that population was 767 over a narrower pattern. §4.2
    ...
    documents** — which cuts against the intuitive reading, because RL-988 Part 2 excludes
    exactly that class from the sweep.
12. **RL-987's acceptance item 2 cannot discriminate a member from a non-member.** It says
...
13. **Check 30's unknown-field rule reds 53 files the moment they are stamped.** Every one of the
    ...
    proof. **Neither RFC-937 §5.4's *"every `SKILL.md` (46) — header stamped"* row nor the
    template anticipates this.**
14. **`docs/_templates/REFERENCE.md` contradicts RL-990, in the words RL-990 rejected.** It
    states the vendored set is decided by *"any directory holding a `LICENSE` that is not the
    repository's own … **not a hand-kept list**"*. RL-990 rejected keying on `LICENSE`
    presence (it under-exempts 240 tracked files) and ruled for a hand-seeded declared constant
    reconciled against the ruff exclude list. The template landed in W37-1, before RL-990,
    and RL-990 §3's obligations name W37-2, W37-4 and W37-6 but not the template — which is
...
- **Every id cited individually, never as a range.** Rulings 66, 67, 68, 69, 70, 71 and 72 are
  each named where they bind; RFC-937's acceptance items are cited as (a) to (h) individually. No
  bare numeric range appears — a range silently drops an append-only id landed inside it.
- **Scope derived from the specification first, then evidenced.** The task list walks RFC-937
...
- **Every acceptance item is a violation that must become detectable.** Fourteen items, each with
  its failing case. Items 11 and 12 are RL-987's own pair, kept in its terms, because a
...
out** — RFC-937 §4: *"one scripted PR, once."*
...
[`../rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md`](../rfcs/RFC-00937-one-id-per-governed-thing-one-sequence-integer-identity-a-self-describing-layout-and-roles-per-family.md), the note wins and
