...
realm whose demo user resolves into the seeded freMTPL2 workspace — so that `FR-PLAT-55`'s
...
**Spec:** [`../specs/07-platform.md`](../specs/07-platform.md) — `FR-PLAT-58` is the whole of
this slice; `FR-PLAT-1`, `FR-PLAT-2`, `FR-PLAT-4`, `FR-PLAT-55`, `FR-PLAT-59` and `NFR-PLAT-4`
bound it.
...
Copied verbatim from `FR-PLAT-58` (`docs/specs/07-platform.md:81`), which is this slice's
entire requirement:
...
- **`deploy/docker-compose.yml` carries its own rule** (lines 3-5): *"Services arrive as the
  ...
  `FR-PLAT-58` quotes that rule as the reason for the profile. The profile is what makes a
...
- **The browser flow.** `FR-PLAT-55` is `W6b-10`. This slice makes it possible; it does not
...
- **The reference production deployment.** `FR-PLAT-59` (`07-platform.md:145`) decides
  `OQ-PLAT-5`'s production half: no identity provider ships in the production stack, and
  `deploy/` carries a **reference** Keycloak deployment the deployer operates, patches and is
  accountable for — **owned by W14**, *"with the rest of `deploy/` beyond compose."* See
  Finding 7 for the directory-name consequence.
- **Group-to-role mapping.** `FR-GOV-41` (`06-governance.md:86`) decided it and marked it
...
names a workspace that exists (FR-PLAT-62's foreign key)"* — so the very first membership row
...
decision — *"There is deliberately **no API to create these in W2**. Self-service membership
would make authentication sufficient for access, which is precisely what FR-PLAT-4 forbids.
...
as one would have proposed re-opening a settled decision. **Verdict: not started, owner W3 /
`FR-GOV-4`.** Untouched by this slice, which writes the demo's membership from a seed rather
...
`FR-PLAT-55` requires the browser to authenticate *"against the same provider and discovery
...
- It cannot be a build-time constant: `FR-PLAT-59` puts production on a deployer's own
...
`w6b-decision-maker` as a `§0` gap** — a capability `FR-PLAT-55` needs and no requirement
...
`FR-PLAT-58` gestures at this — *"mints principals with fresh identifiers and grants them role
...
### Finding 7 — `deploy/keycloak/` is a name W14 will want, so this slice does not take it

`FR-PLAT-59` gives W14 a **reference** Keycloak deployment under `deploy/`, *"with the rest of
...
`deploy/keycloak/` is the name W14's reference deployment will reach for first.

This slice therefore uses **`deploy/keycloak-local/`**, leaving `deploy/keycloak/` free.
**Verdict: not started, owner W14** for the reference deployment itself. The cost of the
...
| `tests/test_repository_invariants.py` | Add the two `FR-PLAT-58` artifact invariants | 1, 2 |
...
| `docs/specs/07-platform.md` | Amend `FR-PLAT-1`; `FR-PLAT-58` discharged | 5 |
...
```yaml
  # The local identity provider (FR-PLAT-58), behind an opt-in profile.
  #
  ...
  # production deployment FR-PLAT-59 says this project does not ship.
  keycloak:
    profiles: ["auth"]
...
`retries: 40` at `interval: 5s` is a 200 s ceiling, inside `NFR-PLAT-4`'s budget — the
...
This is the half of `FR-PLAT-58` a passing suite will not notice:
...
Expected: **PASS**, including the pre-existing `NFR-OVR-9` test — it asserts
...
Four properties, each one clause of `FR-PLAT-58`: *"a public client with PKCE and no secret,
...
token from a username and password with no browser flow at all, and `FR-PLAT-55` specifies
...
would be wrong in any deployment — `FR-PLAT-59` says this file is never the production one.
...
The first test asserts the predicate `FR-PLAT-58` actually states — that a login *resolves into
...
This is `FR-PLAT-58`'s *"both test suites keep running with no container"*:
...
The README's `## Verified` block carries measured start times against `NFR-PLAT-4: < 5 min`,
...
not a number to write down quietly:** report it, because `NFR-PLAT-4` would then be breached by
this slice.
...
- [ ] **Step 1: Amend `FR-PLAT-1`**

`FR-PLAT-1`'s 2026-08-23 amendment ends *"what is outstanding is the local container"*
...
> workspace, user and membership rows a realm login resolves to. `FR-PLAT-58` is discharged;
> the browser half (`FR-PLAT-55`) is not, and remains `W6b-10`.*
...
- [ ] **Step 5: Confirm `FR-PLAT-58` is evidenced**

```bash
uv run python scripts/req-coverage.py | grep -n "FR-PLAT-58"
```

Expected: evidenced by the `@pytest.mark.req("FR-PLAT-58")` tests in
...
| `directAccessGrantsEnabled: false` is "the flow `OQ-PLAT-6` decided against" | `OQ-PLAT-6` (`07-platform.md:465`) decided **PKCE-in-the-SPA over a BFF session cookie**. It says nothing about the password grant. The correct authority is `FR-PLAT-55`, which specifies authorization code with PKCE |

The last of those is the worst kind: a citation that is real, adjacent, and points somewhere
else. It would have survived review by anyone who checked that `OQ-PLAT-6` exists and concerns
...
writes only what the caller supplied. And the plan asserted `NFR-PLAT-4` as "300 s" where
...
decision, so it became Finding 2 with a `not started, owner W3` verdict rather than a proposal
...
the seed would have produced. And it produced Finding 7, from `FR-PLAT-59`'s own wording about
what W14 owns under `deploy/`.

**Spec coverage.** `FR-PLAT-58`'s clauses map to tasks as: the opt-in profile → Task 1; the
...
is unclaimed. `FR-PLAT-1`'s stale sentence → Task 5 Step 1.
