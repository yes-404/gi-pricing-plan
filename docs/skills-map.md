...
| SciPy `cluster.hierarchy` ✔ | 02 FR-MODEL-14 `hierarchical_clustering` | ★★ | Ward linkage and `fcluster(criterion="maxclust")` on **1-D level rates**; weighting a clusterer that takes no sample weights (observation repetition, and its cost); why the resulting clusters are contiguous in rate order and what that implies — a contiguous 1-D partition would give the same answer in O(n log n) rather than O(n²), which is NFR-MODEL-3's shortfall at 10 000 levels. **Measured 2026-08-15:** 6.52 s against a 5 s budget | [scipy.cluster.hierarchy](https://docs.scipy.org/doc/scipy/reference/cluster.hierarchy.html) |
| scikit-learn `tree` ✔ | 02 FR-MODEL-9, FR-MODEL-14, FR-MODEL-85 | ★★ | `DecisionTreeRegressor` as a **banding/grouping instrument**, not a model: `max_leaf_nodes` rather than `max_depth` for an exact band count, `sample_weight` for exposure weighting, and reading cut points off `tree_.threshold` where `tree_.feature >= 0`. Thresholds are midpoints between adjacent observed values, so no cut coincides with a value in the data. Why a one-tree XGBoost is **not** a substitute — its splits answer to `lambda`, `min_child_weight` and `gamma` (OQ-MODEL-9, decided 2026-08-17) | [DecisionTreeRegressor](https://scikit-learn.org/stable/modules/generated/sklearn.tree.DecisionTreeRegressor.html), [Tree structure](https://scikit-learn.org/stable/auto_examples/tree/plot_unveil_tree_structure.html) |
...
| openpyxl | 03 FR-RATE-20 | ★ | XLSX import/export for rate tables — read + write in one library, decimal-string discipline (never float through the file), strict round-trip verification | [openpyxl docs](https://openpyxl.readthedocs.io/) |
