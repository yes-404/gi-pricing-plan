...
id: FD-894
family: finding
title: RFC-840/841 adoption pilot
status: closed                  # active → closed | retired (§1.2a)
created: 2026-08-29
owner: auditor
corrected_by: []
relates: []                     # ids only — the SL-/WK- this discharges through, once known
was: docs/audit/work/nt-0010-0011-adoption/pilot-findings.md
---

# Pilot findings — `CLAUDE.md` §15 step 6

**The pilot:** WK-671 slice 1, 2026-08-29, run by a six-member team spawned from
...
superseded note on `RFC-840`/`RFC-841`) and the maintainer's gate, both of which happen
...
PR. The frozen WK-671 plan's "Slice 1" holds five tasks. So §7's retry caps and §8's
...
first seen with Monitor tasks in WK-670, now with agent lifecycles.
...
(`de4af3b` → `d41f379` → `6f893c6`). The auditor filed *"RL-878 is not reflected"* — true
...
**unnumbered** in `docs/closures/INDEX.md#plan-reviewsmd`, because a number assigned before the thing
...
`docs/plans/PL-00846-wk-671-slice-1-evaluator-core-its-prerequisites-and-the-latency-harness.md` narrates Rulings 6–12 correctly — RL-873's
...
Task 1.2 Step 4 still says *"the booster blob reference for a GBM"*, the option RL-873
...
dereference; the other tunes the per-call booster construction RL-874 exists to delete.
Either alone costs a rewrite. Together they would have looked mutually consistent while both
were wrong.

**The consequence, which nobody named first.** Task 1.4 Step 5 would re-measure NFR-501
through the per-call load RL-874 removes. WK-668's baseline was taken with the booster **already
...
1.626 ms, conclude NFR-501 is failing, and optimise code that already meets its budget. A
...
Task 1.4 Step 5 cited `1.626 ms` as NFR-501's target. WK-668 publishes **two** nthread=1
...
is *incl. DMatrix*, because RL-874's seam amortises the booster **load** while DMatrix
...
never regenerated. That is the **FR-451 drift gate**, one of the repository's named
...
**The instance.** RL-878's addendum imposes a **fourth** obligation on Task 1.4 — lift
...
separate `## RL-878` section; the planner's rule 5 was written over Files and Steps and
...
in flight is a message sent to somebody else. This is `RFC-843`'s relay problem intersecting
...
- **The executor adopted P4 mid-flight** (each task its own process-slice and PR), ran a
  generalisation control unprompted, and booked FR-257 as **explicitly deferred** rather
...
- Two spec-to-contract divergences on `main`, one of which makes a `RateTableVersion`
  **unsatisfiable** for the parquet storage mode `FR-232` defines.
...
- The catches ran in **every direction**, including upward. The planner corrected the
  ...
  and the lead's premise about RL-878's home; the executor refused a role-file clause that
...
hand-authored contract its charter does not grant; the auditor held `FR-218`'s register
...
| # | Disposition | Owner |
|---|---|---|
| **P1** | **fixed** — `watcher.md` now requires liveness proof after arming as an obligation separate from the silence watch, and states that neither a Monitor task id nor the script's own "armed at" banner is that proof (`ps -p` / `kill -0` against the pid you started is) | lead |
| **P1b** | **partly fixed** — its operational form landed in `watcher.md`: when diagnosing from a log you have been writing to, subtract your own attempts first, or diagnose from live state. **Still carried**: a working note of its own, since the reasoning failure generalises past this repository | §14 review |
| **P2** | **fixed** — `watcher.md` and `reporter.md` both now carry the precedence sentence: the skill is authoritative, the handover carries runtime state, the skill wins on conflict. *Partly discharged: the watcher corrected `watcher-state.md` §4 Step 1 to the durable `extract_token.py` path during the pilot* | §15 step 7 |
| **P3** | **fixed** — `reporter.md` now says the role arms its own cycle at spawn from the skill path and proves liveness with `ps -p`. `watcher.md`'s equivalent landed with P1 | lead |
| **P4** | **fixed** — `delivery-process.md` §4 now distinguishes the process-slice from a plan's `Slice N` heading, and says to disambiguate at each use. Ruled for WK-671 (five sequential process-slices, frozen map untouched). **The definitional defect remains**: `delivery-process.md` must distinguish the process-slice from a plan's slice heading, or the plan template must stop using the word for the group | §15 step 7 |
| **P5** | carry — the stand-down procedure must state that stand-down is not termination, that worktree locks persist while the agent lives, and that a successor cuts a fresh worktree. **Not yet fixed**: no document owns the stand-down procedure, which is itself the finding | §15 step 7 |
| **P6** | **fixed** — `reporter.md` step 1 now names the reporter as the marker's writer, as an obligation, and step 4 reads the age from `nudge.log` rather than recomputing. The old passive wording is quoted with the reason. Previously: something must write `.last_lead_status_ts` or the detector is removed, and `reporter-state.md:29`/`:90` are corrected. *Partly discharged: the reporter now writes the marker itself, verified at 15:15Z with no false escalation* | §15 step 7 |
| **P7** | carry — the writer's half, *do not move a branch someone is reading*, is filed **unnumbered** as a pending proposal in `docs/closures/INDEX.md#plan-reviewsmd` (PR #389), deliberately not landed because a §14 review's output is a proposal. *(Corrected: this row previously said the rule "is drafted as rule 6". It was not. It had been promised in a message and never written; the only text ever drafted under that number was the prospective-count rule, and the planner wrote the writer's-half rule for the first time when recovering it. The lead's row recorded a draft that did not exist.)* | §14 review |
...
| **P11** | **fixed** — `delivery-process.md` §15 now requires a gate or check result to name its corpus: the command, the totals and the tree. Placed there rather than in one role's charter because it binds every role that reports a result | lead |
...
| **P14** | **fixed** — `delivery-process.md` §8 now requires announcing an expensive verification when you start it and checking for one in flight before starting, with the announcement named as the load-bearing half; and prefers CI over a reviewer's local re-run | lead |
...
both are P9's own shape.** Told by the auditor that RL-878 was not reflected in #370, the
