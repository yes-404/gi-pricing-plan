# NT-0017 — A public repository needs a public face

| | |
|---|---|
| **Raised** | 2026-08-30, written against tree `7db62ca`, the same day the repository went public |
| **Status** | `open` — proposed, not adopted. Nothing here is in force |
| **Deliverable** | New public-facing files and two repository settings, not a `docs/specs/` change. Per `CLAUDE.md` §0's table, the note's one real decision — the contribution posture (§5 Q1) — is a maintainer ruling that must land before `CONTRIBUTING.md` is written, since (§2 P3) "the file is a policy ruling wearing a filename" |
| **Owner** | The maintainer rules §5's three open questions (Q1 posture, Q2 README granularity, Q3 SECURITY.md response window) — all policy, none mechanical; the lead confirms this note qualifies for the light path (no full reconcile, §7); the auditor's outsider read is the acceptance test for criterion (d) |
| **Lands in** | Nothing yet. §6's impact matrix: new `README.md`, `SECURITY.md`, `CONTRIBUTING.md`, `.github/ISSUE_TEMPLATE/bug.yml` and `question.yml`, `.github/PULL_REQUEST_TEMPLATE.md`; two repo settings (private vulnerability reporting, issues with templates); amendments to `docs/audit/checklists/work-item-close.md`, `docs/audit/security-posture.md`, and `docs/roadmap.md` |
| **Sequencing / Trigger** | §7's light path: freeze → maintainer rules Q1–Q3 → one Work, two slices — **S1** settings + `SECURITY.md` (the private reporting channel must exist before anything advertises the repository) → **S2** `README.md` + `CONTRIBUTING.md` + templates + the checklist line. No pilot needed; the first real external issue is the pilot |
...
`docs/audit/register.md` as the tour stops); how to engage (pointer to CONTRIBUTING and
...
- File: supported scope (the deployed surface once W14 exists; until then, the codebase
...
- Relation to `docs/audit/security-posture.md`: SECURITY.md is the outward one-pager;
...
- **PR template** exists anyway (invited PRs and the agents' own use): scope line,
  ...
  tree).
- Intake rule, one paragraph (the anti-NT-0003 clause): **issues are intake, the register
...
- **C1 — The README copies nothing.** NT-0003 (duplicated status goes stale) is the
...
- **C2 — Living files need owners.** README/SECURITY/CONTRIBUTING are living documents in
  ...
  the close checklist line; SECURITY: W14's deployment changes scope; CONTRIBUTING: the
...
| 7 | `docs/audit/checklists/work-item-close.md` | C2 pointer-freshness line | Amend |
| 8 | `docs/audit/security-posture.md` | One line: SECURITY.md is the public face of this doc | Amend |
