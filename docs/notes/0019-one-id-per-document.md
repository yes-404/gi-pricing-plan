...
**Amended 2026-09-03 — Ruling 105 D5, a dated append; the paragraph above stands as filed and is not edited** (`docs/plans/2026-09-03-w37-6-ruling-105-alias-class-h-breakdown-and-run-window.md`, D5). Row **(f)**'s *"unchanged from `8f5d57d`"* is read as, and from this date reads as, *"unchanged across the migration on the snapshot, per file through the routing table; `8f5d57d` is the tree the baseline was taken at, not the comparand."* This makes Ruling 103 §2.2's reading the sentence's own text rather than a reading held beside it. §7's preamble already scopes every other row to the migration PR's merge tree, and this note's own header records `8f5d57d` as the tree at which its counts were measured — so a literal comparison against that commit would hold the migration answerable for every commit made in between, which is not a property of the migration. The routing table is Ruling 103 §3.3's, not `REDIRECTS.csv`, which is one-to-one as generated and cannot express a split source (Ruling 103 §5.2). A row **(f)** computed against a hard-coded `8f5d57d` is the violation this append exists to make detectable.
