...
expand to `FR-450, FR-451`. It produces the two tokens `FR-450` and `48`, and
`48` alone matches no real requirement — so the intended `FR-451` is silently never
added to scope at all, while the report shows a `NO EVIDENCE` row labelled `48` that reads
as if it were about the missing id. It is not: nothing checked `FR-451` itself. This is
...
`{FR-257, "41", "42", NFR-489, "13", "14"}`, not the six ids it reads as.
Re-running with every id spelled out
(`FR-257,FR-258,FR-259,NFR-489,NFR-502,NFR-501`) landed on the same
...
token, with a "did you mean `FR-258`?" hint whenever a bare number looks like it
...
`FR-450,FR-451` example above — none of them cross a module boundary.
...
requirements — plus FR-450/451 for the API conventions and the generated contract, which
...
```bash
grep -nE '\| \*\*W[0-9]+' docs/roadmap.md | grep PLAT     # e.g. FR-428, FR-429, FR-430, FR-431 → WK-674
```

**Reconcile against the roadmap's own count.** A disagreement is a finding: WK-658's row said
"of 60" where the spec now holds 61, because FR-406 was appended afterwards.
...
WK-660 stood at **49 of 50 requirements with 0 of 28 endpoints published**, and nothing said so.
...
default for four of WK-658's six gaps until an audit went looking.
...
WK-657's re-audit reported half its scope missing while every one of those mechanisms was
working. The mechanisms were fine; the record could not see them.

Where a test is genuinely the wrong instrument, say so and keep the measurement: NFR-529
...
open the test and check the assertion is exact. WK-658 had a test accepting *either* of two
...
WK-657's `docker-compose.yml` sat committed and unrun for days. It worked — but nobody knew
that, and NFR-529 was unverified the whole time. "Committed" is not evidence.
...
```bash
# example: is the import-linter contract actually enforcing ADR-703?
...
Proving a check fires is not enough if the note beside it misattributes *why*. The WK-692
closure tripwire is `^#+ WK-692[a-z]?([ —].*)?: closed <date>`, and its design note credited
`[a-z]?` with keeping slice headings out. It does not. `^#+ WK-692[a-z]?.*: closed …` fires on
...
`WK-692\b.*` **is** that simplification — the obvious tightening of the permissive form, and
...
Worked example, PR #435 (WK-671 Slice 2 Task 2A). RL-882 clause 4 forbids the bundle slot
...
- **Pin the ref.** Both halves must read `git show <ref>:path`, never the working tree.
  ...
  tree it lands in makes a claim about the wrong thing. That confusion misread a WK-692
...
### Search for the thing's shape, not its container's name (RFC-842)
...
order** against the skill text you are reading: delete anything certifying `WK-692\b.*` safe.
...
WK-658's contract drift check passed while the published OpenAPI advertised an error model the
...
was performed *at*. RFC-895 adoption slice H set the field to its own base commit when it
...
> Cold start 21 s, warm 6 s, against NFR-529's 300 s budget.
...
This is the section most worth writing, because "WK-657 closed" reads as "the retrofit list is
...
[`docs/closures/INDEX.md#plan-reviewsmd`](../../../docs/rfcs/RFC-00839-pending-proposals-for-the-14-review-at-wk-671-s-close.md), find every dated
...
```bash
grep -n "Maintainer acceptance" docs/closures/INDEX.md#plan-reviewsmd    # then read each one's clause
```
...
Phase 2's WK-671–WK-674 boundaries and was accepted 2026-08-29. The acceptance says, in the
maintainer's own words, *"Acceptance makes the paragraph above binding. It does not meet
it"* — FR-250 and FR-257 must **each** get an explicit, named, dated deferral in
[`register.md`](../../../docs/findings/register.md) when WK-671 closes, *"not silence, and not a
stub shipped and called done"*, and *"reading this date as having satisfied it would invert
the clause"*.

Two traps that entry also records:

- **A ruling is not a register row.** WK-671's DP1 and DP2 were both ruled, and the acceptance
...
- **A mention inside another row is not a row.** Checking the obligation by grepping the id
  returns F-W9-2's prose *"specialises FR-257's general approval-evidence gate, which WK-671
...
**The precedent for taking this seriously:** F-W9-1 carried NFR-502/501 forward with a
register row that did exist, and the roadmap still lost them for two workstreams — WK-671's row
...
`docs/findings/register.md`'s open rows owed by, or blocking, this close are not a list you
compile by hand while writing the closure record — that is precisely how F41 lost
NFR-502/501 for two workstreams even though a register row, F-W9-1, existed for them the
whole time (§5a's own precedent quotes the same row). Run it and paste the output verbatim:

```bash
python3 scripts/register-owed.py <work-id>     # e.g. WK-671
```

**RL-912** (`docs/rulings/RL-00912-q4-the-generated-output-lands-verbatim-and-it-is-not-the-closure-record-s-findings-table.md`) binds the result's shape,
and the exact wording is in
[`checklists/work-item-close.md`](../../../docs/process/checklists/work-item-close.md)'s Owed
...
- **Component status is `docs/roadmap.md` §6's alone.** `CLAUDE.md` §2 carried layout marks
  ...
  ([`RFC-756`](../../../docs/rfcs/RFC-00756-duplicated-status-in-claude-md-goes-stale.md)) — verify the roadmap's
...
The demo guide (FR-409) is **derived, not written**, so there is nothing to update —
...
**Retry counters (RFC-895 artifact B, `~/gi-pricing-plan.local/handover/
...
2026-08-31 — added the closure record's retry-counters line, RFC-895 adoption slice G
...
2026-08-30 — §5a, written at WK-671's close from an obligation that was live and unmet while
this skill had no step that would have found it. Plan review 8 §5.1's acceptance
(`docs/closures/CR-00830-plan-review-8-at-wk-670-s-close.md:233-251`) requires named, dated register deferrals for
FR-250 and FR-257; `git grep -n "FR-250\|FR-257" docs/findings/register.md`
returns exactly one line, F-W9-2's prose about FR-224, and neither id has a row. The
...
the WK-669 nor the WK-670 close.
...
`13`, `14`, each with a "did you mean" hint against the correct prefix (`FR-258` etc.);
a version with all six ids spelled out
(`FR-257,FR-258,FR-259,NFR-489,NFR-502,NFR-501`) produces
...
directions plus a cross-module id (`FR-450`, real but not RATE's) and a leading bare
...
2026-08-29 — the `--extra` comma-prefix trap in §0, found re-deriving WK-671's own baseline
...
2026-08-24 (second entry, at WK-692's close) — the empty-output gate section. Written from the
...
2026-08-24 — the four §3 subsections on instruments were added from the WK-692 closure gate's
...
RFC-842/843/895 adoption's close. Confirmed live at `9e8783d`:
