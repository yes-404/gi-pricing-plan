...
| [`close-workstream`](close-workstream/SKILL.md) | Audit a workstream against `CLAUDE.md` §13 before writing "closed" into the roadmap — **scope derived from the specs first**, then evidence; gate run locally, new checks proven non-trivial, NFRs measured, every gap given a verdict. Also: a check's design note must say which half **fails open** and which **fails closed**, and a metric a script computes is **run, never re-derived** | self-written | 2026-08-24 |
