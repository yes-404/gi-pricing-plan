...
| [`docs-audit`](docs-audit/SKILL.md) | Verify suite integrity before a commit or PR — 21 checks (8 bookkeeping, 7 structural, 5 over the `.claude/notes/` working notes, 1 over the journeys' interface citations) plus the decision-gate invariant the script does not cover | self-written | 2026-08-18 |
...
| [`python-package`](python-package/SKILL.md) | Write Python in the uv workspace — where code belongs, the import-linter boundaries, the Pydantic v2 idioms the contracts depend on, and **the compiled dependencies that abort rather than raise** | self-written | 2026-08-17 |
