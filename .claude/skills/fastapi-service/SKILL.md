...
frontend generates its type from it (ADR-704).
...
*The four spec examples showed `01J…` until WK-658 implemented this and the mismatch surfaced.*

## Settings are typed, frozen, and validated at startup

FR-447: an invalid setting prevents startup with a message naming the environment
...
| Hash chain (FR-372) | nothing — it *detects* | nothing |
...
- **`REVOKE ... FROM <owner>` does nothing.** After revoking `UPDATE, DELETE` from the
  ...
  privileges. FR-370 is only real if the application connects as a role that does not
  own the table.
...
twice** — a rollback that cannot be re-applied is a one-way door (FR-435).
...
**`total_estimate` is capped, not exact.** FR-410 keeps 13 months of job history; an
...
detours across WK-659 and WK-660, each time because `match=` reads naturally and the failure
...
`outbox.publish_directly()` raises when there *is* one (FR-406). Services take a
...
WK-661's backtest slice, concretely. `POST /models/{id}/backtest` must refuse the version the
...
2026-08-18 — WK-661, backtests. The refusal-ordering rule above was found by a test that expected the specific message and got the generic gate's. Gate green both halves at 1073 Python / 105 frontend.

2026-08-18 — WK-661, peril structures. The `job_kind` `ALTER TYPE` rule was found the way it
...
2026-08-14 — WK-658 OIDC slice. 218 tests pass, the suite run five times to confirm it after
three genuine flakes were found and fixed — two of them real defects in the code and test
data rather than in the harness.

2026-08-14 — WK-658 jobs-routes slice. 173 tests pass; the full suite was run seven times to
...
2026-08-14 — WK-658 Celery slice. 154 tests pass. The full seam was verified with a real
...
registered yet (they arrive with WK-660/WK-661), so the correct end state was
`JOB_HANDLER_NOT_REGISTERED` — which still exercises every hop.

2026-08-14 — WK-658 blob slice. 115 tests pass; blob behaviours verified against real MinIO,
and the suite re-run to confirm it is repeatable rather than passing once.

2026-08-14 — WK-658 persistence slice. 96 tests pass (75 backend, of which 35 run against a
...
2026-08-14 — WK-658 sprint 1. 40 backend tests pass; the middleware-ordering trap was found by
a failing test asserting `trace_id` on the 500 path, not by reading Starlette's source.

2026-08-22 — WK-661 audit remediation. The alembic credential mismatch above, found while adding the `diagnostics_id` immutability migration.
2026-08-14 — WK-660 validation persistence. The `alembic.ini` location, the `status_code`
...
2026-08-14 — WK-660 REST surface. 528 tests pass. The DuckDB sandbox controls were each proven
