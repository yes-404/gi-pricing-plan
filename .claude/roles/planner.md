...
- **Tools:** Read, Grep, Glob; write to `docs/plans/` files only, per `CLAUDE.md` §12 — the
  planner's charter never names other `docs/` content. A roadmap-row correction or other
  `docs/` edit surfaced inside a plan review is a proposal in the review document, applied
  by the lead or decision-maker.
