# NT-0018 — An agent's turn ending strands everything it started: one invariant, two directions, six occurrences, two fixes that each named the mechanism instead

| | |
|---|---|
| **Raised** | 2026-08-31, on the maintainer's instruction, by a subagent working in its own worktree off `main` at `daa6fbe`. Every item in the maintainer's evidence list was re-derived from the repository before being written down; §8 records the three items that could not be, and why |
| **Status** | `open` — raised and assessed, not agreed. Two partial mitigations are already merged (`3c50e03`, `1ccec01`); the invariant itself is stated nowhere, and the orphaned-worker direction of it is stated nowhere at all |
| **Deliverable** | **No code and no spec change**, per [`CLAUDE.md`](../../CLAUDE.md) §0's table — this is working practice, not platform capability. The candidate homes are `.claude/roles/*.md`, `.claude/skills/dev-commands`, and `docs/process/delivery-process.md`; §6 lists them as options and picks none |
| **Owner** | The decision-maker rules §7's questions that are conventions; **the maintainer rules Q3** (whether a role's dispatch discipline becomes a process-spec rule, which amends what `CLAUDE.md` §15 points at). Author is this session; the primary sources are the two merged fixes and plan review 10, all cited inline rather than relayed |
| **Lands in** | Nothing yet. If adopted, the smallest form touches `.claude/roles/executor.md` and `.claude/skills/dev-commands/SKILL.md` (both of which already carry a narrower version of the rule); the largest adds a dispatch-discipline clause to `docs/process/delivery-process.md` §8 and its `.core.json` extract |
| **Sequencing / Trigger** | **Now, not at a phase boundary.** The trigger is that both existing fixes are merged and both were falsified within hours of merging, so the next occurrence is expected under the current rules rather than prevented by them. It costs no workstream capacity: every option in §6 is a wording change to a role file or a skill |
...
counts "the third stall" and "the fifth stall", and [`docs/audit/plan-reviews.md`](../../docs/audit/plan-reviews.md)
finding 3e records itself as "the **sixth occurrence today across five mechanisms**".

| # | Mechanism | Who | Where it is recorded |
|---|---|---|---|
| 1 | Backgrounded shell command — `uv run pytest -q`, 2,347 tests past the 10-minute foreground limit | executor, W11 Task 3A | `3c50e03` commit message; `.claude/skills/dev-commands/SKILL.md` §"Never end a turn with a command still running" |
...
| 6 | A **delegated teammate pair** | **the planner**, writing plan review 10 | `docs/audit/plan-reviews.md`, finding 3e — recorded against itself |
...
repository's count of stalls *noticed and recorded* on 2026-08-30 across W11 Slice 2's
...
- `drift-03` — **not recovered.** Its `03` §5.2 signature-drift result was never obtained.
  ...
  *"The signature axis — `03` §5.2 shapes with no implementation, and implemented shapes `03`
  ...
  It is carried as that review's **proposal 4.2** (*"run the §5.2 direction before W12 opens,
  since W12 builds against those shapes"*) — an open proposal in a merged plan review
...
did not come back is now a plan-review proposal blocking nothing but owed before W12, and
...
Plan review 10 (merged `18831bd`, "docs(audit): plan review 10 and the NT-0014..NT-0017
...
It is related to but distinct from [`NT-0013`](0013-the-lead-is-the-highest-error-node.md):
