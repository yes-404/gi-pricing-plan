# NT-0016 — File taxonomy, reference coding, and custody investigation (rev 2)

| | |
|---|---|
| **Raised** | 2026-08-30 (rev 2 same day, folding in the `docs/`/`.claude/` boundary question rather than filing separately), written against tree `7db62ca` |
| **Status** | `open` — proposed, not adopted. Nothing here is in force |
| **Deliverable** | Investigation first (Stage 0–1: a census script and a taxonomy draft, evidence only), then, per `CLAUDE.md` §0's table, spec/doc change before code — the taxonomy doc, the reference-coding standard and the ownership matrix precede `scripts/file-census.py` and `scripts/file-lint.py`'s gate wiring |
| **Owner** | The decision-maker rules the closed taxonomy set, the two-homes tension, and §3a's notes-relocation exception with its Q5–Q7 (§10); a fresh-context auditor runs the Stage 4 workflow-loop audit; the maintainer accepts the Work's close (`CLAUDE.md` §12) |
| **Lands in** | Nothing yet. Named across §2–§7: a committed census CSV under `docs/audit/`, a taxonomy doc, `docs/notes/` (the §3a move of `.claude/notes/`, with a tombstone left at the old path), `docs/INDEX.md`, `scripts/file-census.py` and `scripts/file-lint.py` (+ tests), the creating skills (`writing-plans`, `close-workstream`, `phase-review`, `adr-write`, `spec-change`), a category × role matrix, register findings rows for unowned categories, the gate, and the phase-close checklist |
| **Sequencing / Trigger** | §8's shape: Stages 0–1 (census + taxonomy draft) run as one investigation session each, read-only fan-out legal → reconcile: decision-maker rules the closed set, the two-homes tension and §3a's exception → Stages 2–3: one Work, ~5 slices (S0 = the §3a notes move, first, self-contained; then lint + index; skills; ownership matrix; findings filed) → Stage 4: one audit slice with the fresh-context auditor → Stage 5: riders on the gate and phase-close checklist. Cross-reference: if NT-0017's Work lands first, its README cites the pre-move `.claude/notes/` path and S0 updates it as a living citation, so the two notes compose in either order |
...
- Precedent findings of exactly this shape already in the register: F31 (a published file
  nothing true derives), F28-P5 (a procedure no document owns), NT-0005 (deferred items
  with no durable custody), NT-0003 (duplicated status goes stale).
- **The boundary evidence (rev 2):** the NT notes — the project's design memory, cited by
  CLAUDE.md §13 and six register rows — live in `.claude/notes/`, a directory whose other
  ...
  is not free: **28 tracked files cite `.claude/notes/` paths** — 7 frozen plans, 7
  ...
  `.claude/notes/**` because `scripts/audit-docs.py` checks 16–20 audit that directory,
...
| note (NT) | frozen proposal | .claude/notes → **docs/notes** (§3a, C5) |
...
1. `git mv .claude/notes docs/notes` (history preserved).
...
3. **Frozen citations are not edited** (C4). Instead `.claude/notes/README.md` remains as
   a tombstone: one paragraph and the old→new mapping, so any frozen plan's citation
   still resolves for a reader — NT-0004's own test ("would it still resolve for a reader
   holding none of your context") satisfied by redirection rather than rewriting history.
4. Acceptance within the PR: `grep -r '\.claude/notes/' --exclude-dir=.git` returns
...
- `scripts/file-lint.py`: new/renamed files must parse to a category grammar; legacy
  ...
  red with a dated flag-day (NT-0014 Q3's posture).
...
with — are filed as findings (register rows per NT-0015's grammar, decaying to the §14
...
- Legacy migrates **opportunistically-on-amendment only** (NT-0015 P4's rule) — a file is
...
after 1; 4 needs 0's data and 3's matrix; 5 needs 2. Cross-reference: NT-0017's README
tour should point at `docs/notes/` — if NT-0017's Work lands first, its README cites the
