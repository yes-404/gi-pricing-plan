...
| **Status** | `accepted` — **corrected 2026-08-30 from `open`, which had become false in both halves.** `README.md` defines `open` as *"Raised, assessed, not agreed"* with *"Nothing is built"*; the maintainer's delegation is dated 2026-08-30 and **slices A–D are merged** (`33b5ef1`, `0be9c3c`, `97965be`), so neither half held. Corrected by the lead, whose records ownership this is — `README.md` check 1 requires a status be *verified*, and only the party that changed the facts can verify it (Ruling 47's record, handed back as not the decision-maker's). **Slices E, F and G remain**, so this is not yet `landed`. |
