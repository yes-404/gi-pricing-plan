...
| **Sequencing / Trigger** | **Not before W11 closes.** `CLAUDE.md` §14 forbids starting next-phase work while an open finding lacks a resolution, and this is a Work-sized change to the process the current workstream is running under — changing the process mid-workstream would invalidate the pilot data §7's retry caps are waiting on. The natural trigger is the W11 §14 plan review, which is already scheduled and already holds three gate-coverage findings (F27(c), F29, F33) that overlap this proposal's C4 |
