# NT-0008 — A closure audit structure for work items and phases
...
| **Trigger** | After W7; the maintainer decides which parts replace versus complement the existing §13/§14 machinery before anything is built |
...
- `docs/audit/README.md` — what a close means, who signs off, the checklists.
...
- `docs/audit/register.md` — the single global list of *open* findings across all phases.
...
5. **It must not duplicate status.** `docs/roadmap.md` §6 owns component and workstream
   status (NT-0003). A `phases/P#/register.md` that restates it will drift; the register
...
([[NT-0009]]): the historical record — closure records, plan reviews, the retrofit-impossible
...
The archive relocation is scheduled after W7 (via NT-0009). This note's fuller structure —
...
>
> Two registers: `phases/P#/register.md` (one row per work item), and `docs/audit/register.md`
