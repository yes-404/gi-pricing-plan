...
session did not itself witness — consistent with `.claude/notes/README.md`'s own
...
- **Strong, adopted mid-session, not yet landed anywhere durable:** remove the relay.
  ...
  practice — which is exactly the kind of institutionalisation a note in `.claude/notes/`
...
There is none in `.claude/notes/README.md`'s sense — no maintainer request produced this
