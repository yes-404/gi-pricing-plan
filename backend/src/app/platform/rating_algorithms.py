...
before a row is written: the shape's own invariants (FR-RATE-1) and the deeper checks
in `pricing_core.rating.compile.validate_algorithm` (FR-RATE-5/13/56/57/58/59).
"""
...
def _parse_algorithm(content: dict[str, Any]) -> RatingAlgorithm:
    ...
    The `RatingAlgorithm` shape enforces the graph invariants (FR-RATE-1) in its own
    ...
    try:
        ...
        if "cycle" in text:
            raise PlatformError(
                ...
                422,
                "A rating algorithm is a directed acyclic graph (FR-RATE-1).",
            ) from exc
        if "undefined value" in text:
            raise PlatformError(
                ...
                422,
                "Every consumed value is produced by a step (FR-RATE-1).",
            ) from exc
        raise PlatformError(
            "VALIDATION_FAILED",
...
async def diff_between(
    ...
    """The structural diff between two versions of one algorithm (FR-RATE-7)."""
