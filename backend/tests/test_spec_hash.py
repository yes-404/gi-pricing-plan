...
FR-204 returns the existing model rather than fitting the same specification twice.
...
to say why. OQ-582 named the version tag as the precondition for the first new field.
...
def test_a_separated_fit_can_be_reported_as_the_named_refusal() -> None:
    ...
    failure FR-115 exists to name arrived as a stack trace about error codes.
    """
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # FR-159/160: `gbm.py` gained `METRIC_REF_UNRESOLVED`,
        ...
        # `EBM_MONOTONE_CONSTRAINT_INCOMPLETE` the day the WK-661 slice wired its fit.
...
def _bound(**over: object) -> GbmSpec:
    """A GBM that *is* one side of another model's interval (FR-199)."""
    base: dict[str, object] = {
        "model_type": "xgboost",
        "model_family_slug": "motor-ad-frequency",
        "dataset_version_id": uuid4(),
...
def test_the_algorithm_version_moved_with_the_new_field() -> None:
    ...
    """
    # v7 -> v8 (2026-08-21, FR-116): the offset-from-another-model slice renamed the
    # scaffold's `model_ref` to `offset_model_ref` — the offset a fit means is now part
    # of the payload, and two specs differing there must not share a digest or FR-204
    # hands the second caller the first caller's model.
    # v8 -> v9 (2026-08-21, FR-140): EbmSpec joined the union — model_type,
    # objective, interactions, max_bins, max_rounds and monotone_constraints join the
    # payload, and two EBM specs differing there must not share a digest or FR-204
    # hands the second caller the first caller's model.
    ...
    # v10 -> v11 (2026-08-26, FR-169, OQ-613): the companion `approximates_model`
    ...
    assert SPEC_HASH_VERSION == 11, (
        "approximates_model joined the payload (FR-169); the tag moves with it"
    )
...
def test_an_ebm_spec_hashes_distinctly() -> None:
    ...
    FR-204 answers by digest, so the moment EbmSpec joins the union it must hash away
    ...
    # The same skeleton as a GLM: poisson would refuse the shared no-offset (FR-111),
...
def test_two_bounds_against_different_central_models_do_not_collide() -> None:
    ...
    (FR-204), and the second central model would silently acquire a bound fitted for
...
def test_two_surrogates_of_different_models_do_not_collide() -> None:
    ...
    FR-204 would answer the second caller with the first caller's fit.
    """
...
def test_the_two_sides_of_one_pair_do_not_collide() -> None:
    ...
    FR-204 and the pair would be one model referenced twice.
    """
