"""FR-273's startup self-check, wired into the app lifespan (WK-671 Task 1.4, F-W11-1-3).
...
(`packages/pricing-core/tests/test_rating_compile.py`) — the W9-2 audit booked FR-273
...
def test_a_failing_round_trip_check_prevents_the_service_starting(
    ...
    """The negative half, stated as the violation FR-273 forbids: a broken check must
    ...
    def _broken() -> None:
        raise AssertionError("simulated FR-273 round-trip failure")

    monkeypatch.setattr(main_module, "assert_integer_minor_round_trip", _broken)

    with (
        pytest.raises(AssertionError, match="simulated FR-273"),
