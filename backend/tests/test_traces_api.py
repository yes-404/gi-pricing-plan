"""`GET /api/v1/traces` (`03` §5.1:603, FR-259; WK-671 Task 4C).
...
batch-produced trace (`environment IS NULL`, RL-890) and a still-`pending` trace (no body
...
`backend/src/app/api/traces.py`'s module docstring for why. That means RL-884's
...
at all, because FR-347 forbids granting a Service Account `RATING_READ`. The two cases that
...
async def _write_real_time(
    ...
    *,
    ...
    via the pending-then-complete path (RL-862) — this file tests the *read* side, which
...
async def _write_batch_produced(
    ...
    *,
    ...
    """A trace written on request for a `score.batch` Job (FR-258, RL-890): no
...
async def _write_pending(
    ...
    """A real-time sampled outcome awaiting its off-path re-score (RL-862): an
...
async def test_a_caller_without_rating_read_is_refused_403(
    ...
    wrong reason (RL-884's discipline, applied to a `RATING_READ`-gated route)."""
...
# -- the two exclusions (Correction 2 / RL-890, and the pending-body corollary) ----------
...
async def test_a_batch_produced_trace_is_never_returned(
    ...
    `score.batch` Job (FR-258, RL-890) must not appear in this production stream."""
    await _write_real_time(
        database, blob_store, workspace_id,
...
async def test_a_pending_trace_is_never_returned(
    ...
    """A row still awaiting its off-path re-score (RL-862) has an `environment` but no
