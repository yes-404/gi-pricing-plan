...
describe("ColumnDrift", () => {
  it("renders nothing at all when no comparison is loaded", () => {
    ...
    const { container } = render(ColumnDrift, { props: { drift: undefined, warnAbove: null } });
  ...
  it("says a column is new rather than showing it as unchanged", () => {
    ...
    render(ColumnDrift, { props: { drift: null, warnAbove: 0.1 } });
    expect(screen.getByText(/new in this version/)).toBeInTheDocument();
  });

  it("renders a measured PSI as unbanded when no warn rule is loaded", () => {
    // The defect `W6b-13` removes: this component invented the 0.25/0.1 thresholds, so a
    // 0.31 PSI rendered "broken" — a band for a `fail` severity VR-DST-1 cannot emit. With
    // no rule handed in it must say so, not guess: falling back to 0.1 here would restore
    // the literal the task deletes.
    const { container } = render(ColumnDrift, {
      props: { drift: MEASURED, warnAbove: null },
    });
    expect(screen.getByText(/PSI 0\.310/)).toHaveClass("text-slate-500");
  ...
  it("bands a PSI above VR-DST-1's warn threshold as shifted, not stable", () => {
    ...
    render(ColumnDrift, { props: { drift: { ...MEASURED, psi: 0.11 }, warnAbove: 0.1 } });
  ...
  it("bands a PSI at or below VR-DST-1's warn threshold as stable", () => {
    render(ColumnDrift, { props: { drift: { ...MEASURED, psi: 0.02 }, warnAbove: 0.1 } });
  ...
  it("does not band a PSI that was never measured", () => {
    ...
    const { container } = render(ColumnDrift, {
      props: { drift: { ...MEASURED, psi: null }, warnAbove: 0.1 },
    });
  ...
  it("reports the level changes and the null-rate shift", () => {
    render(ColumnDrift, { props: { drift: MEASURED, warnAbove: 0.1 } });
