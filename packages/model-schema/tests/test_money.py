"""FR-10: money is integer minor units or Decimal, never float.
...
def test_decimal_str_refuses_a_float(bad):
    """`OQ-547`, decided 2026-08-19: the input shape is policed, not only the wire shape.
    ...
    `Decimal('0.30000000000000004')` inside a field FR-10 calls exact.
    """
...
def test_decimal_str_refuses_a_json_number():
    ...
    exposure `OQ-547` names. The contract has always declared this field a string."""
    with pytest.raises(ValidationError, match="float"):
