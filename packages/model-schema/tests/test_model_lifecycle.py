"""The Model lifecycle as data (`02` FR-202).
...
def test_review_returns_to_fitted_and_never_to_draft() -> None:
    """`06` FR-355 returns a rejected artifact to its pre-submission state. For a Model
...
def test_the_flag_a_dataset_invalidation_raises_is_named_not_freeform() -> None:
    """FR-205's flag is branched on by the approval path and by `06` FR-359's
