"""A banding or grouping factor resolves through its artifact — or not at all (FR-83).
...
fitted `identity` factors. The one thing FR-83's closed set is *for* — that a factor's
...
# -- Intent, the second refusal axis (OQ-595, decided 2026-08-22) -------------------
#
...
# what both *mean*. `offset` and `diagnostic`, the two arms FR-88 never glossed, were
...
def test_an_offset_intent_factor_is_refused_and_the_refusal_is_permanent() -> None:
    """FR-84: superseded, so the message must not say "yet"."""
    ...
    assert "FR-84" in message
    assert "superseded" in message
    # The distinction FR-209 draws for the type arm, held here for the intent arm:
...
def test_a_diagnostic_intent_factor_is_refused_and_the_refusal_is_permanent() -> None:
    """FR-86: superseded, so the refusal says "never" where FR-85 said "yet".

    This assertion is the inverse of the one it replaces, and deliberately so. FR-85
    refused the arm *pending* OQ-597 and this test held it to saying so; OQ-597
    then superseded it, on the finding that the meaning FR-88 never supplied did not
...
def test_risk_and_control_intents_still_resolve_and_are_fitted() -> None:
    ...
    FR-88's amendment of 2026-08-22 states what the enum had never said — `risk` and
...
def test_the_intent_refusal_reaches_the_fit_and_not_only_the_resolver() -> None:
    ...
    is unable to route around it — which is the claim FR-84 actually makes.
    """
    factor = _factor("exposure_offset", "exposure_years", intent=FactorIntent.OFFSET)
    spec = GlmSpec(
        model_family_slug="steps",
