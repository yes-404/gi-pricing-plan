#!/usr/bin/env python3
"""Generate the committed contracts from the code (FR-451, ADR-704).
...
GENERATED_SHAPES: dict[str, str] = {
    ...
    # Added 2026-08-15 (WK-661). Both had hand-authored Phase-0 contracts and no generated
    ...
    "banding": "Banding",
    "grouping": "Grouping",
    # Added 2026-08-16 (WK-661, diagnostics). Same reason as the two above, learned from them:
    ...
    # Added 2026-08-17 (WK-661, model comparison). **No hand-authored Phase-0 counterpart** —
    ...
    # Added 2026-08-17 (WK-661, the GBM arm). `model-spec` is a **union**, not a model class —
    ...
    "model": "Model",
    # Added 2026-08-17 (WK-661, transparency). `transparency-artifact.schema.json` is a
    ...
    # Added 2026-08-18 (WK-661, peril structures), and **superseded 2026-08-24 (W32-11)**: the
    ...
    # `OQ-649`. One-sidedness is now declared in `ONE_SIDED_SLUGS` in
    ...
    # Added 2026-08-18 (WK-661, backtests). No hand-authored Phase-0 counterpart: FR-187
    ...
    "backtest": "Backtest",
    # Added 2026-08-18 (WK-661, custom objectives). Both have hand-authored Phase-0 contracts
    ...
    # an approver's evidence hangs from (FR-146), so a field the contract promises and
    ...
    # No hand-authored counterpart: FR-164 named the blast-radius query and no document
    ...
    # empty on purpose (FR-207), which is only legible if the shape is published.
    "objective-usage": "ObjectiveUsage",
    # Added 2026-08-19 (WK-661, custom metrics, Task 2 of the custom-metrics slice). No
    ...
    # early stopping reads (FR-155/156), so a field the contract promises and the
    # model does not carry is a training-time defect, not a documentation one.
    "custom-metric": "CustomMetric",
    # Added 2026-08-19 (WK-661, custom metrics, Task 2 of the custom-metrics slice, fix
    # round 2). `ObjectiveCertificate` is published (line 87); leaving its sibling
    # unpublished would have FR-162's `GET .../certificate` cross the API
    # boundary as a hand-written frontend type instead of a generated one — the
    # divergence ADR-704/FR-451 exist to prevent.
    ...
    # place, which is what FR-451 exists to remove.
...
def main() -> int:
    ...
    if args.check:
        if stale:
            ...
            print("  ADR-704: the models are the source of truth, so the contract follows")
