...
def bench_proposals(rows: int, levels: int, bands: int, groups: int) -> None:
    ...
    print(f"\nNFR-477 — {rows:,} rows, {levels:,} distinct levels, budget 5 s")
...
def bench_breakdown(rows: int, levels: int, groups: int) -> None:
    ...
    have to pay.
    """
    print(f"\nNFR-477 breakdown — {rows:,} rows, {levels:,} levels")
    frame = levelled_frame(rows, levels)
...
def bench_glm(rows: int, factors: int) -> float:
    print(f"\nNFR-475 — {rows:,} rows x {factors} factors, budget 600 s at 5 M x 60")
...
def bench_gbm(rows: int, factors: int, rounds: int) -> float:
    ...
    """
    print(
        f"\nNFR-476 — {rows:,} rows x {factors} factors x {rounds} trees, "
...
def bench_diagnostics(rows: int, factors: int, *, type_iii: bool) -> None:
    ...
    **One caveat this harness cannot fix on its own.** The denominator below is a *cold*
    ...
    """
    print(
        f"\nNFR-479 / NFR-487 — {rows:,} rows x {factors} factors, "
...
def _artifact_size(computed: object, fit: object) -> None:
    ...
    print(
        f"\nNFR-486 — diagnostics artifact {len(encoded) / 1e6:.2f} MB "
...
def bench_certify() -> None:
    ...
    """
    print("\nNFR-480 — every template, 2 000-point grid, budget 180 s")
...
def bench_curve(scales: list[int], factors: int, rounds: int, *, gbm: bool) -> None:
    ...
    """
    print(f"\nNFR-475/476/485 growth curve — {factors} factors, {len(scales)} scales")
